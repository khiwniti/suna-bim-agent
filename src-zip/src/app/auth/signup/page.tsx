...
function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(password)) {
      setError(t('auth.signup.errors.passwordWeak'));
      return;
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('auth.signup.errors.signupFailed'));
    }
  };
  ...
  const handleOAuth = async (provider: 'google' | 'github') => {
    try {
      ...
      setError(err instanceof Error ? err.message : t('auth.signup.errors.oauthFailed'));
    }
  };
    ...
    return (
      <Card className="w-full max-w-md border-border bg-card/80 backdrop-blur">
        <CardContent className="pt-6 text-center">
          ...
          <h2 className="mb-2 text-xl font-semibold text-foreground">{t('auth.verification.title')}</h2>
          <p className="mb-6 text-muted-foreground">
            {t('auth.verification.description')} <strong className="text-foreground">{email}</strong>
          </p>
          <Button
        ...
        {showAnonymousWarning && (
          <div className="mb-4 rounded-lg bg-warning/10 border border-warning/30 p-4 text-left">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-warning text-sm">{t('auth.signup.anonymousWarning.title')}</h4>
                <p className="text-warning/80 text-xs mt-1">
                  {t('auth.signup.anonymousWarning.description')}
                </p>
              </div>
            </div>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="bg-card px-2 text-muted-foreground">{t('auth.signup.orContinueWithEmail')}</span>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="bg-card px-2 text-muted-foreground">{t('auth.signup.or')}</span>
          </div>
        </div>
