...
def _parse_invoke_block(
    function_name: str, invoke_content: str, full_block: str
    ...
    raw_xml = (
        raw_xml_match.group(0) if raw_xml_match else f'<invoke name="{function_name}">...</invoke>'
    )

    return XMLToolCall(function_name=function_name, parameters=parameters, raw_xml=raw_xml)
...
def strip_xml_tool_calls(content: str) -> str:
    ...
    cleaned = re.sub(
        r"<function_calls[^>]*>[\s\S]*?</function_calls>", "", content, flags=re.IGNORECASE
    )
...
def parse_xml_tool_calls_with_ids(
    xml_chunk: str, assistant_message_id: Optional[str] = None, start_index: int = 0
) -> list[dict[str, Any]]:
    """
    ...
    try:
        ...
        if "<function_calls>" in xml_chunk and "<invoke" in xml_chunk:
            ...
            for idx, xml_tool_call in enumerate(parsed_calls):
                ...
                logger.debug(
                    f"Parsed tool call from chunk: {tool_call['function_name']} (id: {tool_call_id})"
                )
        ...
        logger.error(
            f"XML chunk does not contain expected <function_calls><invoke> format: {xml_chunk[:200]}..."
        )
        return results
