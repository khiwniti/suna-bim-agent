...
class BillingHandler:
    @staticmethod
    async def handle(
        thread_id: str, content: dict, saved_message: dict, account_id: Optional[str]
    ) -> None:
        try:
            ...
            if cache_creation_tokens > 0:
                logger.debug(
                    f"💾 CACHE CREATION TOKENS DETECTED: {cache_creation_tokens} tokens will be charged at cache write rates"
                )

            model = content.get("model")

            usage_type = (
                "FALLBACK ESTIMATE" if is_fallback else ("ESTIMATED" if is_estimated else "EXACT")
            )
            logger.debug(
                f"💰 Usage type: {usage_type} - prompt={prompt_tokens}, completion={completion_tokens}, cache_read={cache_read_tokens}, cache_creation={cache_creation_tokens}"
            )
            ...
            if account_id and (prompt_tokens > 0 or completion_tokens > 0):
                if cache_read_tokens > 0:
                    ...
                    logger.debug(
                        f"🎯 CACHE HIT: {cache_read_tokens}/{prompt_tokens} tokens ({cache_hit_percentage:.1f}%)"
                    )
                elif cache_creation_tokens > 0:
                    logger.debug(
                        f"💾 CACHE WRITE: {cache_creation_tokens} tokens stored for future use"
                    )
                else:
