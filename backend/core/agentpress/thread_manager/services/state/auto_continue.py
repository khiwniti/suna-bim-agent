...
class AutoContinueManager:
    @staticmethod
    def check_trigger(
        chunk: dict[str, Any], auto_continue_state: dict[str, Any], native_max_auto_continues: int
    ) -> bool:
        if chunk.get("type") == "status":
            try:
                ...
                if finish_reason == "agent_terminated":
                    logger.debug(
                        "Stopping auto-continue due to agent termination (ask/complete tool)"
                    )
    ...
    async def track_tool_result_tokens(
        chunk: dict[str, Any], auto_continue_state: dict[str, Any], llm_model: str
    ) -> None:
        ...
        try:
            ...
            if content_str:
                ...
                auto_continue_state["tool_result_tokens"] = (
                    auto_continue_state.get("tool_result_tokens", 0) + tool_tokens
                )
                logger.debug(
                    f"🔧 Tracked {tool_tokens} tool result tokens (total: {auto_continue_state['tool_result_tokens']})"
                )
    ...
    def handle_tool_pairing_error(
        ...
        auto_continue_state["error_retry_count"] = (
            auto_continue_state.get("error_retry_count", 0) + 1
        )

        if auto_continue_state["error_retry_count"] > max_error_retries:
            logger.error(
                f"🛑 Tool call pairing error: max retries ({max_error_retries}) exceeded - failing: {error_str[:200]}"
            )
        ...
        logger.error(
            f"🔧 Tool call pairing error detected (retry {auto_continue_state['error_retry_count']}/{max_error_retries}) - applying emergency fallback: {error_str[:200]}"
        )
    ...
    def handle_anthropic_overload_error(
        ...
        if auto_continue_state["error_retry_count"] > max_error_retries:
            logger.error(
                f"🛑 Anthropic overloaded: max retries ({max_error_retries}) exceeded - failing"
            )
            processed_error = ErrorProcessor.process_system_error(
                e, context={"thread_id": thread_id}
            )
        ...
        logger.error(
            f"Anthropic overloaded (retry {auto_continue_state['error_retry_count']}/{max_error_retries}), falling back to OpenRouter"
        )
    ...
    def is_tool_pairing_error(error_str: str) -> bool:
        return (
            "tool call result does not follow tool call" in error_str.lower()
            or "tool_call_id" in error_str.lower()
        )

    @staticmethod
    def is_non_retryable_error(e: Exception, error_str: str) -> bool:
        return (
            isinstance(e, litellm.BadRequestError)
            or "BadRequestError" in error_str
            or "is blank" in error_str
            or "400" in error_str
            ...
            try:
                ...
                if account_id:
                    try:
                        ...
                        if not can_run:
                            logger.warning(
                                f"Stopping auto-continue - insufficient credits: {message}"
                            )
                            yield {
                                "type": "status",
                                "status": "stopped",
                ...
                if hasattr(response_gen, "__aiter__"):
                    async for chunk in cast(AsyncGenerator, response_gen):
                        ...
                        await AutoContinueManager.track_tool_result_tokens(
                            chunk, auto_continue_state, llm_model
                        )
                ...
                if AutoContinueManager.is_non_retryable_error(e, error_str):
                    logger.error(
                        f"🛑 Non-retryable error detected - stopping immediately: {error_str[:200]}"
                    )
                    processed_error = ErrorProcessor.process_system_error(
                        e, context={"thread_id": thread_id}
                    )
                ...
                if AutoContinueManager.is_anthropic_overload_error(error_str):
                    should_retry, error_response, llm_model = (
                        AutoContinueManager.handle_anthropic_overload_error(
