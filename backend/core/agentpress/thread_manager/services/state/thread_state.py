        ...
        try:
            try:
                ...
                if cached == "1":
                    ...
                    logger.info(
                        f"🖼️ Thread {thread_id} has_images: True (from Redis, {elapsed:.1f}ms)"
                    )
                    return True
                elif cached == "0":
                    elapsed = (time.time() - start) * 1000
                    logger.debug(
                        f"🖼️ Thread {thread_id} has_images: False (from Redis, {elapsed:.1f}ms)"
                    )
                    return False
            except Exception:
                pass
            ...
            try:
                has_images = await asyncio.wait_for(
                    threads_repo.check_thread_has_images(thread_id), timeout=5.0
                )
            except TimeoutError:
                elapsed = (time.time() - start) * 1000
                logger.warning(
                    f"⚠️ thread_has_images QUERY timeout after {elapsed:.1f}ms for {thread_id}"
                )
                return False
            ...
            elapsed = (time.time() - start) * 1000
            logger.debug(
                f"🖼️ Thread {thread_id} has_images: {has_images} (from DB, {elapsed:.1f}ms)"
            )
            return has_images
        except Exception as e:
            elapsed = (time.time() - start) * 1000
