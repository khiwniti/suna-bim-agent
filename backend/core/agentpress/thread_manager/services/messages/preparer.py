...
class MessagePreparer:
    ...
    async def perform_fast_path_check(
        ...
        new_msg_tokens = await self._estimate_new_message_tokens(
            llm_model, is_auto_continue, auto_continue_state, latest_user_message_content, thread_id
        )
        ...
        if memory_context_tokens > 0:
            logger.debug(
                f"⚡ Fast check: {last_total_tokens} + {new_msg_tokens} + {memory_context_tokens} (memory) = {estimated_total} tokens (threshold: {max_tokens})"
            )
        else:
            logger.debug(
    ...
    async def _await_prefetch_tasks(
        ...
        try:
            ...
            if prefetch_messages_result is not None:
                ...
                logger.info(
                    f"⚡ [PREFETCH] Used prefetched data in {query_time:.1f}ms ({len(prefetch_messages_result)} msgs)"
                )
            ...
            logger.warning(
                f"Prefetch failed ({type(e).__name__}), falling back to fresh fetch: {e}"
            )
    ...
    async def _estimate_new_message_tokens(
        ...
        if is_auto_continue:
            ...
            if new_msg_tokens > 0:
                logger.debug(
                    f"🔧 Auto-continue: adding {new_msg_tokens} tool result tokens from state"
                )
            else:
        ...
        latest_msg_content = await asyncio.wait_for(
            threads_repo.get_latest_user_message(thread_id), timeout=5.0
        )
        query_time = (time.time() - start_time) * 1000
        if query_time > 500:
    ...
    async def _estimate_memory_context_tokens(
        self, llm_model: str, is_auto_continue: bool, memory_context: Optional[dict[str, Any]]
    ) -> int:
        ...
        memory_context_tokens = await asyncio.to_thread(
            token_counter, model=llm_model, messages=[memory_context]
        )
    ...
    async def apply_context_compression(
        ...
        if need_compression:
            ...
            logger.debug(
                f"⏱️ [TIMING] Context compression: {(time.time() - compress_start) * 1000:.1f}ms ({len(messages)} -> {len(compressed_messages)} messages)"
            )
            ...
            logger.debug(
                f"⏱️ [TIMING] Compression check: {(time.time() - compress_start) * 1000:.1f}ms"
            )
            return compressed_messages
    ...
    async def prepare_messages_with_caching(
        ...
        if len(messages_with_context) > 2:
            ...
            logger.debug(
                f"First message: Skipping caching and validation ({len(messages_with_context)} messages)"
            )
