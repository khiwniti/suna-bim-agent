        ...
        for tc in tool_calls:
            ...
            if isinstance(args, str):
                try:
                    ...
                    logger.warning(
                        f"Removing tool call {tc.get('id')}: invalid JSON - {str(e)[:50]}"
                    )
                ...
                try:
                    ...
                    logger.debug(
                        f"Normalized tool call {tc.get('id')}: converted dict arguments to JSON string"
                    )
                except (TypeError, ValueError) as e:
                    logger.warning(
                        f"Removing tool call {tc.get('id')}: failed to serialize dict arguments - {str(e)[:50]}"
                    )
            else:
                logger.warning(
                    f"Removing tool call {tc.get('id')}: unexpected arguments type {type(args)}"
                )

        if len(valid_tool_calls) != len(tool_calls) or needs_normalization:
            if len(valid_tool_calls) != len(tool_calls):
                logger.warning(
                    f"Filtered {len(tool_calls) - len(valid_tool_calls)} invalid tool calls from message"
                )
