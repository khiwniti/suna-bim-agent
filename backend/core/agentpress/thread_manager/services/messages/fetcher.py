...
class MessageFetcher:
    ...
    async def get_llm_messages(
        self, thread_id: str, lightweight: bool = False
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if not all_messages:
                ...
                if image_contexts:
                    logger.info(
                        f"🖼️ Found {len(image_contexts)} image_context messages (no regular messages)"
                    )
            ...
            if image_contexts:
                ...
                logger.info(
                    f"🖼️ Injected {len(image_contexts)} image_context messages into conversation"
                )
            ...
            logger.error(
                f"⏱️ Timeout getting messages for thread {thread_id} after {MESSAGE_QUERY_TIMEOUT}s"
            )
            raise
    ...
    def _inject_image_contexts(
        self, messages: list[dict[str, Any]], image_contexts: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    async def _fetch_from_db(
        self, thread_id: str, lightweight: bool, threads_repo, _time
    ) -> list[dict[str, Any]]:
        all_messages = []
        ...
        if lightweight:
            ...
            logger.info(
                f"📊 Lightweight message fetch completed: {elapsed:.0f}ms, {len(all_messages)} messages"
            )
        else:
            ...
            elapsed = (_time.time() - t0) * 1000
            logger.info(
                f"📊 Optimized message fetch completed: {elapsed:.0f}ms, {len(all_messages)} messages"
            )

        return all_messages

    def _parse_messages(
        self, all_messages: list[dict[str, Any]], lightweight: bool
    ) -> list[dict[str, Any]]:
        messages = []
    ...
    def _parse_single_message(
        ...
        if isinstance(content, str):
            try:
                ...
                if parsed_item.get("role") == "user":
                    ...
                    if isinstance(msg_content, str) and not msg_content.strip():
                        logger.warning(
                            f"Skipping empty user message {item['message_id']} from LLM context"
                        )
                        return None

            except json.JSONDecodeError:
                if is_compressed:
                    return {"role": "user", "content": content, "message_id": item["message_id"]}
                else:
            ...
            return {"role": "user", "content": str(content), "message_id": item["message_id"]}
