...
class ExecutionOrchestrator:
    ...
    async def execute_pipeline(
        ...
        if config.native_tool_calling:
            tool_schemas_task = asyncio.create_task(self._get_tool_schemas(tool_registry))
        ...
        (
            ...
            force_rebuild,
        ) = await self._prepare_messages_pipeline(
            thread_id=thread_id,
            llm_model=llm_model,
    ...
    async def _determine_model(
        self, registry_model_id: str, llm_model: str, thread_id: str, thread_has_images_func
    ) -> tuple[str, str]:
        from core.ai_models import model_manager
        from core.ai_models.registry import IMAGE_MODEL_ID

        if not model_manager.supports_vision(registry_model_id) and await thread_has_images_func(
            thread_id
        ):
    ...
    async def _prepare_messages_pipeline(
        ...
        if ExecutionConfig.ENABLE_PROMPT_CACHING and len(messages) > 2:
            if is_auto_continue and not force_rebuild:
                ...
                logger.debug(
                    f"⚡ [AUTO-CONTINUE] Fast path: {len(messages)} messages (skipped token counting)"
                )
            else:
    ...
    async def _fetch_and_refresh_messages(
        self, thread_id: str, messages: Optional[list[dict[str, Any]]], get_llm_messages_func
    ) -> list[dict[str, Any]]:
        if messages is None:
            fetch_start = time.time()
            messages = await get_llm_messages_func(thread_id)
            logger.debug(
                f"⏱️ [TIMING] get_llm_messages(): {(time.time() - fetch_start) * 1000:.1f}ms ({len(messages)} messages)"
            )
        ...
        logger.debug(
            f"⏱️ [TIMING] URL refresh check: {(time.time() - refresh_start) * 1000:.1f}ms ({refresh_count} refreshed)"
        )
    ...
    async def _validate_and_finalize_messages(
        ...
        (
            final_prepared_messages,
            actual_tokens,
        ) = await self.llm_executor.check_and_apply_late_compression(
        ...
        if estimated_total_tokens is not None:
            ...
            diff_pct = (
                (token_diff / estimated_total_tokens * 100) if estimated_total_tokens > 0 else 0
            )
            logger.info(
                f"📤 PRE-SEND: {len(final_prepared_messages)} messages, {actual_tokens} tokens (fast check: {estimated_total_tokens}, diff: {token_diff:+d} / {diff_pct:+.1f}%)"
            )
        else:
            logger.info(
                f"📤 PRE-SEND: {len(final_prepared_messages)} messages, {actual_tokens} tokens (no fast check available)"
            )
