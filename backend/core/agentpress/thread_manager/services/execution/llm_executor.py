...
class LLMExecutor:
    async def validate_and_repair_tool_calls(
        ...
        from core.agentpress.prompt_caching import (
            apply_anthropic_caching_strategy,
            validate_cache_blocks,
        )
        ...
        if force_tool_fallback:
            ...
            if messages is None:
                ...
                logger.debug(
                    f"⏱️ [TIMING] get_llm_messages() for fallback: {(time.time() - fetch_start) * 1000:.1f}ms"
                )
        ...
        is_valid, orphaned_ids, unanswered_ids = context_manager.validate_tool_call_pairing(
            prepared_messages
        )
        ...
        if not is_valid:
            ...
            is_valid_after, orphans_after, unanswered_after = (
                context_manager.validate_tool_call_pairing(prepared_messages)
            )

            if not is_valid_after:
                logger.error(
                    f"🚨 CRITICAL: Could not repair message structure. Orphaned: {len(orphans_after)}, Unanswered: {len(unanswered_after)}"
                )
                logger.error("🚨 Applying emergency fallback: stripping all tool content")
                prepared_messages = context_manager.strip_all_tool_content_as_fallback(
                    prepared_messages
                )
                applied_fallback = True
                ...
                if is_final_valid:
                    ...
                    logger.error(
                        "🚨 CRITICAL: Even fallback failed - proceeding anyway but LLM may error"
                    )
            else:
        ...
        is_ordered, out_of_order_ids, _ = context_manager.validate_tool_call_ordering(
            prepared_messages
        )
        if not is_ordered:
            logger.warning(
                f"⚠️ PRE-SEND ORDERING: Found {len(out_of_order_ids)} out-of-order tool call/result pairs"
            )

            if out_of_order_ids:
                await self._persist_ordering_repair(thread_id, out_of_order_ids)

            prepared_messages = context_manager.remove_out_of_order_tool_pairs(
                prepared_messages, out_of_order_ids
            )
            prepared_messages = context_manager.repair_tool_call_pairing(prepared_messages)

        logger.debug(
            f"⏱️ [TIMING] Pre-send validation: {(time.time() - validation_start) * 1000:.1f}ms"
        )
    ...
    async def _persist_orphan_repair(self, thread_id: str, orphaned_ids: list[str]) -> None:
        try:
            ...
            if marked_count > 0:
                logger.info(
                    f"✅ Persisted orphan repair: marked {marked_count} orphaned tool results as omitted in DB"
                )
    ...
    async def _persist_ordering_repair(self, thread_id: str, out_of_order_ids: list[str]) -> None:
        try:
            ...
            marked_count = await threads_repo.mark_tool_results_as_omitted(
                thread_id, out_of_order_ids
            )
            updated_count = await threads_repo.remove_tool_calls_from_assistants(
                thread_id, out_of_order_ids
            )

            if marked_count > 0 or updated_count > 0:
                logger.info(
                    f"✅ Persisted ordering repair: marked {marked_count} tool results as omitted, updated {updated_count} assistants"
                )
                await invalidate_message_history_cache(thread_id)
        except Exception as e:
    ...
    async def check_and_apply_late_compression(
        ...
        actual_tokens = await asyncio.to_thread(
            token_counter, model=llm_model, messages=prepared_messages
        )
        ...
        if actual_tokens >= safety_threshold:
            logger.warning(
                f"⚠️ PRE-SEND OVER THRESHOLD: actual={actual_tokens} >= threshold={safety_threshold}. Compressing now!"
            )
            ...
            logger.info(
                f"📤 POST-COMPRESSION: {len(prepared_messages)} messages, {actual_tokens} tokens"
            )
    ...
    async def execute(
        ...
        try:
            ...
            if not stream:
                logger.debug(
                    f"⏱️ [TIMING] LLM API call (non-streaming): {(time.time() - llm_call_start) * 1000:.1f}ms"
                )
            else:
                logger.debug(
                    f"⏱️ [TIMING] LLM API call initiated (streaming): {(time.time() - llm_call_start) * 1000:.1f}ms"
                )
