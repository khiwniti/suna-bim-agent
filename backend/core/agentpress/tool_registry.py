...
class ToolRegistry:
    ...
    def register_tool(
        self, tool_class: type[Tool], function_names: Optional[list[str]] = None, **kwargs
    ):
        import time
        ...
        if elapsed > 10:
            ...
            logger.debug(
                f"⏱️ [TIMING] register_tool({tool_class.__name__}): {elapsed:.1f}ms {cache_info}"
            )
    ...
    def get_openapi_schemas(self) -> list[dict[str, Any]]:
        ...
        for tool_name, tool_info in self.tools.items():
            if tool_info["schema"].schema_type == SchemaType.OPENAPI:
                ...
                is_mcp_by_instance = (
                    ...
                    and (
                        "MCPToolWrapper" in str(tool_instance.__class__.__name__)
                        or "MCP" in str(tool_instance.__class__.__name__)
                    )
                )
        ...
        logger.info(
            f"🎯 [HYBRID CACHE] Exposing {native_exposed} native tools, hiding {mcp_hidden} MCP tools (smart separation)"
        )
        return schemas
