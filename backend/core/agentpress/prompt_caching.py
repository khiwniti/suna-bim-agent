...
def get_message_token_count(
    message: dict[str, Any], model: str = "claude-3-5-sonnet-20240620"
) -> int:
...
def get_messages_token_count(
    messages: list[dict[str, Any]], model: str = "claude-3-5-sonnet-20240620"
) -> int:
    return sum(get_message_token_count(msg, model) for msg in messages)


def calculate_optimal_cache_threshold(
    context_window: int, message_count: int, current_tokens: int
) -> int:
    ...
    optimal_threshold = int(
        base_threshold * stage_multiplier * context_multiplier * density_multiplier
    )
    ...
    logger.debug(
        f"Factors - Stage: {stage_multiplier:.1f}, Context: {context_multiplier:.1f}, Density: {density_multiplier:.1f}"
    )
...
async def apply_caching_strategy(
    ...
    if not supports_prompt_caching(model_name):
        ...
        if len(filtered_conversation) < len(conversation_messages):
            logger.debug(
                f"Filtered out {len(conversation_messages) - len(filtered_conversation)} system messages"
            )
    ...
    if cache_threshold_tokens is None or should_recalculate:
        ...
        total_tokens = (
            token_counter(
                model=model_name, messages=[working_system_prompt] + conversation_messages
            )
        ...
        cache_threshold_tokens = calculate_optimal_cache_threshold(
            context_window_tokens, len(conversation_messages), total_tokens
        )
    ...
    logger.debug(
        f"Processing {len(conversation_messages)} messages ({total_conversation_tokens} tokens)"
    )
    ...
    if total_conversation_tokens <= max_cacheable_tokens:
        ...
        if max_conversation_blocks > 0:
            ...
            if optimal_chunk_size > cache_threshold_tokens * 1.8:
                ...
                logger.debug(
                    f"Redistributing cache blocks: {total_conversation_tokens} tokens across {max_conversation_blocks} blocks"
                )
        ...
        logger.warning(
            f"Conversation ({total_conversation_tokens} tokens) exceeds cache limit ({max_cacheable_tokens})"
        )
        prepared_messages.extend(conversation_messages)
    ...
    cache_count = sum(
        ...
        and "cache_control" in msg["content"][0]
    )

    logger.debug(
        f"Final structure: {cache_count} cache breakpoints, {len(prepared_messages)} total blocks"
    )
...
def group_messages_by_tool_calls_for_caching(
...
def create_conversation_chunks(
    ...
    logger.debug(
        f"Creating conversation chunks - chunk threshold: {chunk_threshold_tokens}, max blocks: {max_blocks}"
    )
    ...
    logger.debug(
        f"Grouped {len(messages)} messages into {len(message_groups)} atomic groups for caching"
    )
        ...
        if current_chunk_tokens + group_tokens > chunk_threshold_tokens and current_chunk_groups:
            if chunks_created < max_blocks:
                ...
                if valid_breakpoint_found:
                    ...
                    chunks_created += 1
                    logger.debug(
                        f"Block {chunks_created + 1}: Cached chunk ({current_chunk_tokens} tokens, {len(all_chunk_messages)} messages)"
                    )
...
def get_recent_messages_within_token_limit(
    messages: list[dict[str, Any]], token_limit: int, model: str = "claude-3-5-sonnet-20240620"
) -> list[dict[str, Any]]:
    if not messages:
        return []
...
def validate_cache_blocks(
    messages: list[dict[str, Any]], model_name: str, max_blocks: int = None
) -> list[dict[str, Any]]:
    if not supports_prompt_caching(model_name):
        return messages
    ...
    logger.warning(
        f"Cache validation failed: {cache_count} conversation blocks exceeds limit of {max_blocks}"
    )
    return messages
