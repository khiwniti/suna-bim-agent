...
class ContextManager:
    ...
    def __init__(self, token_threshold: int = DEFAULT_TOKEN_THRESHOLD, db=None):
        ...
        self.keep_recent_assistant_messages = (
            10  # Number of recent assistant messages to keep uncompressed
        )
    ...
    async def count_tokens(
            ...
            try:
                ...
                if bedrock_client:
                    ...
                    from core.ai_models.registry import (
                        KIMI_K2_PROFILE_ID,
                        SONNET_4_5_PROFILE_ID,
                        HAIKU_4_5_PROFILE_ID,
                        MINIMAX_M2_PROFILE_ID,
                    )
                    ...
                    if not bedrock_model_id:
                        ...
                        bedrock_model_id = model_id_mapping.get(
                            KIMI_K2_PROFILE_ID, "moonshot.kimi-k2-thinking"
                        )  # Default to Kimi K2
                    ...
                    if system_content:
                        ...
                        input_to_count["system"] = clean_content_for_bedrock(
                            system_to_count.get("content")
                        )

                    # Call Bedrock count_tokens API
                    response = bedrock_client.count_tokens(
                        modelId=bedrock_model_id, input={"converse": input_to_count}
                    )
        ...
        if system_to_count:
            return await asyncio.to_thread(
                token_counter, model=model, messages=[system_to_count] + messages_to_count
            )
        else:
            return await asyncio.to_thread(token_counter, model=model, messages=messages_to_count)

    async def estimate_token_usage(
        self, prompt_messages: list[dict[str, Any]], completion_content: str, model: str
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if completion_content:
                completion_tokens = await asyncio.to_thread(
                    token_counter, model=model, text=completion_content
                )

            total_tokens = prompt_tokens + completion_tokens

            logger.warning(
                f"⚠️ ESTIMATED TOKEN USAGE: prompt={prompt_tokens}, completion={completion_tokens}, total={total_tokens}"
            )
            ...
            try:
                ...
                completion_tokens = (
                    token_counter(model=model, text=completion_content) if completion_content else 0
                )
                ...
                fallback_prompt = (
                    len(" ".join(str(m.get("content", "")) for m in prompt_messages).split()) * 1.3
                )
                fallback_completion = (
                    len(completion_content.split()) * 1.3 if completion_content else 0
                )
    ...
    def group_messages_by_tool_calls(
        ...
        for msg in messages:
            ...
            if tool_call_ids:
                ...
                if tool_call_id and tool_call_id in expected_tool_call_ids:
                    ...
                    if tool_call_id:
                        logger.warning(
                            f"⚠️ Orphaned tool result detected: tool_call_id={tool_call_id} has no matching assistant message"
                        )
                ...
                if current_group:
                    if expected_tool_call_ids:
                        logger.warning(
                            f"⚠️ Closing tool call group with {len(expected_tool_call_ids)} missing tool results"
                        )
                    groups.append(current_group)
                    current_group = []
                    expected_tool_call_ids = set()
        ...
        if current_group:
            if expected_tool_call_ids:
                logger.warning(
                    f"⚠️ Final group has {len(expected_tool_call_ids)} missing tool results"
                )
            groups.append(current_group)
    ...
    def validate_tool_call_pairing(
        self, messages: list[dict[str, Any]]
        ...
        if orphaned_tool_result_ids:
            logger.error(
                f"🚨 VALIDATION FAILED: {len(orphaned_tool_result_ids)} orphaned tool results (no matching assistant): {orphaned_tool_result_ids}"
            )
        if unanswered_tool_call_ids:
            logger.error(
                f"🚨 VALIDATION FAILED: {len(unanswered_tool_call_ids)} unanswered tool calls (no matching tool result): {unanswered_tool_call_ids}"
            )
    ...
    def remove_orphaned_tool_results(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        if removed_count > 0:
            logger.info(
                f"🔧 Removed {removed_count} orphaned tool results to fix message structure"
            )
    ...
    def remove_unanswered_tool_calls(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for msg in messages:
            ...
            if tool_call_ids:
                ...
                unanswered = [
                    tc_id for tc_id in tool_call_ids if tc_id not in answered_tool_call_ids
                ]
                ...
                if unanswered:
                    ...
                    if not answered and not has_content:
                        ...
                        logger.warning(
                            f"🗑️ Removing assistant message with {len(unanswered)} unanswered tool_calls and no content: {unanswered}"
                        )
                        removed_count += 1
                        continue
                        ...
                        logger.warning(
                            f"🔧 Removing {len(unanswered)} unanswered tool_calls from assistant message (keeping content): {unanswered}"
                        )
                        ...
                        logger.warning(
                            f"🔧 Removed {len(unanswered)} unanswered tool_calls from assistant message (kept {len(answered)}): {unanswered}"
                        )
                        result.append(fixed_msg)
                        fixed_count += 1
                        continue
        ...
        if fixed_count > 0 or removed_count > 0:
            logger.info(
                f"🔧 Fixed {fixed_count} assistant messages, removed {removed_count} to fix unanswered tool_calls"
            )
    ...
    def repair_tool_call_pairing(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        if not is_valid:
            logger.error(
                f"🚨 CRITICAL: Could not fully repair message structure. Orphaned: {len(orphaned)}, Unanswered: {len(unanswered)}"
            )
        else:
    ...
    def validate_tool_call_ordering(
        self, messages: list[dict[str, Any]]
    ) -> tuple[bool, list[str], list[str]]:
        ...
        if not is_valid:
            logger.error(
                f"🚨 ORDERING VALIDATION FAILED: {len(out_of_order_tool_call_ids)} tool_calls have delayed results: {out_of_order_tool_call_ids}"
            )

        return is_valid, out_of_order_tool_call_ids, out_of_order_tool_result_ids

    def remove_out_of_order_tool_pairs(
        self, messages: list[dict[str, Any]], out_of_order_ids: list[str]
    ) -> list[dict[str, Any]]:
                ...
                if removed_ids:
                    ...
                    if not remaining_ids:
                        ...
                        new_tool_calls = [
                            ...
                            if tc.get("id") not in out_of_order_set
                        ]
    ...
    def strip_all_tool_content_as_fallback(
        self, messages: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    def remove_old_tool_outputs(
        self, messages: list[dict[str, Any]], keep_last_n: int = 8
    ) -> list[dict[str, Any]]:
        ...
        if not is_valid:
            logger.warning(
                f"⚠️ Input to remove_old_tool_outputs has pairing issues (orphaned: {len(orphaned_ids)}, unanswered: {len(unanswered_ids)}) - repairing first"
            )
        ...
        if total_tool_results <= keep_last_n:
            ...
            logger.debug(
                f"Only {total_tool_results} tool outputs found, keeping all uncompressed (threshold: {keep_last_n})"
            )
            return messages
        ...
        logger.debug(
            f"Compressing {num_to_compress} old tool outputs in-memory (keeping last {keep_last_n} of {total_tool_results} uncompressed)"
        )
        ...
        if not is_valid_after:
            logger.error(
                f"🚨 BUG: remove_old_tool_outputs broke tool call pairing! Orphaned: {orphaned_after}, Unanswered: {unanswered_after}"
            )

        return result

    def compress_user_messages_in_memory(
        self, messages: list[dict[str, Any]], keep_last_n: int = 10
    ) -> list[dict[str, Any]]:
        ...
        logger.debug(
            f"Compressing {num_to_compress} user messages in-memory (keeping last {keep_last_n})"
        )
    ...
    def compress_assistant_messages_in_memory(
        ...
        logger.debug(
            f"Compressing {num_to_compress} assistant messages in-memory (keeping last {keep_last_n})"
        )
    ...
    def compress_message(
        self, msg_content: str | dict, message_id: Optional[str] = None, max_length: int = 3000
    ) -> str | dict:
    ...
    async def compress_tool_result_messages(
        self,
        messages: list[dict[str, Any]],
        llm_model: str,
            ...
            for msg in reversed(messages):  # Start from the end and work backwards
                ...
                if self.is_tool_result_message(msg):  # Only compress ToolResult messages
                    ...
                    if msg_token_count > token_threshold:  # If the message is too long
                        if (
                            _i > self.keep_recent_tool_outputs
                        ):  # If this is not one of the most recent N ToolResult messages
                            message_id = msg.get("message_id")  # Get the message_id
                            if message_id:
                                msg["content"] = self.compress_message(
                            ...
                            msg["content"] = self.safe_truncate(
                                msg["content"], int(max_tokens_value * 2)
                            )
        return messages

    async def compress_user_messages(
        self,
        messages: list[dict[str, Any]],
        llm_model: str,
        max_tokens: Optional[int],
        ...
        if uncompressed_total_token_count > max_tokens_value:
            ...
            for msg in reversed(messages):  # Start from the end and work backwards
                ...
                if msg.get("role") == "user":  # Only compress User messages
                    ...
                    if msg_token_count > token_threshold:  # If the message is too long
                        if (
                            _i > self.keep_recent_user_messages
                        ):  # If this is not one of the most recent N User messages
                            message_id = msg.get("message_id")  # Get the message_id
                            if message_id:
    ...
    async def compress_assistant_messages(
        self,
        messages: list[dict[str, Any]],
        llm_model: str,
        max_tokens: Optional[int],
        ...
        if uncompressed_total_token_count > max_tokens_value:
            ...
            for msg in reversed(messages):  # Start from the end and work backwards
                ...
                if msg.get("role") == "assistant":  # Only compress Assistant messages
                    ...
                    if msg_token_count > token_threshold:  # If the message is too long
                        if (
                            _i > self.keep_recent_assistant_messages
                        ):  # If this is not one of the most recent N Assistant messages
                            message_id = msg.get("message_id")  # Get the message_id
                            if message_id:
                                # Secondary compression - just placeholder
                                msg["content"] = (
    ...
    async def compress_messages(
        ...
        if actual_total_tokens is not None:
            ...
            uncompressed_total_token_count = await self.count_tokens(
                llm_model, result, system_prompt, apply_caching=True
            )
        ...
        logger.info(
            f"Compression threshold: {max_tokens}, target: {target_tokens} (ratio: {self.compression_target_ratio})"
        )

        # Check if we're already under threshold - no compression needed!
        if uncompressed_total_token_count <= max_tokens:
            logger.info(
                f"✅ Token count ({uncompressed_total_token_count}) under threshold ({max_tokens}), skipping compression"
            )
        ...
        if uncompressed_total_token_count > max_tokens:
            logger.info(
                f"Context over limit ({uncompressed_total_token_count} > {max_tokens}), starting tiered compression..."
            )
            ...
            current_token_count = await self.count_tokens(
                llm_model, result, system_prompt, apply_caching=True
            )

            logger.info(
                f"After tool compression: {uncompressed_total_token_count} -> {current_token_count} tokens"
            )

            # Tier 2: Compress user messages if still above target
            if current_token_count > target_tokens:
                logger.info(
                    f"Still above target ({current_token_count} > {target_tokens}), compressing user messages..."
                )

                # Compress in-memory for this request
                result = self.compress_user_messages_in_memory(
            ...
            if current_token_count > target_tokens:
                logger.info(
                    f"Still above target ({current_token_count} > {target_tokens}), compressing assistant messages..."
                )

                # Compress in-memory for this request
                result = self.compress_assistant_messages_in_memory(
            ...
            logger.info(
                f"Tiered compression complete: {uncompressed_total_token_count} -> {current_token_count} tokens (target: {target_tokens})"
            )
        ...
        if uncompressed_total_token_count > target_tokens:
            logger.info(
                f"Applying secondary compression to reach target ({uncompressed_total_token_count} > {target_tokens})"
            )
            ...
            result = await self.compress_user_messages(
                result,
                llm_model,
                target_tokens,
                aggressive_threshold,
            ...
            result = await self.compress_assistant_messages(
                result,
                llm_model,
                target_tokens,
                aggressive_threshold,
            ...
            result = await self.compress_tool_result_messages(
                result, llm_model, max_tokens, token_threshold, uncompressed_total_token_count
            )
            result = await self.compress_user_messages(
                result, llm_model, max_tokens, token_threshold, uncompressed_total_token_count
            )
        ...
        compressed_total = await self.count_tokens(
            llm_model, result, system_prompt, apply_caching=True
        )

        if compressed_total != uncompressed_total_token_count:
            logger.info(
                f"Context compression: {uncompressed_total_token_count} -> {compressed_total} tokens (saved {uncompressed_total_token_count - compressed_total})"
            )
        else:
            logger.info(
                f"Context compression: {compressed_total} tokens (no compression needed, under threshold)"
            )

        # Recurse if still too large
        if max_iterations <= 0:
            logger.warning(f"Max iterations reached, omitting messages to target ({target_tokens})")
            result = await self.compress_messages_by_omitting_messages(
                result, llm_model, target_tokens, system_prompt=system_prompt
            )
            compressed_total = await self.count_tokens(
                llm_model, result, system_prompt, apply_caching=True
            )
            ...
            logger.info(
                f"Secondary compression didn't reach target ({compressed_total} > {target_tokens}). Using message omission to reach target."
            )
            result = await self.compress_messages_by_omitting_messages(
                result, llm_model, target_tokens, system_prompt=system_prompt
            )
        ...
        logger.info(
            f"✨ Final compression complete: {compressed_total} tokens (target: {target_tokens}, max: {max_tokens})"
        )
            ...
            if message_id and message_id in original_content_map:
                ...
                if current_content != original_content:
                    ...
                    compressed_to_save.append(
                        {"message_id": message_id, "compressed_content": json.dumps(msg_to_save)}
                    )
    ...
    async def compress_messages_by_omitting_messages(
        ...
        initial_token_count = await self.count_tokens(
            llm_model, result, system_prompt, apply_caching=True
        )
        ...
        logger.info(
            f"📦 Grouped {len(result)} messages into {len(message_groups)} atomic groups for compression"
        )
        ...
        while current_token_count > max_allowed_tokens and safety_limit > 0:
            ...
            if len(message_groups) <= min_groups_to_keep:
                logger.warning(
                    f"Cannot compress further: only {len(message_groups)} groups remain (min: {min_groups_to_keep})"
                )
                break
            ...
            if len(message_groups) > (removal_batch_size * 2):
                ...
                logger.debug(
                    f"Removing {len(removed_groups)} groups ({removed_msg_count} messages) from middle"
                )
                ...
                if groups_to_remove > 0:
                    ...
                    removed_msg_count = sum(len(g) for g in removed_groups)
                    logger.debug(
                        f"Removing {groups_to_remove} early groups ({removed_msg_count} messages)"
                    )
            ...
            current_token_count = await self.count_tokens(
                llm_model, conversation_messages, system_message, apply_caching=True
            )
                ...
                try:
                    ...
                    if omitted_tool_call_ids and self.thread_id:
                        ...
                        marked_count = await threads_repo.mark_tool_results_as_omitted(
                            self.thread_id, omitted_tool_call_ids
                        )
                        if marked_count > 0:
                            logger.info(
                                f"📝 Also marked {marked_count} orphaned tool results as omitted"
                            )
                except Exception as e:
        ...
        if not is_valid:
            logger.warning(
                f"⚠️ Post-compression validation found pairing issues (orphaned: {len(orphaned_ids)}, unanswered: {len(unanswered_ids)}) - repairing"
            )
        ...
        final_token_count = await self.count_tokens(
            llm_model, final_messages, system_message, apply_caching=True
        )

        logger.info(
            f"Context compression (omit): {initial_token_count} -> {final_token_count} tokens ({len(messages)} -> {len(final_messages)} messages, {len(message_groups)} groups)"
        )

        return final_messages

    async def middle_out_messages(
        self, messages: list[dict[str, Any]], max_messages: int = 320
    ) -> list[dict[str, Any]]:
        ...
        removed_groups = (
            message_groups[keep_start_groups:-keep_end_groups]
            if keep_end_groups > 0
            else message_groups[keep_start_groups:]
        )

        removed_count = len(message_groups) - len(kept_groups)
        if removed_count > 0:
            logger.info(
                f"📦 Middle-out: removed {removed_count} groups from middle ({len(message_groups)} -> {len(kept_groups)} groups)"
            )
        ...
        if not is_valid:
            logger.warning(
                f"⚠️ Middle-out validation found pairing issues (orphaned: {len(orphaned_ids)}, unanswered: {len(unanswered_ids)}) - repairing"
            )
