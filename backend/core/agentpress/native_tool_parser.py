        ...
        if hasattr(tool_call_chunk, "function"):
            ...
            if hasattr(tool_call_chunk.function, "arguments"):
                ...
                if isinstance(args, str):
                    ...
                    tool_call_data_chunk["function"]["arguments"] = json.dumps(
...
def is_tool_call_complete(tool_call_buffer_entry: dict[str, Any]) -> bool:
    ...
    if not (
        tool_call_buffer_entry.get("id")
        and tool_call_buffer_entry.get("function", {}).get("name")
        and tool_call_buffer_entry.get("function", {}).get("arguments")
    ):
        return False
...
def convert_to_exec_tool_call(
    tool_call: Any, raw_arguments_str: Optional[str] = None
) -> dict[str, Any]:
    """
    ...
    if isinstance(tool_call, dict):
        function_name = tool_call.get("function", {}).get("name") or tool_call.get(
            "function_name", "unknown"
        )
        tool_call_id = tool_call.get("id") or str(uuid.uuid4())
        if raw_arguments_str is None:
            raw_arguments_str = tool_call.get("function", {}).get("arguments") or tool_call.get(
                "raw_arguments", ""
            )
    else:
        # LiteLLM object
    ...
    return {
        ...
        if isinstance(raw_arguments_str, str)
        else json.dumps(raw_arguments_str, ensure_ascii=False),
        "source": "native",
    }


def convert_buffer_to_complete_tool_calls(
    ...
    for idx, tc_buf in tool_calls_buffer.items():
        if (
            tc_buf.get("id")
            and tc_buf.get("function", {}).get("name")
            and tc_buf.get("function", {}).get("arguments")
        ):
            ...
            try:
                ...
                if was_repaired:
                    try:
                        ...
                        pass
                logger.warning(
                    f"Tool call {tc_buf.get('id')} has invalid JSON arguments (repair failed), skipping: {str(e)[:100]}"
                )
                continue
...
def convert_to_unified_tool_call_format(
    tool_call: dict[str, Any], parse_arguments: bool = True
) -> dict[str, Any]:
        ...
        if tc_buf.get("function", {}).get("name"):
            ...
            if delta_mode and sent_lengths is not None:
                ...
                unified_tool_calls.append(
                    {
                        "tool_call_id": tc_buf.get(
                            "id", f"streaming_tool_{idx}_{str(uuid.uuid4())}"
                        ),
