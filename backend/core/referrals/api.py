...
async def refresh_referral_code(
    ...
    try:
        ...
        if not result.get("success"):
            raise HTTPException(
                status_code=400, detail=result.get("message", "Failed to refresh code")
            )
        ...
        return ReferralCodeResponse(referral_code=new_code, referral_url=referral_url)
...
async def get_referral_code(
    ...
    try:
        ...
        return ReferralCodeResponse(referral_code=referral_code, referral_url=referral_url)
    except Exception as e:
        ...
        if referrer_id:
            if referrer_id == user_id:
                return ValidateReferralCodeResponse(
                    valid=False, message="You cannot use your own referral code"
                )

            return ValidateReferralCodeResponse(
                valid=True, referrer_id=referrer_id, message="Valid referral code"
            )
        else:
            return ValidateReferralCodeResponse(valid=False, message="Invalid referral code")
    except Exception as e:
...
async def get_referral_stats(
    ...
    try:
        ...
        return ReferralStats(
            ...
            remaining_earnable_credits=Decimal(
                str(stats.get("remaining_earnable_credits", MAX_EARNABLE_CREDITS_FROM_REFERRAL))
            ),
            max_earnable_credits=Decimal(
                str(stats.get("max_earnable_credits", MAX_EARNABLE_CREDITS_FROM_REFERRAL))
            ),
...
async def get_user_referrals(
    ...
    try:
        ...
        return ReferralListResponse(referrals=referrals, total_count=len(referrals))
    except Exception as e:
