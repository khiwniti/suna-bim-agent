...
class ReferralService:
    ...
    async def expire_and_regenerate_code(self, user_id: str) -> Dict:
        ...
        try:
            ...
            response = await client.rpc("expire_referral_code", {"p_account_id": user_id}).execute()
    ...
    async def get_or_create_referral_code(self, user_id: str) -> str:
        try:
            ...
            result = await client.rpc(
                "get_or_create_referral_code", {"p_account_id": user_id}
            ).execute()
    ...
    async def validate_referral_code(self, code: str) -> Optional[str]:
        try:
            ...
            result = await client.rpc("validate_referral_code", {"p_code": code.upper()}).execute()
    ...
    async def check_total_earned_credits(self, user_id: str) -> Decimal:
        try:
            ...
            result = (
                await client.from_("referral_stats")
            ...
            logger.warning(
                f"Could not fetch total earned credits, defaulting to 0: {e}", user_id=user_id
            )
            return Decimal("0")
    ...
    async def process_referral(
        ...
        if actual_credits_to_award <= 0:
            return {"success": False, "message": "No credits to award", "credits_awarded": 0}
                ...
                if match:
                    try:
                        ...
                        logger.info(
                            ...
                            remaining_limit=MAX_EARNABLE_CREDITS_FROM_REFERRAL
                            - total_earned
                            - actual_credits_to_award,
                        )
                        return result
                    except json.JSONDecodeError:
                        pass
    ...
    async def get_referral_stats(self, user_id: str) -> Dict:
        try:
            ...
            result = await client.rpc("get_referral_stats", {"p_account_id": user_id}).execute()
            ...
            if result.data:
                ...
                stats["remaining_earnable_credits"] = float(
                    (MAX_EARNABLE_CREDITS_FROM_REFERRAL - total_earned) * CREDITS_PER_DOLLAR
                )
                stats["max_earnable_credits"] = float(
                    MAX_EARNABLE_CREDITS_FROM_REFERRAL * CREDITS_PER_DOLLAR
                )
                ...
                return {
                    ...
                    "remaining_earnable_credits": float(
                        MAX_EARNABLE_CREDITS_FROM_REFERRAL * CREDITS_PER_DOLLAR
                    ),
                    "max_earnable_credits": float(
                        MAX_EARNABLE_CREDITS_FROM_REFERRAL * CREDITS_PER_DOLLAR
                    ),
    ...
    async def get_user_referrals(
        ...
        try:
            ...
            result = await client.rpc(
                "get_user_referrals",
                {"p_account_id": user_id, "p_limit": limit, "p_offset": offset},
            ).execute()
    ...
    async def send_referral_email(self, user_id: str, email: str) -> Dict:
        try:
            ...
            result = await self.notification_service.send_referral_code_notification(
                recipient_email=email, referral_url=referral_url, inviter_id=user_id
            )
            ...
            logger.error(
                f"Error sending referral email: {e}", user_id=user_id, recipient_email=email
            )
    ...
    async def send_referral_emails(self, user_id: str, emails: List[str]) -> Dict:
        try:
            ...
            for email in emails:
                ...
                result = await self.notification_service.send_referral_code_notification(
                    recipient_email=email_clean, referral_url=referral_url, inviter_id=user_id
                )
