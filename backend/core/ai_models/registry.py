...
IMAGE_MODEL_ID = "carbon-bim/haiku"
...
class ModelFactory:
    ...
    def create_power_model(main_llm: str, custom_model: Optional[str] = None) -> Model:
        ...
        if main_llm == "kimi":
            return Model(
                id="carbon-bim/power",
                name="Carbon BIM Advanced Mode",
                litellm_model_id=custom_model or default_models["kimi"],
                provider=ModelProvider.OPENROUTER,
                aliases=[
                    "carbon-bim-power",
                    "Carbon BIM POWER Mode",
                    "Carbon BIM Power",
                    "Carbon BIM Advanced Mode",
                ],
                context_window=262_144,
                capabilities=[
                    ModelCapability.CHAT,
    ...
    def create_test_model() -> Model:
        return Model(
            id="carbon-bim/test",
            name="Carbon BIM Test",
            litellm_model_id="openrouter/minimax/minimax-m2.1",
            provider=ModelProvider.OPENROUTER,
            aliases=["carbon-bim-test", "Carbon BIM Test"],
            context_window=200_000,
            capabilities=[
                ModelCapability.CHAT,
                ModelCapability.FUNCTION_CALLING,
...
class ModelRegistry:
    ...
    def _register_pricing_mappings(self):
        ...
        self._litellm_id_to_pricing["openrouter/xiaomi/mimo-v2-flash"] = (
            PricingPresets.MIMO_V2_FLASH
        )
        ...
        self._litellm_id_to_pricing["bedrock/anthropic.claude-3-5-haiku-20241022-v1:0"] = (
            PricingPresets.HAIKU_3_5
        )
        self._litellm_id_to_pricing["openrouter/deepseek/deepseek-chat-v3-0324"] = (
            PricingPresets.DEEPSEEK_V3
        )
    ...
    def get_litellm_params(self, model_id: str, **override_params) -> Dict[str, Any]:
        model = self.get(model_id)
        if not model:
            return {"model": model_id, "num_retries": 5, **override_params}
    ...
    def select_best_model(
        ...
        if required_capabilities:
            models = [
                m for m in models if all(cap in m.capabilities for cap in required_capabilities)
            ]
        ...
        if prefer_cheaper and any(m.pricing for m in models):
            ...
            if models_with_pricing:
                models = sorted(
                    models_with_pricing, key=lambda m: m.pricing.input_cost_per_million_tokens
                )
        else:
            models = sorted(models, key=lambda m: (-m.priority, not m.recommended))
    ...
    def check_token_limit(
        self, model_id: str, token_count: int, is_input: bool = True
    ) -> Tuple[bool, int]:
        model = self.get(model_id)
        if not model:
            return False, 0
    ...
    def list_available_models(
        self, tier: Optional[str] = None, include_disabled: bool = False
    ) -> List[Dict[str, Any]]:
        if tier:
