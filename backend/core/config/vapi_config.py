...
class VoiceConfig:
    ...
    settings: Dict[str, Any] = field(
        default_factory=lambda: {"speed": 1.0, "stability": 0.5, "similarity_boost": 0.75}
    )
...
class VapiConfig:
    ...
    voice_options: Dict[str, Dict[str, str]] = field(
        default_factory=lambda: {
            "11labs": {"EXAVITQu4vr4xnSDxMaL": "Sarah/Bella (Female, Multilingual - Default)"},
            "azure": {
    ...
    model_options: Dict[str, list] = field(
        default_factory=lambda: {
    ...
    server_message_types: list = field(
        default_factory=lambda: [
            "conversation-update",
            "end-of-call-report",
            "function-call",
    ...
    def get_assistant_config(
        ...
        config = {
            ...
            "voice": {"provider": self.voice.provider, "voiceId": voice_id or self.voice.voice_id},
            "transcriber": {
...
MODEL_PROVIDERS = {"openai": "OpenAI", "anthropic": "Anthropic", "groq": "Groq"}
