...
def extract_agent_config(
    ...
    if os.getenv("ENV_MODE", "").upper() == "STAGING":
        print(
            f"[DEBUG] extract_agent_config: Called for agent {agent_id}, is_suna_default={is_suna_default}"
        )
        print(
            f"[DEBUG] extract_agent_config: Input agent_data has icon_name={agent_data.get('icon_name')}, icon_color={agent_data.get('icon_color')}, icon_background={agent_data.get('icon_background')}"
        )
...
def _extract_suna_agent_config(
    agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    ...
    config = {
        ...
        "restrictions": static_config.get(
            "restrictions",
            {
...
def _extract_custom_agent_config(
    ...
    if os.getenv("ENV_MODE", "").upper() == "STAGING":
        print(
            f"[DEBUG] _extract_custom_agent_config: Input agent_data has icon_name={agent_data.get('icon_name')}, icon_color={agent_data.get('icon_color')}, icon_background={agent_data.get('icon_background')}"
        )

    if version_data:
        # Use version configuration when available
        logger.debug(
            f"Using version data for custom agent {agent_id} (version: {version_data.get('version_name', 'unknown')})"
        )
        ...
        if os.getenv("ENV_MODE", "").upper() == "STAGING":
            print(
                f"[DEBUG] _extract_custom_agent_config: Returning config with icon_name={config.get('icon_name')}, icon_color={config.get('icon_color')}, icon_background={config.get('icon_background')}"
            )
    ...
    if os.getenv("ENV_MODE", "").upper() == "STAGING":
        print(
            f"[DEBUG] _extract_custom_agent_config: Fallback config with icon_name={fallback_config.get('icon_name')}, icon_color={fallback_config.get('icon_color')}, icon_background={fallback_config.get('icon_background')}"
        )
...
def _extract_agentpress_tools_for_run(agentpress_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for tool_name, enabled in agentpress_config.items():
        if isinstance(enabled, bool):
            run_tools[tool_name] = {"enabled": enabled, "description": f"{tool_name} tool"}
        elif isinstance(enabled, dict):
            run_tools[tool_name] = enabled
        else:
            run_tools[tool_name] = {"enabled": bool(enabled), "description": f"{tool_name} tool"}
