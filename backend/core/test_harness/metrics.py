...
class MetricsCollector:
    ...
    async def start_run(
        ...
        Args:
            ...
            model_name: Model being tested (e.g., 'carbon-bim/basic')
        ...
        logger.info(
            f"Created benchmark run: {run_id} (type: {run_type}, model: {model_name}, concurrency: {concurrency_level})"
        )

        return run_id

    async def record_prompt_result(self, run_id: str, result: BenchmarkResult):
        """
        ...
        logger.debug(
            f"Recorded result for prompt {result.prompt_id} in run {run_id}: {result.status}"
        )

    async def finalize_run(self, run_id: str, status: str = "completed"):
        """
        ...
        # Get run start time
        run_result = (
            await client.table("benchmark_runs")
            .select("started_at")
            .eq("id", run_id)
            .single()
            .execute()
    ...
    async def get_run_summary(self, run_id: str) -> Dict[str, Any]:
        ...
        # Get run metadata
        run_result = (
            await client.table("benchmark_runs").select("*").eq("id", run_id).single().execute()
        )
        ...
        if completed_results:
            durations = [
                r["total_duration_ms"] for r in completed_results if r.get("total_duration_ms")
            ]
            cold_starts = [
                r["cold_start_time_ms"] for r in completed_results if r.get("cold_start_time_ms")
            ]
            tool_call_times = [
                r["avg_tool_call_time_ms"]
                for r in completed_results
                if r.get("avg_tool_call_time_ms")
            ]
    ...
    async def list_runs(
        self, limit: int = 20, run_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        query = (
            client.table("benchmark_runs").select("*").order("created_at", desc=True).limit(limit)
        )
    ...
    def _calculate_tool_deviations(
        self, tool_call_breakdown: Dict[str, int], expected_tool_calls: Dict[str, int]
