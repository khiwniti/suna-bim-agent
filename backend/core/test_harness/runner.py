...
class TestHarnessRunner:
    ...
    def __init__(
        ...
        self.admin_api_key = admin_api_key or config.CARBON_BIM_ADMIN_API_KEY
    ...
    async def _ensure_test_user(self) -> str:
        ...
        Looks for user with email 'testuser@carbon-bim.ai' and creates if not exists.
        ...
        TEST_USER_EMAIL = "testuser@carbon-bim.ai"
        ...
        try:
            ...
            if result.data:
                ...
                logger.info(
                    f"✅ Using existing test user: {TEST_USER_EMAIL} (ID: {self.test_account_id})"
                )
            else:
                ...
                user_response = await client.auth.admin.create_user(
                    {
                        ...
                        "email_confirm": True,
                        "user_metadata": {"test_user": True, "created_by": "e2e_test_harness"},
                    }
                )
            ...
            if config.SYSTEM_ADMIN_USER_ID:
                logger.warning(
                    f"Falling back to SYSTEM_ADMIN_USER_ID: {config.SYSTEM_ADMIN_USER_ID}"
                )
    ...
    async def _execute_core_test(self, run_id: str, prompts: List, concurrency: int, model: str):
            ...
            async def execute_with_semaphore(prompt):
                async with semaphore:
                    ...
                    return await self.execute_single_prompt(
                        prompt=prompt, run_id=run_id, model=model
                    )
    ...
    async def run_core_test(
        ...
        logger.info(
            f"Starting core test: {len(prompts)} prompts, concurrency={concurrency}, model={model}"
        )
    ...
    async def _execute_stress_test(
        self, run_id: str, prompts: List, concurrency: int, num_executions: int
    ):
        """Internal method to execute stress test with MOCKED responses"""
        try:
            logger.info(
                f"🔥 STRESS TEST: {num_executions} executions with {concurrency} concurrency (MOCK MODE)"
            )
            logger.info(
                f"🔍 DEBUG: run_id={run_id}, active_runs={self._active_runs}, prompts_count={len(prompts)}"
            )
            ...
            if concurrency >= num_executions:
                ...
                async def execute_mock(idx):
                    ...
                    logger.debug(
                        f"🔍 Execution {idx} completed: {result.status if result else 'None'}"
                    )
                    return result
                ...
                results = await asyncio.gather(*tasks, return_exceptions=True)
                logger.info(
                    f"🔍 Gathered {len(results)} results. None count: {sum(1 for r in results if r is None)}"
                )
            else:
            ...
            if exception_count > 0:
                logger.warning(
                    f"Stress test had {exception_count} exceptions out of {num_executions} executions"
                )
                # Log first few exceptions for debugging
                for i, r in enumerate(results[:3]):
                    if isinstance(r, Exception):
                        logger.error(
                            f"Exception in stress test execution {i}: {type(r).__name__}: {r}",
                            exc_info=r,
                        )
    ...
    async def run_stress_test(
        ...
        logger.info(
            f"✅ About to create background task: run_id={run_id}, active_runs={self._active_runs}"
        )

        # Execute tests in background
        task = asyncio.create_task(
            self._execute_stress_test(run_id, prompts, concurrency, num_executions)
        )
    ...
    async def execute_single_prompt(
        self, prompt, run_id: str, model: str = "carbon-bim/basic"
        ...
        try:
            ...
            async with get_http_client() as client:
                ...
                if start_response.status_code != 200:
                    raise Exception(
                        f"Failed to start agent: {start_response.status_code} - {start_response.text}"
                    )
                ...
                async with client.stream(
                    "GET", stream_url, headers=auth_headers
                ) as stream_response:
                    if stream_response.status_code != 200:
                        raise Exception(
                            f"Failed to connect to stream: {stream_response.status_code}"
                        )
        ...
        if status == "completed" and thread_id and not tool_calls:
            try:
                ...
                if messages_response.data and len(messages_response.data) > 0:
                    ...
                    if db_tool_calls:
                        logger.debug(
                            f"Fetched {len(db_tool_calls)} tool calls from DB for thread {thread_id}"
                        )
        ...
        logger.debug(
            f"Prompt {prompt.id} completed: {status} ({total_duration_ms}ms, {tool_calls_count} tool calls)"
        )
