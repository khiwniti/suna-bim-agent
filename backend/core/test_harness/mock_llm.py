...
class MockLLMProvider:
    ...
    async def get_mock_response(
        ...
        for i, tool_call in enumerate(tool_calls):
            ...
            yield MockStreamChunk(
                choices=[MockChoice(delta=delta, finish_reason=None)], model=model
            )
    ...
    def _determine_tool_calls(
        ...
        # File operations
        if any(
            kw in prompt_lower for kw in ["file", "list", "create", "read", "write", "directory"]
        ):
        ...
        if any(
            kw in prompt_lower
            for kw in ["run", "execute", "command", "pwd", "echo", "date", "shell"]
        ):
            if self._has_tool(tools, "sb_shell_tool"):
                ...
                tool_calls.append({"name": "sb_shell_tool", "input": {"command": command}})
                ...
                tool_calls.append(
                    {
                        ...
                        "input": {"query": "mock search query", "max_results": 5},
                    }
                )
    ...
    def _generate_text_response(self, prompt: str, tool_calls: List[Dict[str, Any]]) -> str:
        """
        ...
        if not tool_calls:
            ...
            if len(prompt) < 10:
                ...
                return "I'm Carbon BIM, an AI assistant that can help you with various tasks including file operations, running commands, and searching the web."
            else:
    ...
    def get_mock_tool_result(self, tool_name: str, tool_input: Dict[str, Any]) -> str:
        ...
        if tool_name == "sb_files_tool":
            ...
            if action == "list":
                return json.dumps({"files": ["file1.py", "file2.py", "test.txt"], "count": 3})
            elif action == "write":
