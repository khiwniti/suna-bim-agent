...
class StartTestRequest(BaseModel):
    ...
    prompt_ids: Optional[List[str]] = Field(
        None, description="List of prompt IDs to test (None = all)"
    )
    concurrency: int = Field(1, description="Number of concurrent requests", ge=1, le=100)
    model: str = Field("carbon-bim/basic", description="Model to use (core_test only)")
    num_executions: int = Field(
        100, description="Number of executions (stress_test only)", ge=1, le=10000
    )
...
async def start_benchmark_test(request: StartTestRequest, _: bool = Depends(verify_admin_api_key)):
    """
    ...
    if request.prompt_ids:
        ...
        if invalid_prompts:
            raise HTTPException(status_code=400, detail=f"Invalid prompt IDs: {invalid_prompts}")

    logger.info(
        f"Starting {request.mode} test: concurrency={request.concurrency}, prompts={request.prompt_ids or 'all'}"
    )

    try:
        # Start test - it returns run_id immediately after creating DB record
        logger.info(
            f"Starting {request.mode} with prompts: {request.prompt_ids or 'all'}, cleanup: {request.cleanup_threads}"
        )
        ...
        return StartTestResponse(
            run_id=run_id, status="running", message="Test started successfully"
        )
...
async def get_test_run(run_id: str, _: bool = Depends(verify_admin_api_key)):
    """
...
async def list_test_runs(
    ...
    run_type: Optional[str] = Query(
        None, description="Filter by run type: 'core_test' or 'stress_test'"
    ),
...
async def cancel_test_run(run_id: str, _: bool = Depends(verify_admin_api_key)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to cancel test run: {str(e)}")


@router.post("/emergency-stop", summary="Emergency Stop All Tests")
async def emergency_stop_all_tests(_: bool = Depends(verify_admin_api_key)):
    """
    ...
    try:
        ...
        if not active_runs:
            return {"message": "No active test runs to stop", "cancelled_count": 0}
...
async def list_test_prompts(_: bool = Depends(verify_admin_api_key)):
    """
...
async def run_e2e_tests(
    test_filter: Optional[str] = Query(
        None,
        description="pytest filter expression (e.g., 'test_agents' or 'test_accounts::test_get_accounts')",
    ),
    _: bool = Depends(verify_admin_api_key),
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to run E2E tests: {str(e)}")
