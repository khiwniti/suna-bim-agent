...
try:
    ...
    GA_AVAILABLE = False
    logger.warning(
        "Google Analytics SDK not installed. Install with: pip install google-analytics-data"
    )
...
async def search_paid_subscriptions(
    start_date: datetime, end_date: datetime, include_emails: bool = False
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        logger.debug(
            f"Stripe subscription search: found {count} paid subscriptions between {start_date} and {end_date}"
        )
...
def query_google_analytics(date_str: str) -> dict[str, int]:
    ...
    # Build the request - no hostname filter needed (dedicated carbon-bim property)
    ...
    return {"pageviews": pageviews, "unique_visitors": unique_visitors}
...
async def query_vercel_analytics(date_str: str) -> dict[str, int]:
    ...
    try:
        ...
        result = await client.rpc("get_vercel_analytics", {"target_date": date_str}).execute()
...
async def query_vercel_analytics_range(start_date: str, end_date: str) -> dict[str, int]:
    ...
    try:
        ...
        result = await client.rpc(
            "get_vercel_analytics_range", {"start_date": start_date, "end_date": end_date}
        ).execute()
...
@router.get("/summary")
async def get_analytics_summary(admin: dict = Depends(require_admin)) -> AnalyticsSummary:
    ...
    try:
        ...
        conversion_rate_week = (
            (new_subscriptions_week / new_signups_week * 100) if new_signups_week > 0 else 0
        )
...
async def export_threads(
    ...
    sort_by: str = Query("created_at", description="Sort field"),
    sort_order: str = Query("desc", description="Sort order: asc, desc"),
    share_origin: Optional[str] = Query(
        None, description="Frontend origin used to build share links"
    ),
    admin: dict = Depends(require_admin),
):
...
async def _browse_threads_by_email(
    ...
    try:
        account_result = await client.rpc(
            "get_user_account_by_email", {"email_input": search_email}
        ).execute()
...
async def _browse_threads_filtered(
    ...
    if tier or category:
        ...
        count_result = await client.rpc(
            "get_threads_by_tier_and_category_count", count_params
        ).execute()
        ...
        logger.debug(
            f"Combined filter via RPC: {filter_desc}, page={params.page}, fetched {len(page_threads)} of {total_count} total"
        )
...
async def _enrich_threads(client, threads: list[dict]) -> list[ThreadAnalytics]:
    ...
    async def fetch_messages():
        ...
        result = (
            ...
            .in_("thread_id", threads_with_user_msgs)
            .eq("type", "user")
        ...
        if missing_account_ids:
            ...
            for acc in accounts_result.data or []:
                if acc.get("primary_owner_user_id"):
                    try:
                        email_result = await client.rpc(
                            "get_user_email", {"user_id": acc["primary_owner_user_id"]}
                        ).execute()
    ...
    messages_data, emails_data, projects_data = await asyncio.gather(
        fetch_messages(), fetch_emails(), fetch_projects()
    )
...
async def get_retention_data(
    ...
    min_weeks_active: int = Query(
        2, ge=1, description="Minimum weeks active to be considered recurring"
    ),
    admin: dict = Depends(require_admin),
    ...
    try:
        ...
        return await PaginationService.paginate_with_total_count(
            items=result, total_count=total_count, params=pagination_params
        )
...
async def get_retention_cohorts(
    ...
    try:
        ...
        rpc_result = await client.rpc(
            ...
            {"p_cohorts_back": cohorts_back, "p_weeks_to_measure": weeks_to_measure},
        ).execute()
        ...
        return CohortRetentionResponse(
            cohorts_back=cohorts_back, weeks_to_measure=weeks_to_measure, rows=result_rows
        )
    except Exception as e:
...
async def translate_text(
    request: TranslateRequest, admin: dict = Depends(require_admin)
) -> dict[str, str]:
...
async def get_message_distribution(
    date: Optional[str] = Query(
        None, description="Single date (deprecated, use date_from/date_to)"
    ),
    date_from: Optional[str] = Query(None, description="Start date YYYY-MM-DD"),
    date_to: Optional[str] = Query(None, description="End date YYYY-MM-DD"),
    admin: dict = Depends(require_admin),
    ...
    try:
        ...
        return {"distribution": distribution, "total_threads": data.get("total_threads", 0)}
...
async def get_category_distribution(
    ...
    try:
        ...
        result, count_result = await asyncio.gather(
            ...
            count_query.limit(1).execute()
            if not tier
            else asyncio.sleep(0),  # Skip count query if tier filter
        )
...
async def get_tier_distribution(
    ...
    try:
        ...
        result = await client.rpc(
            "get_thread_tier_distribution", {"start_date": start_of_range, "end_date": end_of_range}
        ).execute()
...
async def get_visitor_stats(
    ...
    source: AnalyticsSource = Query(
        "vercel", description="Analytics source: vercel (primary) or ga"
    ),
    admin: dict = Depends(require_admin),
) -> VisitorStats:
...
async def get_conversion_funnel(
    ...
    try:
        ...
        start_of_range = datetime(
            start_date.year, start_date.month, start_date.day, 0, 0, 0, tzinfo=UTC
        ).isoformat()
        end_of_range = datetime(
            end_date.year, end_date.month, end_date.day, 23, 59, 59, microsecond=999999, tzinfo=UTC
        ).isoformat()
        ...
        visitors, signups, subs_result = await asyncio.gather(
            get_visitors(), get_signups(), get_paid_subscribers()
        )
...
async def get_activation_stats(
    ...
    try:
        ...
        result = await client.rpc(
            "get_signup_activation_stats", {"date_from": start_of_range, "date_to": end_of_range}
        ).execute()
...
async def get_user_funnel(
    ...
    try:
        ...
        counts_result = await client.rpc(
            "get_free_signups_funnel_counts", {"date_from": start_of_range, "date_to": end_of_range}
        ).execute()
        ...
        viewed_then_clicked_rate = (
            (clicked_checkout / tried_and_viewed * 100) if tried_and_viewed > 0 else 0
        )
        clicked_then_converted_rate = (
            (converted / clicked_checkout * 100) if clicked_checkout > 0 else 0
        )
        ...
        try:
            ...
            result = await client.rpc(
                ...
                {"date_from": start_of_range, "date_to": end_of_range},
            ).execute()
...
async def get_signups_by_date(
    ...
    try:
        ...
        result = await client.rpc(
            "get_signups_by_date",
            {"start_date": f"{date_from}T00:00:00Z", "end_date": f"{date_to}T23:59:59.999999Z"},
        ).execute()

        # Transform to dict format
        signups_by_date = {row["signup_date"]: row["count"] for row in (result.data or [])}
...
async def get_new_paid_by_date(
    ...
    try:
        ...
        end_dt = datetime.strptime(date_to, "%Y-%m-%d").replace(
            hour=23, minute=59, second=59, tzinfo=BERLIN_TZ
        )
        ...
        while has_more:
            ...
            for sub in result.data:
                ...
                created_date = datetime.fromtimestamp(sub.created, tz=BERLIN_TZ).strftime(
                    "%Y-%m-%d"
                )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get new paid subscriptions from Stripe"
        )
    except Exception as e:
...
async def _fetch_churn_from_stripe(start_ts: int, end_ts: int) -> dict[str, dict[str, int]]:
    ...
    async def fetch_deleted_churns() -> dict[str, int]:
        ...
        while has_more:
            ...
            for event in result.data:
                ...
                is_paid = any(
                    (item.get("price", {}).get("unit_amount", 0) or 0) > 0 for item in items
                )

                if is_paid:
                    event_date = datetime.fromtimestamp(event.created, tz=BERLIN_TZ).strftime(
                        "%Y-%m-%d"
                    )
    ...
    async def fetch_downgrade_churns() -> dict[str, int]:
        ...
        while has_more:
            ...
            for event in result.data:
                ...
                current_amount = sum(
                    (item.get("price", {}).get("unit_amount", 0) or 0) for item in current_items
                )
                ...
                prev_items = (
                    prev_items_data.get("data", []) if isinstance(prev_items_data, dict) else []
                )

                prev_amount = sum(
                    (item.get("price", {}).get("unit_amount", 0) or 0) for item in prev_items
                )
    ...
    deleted_churns, downgrade_churns = await asyncio.gather(
        fetch_deleted_churns(), fetch_downgrade_churns()
    )
...
async def get_churn_by_date(
    ...
    try:
        ...
        cached_result = (
            await client.from_("arr_daily_churn")
            .select("churn_date, deleted_count, downgrade_count")
            .gte("churn_date", date_from)
            .lte("churn_date", end_date.isoformat())
            .execute()
        )
        ...
        if dates_to_fetch:
            ...
            fetch_start_dt = datetime.combine(fetch_start, datetime.min.time()).replace(
                tzinfo=BERLIN_TZ
            )
            fetch_end_dt = datetime.combine(
                fetch_end, datetime.max.time().replace(microsecond=0)
...
async def update_arr_weekly_actual(
    ...
    try:
        ...
        existing_result = (
            await client.from_("arr_weekly_actuals")
            .select("overrides")
        ...
        result = (
            await client.from_("arr_weekly_actuals")
            .upsert(upsert_data, on_conflict="week_number,platform")
            .execute()
        )
...
async def delete_arr_weekly_actual(
    ...
    try:
        ...
        await (
            client.from_("arr_weekly_actuals")
            .delete()
            .eq("week_number", week_number)
            .eq("platform", platform)
            .execute()
...
async def toggle_field_override(
    ...
    try:
        ...
        if not existing_result.data or len(existing_result.data) == 0:
            raise HTTPException(
                status_code=404, detail=f"Week {week_number} ({platform}) not found"
            )
...
async def update_arr_simulator_config(
    data: SimulatorConfigData, admin: dict = Depends(require_super_admin)
) -> SimulatorConfigData:
    ...
    try:
        ...
        if existing.data and len(existing.data) > 0:
            # Update existing row
            await (
                client.from_("arr_simulator_config")
                .update(update_data)
...
async def get_arr_monthly_actuals(
    ...
    try:
        ...
        result = (
            await client.from_("arr_monthly_actuals").select("*").order("month_index").execute()
        )
...
async def update_arr_monthly_actual(
    ...
    try:
        ...
        # Get existing overrides (if any) to merge with new ones
        existing_result = (
            await client.from_("arr_monthly_actuals")
            .select("overrides")
        ...
        await (
            client.from_("arr_monthly_actuals")
            .upsert(upsert_data, on_conflict="month_index,platform")
            .execute()
        )
...
async def toggle_monthly_field_override(
    ...
    try:
        ...
        if not existing_result.data or len(existing_result.data) == 0:
            raise HTTPException(
                status_code=404, detail=f"Month {month_index} ({platform}) not found"
            )
...
async def get_revenue_summary(admin: dict = Depends(require_super_admin)) -> RevenueSummary:
    """
    ...
    try:
        ...
        new_paid_query = (
            f"status:'active' AND -metadata['tier']:'free' AND created>={month_start_ts}"
        )
        ...
        while new_paid_result.has_more:
            new_paid_result = await stripe.Subscription.search_async(
                query=new_paid_query, limit=100, page=new_paid_result.next_page
            )
            new_paid_this_month += len(new_paid_result.data)
        ...
        churn_result = (
            await client.from_("arr_daily_churn")
            .select("deleted_count, downgrade_count")
            .gte("churn_date", month_start.strftime("%Y-%m-%d"))
            .execute()
        )
...
async def get_engagement_summary(
    ...
    try:
        ...
        week_start = datetime(
            end_date.year, end_date.month, end_date.day, 0, 0, 0, tzinfo=UTC
        ) - timedelta(days=6)
        ...
        metrics_result = await client.rpc(
...
async def get_task_performance(
    ...
    try:
        ...
        if result.data and len(result.data) > 0:
            ...
            total_runs = completed_runs = failed_runs = stopped_runs = running_runs = (
                pending_runs
            ) = 0
            avg_duration = None
        ...
        return TaskPerformance(
            ...
            avg_duration_with_stuck_seconds=round(avg_duration_with_stuck, 1)
            if avg_duration_with_stuck
            else None,
...
async def get_tool_adoption(
    ...
    try:
        ...
        messages_result = (
            await client.from_("messages")
            .select("thread_id, content, type")
            .gte("created_at", today_start.isoformat())
            .lt("created_at", today_end.isoformat())
            .in_("type", ["tool", "assistant"])
            .limit(5000)
            .execute()
        )
        ...
        tool_adoption_rate = (
            (len(threads_with_tools) / total_threads * 100) if total_threads > 0 else 0.0
        )
...
def _prune_stripe_revenue_cache(now: float) -> None:
    expired_keys = [
        key for key, entry in _stripe_revenue_cache.items() if entry.get("expires_at", 0) <= now
    ]
...
async def _fetch_stripe_revenue_for_window(
        ...
        try:
            ...
            def _get_tier_from_subscription(subscription) -> tuple[str, bool]:
                ...
                try:
                    ...
                    if items:
                        items_data = (
                            items.get("data", []) if hasattr(items, "get") else items["data"]
                        )
            ...
            for charge in all_charges:
                ...
                window_results["user_revenue"][customer_id] = (
                    window_results["user_revenue"].get(customer_id, 0) + amount
                )
            ...
            logger.error(
                f"Stripe API error fetching revenue for window (ts {window_start_ts}-{window_end_ts}): {e}"
            )
            raise
    ...
    async def _compute_revenue() -> dict[str, Any]:
        ...
        if failed_indices:
            ...
            for retry in range(2):
                ...
                retry_tasks = [
                    _fetch_stripe_revenue_for_window(*window_ranges[idx]) for idx in failed_indices
                ]
        ...
        for idx, window_results in enumerate(window_results_list):
            if isinstance(window_results, Exception):
                ...
                logger.error(
                    f"Error fetching Stripe window {idx} (ts {ts_start}-{ts_end}): {window_results}"
                )
                continue
...
async def _fetch_revenuecat_revenue(
    client, range_start: datetime, range_end: datetime
) -> dict[str, Any]:
    """
...
async def get_profitability(
    ...
    try:
        ...
        logger.info(
            f"Stripe revenue: ${stripe_revenue['total_revenue']:.2f} ({stripe_revenue['payment_count']} payments)"
        )
        logger.info(
            f"RevenueCat revenue: ${revenuecat_revenue['total_revenue']:.2f} ({revenuecat_revenue['payment_count']} payments)"
        )

        # 2. Get usage costs by tier using RPC (efficient single query instead of fetching 278k+ accounts)
        usage_costs_result = await client.rpc(
            "get_usage_costs_by_tier",
            {"start_date": range_start_utc.isoformat(), "end_date": range_end_utc.isoformat()},
        ).execute()
        ...
        all_tiers = (
            set(stripe_revenue["by_tier"].keys())
            | set(revenuecat_revenue["by_tier"].keys())
            | set(usage_costs_by_tier.keys())
        )
            ...
            if stripe_rev > 0:
                by_tier.append(
                    TierProfitability(
                        ...
                        total_cost=round(
                            cost_with_markup * (stripe_rev / tier_revenue)
                            if tier_revenue > 0
                            else 0,
                            2,
                        ),
                        total_actual_cost=round(
                            actual_cost * (stripe_rev / tier_revenue) if tier_revenue > 0 else 0, 2
                        ),
                        gross_profit=round(
                            stripe_rev
                            - (
                                actual_cost * (stripe_rev / tier_revenue) if tier_revenue > 0 else 0
                            ),
                            2,
                        ),
                        ...
                        avg_revenue_per_user=round(
                            stripe_rev / len(stripe_users) if stripe_users else 0, 2
                        ),
            ...
            if rc_rev > 0:
                by_tier.append(
                    TierProfitability(
                        ...
                        total_cost=round(
                            cost_with_markup * (rc_rev / tier_revenue) if tier_revenue > 0 else 0, 2
                        ),
                        total_actual_cost=round(
                            actual_cost * (rc_rev / tier_revenue) if tier_revenue > 0 else 0, 2
                        ),
                        gross_profit=round(
                            rc_rev
                            - (actual_cost * (rc_rev / tier_revenue) if tier_revenue > 0 else 0),
                            2,
                        ),
                        gross_margin_percent=round(gross_margin, 1),
                        avg_cost_per_user=round(avg_cost, 2),
        ...
        if total_revenue > 0:
            ...
            web_cost = (
                paid_cost * (web_revenue / total_revenue) + free_tier_cost
            )  # Add free tier to web
        ...
        unique_active_users = sum(
            t.get("user_count", 0) for t in usage_costs_by_tier.values()
        )  # Users who had usage

        # Per-paying-user metrics (consistent denominator for Revenue/Cost/Profit per user)
        avg_revenue_per_paid_user = (
            total_revenue / unique_paying_users if unique_paying_users > 0 else 0.0
        )
        avg_cost_per_active_user = (
            total_actual_cost / unique_paying_users if unique_paying_users > 0 else 0.0
        )
        ...
        paying_user_emails = [
            email
            for customer_id, email in stripe_user_emails.items()
...
async def get_conversation_insights(
    ...
    try:
        ...
        return ConversationInsight(
            ...
            intent_distribution=data.get(
                "intent_distribution",
                {"task": 0, "question": 0, "complaint": 0, "feature_request": 0},
            ),
        )
...
async def get_frustrated_conversations(
    ...
    try:
        ...
        data_query = (
            client.from_("conversation_analytics").select("*").gte("frustration_score", threshold)
        )
        ...
        result = (
            await data_query.order("frustration_score", desc=True)
        ...
        if account_ids:
            try:
                ...
                email_map = {
                    e["account_id"]: e["email"]
                    for e in (billing_result.data or [])
                    if e.get("email")
                }
        ...
        pagination_params = PaginationParams(page=page, page_size=page_size)
        return await PaginationService.paginate_with_total_count(
            items, total_items, pagination_params
        )
...
async def get_accounts_by_segment(
    segment: str = Query(
        ..., description="Segment name (champion, loyal, potential, at_risk, hibernating)"
    ),
    ...
    try:
        ...
        """
        rows = await execute(
            sql, {"days": days, "segment": segment, "page_size": page_size, "offset": offset}
        )
...
async def get_feature_requests(
    ...
    try:
        ...
        offset = (page - 1) * page_size
        data_query = (
            client.from_("conversation_analytics").select("*").eq("is_feature_request", True)
        )
        ...
        result = (
            await data_query.order("analyzed_at", desc=True)
            .range(offset, offset + page_size - 1)
            .execute()
        )
...
async def get_conversations_by_sentiment(
    ...
    try:
        ...
        offset = (page - 1) * page_size
        data_query = (
            client.from_("conversation_analytics").select("*").eq("sentiment_label", sentiment)
        )
...
async def get_conversations_by_intent(
    intent: str = Query(
        ..., description="Intent type: question, task, complaint, feature_request, chat"
    ),
    page: int = Query(1, ge=1, description="Page number"),
    page_size: int = Query(20, ge=1, le=100, description="Items per page"),
    date_from: Optional[str] = Query(None, description="Start date YYYY-MM-DD"),
    ...
    try:
        ...
        # Get paginated data
        offset = (page - 1) * page_size
        data_query = client.from_("conversation_analytics").select("*").eq("intent_type", intent)
...
async def get_conversations_by_category(
    ...
    try:
        ...
        offset = (page - 1) * page_size
        data_query = (
            client.from_("conversation_analytics").select("*").eq("use_case_category", category)
        )
...
async def _build_conversation_items(
    client, records: list[dict[str, Any]]
) -> list[ConversationAnalyticsItem]:
    """
        ...
        try:
            ...
            if msg_result.data:
                ...
                for msg in msg_result.data:
                    ...
                    if isinstance(content, dict):
                        content = (
                            content.get("content", "") or content.get("text", "") or str(content)
                        )
...
async def get_topic_distribution(
    ...
    try:
        ...
        # Build base query
        query = client.from_("conversation_analytics").select("primary_topic")
...
async def get_analytics_queue_status(admin: dict = Depends(require_super_admin)) -> dict[str, Any]:
    """
...
async def get_clustered_use_cases_endpoint(
    ...
    distance_threshold: float = Query(
        0.3, ge=0.1, le=0.8, description="Cosine distance threshold (lower = tighter clusters)"
    ),
