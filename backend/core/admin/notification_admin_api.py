...
async def list_workflows(admin: dict = Depends(require_admin)):
    try:
        ...
        if not result.get("success"):
            raise HTTPException(
                status_code=500, detail=result.get("error", "Failed to fetch workflows")
            )
...
async def trigger_workflow_admin(
    request: TriggerWorkflowRequest, admin: dict = Depends(require_admin)
):
    try:
        logger.info(
            f"[ADMIN] Triggering workflow: {request.workflow_id}, broadcast: {request.broadcast}, subscriber_id: {request.subscriber_id}, subscriber_email: {request.subscriber_email}"
        )
        ...
        if request.subscriber_id and request.subscriber_email:
            raise HTTPException(
                status_code=400, detail="Cannot specify both subscriber_id and subscriber_email"
            )
        ...
        if not result.get("success"):
            raise HTTPException(
                status_code=500, detail=result.get("error", "Failed to trigger workflow")
            )

        return {"success": True, "message": "Workflow triggered successfully", **result}
