    ...
    try:
        ...
        if not rpc_result.data:
            return await PaginationService.paginate_with_total_count(
                items=[], total_count=0, params=pagination_params
            )
        ...
        return await PaginationService.paginate_with_total_count(
            items=users, total_count=total_count, params=pagination_params
        )
    ...
    try:
        ...
        if not account.get("billing_customers") or not account["billing_customers"][0].get("email"):
            if account.get("primary_owner_user_id"):
                try:
                    user_email_result = await client.rpc(
                        "get_user_email", {"user_id": account["primary_owner_user_id"]}
                    ).execute()
        ...
        recent_activity = (
            ...
            .select("id, created_at, status, thread_id, threads!inner(account_id)")
        ...
        return {"user": account, "recent_activity": recent_activity.data or []}
...
async def get_user_stats_overview(admin: dict = Depends(require_admin)):
    ...
    try:
        ...
        total_users = (
            await client.schema("basejump").from_("accounts").select("*", count="exact").execute()
        )
        ...
        tier_distribution = [{"tier": tier, "count": count} for tier, count in tier_counts.items()]
        ...
        total_credits = sum(
            float(item.get("balance", 0)) for item in total_credits_result.data or []
        )
...
async def get_user_activity(
    ...
    try:
        ...
        base_query = (
            client.from_("agent_runs")
            .select("*, threads!inner(account_id, thread_id)")
            .eq("threads.account_id", user_id)
        )
        ...
        count_result = (
            await client.from_("agent_runs")
            .select("id, threads!inner(account_id)", count="exact")
            .eq("threads.account_id", user_id)
            .execute()
        )
        ...
        activity_result = (
            await activity_query.order("created_at", desc=True)
            .range(offset, offset + pagination_params.page_size - 1)
            .execute()
        )
            ...
            activities.append(
                {
                    ...
                    "credit_cost": float(
                        run.get("credit_cost", 0) if run.get("credit_cost") else 0
                    ),
                    "error": run.get("error"),
        ...
        return await PaginationService.paginate_with_total_count(
            items=activities, total_count=total_count, params=pagination_params
        )
...
async def get_user_threads_by_email(
    ...
    try:
        ...
        if not email_result.data:
            try:
                oauth_result = await client.rpc(
                    "get_account_by_email", {"search_email": email}
                ).execute()
        ...
        threads_result = (
            await client.from_("threads")
            .select("thread_id, project_id, is_public, created_at, updated_at")
            .eq("account_id", account_id)
        ...
        if not threads_result.data:
            return await PaginationService.paginate_with_total_count(
                items=[], total_count=total_count, params=pagination_params
            )
        ...
        return await PaginationService.paginate_with_total_count(
            items=threads, total_count=total_count, params=pagination_params
        )
...
async def admin_install_suna_for_user(
    account_id: str, replace_existing: bool = False, _: bool = Depends(verify_admin_api_key)
):
    ...
    if agent_id:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to install Suna agent for user {account_id}"
        )
...
def get_env_vars() -> dict[str, str]:
    ...
    if config.ENV_MODE != EnvMode.LOCAL:
        raise HTTPException(
            status_code=403, detail="Env vars management only available in local mode"
        )
