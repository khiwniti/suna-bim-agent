...
async def get_pool_health(admin: dict = Depends(require_admin)):
    try:
        ...
        if not config.enabled:
            ...
            health_status = "warning"
            issues.append(
                f"Pool size ({pool_size}) below replenish threshold ({config.replenish_below})"
            )
...
async def list_pooled_sandboxes(
    limit: int = Query(50, ge=1, le=100), admin: dict = Depends(require_admin)
):
    try:
        from core.sandbox import pool_repo
...
async def force_create_sandboxes(
    request_body: ForceCreateRequest, admin: dict = Depends(require_admin)
):
    try:
        from core.sandbox.pool_service import get_pool_service
        from core.sandbox import pool_repo
...
async def remove_from_pool(
    request_body: RemoveFromPoolRequest, admin: dict = Depends(require_admin)
):
    try:
        from core.sandbox import pool_repo
            ...
            try:
                ...
                if resource.get("status") != "pooled":
                    failed.append(
                        {
                            "id": sandbox_id,
                            "error": f"Status is {resource.get('status')}, not pooled",
                        }
                    )
                    continue
...
async def restart_pool_service(admin: dict = Depends(require_admin)):
    try:
        from core.sandbox.pool_background import (
            stop_pool_service,
            start_pool_service,
