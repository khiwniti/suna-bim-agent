...
#   STRESS_TEST_HOST_HEADER=api.yourdomain.com  (optional, defaults to 20.55.21.69)
STRESS_TEST_ALB_URL = os.getenv("STRESS_TEST_ALB_URL")
STRESS_TEST_HOST_HEADER = os.getenv("STRESS_TEST_HOST_HEADER", "20.55.21.69")
    ...
    try:
        async with httpx.AsyncClient(
            ...
            if data.get("timing_breakdown"):
                logger.info(
                    f"📊 Request {request_id} got timing_breakdown: {data['timing_breakdown']}"
                )
...
async def run_stress_test(
    ...
    logger.info(
        f"Admin stress test: {num_requests} requests, batch size {batch_size}, base_url={base_url}"
    )
    ...
    async def generate():
        ...
        summary = {
            ...
            "avg_request_time": round(sum(request_times) / len(request_times), 3)
            if request_times
            else 0,
        ...
        logger.info(
            f"Stress test completed: {len(successful)}/{num_requests} successful in {total_time:.2f}s"
        )
