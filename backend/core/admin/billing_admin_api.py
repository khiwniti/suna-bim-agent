...
async def adjust_user_credits(
    request: CreditAdjustmentRequest, admin: dict = Depends(require_admin)
):
    if abs(request.amount) > 1000 and admin.get("role") != "super_admin":
        raise HTTPException(
            status_code=403, detail="Adjustments over $1000 require super_admin role"
        )
    ...
    try:
        if request.amount > 0:
            ...
            if result.get("duplicate_prevented"):
                logger.info(
                    f"[ADMIN] Duplicate credit adjustment prevented for {request.account_id}"
                )
            ...
            if not result["success"]:
                raise HTTPException(
                    status_code=400, detail=result.get("error", "Insufficient balance")
                )
        ...
        logger.info(
            f"[ADMIN] Admin {admin['user_id']} adjusted credits for {request.account_id} by {request.amount} (expiring: {request.is_expiring})"
        )
...
@router.post("/refund")
async def process_refund(request: RefundRequest, admin: dict = Depends(require_super_admin)):
    result = await credit_manager.add_credits(
        account_id=request.account_id,
        amount=request.amount,
        is_expiring=request.is_expiring,
    ...
    logger.info(
        f"[ADMIN] Admin {admin['user_id']} processed refund of {request.amount} for user {request.account_id} (expiring: {request.is_expiring})"
    )
...
async def get_user_billing_summary(account_id: str, admin: dict = Depends(require_admin)):
...
async def get_user_transactions(
    ...
    try:
        ...
        transactions_data, total_count = await repo.get_user_transactions_paginated(
            account_id=account_id, page=page, page_size=page_size, type_filter=type_filter
        )
        ...
        return await PaginationService.paginate_with_total_count(
            items=transactions, total_count=total_count, params=pagination_params
        )
