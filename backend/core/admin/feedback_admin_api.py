...
async def get_sentiment_summary(admin: dict = Depends(require_admin)):
    try:
...
async def export_all_feedback(
    ...
    try:
        ...
        feedback_list = await feedback_repo.get_all_feedback(
            rating_filter=rating_filter, has_text=has_text, start_date=start_dt, end_date=end_dt
        )
...
async def analyze_feedback_with_llm(
    request: LLMAnalysisRequest, admin: dict = Depends(require_admin)
):
    try:
        min_rating = None
        max_rating = None
