...
class ComposioProfileService:
    ...
    async def _generate_unique_profile_name(
        self, base_name: str, account_id: str, mcp_qualified_name: str, client
    ) -> str:
    ...
    async def create_profile(
        ...
        try:
            logger.debug(
                f"Creating Composio profile for user: {account_id}, toolkit: {toolkit_slug}"
            )
            ...
            if unique_profile_name != profile_name:
                logger.debug(
                    f"Generated unique profile name: {unique_profile_name} (original: {profile_name})"
                )
    ...
    async def get_mcp_config_for_agent(self, profile_id: str, account_id: str) -> Dict[str, Any]:
        try:
            ...
            if not result.data:
                raise ValueError(
                    f"Profile {profile_id} not found or does not belong to account {account_id}"
                )
        ...
        try:
            ...
            if connected_account_id:
                ...
                if current_connected_account_id != connected_account_id or has_user_id:
                    ...
                    logger.info(
                        f"[MCP URL] Pinned connected_account_id for profile {profile_id}: {upgraded_url}"
                    )
                    return upgraded_url
    ...
    async def get_profiles(
        self, account_id: str, toolkit_slug: Optional[str] = None
