...
class ToolkitService:
    ...
    async def list_categories(self) -> List[CategoryInfo]:
        try:
            ...
            special_apps = ["googlesuper", "googleclassroom", "docusign"]
    ...
    async def list_toolkits(
        ...
        try:
            logger.debug(
                f"Fetching toolkits with limit: {limit}, cursor: {cursor}, category: {category}"
            )
            params = {"limit": limit, "managed_by": "composio"}
            ...
            for item in items:
                ...
                composio_managed_auth_schemes = toolkit_data.get(
            ...
            logger.debug(
                f"Successfully fetched {len(toolkits)} toolkits with OAUTH2 in both auth schemes"
    ...
    async def search_toolkits(
        ...
        try:
            all_toolkits_response = await self.list_toolkits(
                limit=500, cursor=cursor, category=category
            )
            ...
            logger.debug(
                f"Found {len(filtered_toolkits)} toolkits with OAUTH2 in both auth schemes matching query: {query}"
    ...
    async def get_detailed_toolkit_info(self, toolkit_slug: str) -> Optional[DetailedToolkitInfo]:
        try:
            ...
            logger.error(
                f"Failed to get detailed toolkit info for {toolkit_slug}: {e}", exc_info=True
            )
            return None

    async def get_toolkit_tools(
        self, toolkit_slug: str, limit: int = 50, cursor: Optional[str] = None
    ) -> ToolsListResponse:
        try:
            logger.debug(f"Fetching tools for toolkit: {toolkit_slug}")

            params = {"limit": limit, "toolkit_slug": toolkit_slug}
            ...
            for item in items:
                ...
                if isinstance(input_params_raw, dict):
                    input_parameters.properties = input_params_raw.get(
                        "properties", input_params_raw
                    )
                ...
                if isinstance(output_params_raw, dict):
                    output_parameters.properties = output_params_raw.get(
                        "properties", output_params_raw
                    )
            ...
            return ToolsListResponse(items=[], total_items=0, current_page=1, total_pages=1)
