...
class ConnectedAccountService:
    ...
    async def create_connected_account(
        self, auth_config_id: str, user_id: str, initiation_fields: Optional[Dict[str, str]] = None
    ) -> ConnectedAccount:
        try:
            ...
            if connection_data_obj and hasattr(connection_data_obj, "__dict__"):
                ...
                connection_data = ConnectionState(
                    auth_scheme=connection_data_dict.get("auth_scheme", "OAUTH2"), val=val_dict
                )
            else:
                connection_data = ConnectionState()
    ...
    async def get_connected_account(self, connected_account_id: str) -> Optional[ConnectedAccount]:
        try:
            ...
            logger.error(
                f"Failed to get connected account {connected_account_id}: {e}", exc_info=True
            )
            raise
    ...
    async def list_connected_accounts(
        self, auth_config_id: Optional[str] = None
