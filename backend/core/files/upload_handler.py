...
async def fast_parse_files(
    files: List[UploadFile], prompt: str = ""
) -> Tuple[str, List[Tuple[str, bytes, str, Optional[str]]]]:
    ...
    if prompt:
        ...
        for match in existing_matches:
            normalized = (
                match.replace("/workspace/", "") if match.startswith("/workspace/") else match
            )
    ...
    for file in files:
        ...
        try:
            ...
            if file_path.lower() not in existing_refs:
                file_refs.append(
                    f"[Attached: {original_filename} ({format_file_size(result.file_size)}) -> {file_path}]"
                )

            logger.debug(
                f"Fast-parsed {original_filename}: {result.char_count} chars, type={result.file_type.name}"
            )
...
async def upload_files_to_sandbox(
    ...
    try:
        ...
        sandbox_info = await resolve_sandbox(
            project_id=project_id, account_id=account_id, db_client=client, require_started=True
        )

        if not sandbox_info:
            logger.info(
                f"[UPLOAD] No sandbox for project {project_id} - files cached in Redis only"
            )
            return

        logger.info(
            f"[UPLOAD] Sandbox {sandbox_info.sandbox_id} ready, uploading {len(files_data)} files..."
        )
        ...
        for filename, content_bytes, mime_type, _ in files_data:
            try:
                unique_filename = await generate_unique_filename(
                    sandbox_info.sandbox, uploads_dir, filename
                )
                target_path = f"{uploads_dir}/{unique_filename}"

                if hasattr(sandbox_info.sandbox, "fs") and hasattr(
                    sandbox_info.sandbox.fs, "upload_file"
                ):
        ...
        logger.info(
            f"[UPLOAD] Complete: {uploaded_count}/{len(files_data)} files to sandbox {sandbox_info.sandbox_id}"
        )
...
async def ensure_sandbox_for_thread(client, project_id: str, files: Optional[List[Any]] = None):
    result = (
        await client.table("projects").select("account_id").eq("project_id", project_id).execute()
    )
