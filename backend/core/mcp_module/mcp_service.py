...
class MCPService:
    ...
    def _cleanup_old_servers(self) -> None:
        ...
        expired_names = [
            name
            for name, (_, created_at) in self._servers.items()
    ...
    async def execute_tool(
        ...
        request = ToolExecutionRequest(
            tool_name=tool_name, arguments=arguments, external_user_id=external_user_id
        )
        ...
        try:
            ...
            async with asyncio.timeout(30):
                async with streamablehttp_client(
                    server_info.server_url, headers=server_info.headers or {}
    ...
    async def discover_custom_tools(
        self, request_type: str, config: Dict[str, Any]
    ...
    async def _get_server_url(
        ...
        if provider in ["custom", "http", "sse"]:
            ...
            return await self._get_composio_server_url(
                qualified_name, config, account_id=account_id
            )
        else:
    ...
    async def _get_composio_server_url(
        self, qualified_name: str, config: Dict[str, Any], account_id: Optional[str] = None
    ) -> str:
        """Resolve Composio profile_id to actual MCP URL"""
        profile_id = config.get("profile_id")
        if not profile_id:
            raise MCPProviderError(
                f"profile_id not provided for Composio MCP server: {qualified_name}"
            )
