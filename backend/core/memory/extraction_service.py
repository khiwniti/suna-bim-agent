...
class MemoryExtractionService:
    def __init__(self, model: Optional[str] = None):
        self.model = model or config.MEMORY_EXTRACTION_MODEL or "carbon-bim/basic"
        self._client = None
    ...
    async def extract_memories(
        self, messages: List[Dict[str, Any]], account_id: str, thread_id: str
        ...
        try:
            ...
            try:
                ...
                if not worth_extracting:
                    logger.info(
                        f"LLM decided not to extract memories from thread {thread_id}: {reason}"
                    )
                    return []
    ...
    async def should_extract(self, messages: List[Dict[str, Any]], min_messages: int = 1) -> bool:
