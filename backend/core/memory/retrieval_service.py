...
class MemoryRetrievalService:
    ...
    async def retrieve_memories(
        self, account_id: str, query_text: str, tier_name: str, similarity_threshold: float = 0.1
    ) -> List[MemoryItem]:
        try:
            ...
            logger.debug(
                f"Memory retrieval RPC returned {len(result.data) if result.data else 0} results"
            )
            ...
            await Cache.set(
                cache_key, [self._memory_item_to_dict(m) for m in memories], ttl=self.cache_ttl
            )
    ...
    async def get_all_memories(
        ...
        try:
            ...
            result = await memory_repo.get_all_memories(
                account_id, limit, offset, memory_type.value if memory_type else None
            )
            ...
            return {"memories": memories, "total": result.get("total", 0)}
    ...
    async def get_memory_stats(self, account_id: str) -> Dict[str, Any]:
        try:
            ...
            result = await client.rpc("get_memory_stats", {"p_account_id": account_id}).execute()
