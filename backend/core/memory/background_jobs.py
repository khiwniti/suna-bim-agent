...
async def run_memory_extraction(
    ...
    try:
        ...
        messages_result = (
            await client.table("messages").select("*").in_("message_id", message_ids).execute()
        )
        ...
        extracted = await extraction_service.extract_memories(
            messages=messages_result.data, account_id=account_id, thread_id=thread_id
        )
    ...
    try:
        ...
        if current_count + len(to_insert) > max_memories:
            ...
            if old.data:
                ...
                await (
                    client.table("user_memories").delete().in_("memory_id", ids_to_delete).execute()
                )
...
def start_memory_embedding(
    account_id: str, thread_id: str, extracted_memories: List[Dict[str, Any]]
) -> None:
...
async def _embed_memories_wrapper(
    account_id: str, thread_id: str, extracted_memories: List[Dict[str, Any]]
):
    """Wrapper that checks ENABLE_MEMORY before dispatching."""
    from core.utils.config import config
