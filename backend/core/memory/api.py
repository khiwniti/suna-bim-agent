...
async def list_memories(
    ...
    if not config.ENABLE_MEMORY:
        return MemoryListResponse(memories=[], total=0, page=page, limit=limit, pages=0)
    ...
    try:
        ...
        return MemoryListResponse(
            memories=memories, total=total, page=page, limit=limit, pages=pages
        )
...
async def get_memory_stats(user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    if not config.ENABLE_MEMORY:
        try:
            tier_info = await subscription_service.get_user_subscription_tier(user_id)
            tier_name = tier_info["name"]
    ...
    try:
        ...
        memory_enabled_result = await client.rpc(
            "get_user_memory_enabled", {"p_account_id": user_id}
        ).execute()
        memory_enabled = (
            memory_enabled_result.data if memory_enabled_result.data is not None else True
        )
...
async def delete_memory(memory_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    if not config.ENABLE_MEMORY:
    ...
    try:
        ...
        if not is_memory_enabled(tier_name):
            raise HTTPException(
                status_code=403, detail="Memory feature not available for your tier"
            )
...
async def delete_all_memories(
    ...
    try:
        if not confirm:
            raise HTTPException(
                status_code=400,
                detail="Confirmation required to delete all memories. Set confirm=true query parameter.",
            )
        ...
        return {"message": "All memories deleted successfully", "deleted_count": deleted_count}
...
async def create_memory(
    memory_data: CreateMemoryRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    if not config.ENABLE_MEMORY:
        raise HTTPException(status_code=503, detail="Memory feature is currently disabled")
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Invalid memory type: {memory_data.memory_type}"
            )
...
async def get_memory_settings(user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    if not config.ENABLE_MEMORY:
...
async def update_memory_settings(
    enabled: bool = Body(..., embed=True), user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    if not config.ENABLE_MEMORY:
        raise HTTPException(status_code=503, detail="Memory feature is currently disabled")
    ...
    try:
        ...
        await client.rpc(
            "set_user_memory_enabled", {"p_account_id": user_id, "p_enabled": enabled}
        ).execute()
...
async def get_thread_memory_settings(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get thread memory settings: {str(e)}"
        )
...
async def update_thread_memory_settings(
    ...
    try:
        ...
        await client.rpc(
            "set_thread_memory_enabled", {"p_thread_id": thread_id, "p_enabled": enabled}
        ).execute()
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update thread memory settings: {str(e)}"
        )
