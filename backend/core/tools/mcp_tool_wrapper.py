...
class MCPSchemaRedisCache:
    ...
    async def get(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        try:
            ...
            if cached_data:
                logger.debug(
                    f"⚡ Redis cache hit for MCP: {config.get('name', config.get('qualifiedName', 'Unknown'))}"
                )
                return json.loads(cached_data)
            else:
                logger.debug(
                    f"Redis cache miss for MCP: {config.get('name', config.get('qualifiedName', 'Unknown'))}"
                )
                return None
    ...
    async def set(self, config: Dict[str, Any], data: Dict[str, Any]):
        try:
            ...
            logger.debug(
                f"✅ Cached MCP schema in Redis for {config.get('name', config.get('qualifiedName', 'Unknown'))} (TTL: {self._ttl}s)"
            )
...
class MCPToolWrapper(Tool):
    ...
    async def _ensure_initialized(self):
        if not self._initialized:
            try:
                ...
                logger.error(
                    f"Error during MCP server initialization: {e} (continuing with available servers)"
                )
            ...
            try:
                ...
                logger.error(
                    f"Error creating dynamic MCP tools: {e} (continuing with available tools)"
                )
    ...
    async def _initialize_servers(self):
        ...
        if cached_tools_data:
            logger.debug(
                f"⚡ Loaded {len(cached_configs)} MCP schemas from Redis cache: {', '.join(cached_configs)}"
            )
            for cached_data in cached_tools_data:
                try:
                    if cached_data.get("type") == "standard":
                        logger.debug(
                            "Standard MCP tools found in cache but require connection to restore"
                        )
        ...
        if initialization_tasks:
            logger.debug(
                f"🚀 Initializing {len(initialization_tasks)} MCP servers in parallel (cache enabled: {self.use_cache})..."
            )
            ...
            for i, result in enumerate(results):
                ...
                if isinstance(result, Exception):
                    ...
                    if result.get("success", False):
                        ...
                        config_name = result.get(
                            "config_name",
                            config.get("name", config.get("qualifiedName", "Unknown")),
                        )
                        logger.warning(
                            f"MCP server '{config_name}' failed to initialize: {error_msg} (continuing with other servers)"
                        )
                else:
                    # Unexpected result type, but don't fail
                    logger.warning(
                        f"Unexpected result type from MCP initialization: {type(result)}"
                    )
                    failed += 1

            elapsed_time = time.time() - start_time
            logger.debug(
                f"⚡ MCP initialization completed in {elapsed_time:.2f}s - {successful} successful, {failed} failed, {len(cached_configs)} from cache"
            )
        else:
            if cached_configs:
                elapsed_time = time.time() - start_time
                logger.debug(
                    f"⚡ All {len(cached_configs)} MCP schemas loaded from Redis cache in {elapsed_time:.2f}s - instant startup!"
                )
            else:
    ...
    async def _create_dynamic_tools(self):
        try:
            ...
            logger.debug(
                f"MCPManager returned {len(available_tools)} standard tools, Custom handler returned {len(custom_tools)} custom tools"
            )
            ...
            logger.debug(
                f"Created {len(self._dynamic_tools)} dynamic MCP tool methods with {len(self._schemas)} OpenAPI schemas"
            )
    ...
    def __getattr__(self, name: str):
        ...
        raise AttributeError(
            f"'{self.__class__.__name__}' object has no attribute '{name}'. Available MCP tools: {list(self._schemas.keys()) if hasattr(self, '_schemas') else 'not initialized'}"
        )
    ...
    def validate_tool_registration(self) -> Dict[str, Any]:
        ...
        validation_result = {
            ...
            "total_dynamic_tools": len(self._dynamic_tools)
            ...
            for tool_name, tool_data in self._dynamic_tools.items():
                ...
                if method_name:
                    if method_name in self._schemas:
                        ...
                        has_openapi = any(s.schema_type == SchemaType.OPENAPI for s in schema_list)
                        if has_openapi:
