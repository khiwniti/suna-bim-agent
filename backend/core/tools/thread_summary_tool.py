...
This allows you to continue working without losing important context.""",
    ...
    async def compress_thread_history(
        self, preserve_recent: int = 18, reason: Optional[str] = None
    ) -> ToolResult:
        """
        ...
        try:
            ...
            logger.info(
                f"[ThreadSummary] Starting compression for thread {thread_id}, reason: {reason or 'none'}"
            )
            ...
            logger.info(
                f"[ThreadSummary] Compressing {len(to_compress)} messages, keeping {len(working_memory)}"
            )
            ...
            logger.info(
                f"[ThreadSummary] Compression complete: {result.compressed_count} msgs → summary + facts"
            )
            ...
            logger.info(
                f"[ThreadSummary] Updated in-memory state: {len(list(state._messages))} messages"
            )
            ...
            state._pending_writes.append(PendingWrite(write_type="message", data=summary_data))

            # 2. Mark old messages as omitted
            for msg_id in compressed_ids:
                state._pending_writes.append(
                    PendingWrite(
                        write_type="message",
                        data={
                            "message_id": msg_id,
