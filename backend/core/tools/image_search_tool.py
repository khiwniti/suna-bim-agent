...
class SandboxImageSearchTool(SandboxToolsBase):
    ...
    def __init__(self, project_id: str, thread_manager: ThreadManager):
        ...
        if not self.serper_api_key:
            ...
            logger.warning(
                "SERPER_API_KEY not configured - Image Search Tool will not be available"
            )
    ...
    async def image_search(self, query: Union[str, List[str]], num_results: int = 12) -> ToolResult:
        """
        ...
        try:
            ...
            if not self.serper_api_key:
                return self.fail_response(
                    "Image Search is not available. SERPER_API_KEY is not configured."
                )
            ...
            if not self.serper_api_key:
                return self.fail_response(
                    "SERPER_API_KEY not configured. Image search is not available."
                )
            ...
            if is_batch:
                logging.info(
                    f"Executing batch image search for {len(queries)} queries with {num_results} results each"
                )
                # Batch API request
                payload = [{"q": q, "num": num_results} for q in queries]
            else:
                logging.info(
                    f"Executing image search for query: '{queries[0]}' with {num_results} results"
                )
                # Single API request
            ...
            async with get_http_client() as client:
                ...
                response = await client.post(
                    "https://google.serper.dev/images", json=payload, headers=headers, timeout=30.0
                )
            ...
            if is_batch:
                ...
                for i, (q, result_data) in enumerate(zip(queries, data)):
                    ...
                    batch_results.append(
                        {"query": q, "total_found": len(image_urls), "images": image_urls}
                    )
