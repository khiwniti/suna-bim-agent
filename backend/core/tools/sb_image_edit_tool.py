...
class SandboxImageEditTool(SandboxToolsBase):
    ...
    async def _get_user_tier(self, account_id: str) -> str:
        ...
        try:
            ...
            logger.warning(
                f"[IMAGE_TOOL] Failed to get tier for {account_id}: {e}, defaulting to 'none'"
            )
            return "none"
...
**ALWAYS match aspect_ratio to frame orientation!** Default is 1:1 if not specified.""",
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "canvas_path": {
                            "type": "string",
                            "description": "⚠️ REQUIRED when using frame_id! Path to canvas file. Example: 'canvases/instagram-promo.kanvax'",
                        },
                        "canvas_x": {
                            "type": "number",
                            "description": "X position on canvas in pixels. Ignored when frame_id is provided.",
                        },
                        "canvas_y": {
                            "type": "number",
                            "description": "Y position on canvas in pixels. Ignored when frame_id is provided.",
                        },
                        "frame_id": {
                            "type": "string",
                            "description": "Frame ID from add_frame_to_canvas. Image auto-centers and fits inside frame. ⚠️ MUST include canvas_path too!",
                        },
                    },
                    "required": [],
        ...
        try:
            ...
            if action:
                ...
                if mode not in ("generate", "edit", "upscale", "remove_bg", "video"):
                    return ToolResult(
                        success=True,
                        output=f"Invalid action '{action}'. Use: generate, edit, upscale, remove_bg, or video.",
                    )
            else:
            ...
            if mode in ("upscale", "remove_bg") and not image_path:
                return ToolResult(
                    success=True, output=f"'{mode}' action requires 'image_path' parameter."
                )
            if mode in ("generate", "edit", "video") and not prompt:
                return ToolResult(
                    success=True, output=f"'{mode}' action requires 'prompt' parameter."
                )
            ...
            if frame_id and not canvas_path:
                return ToolResult(
                    success=True,
                    output="ERROR: When using frame_id, canvas_path is REQUIRED! Example: canvas_path='canvases/my-design.kanvax'",
                )

            logger.info(
                f"Mode: {mode} (action={action}, image_path={image_path is not None}, video_options={video_options is not None}, frame_id={frame_id})"
            )
            ...
            if mode == "video" and user_tier in FREE_TIERS:
                logger.warning(
                    f"[MEDIA_BILLING] Video generation blocked for free user {account_id}"
                )
                return ToolResult(
                    success=True,
                    output="Video generation requires a paid subscription. Please upgrade your plan to access video generation features.",
                )
            ...
            if account_id and not use_mock:
                ...
                if not has_credits:
                    logger.warning(
                        f"[MEDIA_BILLING] Credit check failed for {account_id}: {credit_msg}"
                    )
            ...
            if is_batch:
                ...
                logger.info(
                    f"Executing batch image operation for {len(prompts)} prompts with mode '{mode}'"
                )
                ...
                if mode == "edit":
                    if not image_path:
                        return self.fail_response(
                            "'image_path' is required for edit mode when using batch prompts."
                        )
                ...
                for i, p in enumerate(prompts):
                    if mode == "edit":
                        ...
                        tasks.append(
                            self._execute_single_image_operation(
                                mode, p, img_path, use_mock, quality_variant, img_aspect_ratio
                            )
                        )
                    else:
                        tasks.append(
                            self._execute_single_image_operation(
                                mode, p, None, use_mock, quality_variant, img_aspect_ratio
                            )
                        )
                results = await asyncio.gather(*tasks, return_exceptions=True)
                elapsed_time = time.time() - start_time
                logger.info(
                    f"Batch image operation completed in {elapsed_time:.2f}s (concurrent execution)"
                )
                ...
                if canvas_info:
                    lines.append(
                        f"Added to canvas: {canvas_path} ({canvas_info['total_elements']} total elements)"
                    )
                if errors:
                ...
                if mode == "video":
                    ...
                    result = await self._execute_video_generation(
                        prompt, image_path, video_options, use_mock
                    )
                ...
                img_aspect_ratio = aspect_ratio or "1:1"
                logger.info(
                    f"Executing single image operation with mode '{mode}' for prompt: '{prompt[:50]}...' (quality={quality_variant}, aspect_ratio={img_aspect_ratio})"
                )

                result = await self._execute_single_image_operation(
                    mode, prompt, image_path, use_mock, quality_variant, img_aspect_ratio
                )
    ...
    def _get_replicate_token(self) -> str:
        ...
        if not token:
            raise Exception(
                "Replicate API token not configured. Add REPLICATE_API_TOKEN to your .env"
            )
    ...
    async def _execute_single_image_operation(
        ...
        try:
            if use_mock:
                logger.warning(
                    f"🎨 Image generation running in MOCK mode for prompt: '{prompt[:50]}...'"
                )
            ...
            if mode == "generate":
                logger.info(
                    f"Calling Replicate openai/gpt-image-1.5 for generation (quality={quality}, aspect_ratio={aspect_ratio})"
                )
                ...
                logger.info(
                    f"Calling Replicate openai/gpt-image-1.5 for editing (quality={quality}, aspect_ratio={aspect_ratio}) with image_path='{image_path}' (image size: {len(image_bytes)} bytes)"
                )
                # Wrap replicate.run() in thread pool to avoid blocking event loop
                output = await asyncio.to_thread(
                    replicate.run,
            ...
            output_list = (
                list(output)
                if hasattr(output, "__iter__") and not hasattr(output, "read")
                else [output]
            )
            ...
            error_message = str(e)
            logger.error(
                f"Error executing image operation for prompt '{prompt[:50]}...': {error_message}"
            )
    ...
    async def _execute_video_generation(
        self, prompt: str, image_path: Optional[str], video_options: Optional[dict], use_mock: bool
    ) -> str | ToolResult:
        """
        ...
        try:
            if use_mock:
                logger.warning(
                    f"🎬 Video generation running in MOCK mode for prompt: '{prompt[:50]}...'"
                )
            ...
            if image_path:
                ...
                if image_path:
                    ...
                    logger.info(
                        f"Video: Using input image_path='{image_path}' (size: {len(image_bytes)} bytes)"
                    )
            ...
            if opts.get("last_frame_image") and image_path:  # Only works with start image
                ...
                if not isinstance(last_frame_bytes, ToolResult):
                    ...
                    logger.info(
                        f"Video: Using last frame image (size: {len(last_frame_bytes)} bytes)"
                    )

            logger.info(f"Calling Replicate bytedance/seedance-1.5-pro for video generation")
            logger.debug(
                f"Video params: duration={input_params.get('duration')}, aspect_ratio={input_params.get('aspect_ratio')}, generate_audio={input_params.get('generate_audio')}, has_image={'image' in input_params}"
            )

            # Wrap replicate.run() in thread pool to avoid blocking event loop
            output = await asyncio.to_thread(
                replicate.run, "bytedance/seedance-1.5-pro", input=input_params
            )
            ...
            # Generate smart filename using LLM based on the prompt
            smart_filename = await generate_smart_filename(
                prompt=prompt, file_type="video", extension="mp4"
            )
            sandbox_path = f"{self.workspace_path}/{smart_filename}"
            await self.sandbox.fs.upload_file(result_bytes, sandbox_path)
    ...
    async def _execute_upscale_operation(self, image_path: str, use_mock: bool) -> str | ToolResult:
        """
    ...
    async def _execute_remove_bg_operation(
        self, image_path: str, use_mock: bool
    ) -> str | ToolResult:
        """
