...
IMPORTANT: For batch searches, pass query as a native array of strings, NOT as a JSON string.""",
    ...
    async def web_search(self, query: str | list[str], num_results: int = 5) -> ToolResult:
        """
        ...
        try:
            ...
            if is_batch:
                ...
                return ToolResult(
                    success=all_successful, output=json.dumps(batch_response, ensure_ascii=False)
                )
            else:
                ...
                if result.get("success", False):
                    return ToolResult(success=True, output=json.dumps(response, ensure_ascii=False))
                else:
                    logging.warning(f"No search results or answer found for query: '{query}'")
                    return ToolResult(
                        success=False, output=json.dumps(response, ensure_ascii=False)
                    )
    ...
    async def _execute_single_search(self, query: str, num_results: int) -> dict:
        ...
        try:
            ...
            logging.info(
                f"Retrieved search results for query: '{query}' - {len(results)} results, answer: {'yes' if answer else 'no'}, {len(enriched_images)} images enriched"
            )
    ...
    async def _enrich_images_with_metadata(self, images: list) -> list:
        ...
        logging.info(
            f"[WebSearch] Starting parallel image enrichment for {len(valid_images)} images"
        )
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                enriched.append(
                    {"url": valid_images[i], "width": 0, "height": 0, "description": ""}
                )
            else:
    ...
    async def _enrich_single_image(self, img_url: str, client) -> dict:
        ...
        """
        image_data = {"url": img_url, "width": 0, "height": 0, "description": ""}

        try:
            headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}

            response = await client.get(
                img_url, headers=headers, timeout=15.0, follow_redirects=True
            )
            ...
            if response.status_code == 200:
                ...
                if content_type.startswith("image/"):
                    ...
                    try:
                        ...
                        logging.debug(
                            f"[WebSearch] Image dimensions: {img_pil.width}x{img_pil.height}"
                        )
    ...
    async def scrape_webpage(self, urls: str, include_html: bool = False) -> ToolResult:
        ...
        try:
            ...
            if successful == len(results):
                ...
                message = (
                    f"Scraped {successful} URLs successfully and {failed} failed. Results saved to:"
                )
                for r in results:
                ...
                error_details = "; ".join(
                    [f"{r.get('url')}: {r.get('error', 'Unknown error')}" for r in results]
                )
                return self.fail_response(
