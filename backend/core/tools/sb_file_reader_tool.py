...
class SandboxFileReaderTool(SandboxToolsBase):
    ...
    async def _wait_for_uploads(self, timeout: int = 60) -> bool:
        ...
        while True:
            ...
            if elapsed >= timeout:
                logger.warning(
                    f"[FileReader] Timeout waiting for uploads to complete for project {self.project_id}"
                )
                return False
    ...
    async def _ensure_kb(self) -> bool:
        ...
        try:
            ...
            url = f"https://github.com/khiwniti/carbon-bim-agent/releases/download/v{KB_VERSION}/kb"
    ...
    async def _read_single_file(self, file_path: str) -> dict:
        try:
            ...
            if file_type == "pdf":
                ...
                result = await self.sandbox.process.exec(
                    f"pdftotext {escaped_path} - 2>/dev/null", timeout=60
                )
                if result.exit_code == 0 and result.result.strip():
                    ...
                    result = await self.sandbox.process.exec(
                        f"pdftotext -layout {escaped_path} - 2>/dev/null", timeout=60
                    )
                    if result.exit_code == 0 and result.result.strip():
                        content = result.result
                ...
                result = await self.sandbox.process.exec(
                    f"catppt {escaped_path} 2>/dev/null", timeout=60
                )
                if result.exit_code != 0:
                    return {
                        "file_path": cleaned_path,
            ...
            logger.info(
                f"[ReadFile] Read '{cleaned_path}' ({file_type}) via {extraction_method}, {len(content)} chars"
            )
        ...
        try:
            ...
            if file_paths:
                ...
                if len(file_paths) > MAX_BATCH_SIZE:
                    logger.warning(
                        f"[ReadFile] Batch limited to {MAX_BATCH_SIZE} files, {len(file_paths)} provided"
                    )
            elif file_path:
            ...
            logger.info(
                f"[ReadFile] Batch read: {len(successful)} success, {len(failed)} failed out of {len(paths_to_read)} files"
            )
    ...
    async def search_file(
        ...
        try:
            ...
            if file_paths:
                ...
                for candidate in default_paths:
                    check = await self.sandbox.process.exec(
                        f"test -e {shlex.quote(candidate)} && echo 'ok'"
                    )
            ...
            if file_paths or file_path:
                for p in paths:
                    check = await self.sandbox.process.exec(
                        f"test -e {shlex.quote(p)} && echo 'ok'"
                    )
            ...
            result = await self.sandbox.process.exec(
                cmd, env=env, cwd=self.workspace_path, timeout=120
            )
                ...
                for query_result in search_results:
                    ...
                    for hit in hits[:10]:
                        ...
                        if not content and hit:
                            logger.info(
                                f"[SearchFile] Hit has no content. Keys: {list(hit.keys())}, first 500 chars: {str(hit)[:500]}"
                            )
                        ...
                        formatted_results.append(
                            {
                                ...
                                if content
                                else "[No content - check hit structure]",
                            }
                        )
