...
class SandboxPresentationTool(SandboxToolsBase):
    ...
    def __init__(self, project_id: str, thread_manager: ThreadManager):
        ...
        self.templates_dir = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "templates", "presentations"
        )
    ...
    def _create_slide_html(
        self, slide_content: str, slide_number: int, total_slides: int, presentation_title: str
    ) -> str:
...
<body>
    ...
    def _extract_style_from_html(self, html_content: str) -> Dict:
        ...
        style_info = {"fonts": [], "colors": [], "layout_patterns": [], "key_css_classes": []}
        ...
        try:
            ...
            if not os.path.exists(self.templates_dir):
                return self.success_response(
                    {"message": "No templates directory found", "templates": []}
                )
            ...
            if not templates:
                return self.success_response({"message": "No templates found", "templates": []})
    ...
    async def load_template_design(
        self, template_name: str, presentation_name: Optional[str] = None
    ) -> ToolResult:
        ...
        try:
            ...
            if presentation_name:
                try:
                    presentation_path = await self._copy_template_to_workspace(
                        template_name, presentation_name
                    )
                except Exception as e:
            ...
            for slide_num, slide_data in sorted(
                metadata["slides"].items(), key=lambda x: int(x[0])
            ):
                slide_filename = slide_data.get("filename", f"slide_{int(slide_num):02d}.html")
            ...
            if not slides:
                return self.fail_response(
                    f"Could not load any slides from template '{template_name}'"
                )
            ...
            if presentation_path:
                ...
                response_data["note"] = (
                    "This template provides ALL slides and extracted design patterns in one response. Study the HTML and CSS to understand the design system, then create your own original slides with similar visual styling. To edit this template directly, provide a presentation_name parameter."
                )
    ...
    async def create_slide(
        ...
        **kwargs,
        ...
        print(
            f"[create_slide] ENTERED FUNCTION - presentation_name={repr(presentation_name)}, slide_number={repr(slide_number)}"
        )
        logger.info(
            f"[create_slide] Received params: presentation_name={repr(presentation_name)}, "
        ...
        try:
            ...
            if kwargs:
                logger.warning(
                    f"create_slide received unexpected arguments: {list(kwargs.keys())}. These will be ignored."
                )
        ...
        try:
            ...
            try:
                ...
                for file_info in files:
                    ...
                    is_dir = getattr(file_info, "is_dir", False) or getattr(
                    ...
                    if is_dir:
                        try:
                            ...
                            logger.warning(
                                f"Failed to load metadata for presentation '{file_info.name}': {str(e)}"
                            )
                            continue
                ...
                if any(
                    phrase in error_msg
                    for phrase in ["not found", "no such file", "does not exist", "doesn't exist"]
                ):
                    ...
                    logger.error(
                        f"Error listing presentations in {presentations_path}: {str(e)}",
                        exc_info=True,
                    )
    ...
    async def _export_to_format(
        ...
        try:
            convert_response = await client.post(
                ...
                json={"presentation_path": presentation_path, "download": not store_locally},
                timeout=180.0,
            )

            if not convert_response.is_success:
                error_detail = (
                    convert_response.json().get("detail", "Unknown error")
                    if convert_response.headers.get("content-type", "").startswith(
                        "application/json"
                    )
                    else convert_response.text
                )
                return {
                    "success": False,
                    "format": format_type,
                    "error": f"{format_type.upper()} conversion failed: {error_detail}",
                }
    ...
    async def export_presentation(
        self, presentation_name: str, store_locally: bool = True
    ) -> ToolResult:
        ...
        try:
            ...
            if not metadata.get("slides"):
                return self.fail_response(
                    f"Presentation '{presentation_name}' not found or has no slides"
                )
            ...
            if len(successes) == 2:
                response_data["message"] = (
                    f"Presentation '{presentation_name}' exported to PPTX and PDF successfully"
                )
                response_data["note"] = (
                    "Both files are stored in /workspace/downloads/ and can be downloaded repeatedly"
                )
            elif len(successes) == 1:
                response_data["message"] = (
                    f"Presentation '{presentation_name}' exported to {successes[0]} successfully. {errors[0] if errors else ''}"
                )
