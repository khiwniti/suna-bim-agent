...
class AgentCreationTool(Tool):
    ...
    async def create_new_agent(
        ...
        try:
            ...
            try:
                ...
                await (
                    client.table("agents")
                    .update({"current_version_id": version.version_id})
                ...
                success_message += (
                    f"**Tools Enabled**: {len([k for k, v in agentpress_tools.items() if v])}\n"
                )
                success_message += f"**MCPs Configured**: {len(configured_mcps)}\n\n"
                success_message += (
                    "The agent is now available in your agent library and ready to use!\n\n"
                )
    ...
    async def search_mcp_servers_for_agent(self, search_query: str) -> ToolResult:
        try:
            ...
            if not toolkits:
                return self.success_response(
                    {"message": f"No MCP servers found matching '{search_query}'", "toolkits": []}
                )
    ...
    async def get_mcp_server_details(self, toolkit_slug: str) -> ToolResult:
        try:
            ...
            if toolkit_data.auth_schemes:
                result_text += (
                    f"**Authentication Methods**: {', '.join(toolkit_data.auth_schemes)}\n"
                )
            ...
            return self.success_response(
                {
                    ...
                    "toolkit": {
                        ...
                        or f"Toolkit for {toolkit_data.name}",
    ...
    async def create_credential_profile_for_agent(
        ...
        try:
            ...
            success_message = (
                f"🔐 **AUTHENTICATION REQUIRED FOR {result.toolkit.name.upper()}**\n\n"
            )
            ...
            logger.error(
                f"Failed to create credential profile for '{toolkit_slug}': {e}", exc_info=True
            )
    ...
    async def discover_mcp_tools_for_agent(self, profile_name: str) -> ToolResult:
        try:
            ...
            result = await mcp_service.discover_custom_tools(
                request_type="http", config={"url": profile.mcp_url}
            )
            ...
            response_text += (
                f"Found **{len(available_tools)} tools** available for {profile.profile_name}:\n\n"
            )
    ...
    async def configure_agent_integration(
        ...
        try:
            ...
            requested_agent_id = (
                (agent_id.strip() or "default") if isinstance(agent_id, str) else "default"
            )
            ...
            success_message = (
                f"✅ Successfully configured {profile.toolkit_name} integration for agent!\n\n"
            )
            ...
            success_message += "\n\n"
            success_message += (
                f"The {profile.toolkit_name} integration is now active and ready to use!"
            )
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "name": {"type": "string", "description": "Name of the scheduled trigger"},
                        "description": {
                            "type": "string",
                        ...
                        "model": {
                            "type": "string",
                            "description": "Model to use for scheduled runs. Defaults to 'carbon-bim/basic'.",
                        },
                    },
    ...
    async def create_agent_scheduled_trigger(
        ...
        try:
            ...
            selected_model = model or "carbon-bim/basic"
            ...
            try:
                ...
                success_message = (
                    f"✅ Successfully created scheduled trigger '{name}' for agent!\n\n"
                )
                ...
                success_message += (
                    f"- Prompt: {agent_prompt[:50]}{'...' if len(agent_prompt) > 50 else ''}\n"
                )
                success_message += f"- Status: **Active**\n\n"
                success_message += (
                    f"The trigger is now active and will run according to the schedule."
                )
    ...
    async def toggle_agent_scheduled_trigger(
        self, agent_id: str, trigger_id: str, is_active: bool
    ) -> ToolResult:
        try:
            account_id = self.account_id
            if not account_id:
            ...
            if updated_config:
                ...
                if is_active:
                    success_message += (
                        "The trigger is now active and will run according to its schedule."
                    )
                else:
    ...
    async def update_agent_config(
        ...
        try:
            ...
            if agent_updates:
                await (
                    client.table("agents").update(agent_updates).eq("agent_id", agent_id).execute()
                )
            ...
            new_system_prompt = (
                ...
                else current_config.get("system_prompt", "")
            )
            new_model = model if model is not None else current_config.get("model")
            new_agentpress_tools = (
                agentpress_tools
                if agentpress_tools is not None
                else current_config.get("tools", {}).get("agentpress", {})
            )
            ...
            updated_agent_result = (
                await client.table("agents").select("*").eq("agent_id", agent_id).execute()
            )
            updated_agent = (
                updated_agent_result.data[0] if updated_agent_result.data else agent_data
            )
            ...
            success_message += (
                f"• Description: {updated_agent.get('description', 'No description')}\n"
            )
            success_message += f"• Icon: {updated_agent['icon_name']} ({updated_agent['icon_color']} on {updated_agent['icon_background']})\n"
            success_message += (
                f"• Default Agent: {'Yes' if updated_agent['is_default'] else 'No'}\n"
            )
            if version_changes:
                success_message += f"• Model: {new_model}\n"
                success_message += (
                    f"• Tools Enabled: {len([k for k, v in new_agentpress_tools.items() if v])}\n"
                )
