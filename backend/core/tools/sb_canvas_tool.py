...
class SandboxCanvasTool(SandboxToolsBase):
    ...
    async def create_canvas(
        self, name: str, description: Optional[str] = None, background: str = "#1a1a1a"
    ) -> ToolResult:
        """Create a new infinite canvas"""
        try:
            await self._ensure_sandbox()
            logger.info(
                f"[Canvas] Creating canvas '{name}' in sandbox {self.sandbox_id} for project {self.project_id}"
            )
                        ...
                        "elements": {
                            ...
                            "items": {
                                ...
                                "properties": {
                                    ...
                                    "src": {
                                        "type": "string",
                                        "description": "Image source path (for image type only)",
                                    },
                                    "x": {"type": "number"},
                                    "y": {"type": "number"},
                                    "width": {"type": "number"},
                                    ...
                                    "backgroundColor": {
                                        "type": "string",
                                        "description": "**FOR FRAMES ONLY** - Fill color in hex format (e.g. '#000000'). MUST preserve from list_canvas_elements!",
                                    },
                                },
                            },
                        },
    ...
    async def save_canvas(self, canvas_path: str, elements: List[Dict[str, Any]]) -> ToolResult:
        """
        ...
        async with canvas_lock:
            try:
                ...
                if frame_id:
                    ...
                    if not target_frame:
                        logger.warning(
                            f"Frame {frame_id} not found in canvas, using default positioning"
                        )
                ...
                if target_frame:
                    ...
                    img_aspect = (
                        actual_img_width / actual_img_height if actual_img_height > 0 else 1
                    )
...
**SIZES:** IG Story/Reel/TikTok=1080x1920, IG Post=1080x1080, YouTube=1280x720, Twitter=1200x675""",
    ...
    async def add_frame_to_canvas(
        ...
        try:
            ...
            return self.fail_response(
                "Invalid dimensions provided. Width and height must be numbers."
            )
        ...
        async with canvas_lock:
            try:
                ...
                if x == 100 and y == 100:
                    ...
                    existing_frames = [
                        el for el in canvas_data["elements"] if el.get("type") == "frame"
                    ]
                    if existing_frames:
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "x": {
                            "type": "number",
                            "description": "**OPTIONAL** - New X position in pixels.",
                        },
                        "y": {
                            "type": "number",
                            "description": "**OPTIONAL** - New Y position in pixels.",
                        },
                        "width": {
                            "type": "number",
                        ...
                        "opacity": {
                            "type": "number",
                            "description": "**OPTIONAL** - Opacity value between 0 and 1. Minimum: 0, Maximum: 1.",
                            "minimum": 0,
                            "maximum": 1,
                        },
                        "locked": {
                            "type": "boolean",
                            "description": "**OPTIONAL** - Lock element to prevent changes.",
                        },
    ...
    async def ai_process_canvas_element(
        self, canvas_path: str, element_id: str, action: Literal["upscale", "remove_bg"]
    ) -> ToolResult:
        """
        ...
        try:
            ...
            if action not in ("upscale", "remove_bg"):
                return self.fail_response(
                    f"Invalid action '{action}'. Use 'upscale' or 'remove_bg'."
                )
            ...
            if source_element.get("type") != "image":
                return self.fail_response(
                    f"Element '{element_id}' is not an image (type: {source_element.get('type')})"
                )
            ...
            # Process the image
            logger.info(
                f"[Canvas AI] Processing element '{source_element.get('name', element_id)}' with action '{action}'"
            )
