...
def normalize_phone_number(raw_number: str, default_region: str = "US") -> tuple[str, str, str]:
    ...
    if "+" in cleaned and not cleaned.startswith("+"):
        raise ValueError(
            f"Phone number '{original_input}' has '+' in wrong position (must be at start)"
        )

    max_length = 17
    if len(cleaned) > max_length:
        raise ValueError(
            f"Phone number '{original_input}' is too long ({len(cleaned)} digits, max {max_length})"
        )
    ...
    if len(digits_only) < min_length:
        raise ValueError(
            f"Phone number '{original_input}' is too short (minimum {min_length} digits required)"
        )
    ...
    if parsed_number is None:
        ...
        if len(parsing_errors) <= 5:
            error_context += f"\nParsing attempts:\n" + "\n".join(
                f"  - {err}" for err in parsing_errors[:5]
            )
        raise ValueError(error_context)

    if not phonenumbers.is_valid_number(parsed_number):
        logger.warning(
            f"Number '{original_input}' parsed but may not be fully valid (using: {successful_strategy})"
        )
...
def check_content_safety(text: str, context: str = "content") -> tuple[bool, Optional[str]]:
    ...
    if found_prohibited:
        ...
        return (
            False,
            f"Content contains prohibited keywords related to illegal, unethical, or emergency activities: {', '.join(found_prohibited[:3])}",
        )
    ...
    if len(found_sensitive) >= 2:
        logger.warning(
            f"Multiple sensitive keywords detected in {context}: {', '.join(found_sensitive)}"
        )
        return (
            False,
            f"Content appears to request sensitive personal or financial information, which may indicate a scam or fraud attempt: {', '.join(found_sensitive[:3])}",
        )
...
def validate_call_safety(
    phone_number: str, first_message: str, system_prompt: Optional[str]
    ...
    if is_emergency:
        ...
        return (
            False,
            f"❌ CALL BLOCKED: {emergency_reason}. Emergency numbers cannot be called through this system. For emergencies, dial directly from your phone.",
        )
...
class VapiVoiceTool(Tool):
    def __init__(self, thread_manager: ThreadManager):
        ...
        if not self.api_key:
            logger.warning(
                "VAPI_PRIVATE_KEY not configured - Voice Calls tool will not be available"
            )
        else:
    ...
    async def _get_current_thread_and_user(
        self,
        ...
        logger.info(
            f"[VapiVoiceTool] Final context - thread_id: {thread_id}, user_id: {user_id}, agent_id: {agent_id}"
        )
    ...
    async def make_phone_call(
        ...
        if not self.api_key:
            return self.fail_response(
                "VAPI_PRIVATE_KEY not configured. Please add your Vapi API key to the configuration."
            )
        ...
        try:
            ...
            is_safe_normalized, safety_message_normalized = validate_call_safety(
                normalized_phone, first_message, system_prompt
            )
            ...
            logger.info(
                f"[VapiVoiceTool] Creating call record - call_id: {call_id}, thread_id: {thread_id}, agent_id: {agent_id}"
            )
            ...
            try:
                result = (
                    ...
                    .upsert(call_record, on_conflict="call_id")
                    .execute()
                )
                if result.data:
                    logger.info(
                        f"Successfully created/updated call record in database for {call_id}"
                    )
                    logger.info(
                        f"Initial call record: {result.data[0] if result.data else 'No data returned'}"
                    )
                else:
                ...
                try:
                    ...
                    logger.info(
                        f"Created minimal call record via fallback for {call_id} with thread_id: {thread_id}"
                    )
            ...
            if thread_id:
                try:
                    ...
                    logger.info(
                        f"[VapiVoiceTool] Added call initiation message to thread {thread_id}"
                    )
                except Exception as e:
        ...
        try:
            async with get_http_client() as http_client:
                response = await http_client.get(
                    f"{self.base_url}/call/{call_id}", headers=self._get_headers(), timeout=30.0
                )
            ...
            while total_wait < max_wait_time:
                ...
                if result.data:
                    ...
                    if transcript_count > last_transcript_count and thread_id:
                        new_messages = (
                            transcript[last_transcript_count:]
                        ...
                        for msg in new_messages:
                            if isinstance(msg, dict) and msg.get("role") and msg.get("content"):
                                try:
                                    ...
                                    logger.warning(
                                        f"Failed to add transcript message to thread: {e}"
                                    )
                        last_transcript_count = transcript_count

                    if status in ["ended", "completed", "failed", "error"]:
                        logger.info(
                            f"[wait_for_call_completion] Call {call_id} has ended with status: {status}"
                        )
                        ...
                        if thread_id:
                            try:
                                ...
                                credits_deducted = (
                                    float(cost) * float(TOKEN_PRICE_MULTIPLIER) if cost else 0
                                )
...
The voice call has ended. You can continue with any follow-up actions."""
                    ...
                    logger.debug(
                        f"[wait_for_call_completion] Call {call_id} still active with status: {status}, checking again in {check_interval}s"
                    )
            ...
            return self.fail_response(
                f"Call monitoring timed out after {max_wait_time} seconds. Call may still be active."
            )
    ...
    async def list_calls(self, limit: int = 10) -> ToolResult:
        ...
        try:
            ...
            query = (
                client.table("vapi_calls").select("*").order("created_at", desc=True).limit(limit)
            )
            ...
            return self.success_response(
                {"calls": calls, "count": len(calls), "message": f"Retrieved {len(calls)} call(s)"}
            )
