    ...
    async def create_tasks(
        ...
        try:
                ...
                try:
                    ...
                    if was_repaired:
                        try:
                            ...
                            return ToolResult(
                                success=False, output=f"❌ Invalid JSON in sections: {str(e)}"
                            )
                    else:
                        return ToolResult(
                            success=False, output=f"❌ Invalid JSON in sections: {str(e)}"
            ...
            if sections is not None and not isinstance(sections, list):
                return ToolResult(
                    success=False,
                    output=f"❌ Sections must be an array, got {type(sections).__name__}",
                )
            ...
            if sections:
                for idx, section_data in enumerate(sections):
                    if not isinstance(section_data, dict):
                        return ToolResult(
                            success=False, output=f"❌ Section {idx} must be an object"
                        )
                    if "title" not in section_data or "tasks" not in section_data:
                        return ToolResult(
                            success=False, output=f"❌ Section {idx} missing 'title' or 'tasks'"
                        )
                    if not isinstance(section_data["tasks"], list):
                        return ToolResult(
                            success=False,
                            output=f"❌ Section '{section_data.get('title')}' tasks must be an array",
                        )
                ...
                try:
                    ...
                    if was_repaired:
                        try:
                            ...
                            return ToolResult(
                                success=False, output=f"❌ Invalid JSON in task_contents"
                            )
                    else:
            ...
            if sections:
                ...
                if not task_contents:
                    return ToolResult(
                        success=False,
                        output="❌ Provide 'sections' array OR 'task_contents' with section info",
                    )

                if not section_id and not section_title:
                    return ToolResult(
                        success=False,
                        output="❌ Specify 'section_id' or 'section_title' with 'task_contents'",
                    )
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "content": {"type": "string", "description": "New content for the task(s)"},
                        "status": {
                            "type": "string",
