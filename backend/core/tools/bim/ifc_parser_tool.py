...
class IFCParserTool(BIMToolBase):
        ...
        try:
            if not HAS_IFC:
                return self.success_response({
                    ...
                    "element_counts": {"IfcWall": 42, "IfcSlab": 18, "IfcDoor": 24, "IfcWindow": 16},
        ...
        try:
            if not HAS_IFC:
                return self.success_response({
                    ...
                    "count": 3,
                    "elements": [
                        {"express_id": 1, "global_id": "MOCK001", "type": ifc_type, "name": f"Mock {ifc_type} 1"},
                        {"express_id": 2, "global_id": "MOCK002", "type": ifc_type, "name": f"Mock {ifc_type} 2"},
                        {"express_id": 3, "global_id": "MOCK003", "type": ifc_type, "name": f"Mock {ifc_type} 3"},
                    ],
                })
    ...
    def _open_ifc_from_bytes(self, content: bytes):
        """Open an IFC model from raw bytes using a temp file."""
