...
class ExpandMessageTool(Tool):
    ...
    def _mcp_registry_out_of_sync(mcp_registry, mcp_loader) -> bool:
        ...
        for tool_name, loader_info in loader_tool_map.items():
            ...
            registry_profile_id = (registry_info.mcp_config.get("config", {}) or {}).get(
                "profile_id"
            )
    ...
    async def initialize_tools(self, tool_names: str | List[str]) -> ToolResult:
        ...
        activation_tasks = [
            JITLoader.activate_tool(
                tool_name, self.thread_manager, project_id, jit_config=jit_config
            )
        ...
        logger.info(
            f"⏱️ [INIT TOOLS] Parallel activation completed in {(time.time() - activation_start) * 1000:.1f}ms"
        )
        ...
        logger.info(
            f"✅ [INIT TOOLS] Returned {len(guides)} guide(s) in {total_time:.1f}ms, total size: {total_guide_size:,} chars"
        )
        logger.info(
            f"🎯 [INIT TOOLS] Tools now available for use: {[t for t in valid_tool_names if t not in activation_failures]}"
        )
    ...
    async def _discover_tools(self, filter: Optional[str] = None) -> ToolResult:
        ...
        if not loader_tool_map:
            logger.info(
                f"🔒 [MCP DISCOVERY] No MCP tools available for thread {self.thread_id}; returning empty discovery"
            )
            return self.success_response(
                {
                    "available_tools": {},
        ...
        async with _get_mcp_registry_lock():
            ...
            discovery_info = await mcp_registry.get_discovery_info(
                filter, load_schemas=True, account_id=account_id
            )

        logger.info(
            f"🔍 [MCP DISCOVERY] Found {discovery_info['total_count']} MCP tools across {len(discovery_info['toolkits'])} toolkits with full schemas"
        )
    ...
    async def _call_tool(self, tool_name: str, args: dict) -> ToolResult:
        ...
        if tool_name in native_tools:
            return self.fail_response(
                f"Tool '{tool_name}' is a native tool. Use initialize_tools to load {tool_name}_tool first, then use {tool_name} directly."
            )
        ...
        from core.agentpress.mcp_registry import (
            get_mcp_registry,
            MCPExecutionContext,
            init_mcp_registry_from_loader,
        )
    ...
    async def _save_dynamic_tools_to_metadata(self, new_tool_names: List[str]) -> None:
        ...
        try:
            ...
            if not result.data:
                logger.warning(
                    f"⚠️  [DYNAMIC TOOLS] Thread {self.thread_id} not found, cannot save tools"
                )
                return
            ...
            logger.info(
                f"💾 [DYNAMIC TOOLS] Saved {len(new_tool_names)} tools to thread metadata (total: {len(updated_tools)})"
            )
