        ...
        try:
            ...
            if not config.GEMINI_API_KEY:
                return self.fail_response(
                    "Vision tool is not available. GEMINI_API_KEY is not configured."
                )
            ...
            curl_cmd = (
                f"curl -s -X POST '{url}' -H 'Content-Type: application/json' -d '{json_data}'"
            )
            ...
            if response.exit_code == 0:
                try:
                    ...
                    if response_data.get("success"):
                        ...
                        if screenshot_base64:
                            ...
                            print(
                                f"[SeeImage] Converted SVG '{os.path.basename(svg_full_path)}' to PNG using sandbox browser"
                            )
                ...
                raise Exception(
                    f"Browser API call failed with exit code {response.exit_code}: {response.result}"
                )
    ...
    async def compress_image(
        ...
        try:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(
                                f"[SeeImage] Converted SVG '{file_path}' to PNG using fallback method (svglib)"
                            )
                        ...
                        raise Exception(
                            f"SVG conversion libraries not available. Cannot display SVG file '{file_path}'. Please convert to PNG manually."
                        )
                    except Exception as e:
                        raise Exception(
                            f"SVG conversion failed for '{file_path}': {str(e)}. Please convert to PNG manually."
                        )
            ...
            if mime_type == "image/gif":
                ...
                # Compress PNG
                img.save(
                    output, format="PNG", optimize=True, compress_level=DEFAULT_PNG_COMPRESS_LEVEL
                )
            ...
            if mime_type in ["image/jpeg", "image/png", "image/gif", "image/webp"]:
                print(
                    f"[SeeImage] Failed to compress image: {str(e)}. Using original (format is supported)."
                )
                return image_bytes, mime_type
            else:
                # Unsupported format and compression failed - must fail
                raise Exception(
                    f"Failed to process image '{file_path}' with unsupported format '{mime_type}': {str(e)}"
                )
...
Images remain in the sandbox and can be loaded again anytime. SVG files are automatically converted to PNG. **🚨 PARAMETER NAMES**: Use EXACTLY this parameter name: `file_path` (REQUIRED).""",
    ...
    async def load_image(self, file_path: str) -> ToolResult:
        ...
        try:
            ...
            if is_url:
                ...
                if not mime_type or not mime_type.startswith("image/"):
                    ...
                    if ext == ".jpg" or ext == ".jpeg":
                        ...
                        return self.fail_response(
                            f"Unsupported or unknown image format for file: '{cleaned_path}'. Supported: JPG, PNG, GIF, WEBP, SVG."
                        )

                original_size = file_info.size

            # Compress the image
            compressed_bytes, compressed_mime_type = await self.compress_image(
            ...
            if (
                mime_type == "image/svg+xml" or cleaned_path.lower().endswith(".svg")
            ) and compressed_mime_type == "image/png":
                ...
                try:
                    ...
                    print(
                        f"[SeeImage] Saved converted PNG to sandbox as '{png_filename}' for frontend display"
                    )
                except Exception as e:
            ...
            try:
                ...
                storage_response = await client.storage.from_("image-uploads").upload(
                    storage_filename, compressed_bytes, {"content-type": compressed_mime_type}
                )

                # Get public URL
                public_url = await client.storage.from_("image-uploads").get_public_url(
                    storage_filename
                )
                ...
                return self.fail_response(
                    f"Failed to upload image to cloud storage: {str(upload_error)}"
                )
            ...
            if current_image_count >= 3:
                ...
                logger.info(
                    f"[LoadImage] Auto-cleared {cleared} image(s) to make room (was {current_image_count}/3)"
                )
            ...
            logger.info(
                f"[LoadImage] Prepared '{cleaned_path}' for context (will be saved after tool result)"
            )
