    ...
    async def _check_stagehand_api_health(self) -> bool:
        ...
        try:
            ...
            for attempt in range(max_retries):
                ...
                curl_cmd = (
                    "curl -s -X GET 'http://localhost:8004/api' -H 'Content-Type: application/json'"
                )

                if attempt > 0:
                    logger.info(
                        f"Retrying Stagehand API health check (attempt {attempt + 1}/{max_retries})..."
                    )
                            ...
                            if response.exit_code == 0:
                                try:
                                    ...
                                    if init_result.get("status") == "healthy":
                                        ...
                                        logger.warning(
                                            f"Stagehand API initialization failed: {init_result}"
                                        )
                                        # Don't return False yet, might succeed on retry
                                except json.JSONDecodeError:
                                    logger.warning(
                                        f"Init endpoint returned invalid JSON: {response.result}"
                                    )
                            else:
                                logger.warning(
                                    f"Stagehand API initialization request failed: {response.result}"
                                )
                    except json.JSONDecodeError:
                        logger.warning(
        ...
        try:
            ...
            if not config.GEMINI_API_KEY:
                return self.fail_response(
                    "Browser tool is not available. GEMINI_API_KEY is not configured."
                )
            ...
            if response.exit_code == 0:
                try:
                        ...
                        try:
                            ...
                            if is_valid:
                                ...
                                image_url = await upload_base64_image(
                                    screenshot_data, "browser-screenshots"
                                )
                    ...
                    if result.get("image_upload_error"):
                        clean_result["screenshot_issue"] = (
                            f"Screenshot upload issue: {result['image_upload_error']}"
                        )
