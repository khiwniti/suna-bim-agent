...
- Use `replace_all` for replacing and renaming strings across the file (useful for renaming variables).""",
    ...
    async def str_replace(
        self, file_path: str, old_str: str, new_str: str, replace_all: bool = False
    ) -> ToolResult:
        try:
            # Ensure sandbox is initialized
            await self._ensure_sandbox()
            ...
            if occurrences > 1 and not replace_all:
                ...
                return self.fail_response(
                    f"Multiple occurrences found in lines {lines}. Use replace_all=true or provide more context to make it unique"
                )
    ...
    async def delete_file(self, file_path: str) -> ToolResult:
        try:
            ...
            if await self._file_exists(full_path):
                logger.warning(
                    f"File '{file_path}' still exists after delete_file call. Attempting alternative deletion method."
                )
                ...
                try:
                    ...
                    if result.exit_code != 0:
                        return self.fail_response(
                            f"Failed to delete file '{file_path}'. File still exists after deletion attempt. Exit code: {result.exit_code}, stderr: {result.stderr}"
                        )
                    # Verify again
                    await asyncio.sleep(0.1)
                    if await self._file_exists(full_path):
                        return self.fail_response(
                            f"Failed to delete file '{file_path}'. File still exists after deletion attempt."
                        )
                except Exception as shell_error:
                    return self.fail_response(
                        f"Failed to delete file '{file_path}'. File still exists and shell deletion also failed: {str(shell_error)}"
                    )
        ...
        try:
            ...
            if error_message:
                if is_tiptap_doc and original_wrapper:
                    logger.debug(
                        f"Morph AI edit failed for TipTap doc: {error_message}, attempting fallback manual update"
                    )
    ...
    async def export_to_pdf(self, file_path: str) -> ToolResult:
        ...
        try:
            ...
            if not cleaned_path.lower().endswith((".html", ".htm")):
                return self.fail_response(
                    f"File '{file_path}' is not an HTML file. Only .html/.htm files can be exported to PDF."
                )
            ...
            async with get_http_client() as client:
                response = await client.post(
                    ...
                    json={"file_path": cleaned_path, "file_name": base_name.replace("/", "_")},
                    timeout=120.0,
                )
