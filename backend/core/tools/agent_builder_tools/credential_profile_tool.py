...
class CredentialProfileTool(AgentBuilderBaseTool):
    ...
    async def create_credential_profile(
        self, toolkit_slug: str, profile_name: str, display_name: Optional[str] = None
    ) -> ToolResult:
        try:
            account_id = await self._get_current_account_id()
            ...
            logger.debug(
                f"Generated integration user_id: {integration_user_id} for account: {account_id}"
            )
            ...
            if result.connected_account.redirect_url:
                ...
                response_data[
                    "instructions"
                ] = f"""🔗 **{result.toolkit.name} Authentication Required**
...
After connecting, you'll be able to use {result.toolkit.name} tools in your agent."""
            ...
            logger.error(
                f"Error creating credential profile for '{toolkit_slug}': {e}", exc_info=True
            )
    ...
    async def configure_profile_for_agent(
        self, profile_id: str, enabled_tools: List[str], display_name: Optional[str] = None
    ) -> ToolResult:
        try:
            account_id = await self._get_current_account_id()
