...
class MCPSearchTool(AgentBuilderBaseTool):
    ...
    async def search_mcp_servers(
        ...
        try:
            ...
            if query:
                toolkits_response = await integration_service.search_toolkits(
                    query, category=category
                )
                toolkits = toolkits_response.get("items", [])
            else:
                toolkits_response = await toolkit_service.list_toolkits(
            ...
            if not formatted_toolkits:
                return ToolResult(success=False, output=json.dumps([], ensure_ascii=False))

            return ToolResult(
                success=True, output=json.dumps(formatted_toolkits, ensure_ascii=False)
            )
