...
class AgentConfigTool(AgentBuilderBaseTool):
            ...
            if is_suna_default:
                ...
                if restricted_fields:
                    return self.fail_response(
                        f"Cannot modify {', '.join(restricted_fields)} for Carbon BIM. "
                        f"Carbon BIM's core identity is centrally managed. You can still add MCPs and triggers."
                    )
            ...
            if config_changed:
                try:
                    ...
                    if agentpress_tools is not None:
                        ...
                        current_agentpress_tools = ensure_core_tools_enabled(
                            current_version.get("agentpress_tools", {})
                        )
    ...
    async def get_current_agent_config(self) -> ToolResult:
        try:
            ...
            return self.success_response({"summary": summary_text, "configuration": config_summary})
