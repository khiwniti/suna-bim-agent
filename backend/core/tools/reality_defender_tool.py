...
class RealityDefenderTool(SandboxToolsBase):
    ...
    def __init__(self, project_id: str, thread_manager: ThreadManager):
        ...
        if not self.api_key:
            logger.warning(
                "REALITY_DEFENDER_API_KEY not configured - Reality Defender Tool will not be available"
            )
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                async with session.get(url, headers=headers, allow_redirects=True) as response:
                    ...
                    if not media_type:
                        raise Exception(
                            f"Unsupported file type. URL must point to an image, audio, or video file."
                        )
                    ...
                    if len(file_bytes) > max_size:
                        raise Exception(
                            f"File is too large ({len(file_bytes) / (1024 * 1024):.2f}MB). Maximum size for {media_type} is {max_size / (1024 * 1024)}MB"
                        )
    ...
    async def get_file_from_sandbox(self, file_path: str) -> Tuple[bytes, str, Optional[str]]:
        ...
        if file_info.size > max_size:
            raise Exception(
                f"File '{cleaned_path}' is too large ({file_info.size / (1024 * 1024):.2f}MB). Maximum size for {media_type} is {max_size / (1024 * 1024)}MB"
            )
        ...
        try:
            ...
            if not self.api_key:
                return self.fail_response(
                    "Deepfake detection is not available. REALITY_DEFENDER_API_KEY is not configured."
                )
            ...
            if not media_type:
                return self.fail_response(
                    f"Could not determine media type for file: {cleaned_path}"
                )
            ...
            try:
                ...
                ext = os.path.splitext(cleaned_path)[1] or (
                    ".jpg"
                    if media_type == "image"
                    else (".mp3" if media_type == "audio" else ".mp4")
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Analyzing {media_type} file '{cleaned_path}' for deepfake detection..."
                    )
                    ...
                    if not request_id:
                        return self.fail_response(
                            "Failed to upload file to Reality Defender - no request_id returned"
                        )
                    ...
                    while elapsed < max_polling_time:
                        try:
                            ...
                            if not isinstance(result, dict):
                                return self.fail_response(
                                    f"Unexpected result format from Reality Defender: {type(result)}"
                                )
                            ...
                            if e.code == "not_found":
                                ...
                                if elapsed < max_polling_time - polling_interval:
                                    logger.warning(
                                        f"Reality Defender API returned error (will retry): {e}"
                                    )
                                    await asyncio.sleep(polling_interval)
                                    elapsed += polling_interval
                                    continue
                                    ...
                                    return self.fail_response(
                                        f"Reality Defender API timeout - analysis is taking longer than expected. Please try again later."
                                    )
                            else:
                    ...
                    if isinstance(models, list):
                        for model in models:
                            ...
                            try:
                                model_score = (
                                    float(model_score_value)
                                    if model_score_value is not None
                                    else 0.5
                                )
                            except (ValueError, TypeError):
                                model_score = 0.5
                    ...
                    logger.info(
                        f"Deepfake detection completed: verdict={verdict}, confidence={score:.3f}"
                    )
