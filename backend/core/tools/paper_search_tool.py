...
class PaperSearchTool(Tool):
    def __init__(self, thread_manager: ThreadManager):
        ...
        if self.api_key:
            ...
            logger.warning(
                "SEMANTIC_SCHOLAR_API_KEY not configured - Paper Search Tool will not be available"
            )
        ...
        try:
            ...
            for idx, paper in enumerate(results, 1):
                ...
                for author in paper.get("authors", []):
                    authors_list.append(
                        {"name": author.get("name", ""), "author_id": author.get("authorId", "")}
                    )
            ...
            logger.info(
                f"Successfully completed paper search with {len(formatted_results)} results"
            )
            ...
            return self.fail_response(
                "Paper search timed out. Please try again with a simpler query."
            )
    ...
    async def get_paper_details(
        self, paper_id: str, include_citations: bool = False, include_references: bool = False
    ) -> ToolResult:
        if not self.api_key:
            return self.fail_response(
    ...
    async def search_authors(self, query: str, limit: int = 10) -> ToolResult:
        if not self.api_key:
            return self.fail_response(
        ...
        try:
            ...
            logger.info(
                f"Successfully completed author search with {len(formatted_results)} results"
            )
            ...
            return self.fail_response(
                "Author search timed out. Please try again with a simpler query."
            )
        except Exception as e:
    ...
    async def get_author_details(
        self, author_id: str, include_papers: bool = False, papers_limit: int = 10
    ) -> ToolResult:
        if not self.api_key:
            return self.fail_response(
    ...
    async def get_author_papers(
        ...
        try:
            logger.info(
                f"Fetching papers for author: {author_id} (limit: {limit}, offset: {offset})"
            )
            ...
            logger.info(
                f"Successfully fetched {len(formatted_papers)} papers for author: {author_id}"
            )
