...
class SandboxKbTool(SandboxToolsBase):
    ...
    async def _ensure_kb_initialized(self) -> dict:
        ...
        try:
            ...
            if download_result.exit_code != 0:
                return {
                    "success": False,
                    "error": f"Failed to download kb: {download_result.result}",
                }
            ...
            if verify_result["exit_code"] != 0:
                return {
                    "success": False,
                    "error": f"kb installation verification failed: {verify_result['output']}",
                }
        ...
        try:
            ...
            if not queries:
                return self.fail_response(
                    "At least one query is required for search. Use 'query' for a single search or 'queries' for multiple searches."
                )
            ...
            if not init_result.get("success"):
                return self.fail_response(
                    f"Failed to initialize kb-fusion: {init_result.get('error', 'Unknown error')}"
                )
            ...
            check_path = await self.sandbox.process.exec(
                f"test -e {search_path} && echo 'exists' || echo 'not_found'"
            )
            ...
            try:
                ...
                if total_hits == 0:
                    response["note"] = (
                        "No results found. Files may still be indexing in the background. Use ls_kb to check which files are currently indexed."
                    )

                return self.success_response(response)
            except json.JSONDecodeError:
                # If parsing fails, just return raw results
                return self.success_response({"results": result["output"]})
    ...
    async def _download_with_retry(
        self, client, file_path: str, max_retries: int = 3
    ) -> Optional[bytes]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_error = e
                logger.warning(
                    f"[KB_SYNC] Download attempt {attempt + 1} failed for {file_path}: {e}"
                )
        ...
        if last_error:
            logger.error(
                f"[KB_SYNC] All {max_retries} download attempts failed for {file_path}: {last_error}"
            )
        return None

    async def _upload_file_with_retry(
        self, file_content: bytes, destination: str, max_retries: int = 3
    ) -> bool:
                ...
                if verify_result.exit_code == 0:
                    try:
                        ...
                        if actual_size == expected_size:
                            logger.info(
                                f"[KB_SYNC] Upload verified via shell: {destination} ({actual_size} bytes)"
                            )
                            return True
                        else:
                            logger.warning(
                                f"[KB_SYNC] Size mismatch for {destination}: expected {expected_size}, got {actual_size}"
                            )
                    except ValueError:
                        logger.warning(
                            f"[KB_SYNC] Could not parse size from: {verify_result.result}"
                        )
                else:
                    logger.warning(
                        f"[KB_SYNC] Shell verification failed for {destination}: {verify_result.result}"
                    )

                if attempt < max_retries - 1:
                    logger.info(
                        f"[KB_SYNC] Retrying upload for {destination} (attempt {attempt + 2}/{max_retries})"
                    )
                    await asyncio.sleep(0.5 * (attempt + 1))

            except Exception as e:
                logger.error(
                    f"[KB_SYNC] Upload attempt {attempt + 1} failed for {destination}: {e}",
                    exc_info=True,
                )
                if attempt < max_retries - 1:
                    await asyncio.sleep(0.5 * (attempt + 1))
    ...
    async def global_kb_sync(self) -> ToolResult:
        ...
        try:
            ...
            agent_id = (
                getattr(self.thread_manager, "agent_config", {}).get("agent_id")
                if hasattr(self.thread_manager, "agent_config")
                else None
            )
            if not agent_id:
            ...
            result = (
                await client.from_("agent_knowledge_entry_assignments")
                .select("""
                entry_id,
                enabled,
            ...
            if not result.data:
                ...
                if account_id:
                    ...
                    if total_entries > 0:
                        ...
                        assignments = (
                            await client.table("agent_knowledge_entry_assignments")
                ...
                logger.info(
                    f"[KB_SYNC] No files to sync for agent {agent_id}. Account {account_id} has {total_entries} total KB entries.{unassigned_info}"
                )
            ...
            for assignment in result.data:
                ...
                try:
                    ...
                    logger.info(
                        f"[KB_SYNC] Uploading {len(file_response)} bytes to: {file_destination}"
                    )

                    upload_success = await self._upload_file_with_retry(
                        file_response, file_destination
                    )
                    ...
                    synced_files += 1
                    logger.info(
                        f"[KB_SYNC] Successfully synced: {safe_folder_name}/{safe_filename}"
                    )
                    ...
                    logger.error(
                        f"[KB_SYNC] Error syncing {folder_name}/{filename}: {e}",
                        exc_info=True,
                    )
                    failed_files.append(
                        {"filename": filename, "folder": folder_name, "error": str(e)}
                    )
                    continue
...
"""
            ...
            if verify_ls.exit_code == 0 and verify_ls.result.strip():
                verified_files = [
                    f.strip() for f in verify_ls.result.strip().split("\n") if f.strip()
                ]
                logger.info(
                    f"[KB_SYNC] Final verification - files in KB directory: {verified_files}"
                )
            ...
            for expected_path in all_file_paths:
                if expected_path not in verified_files:
                    ...
                    logger.warning(
                        f"[KB_SYNC] Expected file not found in verification: {expected_path}"
                    )
            ...
            response = {
                "message": f"Synced {synced_files} files to knowledge base"
            ...
            if failed_files:
                ...
                response["failure_messages"] = [
                    f"{f['folder']}/{f['filename']}: {f['error']}" for f in failed_files
                ]
    ...
    async def global_kb_create_folder(self, name: str, description: str = None) -> ToolResult:
        ...
        try:
            ...
            final_name = FileNameValidator.generate_unique_name(
                sanitized_name, existing_names, "folder"
            )
            ...
            if final_name != sanitized_name:
                ...
                response_data["message"] = (
                    f"Successfully created folder '{sanitized_name}' as '{final_name}' (name auto-adjusted to avoid conflicts)"
                )
    ...
    async def global_kb_upload_file(
        self, sandbox_file_path: str, folder_name: str, description: str = None
    ) -> ToolResult:
        ...
        try:
            ...
            if not folder_result.data:
                return self.fail_response(
                    f"Folder '{folder_name}' not found. Create it first with global_kb_create_folder."
                )
            ...
            if current_total + len(file_content) > MAX_TOTAL_SIZE:
                ...
                return self.fail_response(
                    f"File size limit exceeded. Current: {current_mb:.1f}MB, New: {new_mb:.1f}MB, Limit: 50MB"
                )

            # Generate unique filename if there's a conflict
            from core.knowledge_base.validation import (
            ...
            if final_filename != filename:
                ...
                response_data["message"] = (
                    f"Successfully uploaded '{filename}' as '{final_filename}' to folder '{folder_name}' (name was auto-adjusted to avoid conflicts)"
                )
    ...
    async def global_kb_enable_item(
        self, item_type: str, item_id: str, enabled: bool
    ) -> ToolResult:
        ...
        try:
            ...
            if item_type != "file":
                return self.fail_response(
                    "Only 'file' type is supported for enable/disable operations"
                )
            ...
            if assignment_result.data:
                ...
                await (
                    client.table("agent_knowledge_entry_assignments")
                    ...
                    kb_structure[folder_name]["files"].append(
                        {
                            ...
                            "summary": file["summary"][:100] + "..."
                            if file.get("summary") and len(file["summary"]) > 100
                            else (file.get("summary") or ""),
