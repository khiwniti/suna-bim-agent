...
class ResourceService:
    ...
    async def create_resource(
        ...
        logger.debug(
            f"Created resource {result.data[0]['id']} of type {resource_type.value} for account {account_id}"
        )
    ...
    async def get_resource_by_external_id(
        self, external_id: str, resource_type: Optional[ResourceType] = None
    ...
    async def update_resource(
        ...
        result = (
            await self.client.table("resources").update(update_data).eq("id", resource_id).execute()
        )
    ...
    async def link_resource_to_project(self, project_id: str, resource_id: str) -> bool:
        """
        ...
        result = (
            await self.client.table("projects")
            .update({"sandbox_resource_id": resource_id})
            .eq("project_id", project_id)
            .execute()
        )
    ...
    async def migrate_project_sandbox_if_needed(self, project_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                sandbox_jsonb = (
                    sandbox_result.data[0].get("sandbox") if sandbox_result.data else None
                )
            ...
            existing_resource = await self.get_resource_by_external_id(
                sandbox_id, ResourceType.SANDBOX
            )
            ...
            if existing_resource:
                ...
                if await self.link_resource_to_project(project_id, resource_id):
                    logger.info(
                        f"Lazy migration: Linked existing resource {resource_id} to project {project_id}"
                    )
            ...
            if await self.link_resource_to_project(project_id, resource["id"]):
                logger.info(
                    f"Lazy migration: Migrated sandbox data for project {project_id} to resource {resource['id']}"
                )
                return resource
            else:
                ...
                logger.error(
                    f"Lazy migration: Failed to link resource to project {project_id}, cleaned up resource"
                )
                return None
    ...
    async def get_pooled_sandboxes(self, limit: int = 100) -> list[Dict[str, Any]]:
        result = (
            ...
            .eq("status", ResourceStatus.POOLED.value)
            .order("pooled_at", desc=False)
            .limit(limit)
            .execute()
        )
    ...
    async def claim_pooled_sandbox(
        ...
        update_result = (
            ...
            .eq(
                "status",
                ResourceStatus.POOLED.value,  # Optimistic lock
            )
            .execute()
        )
        ...
        logger.info(
            f"Claimed pooled sandbox {claimed_resource['external_id']} for project {project_id}"
        )
