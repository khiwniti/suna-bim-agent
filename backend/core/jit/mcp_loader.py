...
class MCPJITLoader:
    ...
    async def build_tool_map(self, cache_only: bool = False, force_rebuild: bool = False) -> None:
        ...
        if not cache_only and self._tool_map_built and len(self.tool_map) == 0:
            logger.info(
                "⚡ [MCP JIT] Rebuilding tool map with full discovery (previous build was cache-only with no results)"
            )
        ...
        logger.debug(
            f"⚡ [MCP JIT] Processing {len(custom_mcps)} custom MCPs and {len(configured_mcps)} configured MCPs ({mode_str})"
        )

        for i, mcp in enumerate(custom_mcps):
            logger.debug(
                f"⚡ [MCP JIT] custom_mcp[{i}]: name={mcp.get('name')}, toolkit={mcp.get('toolkit_slug') or mcp.get('config', {}).get('toolkit_name')}"
            )

        for i, mcp in enumerate(configured_mcps):
            logger.debug(
                f"⚡ [MCP JIT] configured_mcp[{i}]: name={mcp.get('name')}, toolkit={mcp.get('toolkit_slug')}"
            )

        process_tasks = []
        for mcp_config in custom_mcps:
            process_tasks.append(
                self._process_mcp_config(mcp_config, "custom", cache_only=cache_only)
            )

        for mcp_config in configured_mcps:
            process_tasks.append(
                self._process_mcp_config(mcp_config, "configured", cache_only=cache_only)
            )
        ...
        logger.info(
            f"⚡ [MCP JIT] Built tool map: {len(self.tool_map)} tools from {len(custom_mcps + configured_mcps)} servers in {elapsed_ms:.1f}ms ({mode_str})"
        )
    ...
    async def _process_mcp_config(
        self, mcp_config: Dict[str, Any], config_type: str, cache_only: bool = False
    ) -> None:
        ...
        if enabled_tools:
            for tool_name in enabled_tools:
                if tool_name in self.tool_map:
                    previous_profile_id = (
                        self.tool_map[tool_name].mcp_config.get("config", {}) or {}
                    ).get("profile_id")
                ...
                self.tool_map[tool_name] = MCPToolInfo(
                    tool_name=tool_name, toolkit_slug=toolkit_slug, mcp_config=mcp_config
                )
            return

        account_id = self.agent_config.get("account_id")
        available_tools = await get_toolkit_tools(
            toolkit_slug, account_id=account_id, cache_only=cache_only
        )
    ...
    async def _process_custom_mcp_config(
        self,
        mcp_config: Dict[str, Any],
        custom_type: str,
        server_name: str,
        ...
        if cache_only:
            if enabled_tools:
                ...
                logger.debug(
                    f"⚡ [MCP JIT] Custom MCP {server_name}: Added {len(enabled_tools)} enabled tools from config (cache-only mode)"
                )
            else:
                logger.debug(
                    f"⚡ [MCP JIT] Custom MCP {server_name}: No enabled tools in config, will discover later"
                )
            return
        ...
        try:
            ...
            if enabled_tools:
                ...
                logger.debug(
                    f"⚡ [MCP JIT] Custom MCP {server_name}: Filtered to {len(tools_to_add)}/{len(available_tools)} enabled tools"
                )
            else:
                tools_to_add = available_tools
                logger.debug(
                    f"⚡ [MCP JIT] Custom MCP {server_name}: No enabledTools filter, loading all {len(tools_to_add)} tools"
                )
            ...
            for tool_name in tools_to_add:
                if tool_name in self.tool_map:
                    logger.warning(
                        f"⚠️  [MCP JIT] Tool '{tool_name}' already registered, skipping duplicate"
                    )
                    continue
            ...
            logger.info(
                f"⚡ [MCP JIT] Custom MCP {server_name}: Registered {len(tools_to_add)} tools"
            )

        except Exception as e:
            logger.error(f"❌ [MCP JIT] Failed to discover tools for custom MCP {server_name}: {e}")

    async def _discover_custom_mcp_tools(
        self, custom_type: str, config: Dict[str, Any]
    ) -> List[str]:
        if custom_type == "sse":
                ...
                if "unexpected keyword argument" in str(e):
                    async with sse_client(url) as (read_stream, write_stream):
                        async with ClientSession(read_stream, write_stream) as session:
                            ...
                            tool_names = [tool.name for tool in tools]
                            logger.debug(
                                f"⚡ [MCP JIT] Discovered {len(tool_names)} SSE tools (no headers)"
                            )
                            return tool_names
                else:
                    raise
    ...
    async def activate_tool(self, tool_name: str) -> ActivationResult:
        ...
        try:
            ...
            return ActivationSuccess(
                tool_name=tool_name, load_time_ms=tool_info.load_time_ms, dependencies_loaded=[]
            )
    ...
    async def activate_multiple(self, tool_names: List[str]) -> Dict[str, ActivationResult]:
        ...
        logger.info(
            f"⚡ [MCP JIT] Parallel activation completed: {successful}/{len(tool_names)} successful"
        )
    ...
    async def _load_tool_schema(self, tool_name: str, tool_info: MCPToolInfo) -> Dict[str, Any]:
        ...
        if custom_type == "composio":
            ...
            return await self._load_custom_mcp_schema(
                tool_name, toolkit_slug, mcp_config, custom_type
            )
        else:
            return await self._load_custom_mcp_schema(tool_name, toolkit_slug, mcp_config, "http")

    async def _load_composio_schema(
        self, tool_name: str, toolkit_slug: str, mcp_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        try:
            config = mcp_config.get("config", {})
            ...
            logger.debug(
                f"⚡ [MCP JIT] Resolved Composio profile {profile_id} to MCP URL for {tool_name}"
            )
    ...
    async def _load_custom_mcp_schema(
        self, tool_name: str, toolkit_slug: str, mcp_config: Dict[str, Any], custom_type: str
    ) -> Dict[str, Any]:
        try:
            config = mcp_config.get("config", {})
            url = config.get("url")
            ...
            logger.error(
                f"❌ [MCP JIT] Failed to load {custom_type} MCP schema for {tool_name}: {e}"
            )
            raise

    async def _load_sse_schema(
        self, tool_name: str, url: str, config: Dict[str, Any]
    ) -> Dict[str, Any]:
        if not url:
        ...
        try:
            async with sse_client(url, headers=headers) as (read_stream, write_stream):
                async with ClientSession(read_stream, write_stream) as session:
                    ...
                    available_tools = [tool.name for tool in tools]
                    raise ValueError(
                        f"Tool '{tool_name}' not found in SSE server. Available: {available_tools}"
                    )
        except TypeError as e:
            if "unexpected keyword argument" in str(e):
                async with sse_client(url) as (read_stream, write_stream):
                    async with ClientSession(read_stream, write_stream) as session:
                        ...
                        for tool in tools:
                            if tool.name == tool_name:
                                ...
                                logger.debug(
                                    f"⚡ [MCP JIT] Found SSE schema for {tool_name} (no headers)"
                                )
                                return schema
    ...
    async def _load_http_schema(
        ...
        async with streamablehttp_client(url) as (read_stream, write_stream, _):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                available_tools = [tool.name for tool in tools]
                raise ValueError(
                    f"Tool '{tool_name}' not found in HTTP server. Available: {available_tools}"
                )
    ...
    async def _load_json_schema(self, tool_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with stdio_client(server_params) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                available_tools = [tool.name for tool in tools]
                raise ValueError(
                    f"Tool '{tool_name}' not found in JSON/stdio server. Available: {available_tools}"
                )
