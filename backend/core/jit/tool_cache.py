...
class ToolGuideCache:
    ...
    def __init__(self, ttl: Optional[timedelta] = None):
        ...
        self.enabled = True
        logger.info(
            f"⚡ [TOOL CACHE] Initialized with TTL={self.ttl}, using shared async Redis pool"
        )
    ...
    async def set_tool_guide(self, tool_name: str, guide: str) -> bool:
        ...
        try:
            ...
            cache_key = self._make_cache_key(tool_name)
            data = {"tool_name": tool_name, "guide": guide, "version": self.CACHE_VERSION}

            # Use module-level function with timeout protection
            await redis_service.setex(
                cache_key, int(self.ttl.total_seconds()), json.dumps(data), timeout=5.0
            )
