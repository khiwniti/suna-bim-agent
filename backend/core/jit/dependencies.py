...
class DependencyResolver:
    ...
    def topological_sort(
        self, tool_names: List[str], prioritized: Optional[Set[str]] = None
    ) -> List[str]:
        ...
        if len(result) != len(all_tools):
            logger.warning(
                f"⚠️  [JIT DEP] Circular dependency detected! Loaded {len(result)}/{len(all_tools)} tools"
            )
            return result
    ...
    def resolve_loading_order(
        self, requested_tools: List[str], allowed_tools: Optional[Set[str]] = None
    ) -> Dict[str, List[str]]:
