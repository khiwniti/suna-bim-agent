...
class MCPToolExecutor:
    ...
    async def _execute_composio_tool(self, tool_name: str, args: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            mcp_url = await profile_service.get_mcp_url_for_runtime(
                profile_id, account_id=self.account_id
            )
            ...
            return ToolResult(success=False, output=f"Failed to execute Composio tool: {str(e)}")
    ...
    async def _execute_sse_tool(self, tool_name: str, args: Dict[str, Any]) -> Any:
        ...
        if not url:
            return ToolResult(success=False, output="Missing 'url' in SSE MCP config")
        ...
        try:
                    ...
                    if "unexpected keyword argument" in str(e):
                        async with sse_client(url) as (read, write):
                            async with ClientSession(read, write) as session:
                                ...
                                content = self._extract_result_content(result)
                                logger.debug(
                                    f"⚡ [MCP EXEC] Executed {tool_name} via SSE (no headers)"
                                )
                                return ToolResult(success=True, output=str(content))
                    else:
                        raise
            ...
            return ToolResult(success=False, output=f"SSE tool execution timeout after 30 seconds")
        except Exception as e:
            logger.error(f"❌ [MCP EXEC] SSE execution failed for {tool_name}: {e}")
            return ToolResult(success=False, output=f"Failed to execute SSE tool: {str(e)}")
    ...
    async def _execute_http_tool(self, tool_name: str, args: Dict[str, Any]) -> Any:
        ...
        if not url:
            return ToolResult(success=False, output="Missing 'url' in HTTP MCP config")
        ...
        try:
            ...
            return ToolResult(success=False, output=f"HTTP tool execution timeout after 30 seconds")
        except Exception as e:
            logger.error(f"❌ [MCP EXEC] HTTP execution failed for {tool_name}: {e}")
            return ToolResult(success=False, output=f"Failed to execute HTTP tool: {str(e)}")
    ...
    async def _execute_json_tool(self, tool_name: str, args: Dict[str, Any]) -> Any:
        ...
        if not command:
            return ToolResult(success=False, output="Missing 'command' in JSON/stdio MCP config")
        ...
        try:
            ...
            return ToolResult(
                success=False, output=f"JSON/stdio tool execution timeout after 30 seconds"
            )
        except Exception as e:
            logger.error(f"❌ [MCP EXEC] JSON/stdio execution failed for {tool_name}: {e}")
            return ToolResult(success=False, output=f"Failed to execute JSON/stdio tool: {str(e)}")
