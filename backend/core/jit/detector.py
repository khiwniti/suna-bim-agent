...
class ParameterDetector:
    ...
    def detect_init_parameters(tool_class) -> Dict[str, type]:
        try:
            ...
            logger.debug(
                f"⚡ [JIT] Detected parameters for {tool_class.__name__}: {list(params.keys())}"
            )
            return params
    ...
    @staticmethod
    def build_kwargs(
        init_params: Dict[str, type], thread_manager, project_id: Optional[str] = None
    ) -> Dict[str, Any]:
        kwargs = {}

        for param_name in init_params.keys():
            value = ParameterDetector._resolve_parameter(param_name, thread_manager, project_id)
    ...
    def _resolve_parameter(
        param_name: str, thread_manager, project_id: Optional[str]
    ) -> Optional[Any]:
