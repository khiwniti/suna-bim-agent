...
class JITLoader:
    ...
    async def activate_tool(
        ...
        try:
            ...
            return ActivationSuccess(
                tool_name=tool_name, load_time_ms=elapsed_ms, dependencies_loaded=[]
            )

        except Exception as e:
            logger.error(f"❌ [JIT] Failed to activate tool '{tool_name}': {e}", exc_info=True)
            return ActivationError(
                error_type=ActivationErrorType.INIT_FAILED, message=str(e), tool_name=tool_name
            )
    ...
    async def activate_multiple(
        ...
        if dependencies:
            logger.info(
                f"⚡ [JIT DEP] Auto-loading {len(dependencies)} dependencies: {dependencies}"
            )
        if skipped:
        ...
        for tool_name in sorted_tools:
            result = await JITLoader.activate_tool(
                tool_name, thread_manager, project_id, jit_config
            )
        ...
        batch_result = BatchActivationResult(
            successful=successful, failed=failed, total_time_ms=total_time
        )
            ...
            if isinstance(result, ActivationSuccess):
                ...
                if schema:
                    ...
                    tool_wrapper = await JITLoader._create_mcp_tool_wrapper(
                        tool_name, schema, tool_info, account_id=account_id
                    )
                    ...
                    tool_schema = ToolSchema(schema=openapi_schema, schema_type=SchemaType.OPENAPI)
                ...
                elapsed_ms = (time.time() - start_time) * 1000
                logger.info(
                    f"✅ [JIT MCP] Tool '{tool_name}' activated successfully in {elapsed_ms:.1f}ms"
                )
    ...
    async def _create_mcp_tool_wrapper(
        tool_name: str, schema: Dict, tool_info, account_id: str = None
    ):
    ...
    async def activate_multiple_with_mcp(
        ...
        start_time = time.time()
        logger.info(
            f"⚡ [JIT] Activating {len(tool_names)} tools (regular + MCP with dependency resolution)"
        )
        ...
        logger.info(
            f"⚡ [JIT] Split: {len(regular_tools)} regular tools, {len(mcp_tools)} MCP tools"
        )
