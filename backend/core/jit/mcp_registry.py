...
class MCPRegistry:
    ...
    async def get_toolkit_tools(
        self, toolkit_slug: str, account_id: Optional[str] = None, cache_only: bool = False
    ) -> List[str]:
    ...
    async def _query_composio_toolkit(
        self,
        toolkit_slug: str,
        account_id: Optional[str] = None,
        sample_profile_id: Optional[str] = None,
    ) -> List[str]:
        ...
        try:
            ...
            try:
                if account_id:
                    connected_profiles = await self._find_connected_profiles_for_toolkit(
                        profile_service, toolkit_slug, account_id
                    )
                else:
                    logger.warning(
                        f"⚠️  [MCP DYNAMIC] No account_id provided, cannot find profiles for {toolkit_slug}"
                    )
                    connected_profiles = []
                ...
                logger.debug(
                    f"⚡ [MCP DYNAMIC] No connected profiles found for {toolkit_slug}: {profile_err}"
                )

            logger.warning(
                f"⚠️  [MCP DYNAMIC] No connected profiles for {toolkit_slug} - cannot discover real tool names"
            )
            return []
    ...
    async def _find_connected_profiles_for_toolkit(
        self, profile_service, toolkit_slug: str, account_id: str
    ):
        try:
    ...
    async def _query_via_mcp_service_with_url(
        self, toolkit_slug: str, mcp_url: str, start_time: float
    ) -> List[str]:
        try:
            ...
            if result.success and result.tools:
                ...
                logger.info(
                    f"⚡ [MCP DYNAMIC] Discovered {len(real_tools)} REAL tool names for {toolkit_slug} via MCP server in {elapsed_ms:.1f}ms"
                )
                return real_tools
    ...
    async def _query_toolkit_service_with_mapping(
        self, toolkit_slug: str, start_time: float
    ) -> List[str]:
        ...
        tools_response = await toolkit_service.get_toolkit_tools(
            toolkit_slug=toolkit_slug, limit=500
        )
        ...
        elapsed_ms = (time.time() - start_time) * 1000
        logger.info(
            f"⚡ [MCP DYNAMIC] Mapped {len(tools)} display names to API names for {toolkit_slug} in {elapsed_ms:.1f}ms"
        )
        return tools
    ...
    async def _cache_toolkit_tools(self, toolkit_slug: str, tools: List[str]) -> None:
        ...
        if self._cache_enabled:
            try:
                ...
                # Use module-level function with timeout protection
                await redis_service.setex(cache_key, ttl_seconds, json.dumps(tools), timeout=5.0)

                logger.debug(
                    f"💾 [MCP DYNAMIC] Cached {toolkit_slug}: {len(tools)} tools (TTL={ttl_seconds}s)"
                )
...
async def get_toolkit_tools(
    toolkit_slug: str, account_id: Optional[str] = None, cache_only: bool = False
) -> List[str]:
    registry = await get_dynamic_registry()
    return await registry.get_toolkit_tools(
...
async def get_all_available_tools_from_toolkits(toolkit_slugs: List[str]) -> Dict[str, str]:
    ...
    logger.debug(
        f"⚡ [MCP DYNAMIC] Built tool map: {len(tool_map)} tools from {len(toolkit_slugs)} toolkits"
    )
    return tool_map
