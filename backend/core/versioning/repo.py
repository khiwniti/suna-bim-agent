...
async def get_agent_current_version(agent_id: str) -> Optional[Dict[str, Any]]:
    ...
    result = await execute_one(
        version_sql, {"version_id": current_version_id, "agent_id": agent_id}
    )
...
async def deactivate_agent_versions(agent_id: str) -> bool:
    ...
    """
    await execute_mutate(sql, {"agent_id": agent_id, "updated_at": datetime.now(timezone.utc)})
    return True
...
async def activate_agent_version(version_id: str) -> bool:
    ...
    """
    await execute_mutate(sql, {"version_id": version_id, "updated_at": datetime.now(timezone.utc)})
    return True
...
async def update_agent_current_version(agent_id: str, version_id: str) -> bool:
    ...
    await execute_mutate(
        sql,
        {"agent_id": agent_id, "version_id": version_id, "updated_at": datetime.now(timezone.utc)},
    )
    return True


async def update_agent_version(
    version_id: str, version_name: Optional[str] = None, change_description: Optional[str] = None
) -> bool:
    from datetime import datetime, timezone
