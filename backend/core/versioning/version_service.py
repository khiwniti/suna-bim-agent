...
class VersionService:
    ...
    async def _verify_and_authorize_agent_access(
        ...
        for mcp in custom_mcps:
            ...
            if mcp_type == "composio":
                if "mcp_qualified_name" not in mcp_copy:
                    mcp_copy["mcp_qualified_name"] = (
                        config.get("mcp_qualified_name")
                        or config.get("qualifiedName")
                        or f"composio.{mcp_name.lower().replace(' ', '_')}"
                    )
                if "toolkit_slug" not in mcp_copy:
                    mcp_copy["toolkit_slug"] = config.get(
                        "toolkit_slug"
                    ) or mcp_name.lower().replace(" ", "_")
        ...
        for attempt in range(max_retries):
            try:
                ...
                for trigger in triggers:
                    if "config" in trigger and isinstance(trigger["config"], str):
                        try:
                            ...
                            logger.warning(
                                f"Failed to parse trigger config for {trigger.get('trigger_id')}"
                            )
                            trigger["config"] = {}
                ...
                is_version_conflict = (
                    ...
                    and (
                        "agent_versions_agent_id_version_number_key" in error_msg
                        or "agent_versions_agent_id_version_name_key" in error_msg
                    )
                )
                if is_version_conflict:
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"Version conflict for agent {agent_id}, attempt {attempt + 1}/{max_retries}, retrying..."
                        )
                        continue
                    else:
                        logger.error(
                            f"Failed to create version after {max_retries} attempts due to version conflicts"
                        )
                        raise VersionConflictError(
                            "Unable to create version due to concurrent modifications. Please try again."
                        )
                else:
                    raise e
    ...
    async def get_active_version(
        ...
        logger.debug(
            f"Retrieved active version for agent {agent_id}: model='{version.model}', version_name='{version.version_name}'"
        )
        return version
    ...
    def _calculate_differences(self, v1: AgentVersion, v2: AgentVersion) -> List[Dict[str, Any]]:
        ...
        if v1.model != v2.model:
            differences.append(
                {"field": "model", "type": "modified", "old_value": v1.model, "new_value": v2.model}
            )
        ...
        for tool in v2_tools - v1_tools:
            differences.append(
                {"field": f"tool.{tool}", "type": "added", "new_value": v2.agentpress_tools[tool]}
            )

        for tool in v1_tools - v2_tools:
            differences.append(
                {"field": f"tool.{tool}", "type": "removed", "old_value": v1.agentpress_tools[tool]}
            )
    ...
    async def get_current_mcp_config(
        ...
        logger.debug(
            f"⏱️ [MCP CONFIG] Cache MISS for {agent_id}, fetching from DB (cache check: {cache_time:.1f}ms)"
        )
        ...
        try:
            ...
            rpc_result = await client.rpc(
                "get_agent_mcp_config", {"p_agent_id": agent_id}
            ).execute()
            ...
            if not rpc_result.data:
                ...
                await set_cached_mcp_version_config(
                    agent_id, {"custom_mcp": [], "configured_mcps": []}
                )
                return empty_config
            ...
            logger.info(
                f"✅ [MCP CONFIG] Loaded for {agent_id} in {total_time:.1f}ms (client:{client_time:.0f}ms, rpc:{rpc_time:.0f}ms, cache_set:{cache_set_time:.0f}ms)"
            )
            ...
            total_time = (time.time() - start_time) * 1000
            logger.error(
                f"❌ [MCP CONFIG] Error loading for agent {agent_id} after {total_time:.1f}ms: {e}",
                exc_info=True,
            )
