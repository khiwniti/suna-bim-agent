...
def load_static_suna_config() -> Dict[str, Any]:
    ...
    logger.info(
        f"✅ Loaded static Suna config into memory (prompt: {len(_SUNA_STATIC_CONFIG['system_prompt'])} chars)"
    )
    return _SUNA_STATIC_CONFIG
...
async def set_cached_user_mcps(
    agent_id: str, configured_mcps: list, custom_mcps: list, triggers: list = None
) -> None:
    cache_key = _get_user_mcps_key(agent_id)
    data = {
...
async def get_cached_agent_config(
    agent_id: str, version_id: Optional[str] = None
    ...
    try:
        ...
        for i in range(0, len(agent_ids), batch_size):
            ...
            results = await asyncio.gather(
                *[prewarm_single(aid) for aid in batch], return_exceptions=True
            )
        ...
        elapsed = (time.time() - t_start) * 1000
        logger.info(
            f"✅ [PREWARM] User {user_id[:8]}...: {prewarmed} loaded, {skipped} cached, {errors} errors ({elapsed:.0f}ms)"
        )
...
async def set_cached_running_runs(
    account_id: str, running_count: int, running_thread_ids: list
) -> None:
    cache_key = _get_running_runs_key(account_id)
...
async def warm_kb_context_for_agent(agent_id: str, client) -> None:
    try:
        ...
        if kb_data and kb_data.strip():
            ...
            logger.debug(
                f"✅ [PREWARM] Warmed KB cache for agent {agent_id} ({len(kb_data)} chars)"
            )
        else:
            await set_cached_kb_context(agent_id, "")
    except Exception as e:
...
async def get_cached_message_history(thread_id: str) -> Optional[list]:
    ...
    try:
        ...
        if cached:
            data = _json_loads(cached) if isinstance(cached, (str, bytes)) else cached
            logger.debug(
                f"⚡ Redis cache hit for message history: {thread_id} ({len(data)} messages)"
            )
            return data
    except Exception as e:
...
async def append_to_cached_message_history(thread_id: str, message: dict) -> bool:
    ...
    try:
        ...
        if cached:
            ...
            await redis_service.set(cache_key, _json_dumps(messages), ex=MESSAGE_HISTORY_TTL)
            logger.debug(
                f"✅ Appended message to cached history: {thread_id} ({len(messages)} messages)"
            )
            return True
    except Exception as e:
...
async def set_cached_tier_info(account_id: str, tier_info: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.debug(
            f"✅ Cached tier info in Redis: {account_id} (tier: {tier_info.get('name', 'unknown')})"
        )
    except Exception as e:
...
async def set_pending_thread(
    thread_id: str, project_id: str, account_id: str, agent_run_id: str, prompt: str
) -> None:
