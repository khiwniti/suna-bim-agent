...
class SubscriptionRepository:
    ...
    async def update_account_tier(
        ...
        try:
            ...
            if after_result:
                ...
                logger.info(
                    f"[REVENUECAT] Current state AFTER update: tier={final_state.get('tier')}"
                )
    ...
    async def update_tier_only(client, app_user_id: str, tier_name: str) -> None:
        await billing_repo.update_credit_account(
            app_user_id, {"tier": tier_name, "updated_at": datetime.now(timezone.utc).isoformat()}
        )
    ...
    async def sync_customer_info_to_db(
        client, account_id: str, tier_name: str, customer_info: Dict
    ) -> None:
        ...
        logger.info(
            f"[REVENUECAT SYNC] Synced tier {tier_name} (product: {product_id}) for {account_id}"
        )
