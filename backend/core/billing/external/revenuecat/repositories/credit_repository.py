...
class CreditRepository:
    ...
    async def grant_renewal_credits(
        ...
        try:
            ...
            logger.error(f"[REVENUECAT RENEWAL] Exception for {app_user_id}: {e}", exc_info=True)
            return {}

    @staticmethod
    def log_renewal_result(result_data: Dict, app_user_id: str) -> None:
        if not result_data:
            logger.error(f"[REVENUECAT RENEWAL] No data returned from atomic_grant_renewal_credits")
            return
    ...
    async def check_duplicate_topup(
        ...
        """
        return await execute_one(sql, {"account_id": app_user_id, "transaction_id": transaction_id})
    ...
    async def fail_credit_purchase(client, transaction_id: str, error_message: str) -> None:
        ...
        try:
            ...
            """
            await execute_mutate(
                sql, {"transaction_id": transaction_id, "error_message": error_message}
            )
        except:
            pass
