...
class RevenueCatService:
    ...
    async def process_webhook(self, request: Request) -> Dict:
        try:
            ...
            event_id = (
                event.get("id")
                or f"rc_{event.get('app_user_id')}_{event.get('event_timestamp_ms')}"
            )

            can_process, reason = await WebhookLock.check_and_mark_webhook_processing(
                event_id, event_type, payload=webhook_data
            )
            ...
            if not can_process:
                    ...
                    if event_type == "INITIAL_PURCHASE":
                        ...
                        if needs_processing:
                            ...
                            (
                                can_process,
                                reason,
                            ) = await WebhookLock.check_and_mark_webhook_processing(
                                event_id, event_type, payload=webhook_data, force_reprocess=True
                            )
                            if not can_process:
                                logger.error(
                                    f"[REVENUECAT] Failed to force reprocess event {event_id}"
                                )
                                raise HTTPException(
                                    status_code=500, detail="Failed to reprocess webhook"
                                )
                ...
                if not can_process:
                    if reason in ("processing_retry_later", "in_progress"):
                        ...
                        logger.warning(
                            f"[REVENUECAT] Event {event_id} being processed by another request, requesting retry"
                        )
                        raise HTTPException(
                            status_code=503, detail="Event is being processed, please retry"
                        )
    ...
    async def _verify_needs_processing(self, webhook_data: Dict) -> bool:
        ...
        try:
            ...
            if not account:
                logger.info(
                    f"[REVENUECAT VERIFY] No account found for {app_user_id} - needs processing"
                )
                return True
            ...
            is_provisioned = (
                current_provider == "revenuecat"
                and current_tier == tier_name
                and current_product == product_id
            )
