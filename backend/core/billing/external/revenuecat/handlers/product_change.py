...
class ProductChangeHandler:
    ...
    async def handle(webhook_data: Dict) -> None:
        ...
        if not ProductMapper.validate_product_id(new_product_id):
            logger.error(
                f"[REVENUECAT] Skipping PRODUCT_CHANGE for invalid new product: {new_product_id}"
            )
            return

        old_tier, old_tier_info = (
            ProductMapper.get_tier_info(old_product_id) if old_product_id else (None, None)
        )
        new_tier, new_tier_info = ProductMapper.get_tier_info(new_product_id)

        if not new_tier_info:
            logger.error(
                f"[REVENUECAT PRODUCT_CHANGE] Unknown new product: {new_product_id}, skipping"
            )
            return
