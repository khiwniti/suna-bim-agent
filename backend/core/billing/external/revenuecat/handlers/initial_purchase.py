...
class InitialPurchaseHandler:
    ...
    async def handle(webhook_data: Dict) -> None:
        ...
        if not ProductMapper.validate_product_id(product_id):
            logger.error(
                f"[REVENUECAT] Skipping INITIAL_PURCHASE for invalid product: {product_id}"
            )
            return
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"[REVENUECAT INITIAL_PURCHASE] Cache invalidation failed: {cache_error}"
                )

            logger.info(
                f"[REVENUECAT INITIAL_PURCHASE] ✅ Successfully processed purchase for {app_user_id}"
            )
