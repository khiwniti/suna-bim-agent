...
class SubscriptionService:
    ...
    async def apply_subscription_change(
        app_user_id: str, product_id: str, price: float, event_type: str, webhook_data: Dict
    ) -> None:
        ...
        if existing_account:
            ...
            logger.info(
                f"[REVENUECAT] Current account state: tier={existing_account.get('tier')}, provider={existing_account.get('provider')}"
            )
        else:
        ...
        if not tier_info.monthly_refill_enabled:
            logger.info(
                f"[REVENUECAT] Skipping initial credit grant for tier {tier_name} - monthly_refill_enabled=False"
            )
        else:
            try:
                if existing_account:
        ...
        try:
            ...
            if existing_account:
                ...
                await SubscriptionService._cancel_existing_stripe_subscription(
                    existing_account, app_user_id
                )
    ...
    @staticmethod
    async def process_renewal(app_user_id: str, product_id: str, webhook_data: Dict) -> None:
        ...
        if period_type == "yearly":
            logger.info(
                f"[REVENUECAT RENEWAL] Yearly plan renewal - granting 1 monthly credit batch"
            )
            ...
            await SubscriptionRepository.update_renewal_data(
                None, app_user_id, billing_cycle_anchor=anchor_date, next_credit_grant=next_grant
            )
        ...
        await CreditService.grant_renewal_credits(
            app_user_id, period_start, real_period_end, credits_amount, event, product_id, tier_name
        )

    @staticmethod
    async def schedule_plan_change_for_period_end(
        app_user_id: str, old_product_id: str, new_product_id: str, event: Dict, change_type: str
    ) -> None:
    ...
    async def _cancel_existing_stripe_subscription(
        existing_account: Dict, app_user_id: str
    ) -> None:
