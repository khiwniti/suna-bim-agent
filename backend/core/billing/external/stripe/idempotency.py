...
class StripeIdempotencyManager(IdempotencyManagerInterface):
    def generate_key(
        self, operation: str, account_id: str, *args, time_bucket_minutes: int = 5, **kwargs
    ) -> str:
    ...
    def generate_checkout_key(
        self, account_id: str, price_id: str, commitment_type: Optional[str] = None
    ) -> str:
        return self.generate_key(
            "checkout", account_id, price_id, commitment_type=commitment_type or "none"
        )

    def generate_trial_key(self, account_id: str, trial_days: int) -> str:
        return self.generate_key("trial_checkout", account_id, trial_days)

    def generate_credit_purchase_key(self, account_id: str, amount: float) -> str:
        return self.generate_key("credit_purchase", account_id, amount)

    def generate_subscription_modify_key(self, subscription_id: str, new_price_id: str) -> str:
        return self.generate_key("modify_subscription", subscription_id, new_price_id)

    def generate_subscription_cancel_key(
        self, subscription_id: str, cancel_type: str = "at_period_end"
    ) -> str:
        return self.generate_key("cancel_subscription", subscription_id, cancel_type)

    def generate_refund_key(self, payment_intent_id: str, amount: Optional[float] = None) -> str:
        return self.generate_key("refund", payment_intent_id, amount or "full")
...
def generate_checkout_idempotency_key(
    account_id: str, price_id: str, commitment_type: Optional[str] = None
) -> str:
...
def generate_subscription_modify_idempotency_key(subscription_id: str, new_price_id: str) -> str:
    return stripe_idempotency_manager.generate_subscription_modify_key(
        subscription_id, new_price_id
    )


def generate_subscription_cancel_idempotency_key(
    subscription_id: str, cancel_type: str = "at_period_end"
) -> str:
