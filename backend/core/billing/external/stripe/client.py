...
class StripeCircuitBreaker(CircuitBreakerInterface):
    ...
    async def safe_call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if await self._should_allow_request(state_info):
            ...
            logger.warning(
                f"[CIRCUIT BREAKER] Request blocked - circuit is {state_info['state'].value}"
            )
            raise Exception(
                f"Circuit breaker is {state_info['state'].value} - blocking request to Stripe API"
            )
    ...
    def _get_memory_state(self) -> Optional[Dict]:
        ...
        now = time.time()
        if (
            StripeCircuitBreaker._memory_state is not None
            and (now - StripeCircuitBreaker._memory_state_time)
            < StripeCircuitBreaker._memory_state_ttl
        ):
    ...
    async def _fetch_state_from_db(self) -> Dict:
        ...
        try:
            ...
            result = (
                await client.from_("circuit_breaker_state")
                .select("*")
                .eq("circuit_name", self.circuit_name)
                .execute()
            )
            ...
            return {"state": CircuitState.CLOSED, "failure_count": 0, "last_failure_time": None}

        except Exception as e:
            logger.error(
                f"[CIRCUIT BREAKER] Error reading state from DB: {e}, defaulting to CLOSED"
            )
            return {"state": CircuitState.CLOSED, "failure_count": 0, "last_failure_time": None}
    ...
    async def get_status(self) -> Dict:
        ...
        return {
            ...
            "last_failure_time": state_info["last_failure_time"].isoformat()
            ...
            "status": "✅ Healthy"
            if state_info["state"] == CircuitState.CLOSED
            else f"🔴 {state_info['state'].value.upper()}",
            "cache_age_seconds": time.time() - StripeCircuitBreaker._memory_state_time
            if StripeCircuitBreaker._memory_state
            else None,
        }
    ...
    async def _record_success_async(self):
        ...
        try:
            ...
            client = await self.db.client
            await (
                client.from_("circuit_breaker_state")
                .upsert(
                    {
                        "circuit_name": self.circuit_name,
        ...
        try:
            async with self._get_lock():
                ...
                if new_state == CircuitState.OPEN:
                    logger.warning(
                        f"[CIRCUIT BREAKER] Circuit OPENED due to {new_failure_count} failures: {error_message}"
                    )
                else:
                    logger.debug(
                        f"[CIRCUIT BREAKER] Recorded failure #{new_failure_count} for {self.circuit_name}"
                    )
    ...
    async def _transition_to_half_open(self):
        ...
        try:
            half_open_state = {
                ...
                "failure_count": 0,
                "last_failure_time": self._get_memory_state().get("last_failure_time")
                if self._get_memory_state()
                else None,
            }
            ...
            logger.info(
                f"[CIRCUIT BREAKER] Circuit transitioned to HALF_OPEN for {self.circuit_name}"
            )
...
class StripeAPIWrapper:
    ...
    async def safe_stripe_call(
        cls, func: Callable, *args, timeout: Optional[float] = None, **kwargs
    ) -> Any:
        """Execute Stripe API call with timeout and circuit breaker protection."""
        request_timeout = timeout or cls.DEFAULT_TIMEOUT
        try:
            return await asyncio.wait_for(
                cls._circuit_breaker.safe_call(func, *args, **kwargs), timeout=request_timeout
            )
    ...
    async def cancel_subscription(
        cls, subscription_id: str, cancel_immediately: bool = True
    ) -> stripe.Subscription:
        if cancel_immediately:
            return await cls.safe_stripe_call(
                stripe.Subscription.cancel_async, subscription_id, prorate=True
            )
        else:
            return await cls.safe_stripe_call(
                stripe.Subscription.modify_async, subscription_id, cancel_at_period_end=True
            )

    @classmethod
    async def modify_subscription(cls, subscription_id: str, **kwargs) -> stripe.Subscription:
        return await cls.safe_stripe_call(
            stripe.Subscription.modify_async, subscription_id, **kwargs
        )
