...
class SubscriptionRepository:
    ...
    async def update_subscription_status_and_anchor(
        self, account_id: str, status: str, billing_anchor: str
    ) -> None:
        await billing_repo.update_credit_account(
            account_id,
            {"stripe_subscription_status": status, "billing_cycle_anchor": billing_anchor},
        )
    ...
    async def add_credit_ledger_entry(
        self, account_id: str, amount: float, balance_after: float, type: str, description: str
    ) -> None:
