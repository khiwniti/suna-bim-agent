...
from core.billing.shared.config import is_commitment_price_id, get_commitment_duration_months
...
class CommitmentService:
    ...
    async def track_commitment_if_needed(
        self, account_id: str, price_id: str, subscription: Dict, commitment_type: str = None
    ) -> None:
        ...
        if await self.commitment_repo.get_existing_commitment(subscription["id"]):
            logger.info(
                f"[COMMITMENT] Commitment already tracked for subscription {subscription['id']}"
            )
            return
        ...
        if await self.commitment_repo.check_user_exists(account_id):
            ...
            logger.info(
                f"[COMMITMENT] Tracked yearly commitment for {account_id}, ends {end_date.date()}"
            )
        else:
    ...
    async def clear_commitment_if_needed(self, account_id: str) -> None:
        await self.commitment_repo.update_commitment_in_credit_account(
            account_id,
            {
    ...
    def is_scheduled_downgrade_ready(self, scheduled_changes: Dict, current_price_id: str) -> bool:
