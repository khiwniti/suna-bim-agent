...
class SubscriptionUpgradeService:
    ...
    async def handle_tier_upgrade_from_free(
        self, account_id: str, subscription: Dict, current_tier: str
    ) -> None:
        price_id = (
        ...
        if subscription_status != "active":
            logger.info(
                f"[UPGRADE] Subscription status '{subscription_status}' is not active - storing pending subscription without setting tier for {account_id}"
            )

            await self.subscription_repo.update_subscription_metadata(
                account_id,
                {
        ...
        logger.info(
            f"[UPGRADE] User {account_id} upgrading from {current_tier} to {tier_info.name}"
        )
        ...
        if not acquired:
            logger.error(
                f"[UPGRADE] Failed to acquire lock for free tier upgrade credit grant to {account_id}"
            )
            await self.subscription_repo.update_subscription_metadata(
                account_id,
                {
        ...
        try:
            if tier_info.monthly_refill_enabled:
                ...
                logger.info(
                    f"[UPGRADE] Granting ${credits_amount} credits for new {tier_info.name} subscription (upgrade from {current_tier})"
                )
                ...
                logger.info(
                    f"[UPGRADE] Skipping credits for tier {tier_info.name} - monthly_refill_enabled=False"
                )
    ...
    async def handle_incomplete_to_active_upgrade(
        ...
        if not tier_info:
            logger.error(
                f"[UPGRADE] Cannot process incomplete→active - price_id {price_id} not recognized"
            )
            raise ValueError(f"Unrecognized price_id: {price_id}")
        ...
        logger.info(
            f"[UPGRADE] User {account_id} upgrading from {current_tier} via incomplete→active to {tier_info.name}"
        )
    ...
    async def handle_cancelled_trial_resubscription(
        self, account_id: str, subscription: Dict
    ) -> None:
        ...
        logger.info(
            f"[RESUBSCRIPTION] User {account_id} with cancelled trial subscribing to {tier_info.name}"
        )
