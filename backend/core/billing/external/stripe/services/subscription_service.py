...
class SubscriptionService:
    ...
    def extract_subscription_info(self, subscription: Dict) -> Dict:
        ...
        price_amount = (
            subscription["items"]["data"][0]["price"].get("unit_amount", 0) or 0
    ...
    def is_tier_upgrade(self, current_tier_info, prev_tier_info) -> bool:
        return (
            current_tier_info
            and prev_tier_info
            and current_tier_info.name != prev_tier_info.name
            and float(current_tier_info.monthly_credits) > float(prev_tier_info.monthly_credits)
        )
    ...
    async def update_subscription_metadata_via_stripe(
        self, subscription_id: str, metadata: Dict
    ) -> None:
        try:
            await StripeAPIWrapper.modify_subscription(subscription_id, metadata=metadata)
            logger.info(
                f"[SUBSCRIPTION] Updated Stripe metadata for subscription {subscription_id}"
            )
