...
class CleanupService:
    async def cleanup_duplicate_subscriptions(
        ...
        if previous_subscription_id:
            canceled_subs.extend(
                await self._cancel_previous_subscription(
                    previous_subscription_id, new_subscription_id
                )
            )

        canceled_subs.extend(
            await self._cleanup_active_subscriptions(customer_id, new_subscription_id, new_amount)
        )

        if canceled_subs:
            logger.info(
                f"[CLEANUP SUMMARY] ✅ Canceled {len(canceled_subs)} old subscriptions: {canceled_subs}"
            )
        else:
    ...
    async def _cancel_previous_subscription(
        self, previous_subscription_id: str, account_id: str
    ) -> List[str]:
        try:
            logger.info(
                f"[UPGRADE CLEANUP] Immediately canceling previous subscription {previous_subscription_id} for {account_id}"
            )
            await StripeAPIWrapper.cancel_subscription(previous_subscription_id)
            logger.info(
                f"[UPGRADE CLEANUP] ✅ Canceled previous subscription {previous_subscription_id}"
            )
            return [previous_subscription_id]
        except stripe.error.StripeError as e:
            logger.warning(
                f"[UPGRADE CLEANUP] Could not cancel previous subscription {previous_subscription_id}: {e}"
            )
            return []

    async def _cleanup_active_subscriptions(
        self, customer_id: str, keep_subscription_id: str, new_amount: int
    ) -> List[str]:
        try:
            logger.info(
                f"[SUBSCRIPTION CLEANUP] Fetching all active subscriptions for customer {customer_id}"
            )
            ...
            logger.info(
                f"[SUBSCRIPTION CLEANUP] Found {len(customer_subs.data)} active subscriptions for customer"
            )
            ...
            for old_sub in customer_subs.data:
                if old_sub.id != keep_subscription_id:
                    ...
                    logger.info(
                        f"[SUBSCRIPTION CLEANUP] Checking subscription {old_sub.id} with amount ${old_amount / 100:.2f}"
                    )
                    ...
                    if should_cancel:
                        ...
                        logger.info(
                            f"[DUPLICATE CLEANUP] ✅ Canceled old subscription {old_sub.id}"
                        )
    ...
    async def check_for_other_active_subscriptions(
        self, customer_id: str, exclude_subscription_id: str
    ) -> List[Dict]:
        try:
            ...
            other_active_subs = [
                ...
                and sub.status in ["active", "trialing", "incomplete"]
            ]
