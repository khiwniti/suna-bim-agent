...
class SubscriptionCancellationService:
    ...
    async def process_subscription_deletion(
        self, subscription: Dict, other_active_subs: list
    ) -> None:
        account_id = subscription.get("metadata", {}).get("account_id")
        if not account_id:
            customer_id = subscription.get("customer")
            if customer_id:
                account_id = await self.subscription_repo.get_account_from_customer(
                    str(customer_id)
                )
        ...
        if other_active_subs:
            logger.info(
                f"[DELETION] User {account_id} has {len(other_active_subs)} other active subscriptions - skipping credit removal"
            )
            return

        logger.info(
            f"[DELETION] No other subscriptions found for {account_id} - proceeding with cleanup"
        )
    ...
    async def _handle_deletion_by_type(
        self, account_id: str, account_data: Dict, subscription: Dict
    ) -> None:
        ...
        if provider == "revenuecat" or account_data.get("revenuecat_subscription_id"):
            logger.info(
                f"[DELETION] Account {account_id} uses RevenueCat - skipping Stripe cleanup"
            )
            return
    ...
    async def _handle_paid_subscription_cancellation(
        self, account_id: str, expiring_credits: float, non_expiring_credits: float
    ) -> None:
