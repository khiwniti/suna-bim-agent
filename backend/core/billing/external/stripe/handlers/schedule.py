...
class ScheduleHandler:
    ...
    async def handle_subscription_schedule_event(event, client=None):
        ...
        logger.info(
            f"[SCHEDULE] Processing {event.type} for schedule {schedule_id}, subscription {subscription_id}"
        )
        ...
        if event.type == "subscription_schedule.completed":
            ...
            if (
                account_id
                and scheduled_tier
                and schedule.get("metadata", {}).get("downgrade") == "true"
            ):
                ...
                if acquired:
                    try:
                        ...
                        recheck = await billing_repo.get_credit_account_scheduled_changes(
                            account_id
                        )
                        ...
                        if recheck and recheck.get("scheduled_tier_change"):
                            ...
                            logger.info(
                                f"[SCHEDULE COMPLETED] ✅ Cleared scheduled change fields for {account_id}"
                            )
                    finally:
