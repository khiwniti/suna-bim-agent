...
class SubscriptionHandler:
    ...
    async def _handle_subscription_created_or_updated(self, event, client):
        ...
        logger.info(
            f"[SUBSCRIPTION HANDLER] Event: {event.type}, Subscription: {subscription_info['subscription_id']}, Status: {subscription_info['status']}"
        )
    ...
    async def _handle_subscription_created(
        self, subscription: Dict, subscription_info: Dict, client
    ):
        logger.info(
            f"[SUBSCRIPTION.CREATED] Processing subscription.created for {subscription_info['subscription_id']}"
        )
        ...
        logger.info(
            f"[SUBSCRIPTION.CREATED] account_id={account_id}, customer_id={subscription_info['customer_id']}, price_id={subscription_info['price_id']}"
        )

        await self._cleanup_duplicate_subscriptions(subscription, subscription_info, account_id)

        await self._process_new_subscription_by_state(
            account_id, subscription, subscription_info, client
        )
    ...
    async def _cleanup_duplicate_subscriptions(
        self, subscription: Dict, subscription_info: Dict, account_id: str
    ):
    ...
    async def _process_new_subscription_by_state(
        self, account_id: str, subscription: Dict, subscription_info: Dict, client
    ):
        account_data = await self.subscription_service.repository.get_credit_account_basic(
            account_id
        )
        if not account_data:
    ...
    async def _handle_trial_conversion(
        self, account_id: str, subscription: Dict, subscription_info: Dict
    ):
        tier_info = get_tier_by_price_id(subscription_info["price_id"])
        if not tier_info:
            logger.error(
                f"[TRIAL] Cannot process trial conversion - price_id {subscription_info['price_id']} not recognized"
            )
    ...
    async def _handle_subscription_deleted(self, event, client):
        ...
        await self.cancellation_service.process_subscription_deletion(
            subscription, other_active_subs
        )
    ...
    async def _sync_subscription_status(self, account_id: str, subscription: Dict):
        try:
            billing_anchor = datetime.fromtimestamp(
                subscription["billing_cycle_anchor"], tz=timezone.utc
            )
            ...
            logger.info(
                f"[SYNC] Synced status='{subscription.status}' & anchor='{billing_anchor}' for {account_id}"
            )
    ...
    async def _handle_unpaid_status(self, subscription: Dict, account_id: str):
        if subscription.status == "unpaid":
            logger.info(
                f"[GRACE PERIOD ENDED] Subscription {subscription.id} is unpaid - revoking access for {account_id}"
            )
            await self.cancellation_service.process_subscription_deletion(subscription, [])

    async def _handle_trial_status_changes(
        self, event, subscription: Dict, account_id: str, client
    ):
        previous_attributes = event.data.get("previous_attributes", {})
        ...
        if (
            ...
            and subscription.get("default_payment_method")
            and not prev_default_payment
        ):
    ...
    async def _handle_scheduled_downgrades(self, subscription: Dict, account_id: str, client):
        scheduled_changes = await self.subscription_service.repository.get_scheduled_changes(
            account_id
        )
        ...
        if self.commitment_service.is_scheduled_downgrade_ready(
            scheduled_changes, current_price_id
        ):
            if current_tier_info:
                await self.subscription_service.repository.clear_scheduled_changes(
                    account_id, current_tier_info.name
                )
                await Cache.invalidate(f"subscription_tier:{account_id}")
                logger.info(
                    f"[DOWNGRADE] ✅ Applied scheduled downgrade to {current_tier_info.name}"
                )
            return
        ...
        if scheduled_tier_key and current_tier_info:
            ...
            if scheduled_tier_info:
                current_credits = (
                    float(current_tier_info.monthly_credits)
                    if current_tier_info.monthly_credits
                    else 0
                )
                scheduled_credits = (
                    float(scheduled_tier_info.monthly_credits)
                    if scheduled_tier_info.monthly_credits
                    else 0
                )
                ...
                if current_credits > scheduled_credits:
                    ...
                    await Cache.invalidate(f"subscription_tier:{account_id}")
                    logger.info(
                        f"[UPGRADE] Cancelled scheduled downgrade to {scheduled_tier_info.display_name} - user upgraded to {current_tier_info.display_name}"
                    )
    ...
    async def _handle_commitment_updates(self, event, subscription: Dict, account_id: str):
        ...
        previous_attributes = event.data.get("previous_attributes", {})
        prev_price_id = (
            previous_attributes.get("items", {}).get("data", [{}])[0].get("price", {}).get("id")
            if previous_attributes.get("items")
            else None
        )
        commitment_type = subscription.metadata.get("commitment_type")

        if price_id and (
            (price_id != prev_price_id and is_commitment_price_id(price_id))
            or (commitment_type == "yearly_commitment" and is_commitment_price_id(price_id))
        ):
            await self.commitment_service.track_commitment_if_needed(
    ...
    async def handle_trial_subscription(cls, subscription, account_id, new_tier, client):
        handler = cls()
        await handler.trial_service.activate_trial_for_subscription(
            subscription, account_id, new_tier
        )
