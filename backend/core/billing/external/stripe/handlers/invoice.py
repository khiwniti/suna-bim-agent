...
class InvoiceHandler:
    ...
    async def handle_subscription_renewal(invoice: Dict, stripe_event_id: str = None):
        try:
            ...
            guard_check = await billing_repo.check_renewal_already_processed(
            ...
            if not acquired:
                logger.error(
                    f"[RENEWAL] Failed to acquire lock for {account_id} period {period_start}"
                )
                return

            try:
                logger.debug(
                    f"[RENEWAL] Starting renewal logic for account {account_id}, billing_reason={billing_reason}"
                )
                ...
                logger.debug(
                    f"[RENEWAL] is_prorated={is_prorated_upgrade}, has_full_cycle={has_full_cycle_charge}"
                )
                ...
                if billing_reason == "subscription_cycle":
                    ...
                    has_full_cycle_charge = True
                    logger.debug(
                        f"[RENEWAL] Billing reason is subscription_cycle, forcing has_full_cycle=True"
                    )
                    ...
                    if is_prorated_upgrade:
                        ...
                        logger.info(
                            f"[RENEWAL] Processing prorated upgrade for account {account_id}"
                        )
                        ...
                        existing_tier_result = await billing_repo.get_credit_account_tier(
                            account_id
                        )
                        ...
                        if tier_info and existing_tier_result:
                            ...
                            existing_tier = (
                                get_tier_by_name(existing_tier_name) if existing_tier_name else None
                            )

                            if (
                                existing_tier
                                and tier_info.name != existing_tier.name
                                and float(tier_info.monthly_credits)
                                > float(existing_tier.monthly_credits)
                            ):
                                logger.info(
                                    f"[RENEWAL] Prorated upgrade detected ({existing_tier.name} -> {tier_info.name}) - credits handled by subscription.updated, updating metadata only"
                                )
                    ...
                    if not has_full_cycle_charge:
                        logger.info(
                            f"[RENEWAL] subscription_update without full cycle charge - skipping (likely mid-period change)"
                        )
                        return

                    logger.info(
                        f"[RENEWAL] subscription_update WITH full cycle charge detected - treating as renewal and continuing processing"
                    )
                ...
                logger.debug(
                    f"[RENEWAL] Account tier={tier}, trial_status={trial_status}, invoice_id={invoice_id}"
                )
                ...
                logger.debug(
                    f"[RENEWAL] subscription_status={subscription_status}, trial_status={trial_status}, billing_reason={billing_reason}"
                )

                if (
                    trial_status == "active"
                    and billing_reason == "subscription_create"
                    and is_still_trialing
                ):
                    logger.debug(
                        f"[RENEWAL] Trial + subscription_create + still trialing, updating invoice ID only"
                    )
                    await billing_repo.update_credit_account(
                        account_id, {"last_processed_invoice_id": invoice_id}
                    )
                    return

                if trial_status == "active" and not is_still_trialing:
                    logger.info(
                        f"[RENEWAL] Trial ended (subscription is {subscription_status}), will grant first paid period credits and mark trial as converted"
                    )
                    await billing_repo.update_credit_account(
                        account_id, {"trial_status": "converted"}
                    )
                    await billing_repo.mark_trial_converted(account_id)
                    trial_status = "converted"
                    logger.info(
                        f"[RENEWAL] Trial status updated to 'converted' for account {account_id}"
                    )
                ...
                if not price_id:
                    logger.warning(
                        f"[RENEWAL] No price_id from subscription, attempting to extract from invoice"
                    )
                    if invoice.get("lines", {}).get("data"):
                        for line in invoice["lines"]["data"]:
                            if line.get("price") and line.get("price", {}).get("id"):
                                ...
                                logger.info(
                                    f"[RENEWAL] Found price_id from invoice line: {price_id}"
                                )
                                break
                ...
                if price_id:
                    ...
                    if tier_info:
                        ...
                        logger.error(
                            f"[RENEWAL] Price ID {price_id} not recognized in tier configuration - cannot process invoice"
                        )
                        raise ValueError(f"Unrecognized price_id: {price_id}")
                else:
                    logger.error(
                        f"[RENEWAL] Could not determine price_id from subscription or invoice - cannot process invoice"
                    )
                ...
                if trial_status == "cancelled" and billing_reason == "subscription_create":
                    logger.info(
                        f"[RENEWAL] Cancelled trial user subscribing - resetting trial status to 'none'"
                    )
                    trial_status = "none"
                ...
                if not tier_config:
                    logger.error(
                        f"[RENEWAL] Tier {tier} not found in configuration - cannot process invoice"
                    )
                ...
                if not tier_config.monthly_refill_enabled:
                    logger.info(
                        f"[RENEWAL] Skipping credit grant for tier {tier} - monthly_refill_enabled=False"
                    )
                    ...
                    await billing_repo.update_credit_account(
                        account_id,
                        {
                            ...
                            "next_credit_grant": BillingPeriodHandler._calculate_next_credit_grant(
                ...
                logger.info(
                    f"[RENEWAL] invoice_id={invoice_id}, billing_reason={billing_reason}, monthly_credits={monthly_credits}, tier={tier}"
                )

                if monthly_credits <= 0:
                    logger.error(
                        f"[RENEWAL] No credits configured for tier {tier} - cannot process invoice"
                    )
                ...
                if is_initial_subscription:
                    ...
                    if last_grant and current_db_tier == tier:
                        ...
                        seconds_since_grant = (
                            datetime.now(timezone.utc) - last_grant_dt
                        ).total_seconds()

                        if seconds_since_grant < 60:
                            logger.info(
                                f"[INITIAL GRANT SKIP] Credits already granted {seconds_since_grant:.0f}s ago for tier {tier}, skipping duplicate grant"
                            )
                            update_data = {
                                "last_processed_invoice_id": invoice_id,
                                "tier": tier,
                        ...
                        logger.info(
                            f"[TIER CHANGE DETECTED] Last grant was for tier {current_db_tier}, but invoice is for tier {tier} - will grant credits for new tier"
                        )
                ...
                if is_true_renewal:
                    ...
                    if tier_config and not tier_config.monthly_refill_enabled:
                        logger.info(
                            f"[RENEWAL SKIP] Skipping monthly credit grant for {account_id} - tier {tier} has monthly_refill_enabled=False"
                        )
                        await billing_repo.update_credit_account(
                            account_id,
                            {
                    ...
                    logger.info(
                        f"[RENEWAL] Using atomic function to grant ${monthly_credits} credits for {account_id} (TRUE RENEWAL)"
                    )
                    ...
                    if recent_credits:
                        for entry in recent_credits:
                            ...
                            if (
                                (
                                    (
                                        "checkout.session.completed" in description
                                        or "checkout_" in stripe_event_id_check
                                        or "free_upgrade_" in stripe_event_id_check
                                    )
                                    and float(amount) == float(monthly_credits)
                                    and tier in description
                                )
                                or (
                                    "Tier upgrade to" in description
                                    and float(amount) == float(monthly_credits)
                                )
                                or (
                                    "Subscription credits for" in description
                                    and float(amount) == float(monthly_credits)
                                )
                                or (
                                    "New subscription:" in description
                                    and tier in description
                                    and float(amount) == float(monthly_credits)
                                )
                            ):
                                ...
                                logger.info(
                                    f"[INITIAL GRANT SKIP] Found recent upgrade credit grant: {description} (${amount})"
                                )
                                break

                    if checkout_credits_already_granted:
                        logger.info(
                            f"[INITIAL GRANT SKIP] Checkout handler already granted ${monthly_credits} credits for {account_id}, skipping invoice grant"
                        )
                    ...
                    if tier_config and not tier_config.monthly_refill_enabled:
                        logger.info(
                            f"[INITIAL GRANT SKIP] Skipping initial credit grant for {account_id} - tier {tier} has monthly_refill_enabled=False"
                        )
                    else:
                        logger.info(
                            f"[INITIAL GRANT] Granting ${monthly_credits} credits for {account_id} (billing_reason={billing_reason}, NOT a renewal - will not block future renewals)"
                        )
                    ...
                    if trial_status in ["cancelled", "expired"]:
                        update_data["trial_status"] = "none"
                        logger.info(
                            f"[RENEWAL] Resetting trial_status from {trial_status} to 'none'"
                        )
                ...
                if is_true_renewal and result:
                    ...
                    if result.get("duplicate_prevented"):
                        ...
                        try:
                            ...
                            logger.warning(
                                f"[RENEWAL] Cache invalidation failed for {account_id}: {str(cache_err)}"
                            )
                    else:
                        error_msg = result.get("error", "Unknown error")
                        logger.error(
                            f"[RENEWAL ERROR] Failed to grant credits for account {account_id}: {error_msg}"
                        )
                elif is_true_renewal:
                    logger.error(
                        f"[RENEWAL ERROR] Failed to grant credits for account {account_id}: No response from atomic function"
                    )
                elif not is_true_renewal and result and result.get("success"):
                    logger.info(
                        f"[INITIAL GRANT SUCCESS] ✅ Granted ${monthly_credits} initial subscription credits to {account_id}"
                    )
    ...
    async def handle_invoice_payment_failed(event, client=None):
        ...
        logger.info(
            f"[PAYMENT FAILED] Processing: subscription_id={subscription_id}, billing_reason={billing_reason}"
        )
        ...
        try:
            ...
            logger.debug(
                f"[PAYMENT FAILED] Retrieved subscription: status={subscription_status}, account_id={account_id}"
            )
            ...
            if not account_id:
                ...
                if customer_data:
                    account_id = customer_data["account_id"]
                    logger.info(
                        f"[PAYMENT FAILED] Found account_id from billing_customers: {account_id}"
                    )
            ...
            if account_id:
                try:
                    is_new_subscription_failure = (
                        ...
                        and subscription_status in ["incomplete", "incomplete_expired", "canceled"]
                    )

                    if is_new_subscription_failure:
                        logger.info(
                            f"[PAYMENT FAILED] Initial subscription payment failed for {account_id} - reverting tier to 'free'"
                        )
                        ...
                        logger.info(
                            f"[PAYMENT FAILED] Successfully reverted {account_id} to free tier"
                        )
                    else:
                        await billing_repo.update_credit_account(
                            account_id,
                        ...
                        logger.info(
                            f"[PAYMENT FAILED] Marked payment as failed for account {account_id}"
                        )
                ...
                logger.warning(
                    f"[PAYMENT FAILED] Could not find account_id for subscription {subscription_id}"
                )
