...
class CheckoutHandler:
    ...
    async def handle_checkout_session_completed(event, client=None):
        ...
        logger.info(
            f"[WEBHOOK] Checkout session completed - ID: {session.get('id')}, Has subscription: {bool(session.get('subscription'))}, Metadata: {session.get('metadata', {})}"
        )
        ...
        if session.get("metadata", {}).get("type") == "credit_purchase":
            ...
            logger.warning(
                f"[WEBHOOK] Checkout session has neither credit_purchase type nor subscription"
            )
    ...
    async def _handle_credit_purchase(session):
        ...
        try:
            ...
            try:
                ...
                if purchase_id:
                    await billing_repo.update_purchase_by_id(
                        purchase_id=purchase_id, status="failed", error_message=str(e)
                    )
            except:
                pass
            return
        ...
        if final_state:
            before = (
                current_state
                if current_state
                else {"balance": 0, "expiring_credits": 0, "non_expiring_credits": 0}
            )
            after = final_state
    ...
    async def _handle_yearly_upgrade_payment(session):
        ...
        if not account_id or not target_price_id:
            logger.error(
                f"[YEARLY UPGRADE] Missing required metadata: account_id={account_id}, target_price_id={target_price_id}"
            )
            return

        logger.info(
            f"[YEARLY UPGRADE] Processing upgrade payment for {account_id} → {target_price_id}"
        )
        ...
        try:
            ...
            logger.info(
                f"[YEARLY UPGRADE] Created new subscription {new_subscription.id} for {account_id}"
            )
            ...
            await billing_repo.update_credit_account(
                account_id,
                {
            ...
            logger.info(
                f"[YEARLY UPGRADE] ✅ Successfully completed yearly upgrade for {account_id}"
            )
    ...
    async def _handle_subscription_checkout(session):
        logger.info(
            f"[WEBHOOK CHECKOUT] Starting subscription checkout handler for session {session.get('id')}"
        )
        account_id = session.get("metadata", {}).get("account_id")

        if not account_id:
            customer_data = await billing_repo.get_billing_customer_by_stripe_id(
        ...
        logger.info(
            f"[WEBHOOK CHECKOUT] Account ID: {account_id}, Has subscription: {bool(session.get('subscription'))}"
        )
        ...
        if account_id:
            ...
            logger.info(
                f"[WEBHOOK CHECKOUT] Trial check data: {trial_check if trial_check else 'No data'}"
            )
            ...
            if trial_check and trial_check.get("trial_status") == "active":
                ...
                if subscription_id:
                    ...
                    if not tier_info:
                        logger.error(
                            f"[WEBHOOK CHECKOUT] Cannot process checkout - price_id {price_id} not recognized"
                        )
        ...
        logger.info(
            f"[WEBHOOK CHECKOUT] Checking converting_from_trial metadata: {session.get('metadata', {}).get('converting_from_trial')}"
        )
        ...
        logger.info(
            f"[WEBHOOK CHECKOUT] Checking trial_start metadata: {session.get('metadata', {}).get('trial_start')}"
        )
        ...
        logger.info(
            f"[WEBHOOK CHECKOUT] Reached default handler section. Has subscription: {bool(session.get('subscription'))}"
        )
    ...
    async def _handle_trial_conversion_webhook(session):
        ...
        if session.get("subscription"):
            ...
            if not tier_info:
                logger.error(
                    f"[TRIAL CONVERSION] Cannot process trial conversion - price_id {price_id} not recognized"
                )
                raise ValueError(f"Unrecognized price_id: {price_id}")
            ...
            try:
                logger.info(
                    f"[TRIAL CONVERSION] 🔒 Acquired lock for trial conversion for {account_id}"
                )
                ...
                next_grant_date = datetime.fromtimestamp(
                ...
                if plan_type == "yearly":
                    ...
                    logger.info(
                        f"[WEBHOOK TRIAL CONVERSION] Yearly plan detected - setting next_credit_grant to 1 month from now: {next_grant_date}"
                    )
                ...
                cancel_after_checkout = subscription.get("metadata", {}).get(
                    "cancel_after_checkout"
                )
                if cancel_after_checkout:
                    logger.info(
                        f"[TRIAL CONVERSION] Cancelling old subscription {cancel_after_checkout} after checkout completion"
                    )
                    try:
                        await StripeAPIWrapper.cancel_subscription(
                            cancel_after_checkout, cancel_immediately=True
                        )
                        logger.info(
                            f"[TRIAL CONVERSION] ✅ Successfully cancelled old subscription {cancel_after_checkout}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"[TRIAL CONVERSION] Could not cancel old subscription {cancel_after_checkout}: {e}"
                        )
    ...
    async def _handle_trial_start_webhook(session):
        ...
        if session.get("subscription"):
            ...
            if subscription.status == "trialing":
                ...
                if not acquired:
                    logger.error(
                        f"[WEBHOOK TRIAL] Failed to acquire lock for trial activation {account_id}"
                    )
                    return

                try:
                    logger.info(
                        f"[WEBHOOK TRIAL] 🔒 Acquired lock for trial activation for {account_id}"
                    )

                    existing_account = await billing_repo.get_credit_account_subscription_info(
                        account_id
                    )
                    if existing_account and existing_account.get("trial_status") == "active":
                        logger.info(
                            f"[WEBHOOK] Trial already active for account {account_id} in checkout handler, skipping duplicate credits"
                        )
                        return
                    ...
                    recent_trial_credits = await billing_repo.get_recent_credit_ledger_entries(
                        account_id, recent_time
                    )

                    trial_description = f"{TRIAL_DURATION_DAYS}-day free trial credits"
                    if recent_trial_credits and any(
                        e.get("description") == trial_description for e in recent_trial_credits
                    ):
                        logger.warning(
                            f"[WEBHOOK] Trial credits already granted for account {account_id} (found in ledger), skipping duplicate"
                        )
                        return
                    ...
                    await billing_repo.upsert_trial_history(
                        account_id,
                        {
                    ...
                    if cancel_after_checkout:
                        logger.info(
                            f"[WEBHOOK] Cancelling old subscription {cancel_after_checkout} after trial start"
                        )
                        try:
                            await StripeAPIWrapper.cancel_subscription(
                                cancel_after_checkout, cancel_immediately=True
                            ...
                            logger.info(
                                f"[WEBHOOK] ✅ Successfully cancelled old subscription {cancel_after_checkout}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"[WEBHOOK] Could not cancel old subscription {cancel_after_checkout}: {e}"
                            )
                    ...
                    logger.info(
                        f"[WEBHOOK] ✅ Trial activated for account {account_id} via checkout.session.completed - granted ${TRIAL_CREDITS} credits"
                    )
                finally:
                    await lock.release()
            else:
    ...
    async def _handle_default_subscription_webhook(session):
        subscription_id = session["subscription"]
        subscription = await StripeAPIWrapper.safe_stripe_call(
            stripe.Subscription.retrieve_async, subscription_id, expand=["default_payment_method"]
        )
        ...
        if subscription.status == "active":
            ...
            if not account_id:
                logger.warning(
                    f"[WEBHOOK] Could not find account_id for checkout session {session.get('id')}"
                )
                return
            ...
            if not tier_info:
                logger.warning(
                    f"[WEBHOOK] Unknown price_id {price_id} for subscription {subscription_id}"
                )
                return
            ...
            if credit_account:
                ...
                upgrade_from_subscription = session.get("metadata", {}).get(
                    "upgrade_from_subscription"
                )
                if (
                    upgrade_from_subscription
                    and current_subscription_id == upgrade_from_subscription
                ):
                    logger.info(
                        f"[WEBHOOK DEFAULT] Processing upgrade via checkout: {upgrade_from_subscription} -> {subscription_id}"
                    )
                    # This is an upgrade - proceed with processing
                elif (
                    current_subscription_id == subscription_id
                    and credit_account.get("tier") == tier_info.name
                ):
                ...
                if credit_account.get("trial_status") == "cancelled":
                    logger.info(
                        f"[WEBHOOK DEFAULT] User {account_id} with cancelled trial is subscribing - handling as new subscription"
                    )
            ...
            try:
                ...
                if plan_type == "yearly":
                    next_grant_date = billing_anchor + relativedelta(months=1)
                    logger.info(
                        f"[WEBHOOK DEFAULT] Yearly plan detected - setting next_credit_grant to 1 month from now: {next_grant_date}"
                    )
                    ...
                    logger.info(
                        f"[WEBHOOK DEFAULT] Yearly commitment detected - setting next_credit_grant to period end: {next_grant_date}"
                    )
                else:
                    next_grant_date = datetime.fromtimestamp(
                        subscription["current_period_end"], tz=timezone.utc
                    ...
                    logger.info(
                        f"[WEBHOOK DEFAULT] Monthly plan - setting next_credit_grant to period end: {next_grant_date}"
                    )

                current_tier = credit_account.get("tier") if credit_account else "none"
                is_tier_upgrade = (
                    current_tier and current_tier != "none" and current_tier != tier_info.name
                )
                ...
                if is_tier_upgrade:
                    ...
                    if not tier_info.monthly_refill_enabled:
                        logger.info(
                            f"[WEBHOOK DEFAULT] Skipping upgrade credits for tier {tier_info.name} - monthly_refill_enabled=False"
                        )
                    else:
                        logger.info(
                            f"[WEBHOOK DEFAULT] Tier upgrade detected: {current_tier} -> {tier_info.name}"
                        )
                        logger.info(
                            f"[WEBHOOK DEFAULT] Replacing existing credits with ${tier_info.monthly_credits} for {tier_info.name} (Stripe handled payment proration)"
                        )

                        import time

                        unique_id = (
                            f"checkout_upgrade_{account_id}_{tier_info.name}_{int(time.time())}"
                        )
                    ...
                    if not tier_info.monthly_refill_enabled:
                        logger.info(
                            f"[WEBHOOK DEFAULT] Skipping initial credits for tier {tier_info.name} - monthly_refill_enabled=False"
                        )
                    else:
                        logger.info(
                            f"[WEBHOOK DEFAULT] Granting ${tier_info.monthly_credits} credits for new {plan_type} subscription"
                        )
                ...
                logger.info(
                    f"[WEBHOOK DEFAULT] Granted {tier_info.monthly_credits} credits to {account_id}"
                )
                ...
                if subscription.get("metadata", {}).get("requires_cleanup") == "true":
                    logger.info(
                        f"[WEBHOOK] Cleanup required for account {account_id}, removing duplicate subscriptions"
                    )
                    await CheckoutHandler._cleanup_duplicate_subscriptions(
                ...
                if cancel_after_checkout:
                    logger.info(
                        f"[WEBHOOK DEFAULT] Cancelling old subscription {cancel_after_checkout} after checkout completion"
                    )
                    try:
                        await StripeAPIWrapper.cancel_subscription(
                            cancel_after_checkout, cancel_immediately=True
                        ...
                        logger.info(
                            f"[WEBHOOK DEFAULT] ✅ Successfully cancelled old subscription {cancel_after_checkout}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"[WEBHOOK DEFAULT] Could not cancel old subscription {cancel_after_checkout}: {e}"
                        )
            finally:
                await lock.release()

    @staticmethod
    async def _cleanup_duplicate_subscriptions(
        customer_id: str, keep_subscription_id: str, account_id: str
    ):
        try:
            logger.info(
                f"[WEBHOOK CLEANUP] Checking for duplicate subscriptions for customer {customer_id}"
            )

            customer_subs = await StripeAPIWrapper.safe_stripe_call(
                stripe.Subscription.list_async, customer=customer_id, status="active", limit=20
            )
            ...
            for sub in customer_subs.data:
                if sub.id != keep_subscription_id:
                    logger.info(
                        f"[WEBHOOK CLEANUP] Found duplicate subscription {sub.id}, canceling..."
                    )
                    try:
                        await StripeAPIWrapper.safe_stripe_call(
                            stripe.Subscription.cancel_async, sub.id, prorate=True
                        )
                        duplicates_found.append(sub.id)
                        logger.info(
                            f"[WEBHOOK CLEANUP] ✅ Canceled duplicate subscription {sub.id}"
                        )
                    except Exception as e:
                        logger.error(
                            f"[WEBHOOK CLEANUP] Failed to cancel duplicate subscription {sub.id}: {e}"
                        )

            if duplicates_found:
                logger.info(
                    f"[WEBHOOK CLEANUP] Cleaned up {len(duplicates_found)} duplicate subscriptions for {account_id}"
                )
            else:
            ...
            logger.error(
                f"[WEBHOOK CLEANUP] Error during subscription cleanup for {account_id}: {e}"
            )
