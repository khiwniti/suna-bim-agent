...
async def update_scheduled_tier_change(
    account_id: str, scheduled_tier: str, scheduled_date: str, price_id: str
) -> None:
    sql = """
...
async def update_credit_account(account_id: str, update_data: Dict[str, Any]) -> bool:
    ...
    if "tier" in update_data or "trial_status" in update_data:
        try:
            ...
            logger.warning(
                f"[CREDIT_ACCOUNT] Failed to invalidate tier caches for {account_id}: {e}"
            )
...
async def update_credit_account_balances(
    account_id: str, expiring_credits: float, non_expiring_credits: float, balance: float
) -> None:
    sql = """
    UPDATE credit_accounts
...
async def clear_credit_account_scheduled_changes(account_id: str) -> bool:
    return await update_credit_account(
        account_id,
        {
...
async def check_renewal_already_processed(
    account_id: str, period_start: int
) -> Optional[Dict[str, Any]]:
    sql = "SELECT check_renewal_already_processed(:p_account_id, :p_period_start)"
    result = await execute_one(sql, {"p_account_id": account_id, "p_period_start": period_start})
