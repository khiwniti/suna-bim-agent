...
async def update_trial_history(
    account_id: str, update_data: Dict[str, Any], status_filter: Optional[str] = None
) -> None:
    if not update_data:
        return
...
async def mark_trial_converted(account_id: str) -> None:
    ...
    """
    await execute_mutate(
        sql, {"account_id": account_id, "ended_at": datetime.now(timezone.utc).isoformat()}
    )
...
async def insert_credit_ledger_entry(
    account_id: str, amount: float, balance_after: float, entry_type: str, description: str
) -> None:
    sql = """
...
async def get_accounts_due_for_refill(plan_type: str, limit: Optional[int] = None) -> list:
    ...
    """
    params = {"plan_type": plan_type, "now": datetime.now(timezone.utc).isoformat()}
