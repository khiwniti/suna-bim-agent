...
class FreeTierService:
    ...
    async def auto_subscribe_to_free_tier(
        ...
        if not acquired:
            logger.warning(
                f"[FREE TIER] Could not acquire lock for {account_id}, another process may be setting up free tier"
            )
            return {"success": False, "message": "Lock acquisition failed"}

        try:
            logger.info(
                f"[FREE TIER] Auto-subscribing user {account_id} to free tier (lock acquired)"
            )
            ...
            if self.is_local_mode:
                ...
                if not stripe_customer_id:
                    await billing_repo.create_billing_customer(
                        mock_customer_id, account_id, email or f"{account_id}@local.dev"
                    )
                    stripe_customer_id = mock_customer_id

                # Create credit account with mock subscription
                await billing_repo.upsert_credit_account(
                    account_id,
                    {
                        "tier": "free",
                ...
                if refreshed:
                    logger.info(
                        f"[FREE TIER] ✅ LOCAL mode: Triggered initial daily refresh: ${amount} credits granted to {account_id}"
                    )

                current_balance_result = await billing_repo.get_credit_account_balance(account_id)
                current_balance = (
                    float(current_balance_result.get("balance", 0)) if current_balance_result else 0
                )
                ...
                if current_balance < FREE_TIER_INITIAL_CREDITS:
                    ...
                    logger.info(
                        f"[FREE TIER] LOCAL mode: Granting {FREE_TIER_INITIAL_CREDITS} initial credits to {account_id}"
                    )
                ...
                logger.info(
                    f"[FREE TIER] ✅ LOCAL mode: Created mock subscription {mock_subscription_id} for {account_id}"
                )
            ...
            if stripe_customer_id:
                try:
                    ...
                    logger.info(
                        f"[FREE TIER] Verified existing Stripe customer {stripe_customer_id} for {account_id}"
                    )
                except stripe.error.InvalidRequestError as e:
                    if "No such customer" in str(e):
                        logger.warning(
                            f"[FREE TIER] Customer {stripe_customer_id} not found in Stripe, will create new customer"
                        )
                ...
                if account_details:
                    ...
                    if user_id:
                        ...
                        try:
                            ...
                            email = (
                                user_result.user.email if user_result and user_result.user else None
                            )
                        except:
                            pass
            ...
            subscription = await self.stripe.Subscription.create_async(
                ...
                days_until_due=None,
                metadata={"account_id": account_id, "tier": "free"},
            )
            ...
            if refreshed:
                logger.info(
                    f"[FREE TIER] ✅ Triggered initial daily refresh: ${amount} credits granted to {account_id}"
                )
            else:
                logger.warning(
                    f"[FREE TIER] Daily refresh did not grant credits on signup for {account_id}"
                )
            ...
            if current_balance < FREE_TIER_INITIAL_CREDITS:
                ...
                logger.info(
                    f"[FREE TIER] Granting {FREE_TIER_INITIAL_CREDITS} initial credits to {account_id}"
                )
                await credit_manager.add_credits(
                    account_id=account_id,
                    amount=Decimal(str(FREE_TIER_INITIAL_CREDITS)),
                ...
                logger.info(
                    f"[FREE TIER] ✅ Granted {FREE_TIER_INITIAL_CREDITS} credits to new free tier user {account_id}"
                )
            else:
                logger.info(
                    f"[FREE TIER] User {account_id} already has sufficient credits, skipping grant"
                )

            logger.info(
                f"[FREE TIER] ✅ Successfully created free tier subscription {subscription.id} for {account_id}"
            )
