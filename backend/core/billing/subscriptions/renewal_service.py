...
class RenewalService:
    async def process_yearly_plan_refills(self) -> Dict:
        try:
            ...
            if result:
                ...
                logger.info(
                    f"[YEARLY REFILL] Processed {accounts_processed} accounts: {successful} successful, {failed} failed"
                )
            ...
            return {"success": False, "error": str(e), "accounts_processed": 0}
    ...
    async def grant_monthly_credits(
        ...
        if not tier.monthly_refill_enabled:
            logger.info(
                f"[RENEWAL] Skipping monthly credit grant for {account_id} - tier {tier_name} has monthly_refill_enabled=False"
            )
            return {
                "success": True,
        ...
        try:
            ...
            logger.info(
                f"[RENEWAL] Granted {monthly_credits} credits to {account_id} for {tier_name}"
            )
    ...
    async def update_plan_type(
        self, account_id: str, plan_type: str, billing_cycle_anchor: Optional[datetime] = None
    ) -> Dict:
        if plan_type not in ["monthly", "yearly", "yearly_commitment"]:
            raise ValueError(f"Invalid plan_type: {plan_type}")

        update_data = {"plan_type": plan_type, "updated_at": datetime.now(timezone.utc).isoformat()}
        ...
        try:
            ...
            return {"success": True, "account_id": account_id, "plan_type": plan_type}
    ...
    async def get_accounts_due_for_refill(
        self, plan_type: str = "yearly", limit: Optional[int] = None
    ) -> List[Dict]:
        return await billing_repo.get_accounts_due_for_refill(plan_type, limit)

    async def calculate_next_refill_date(self, account_id: str) -> Optional[datetime]:
