    ...
    async def get_user_subscription_tier(account_id: str, skip_cache: bool = False) -> Dict:
        ...
        if not skip_cache:
            try:
                ...
                if redis_cached:
                    logger.info(
                        f"[TIER] Cache hit for {account_id[:8]}... tier={redis_cached.get('name')} thread_limit={redis_cached.get('thread_limit')} project_limit={redis_cached.get('project_limit')}"
                    )
                    return redis_cached
            except Exception:
                pass
        ...
        if trial_status == "active" and tier_name == "none":
            ...
            logger.info(
                f"[TIER] Trial active but tier=none for {account_id}, using TRIAL_TIER: {TRIAL_TIER}"
            )
        ...
        logger.info(
            f"[TIER] Fresh fetch for {account_id[:8]}... tier={tier_name} thread_limit={tier_obj.thread_limit} project_limit={tier_obj.project_limit}"
        )
    ...
    async def get_allowed_models_for_user(user_id: str, client=None) -> List[str]:
        try:
            ...
            if tier_info.get("models"):
                ...
                logger.debug(
                    f"[ALLOWED_MODELS] User {user_id} has access to {len(allowed_model_ids)} models: {[m for m in allowed_model_ids]}"
                )
                return allowed_model_ids

            else:
                logger.debug(
                    f"[ALLOWED_MODELS] User {user_id} has no model access (tier: {tier_name})"
                )
                return []
