...
from core.billing.shared.config import get_tier_by_price_id, get_plan_type, get_price_type
...
class BillingPeriodHandler:
    ...
    async def handle_subscription_for_billing_period(
        ...
        logger.info(
            f"[BILLING PERIOD] Processing {plan_type} subscription for account {account_id}, tier {tier_info.name}"
        )

        if plan_type == "yearly":
            next_credit_grant = billing_anchor + relativedelta(months=1)
            logger.info(
                f"[BILLING PERIOD] Yearly plan: next credits in 1 month ({next_credit_grant})"
            )
            ...
            logger.info(
                f"[BILLING PERIOD] Yearly commitment: next credits at period end ({next_credit_grant})"
            )
            ...
            logger.info(
                f"[BILLING PERIOD] Monthly plan: next credits at period end ({next_credit_grant})"
            )
        ...
        logger.info(
            f"[BILLING PERIOD] ✅ Successfully processed {plan_type} subscription for {account_id}"
        )
    ...
    async def _grant_initial_credits_for_billing_period(
        ...
        logger.info(
            f"[BILLING PERIOD] Granting ${credits_amount} {plan_type} credits to {account_id}, expires {expires_at}"
        )
        ...
        logger.info(
            f"[BILLING PERIOD] ✅ Granted ${credits_amount} credits for {plan_type} subscription"
        )

    @staticmethod
    def is_billing_period_change(
        current_tier: str, new_tier: str, old_plan_type: str, new_plan_type: str
    ) -> bool:
        return (
    ...
    async def handle_billing_period_switch(
        account_id: str, subscription: Dict, old_plan_type: str, new_plan_type: str
    ) -> Dict:
