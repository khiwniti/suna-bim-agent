...
class SubscriptionLifecycleHandler:
    ...
    async def _cancel_subscription(self, account_id: str, feedback: Optional[str] = None) -> Dict:
        logger.info(
            f"[CANCEL] Processing cancellation for {account_id} - will downgrade to free tier at period end"
        )
        ...
        try:
            ...
            result = await SchedulingHandler.schedule_tier_downgrade(
                account_id=account_id, target_tier_key="free", commitment_type="monthly"
            )
            ...
            return {
                ...
                "message": result.get(
                    "message",
                    "Your plan will be downgraded to the free tier at the end of your current billing period",
                ),
    ...
    async def _save_cancellation_feedback(self, account_id: str, feedback: str):
        try:
            credit_account = await self.credit_repo.get_credit_account(
                account_id, "stripe_subscription_id"
            )
            ...
            if credit_account and credit_account.get("stripe_subscription_id"):
                ...
                await StripeAPIWrapper.modify_subscription(
                    subscription_id, metadata={"cancellation_feedback": feedback}
                )
    ...
    async def _reactivate_subscription(self, account_id: str) -> Dict:
        ...
        try:
            ...
            await StripeAPIWrapper.modify_subscription(
                subscription_id, cancel_at_period_end=False, cancel_at=None
            )
    ...
    async def _handle_subscription_change(
        self, subscription: Dict, previous_attributes: Dict = None
    ):
        logger.info(
            f"[SUBSCRIPTION] Processing change for subscription {subscription.get('id')}, status: {subscription.get('status')}"
        )

        customer_id = subscription.get("customer")
        account_id = await self.lifecycle_service.get_account_id_from_subscription(
        ...
        price_id = (
            subscription["items"]["data"][0]["price"]["id"] if subscription.get("items") else None
        )
        ...
        logger.debug(
            f"[SUBSCRIPTION] Account: {account_id}, Price: {price_id}, Billing anchor: {billing_anchor}"
        )

        if subscription.get("status") == "past_due":
            logger.info(
                f"[GRACE PERIOD] Subscription {subscription.get('id')} is past_due - updating metadata only, NO CREDITS granted (credits only granted on invoice.payment_succeeded)"
            )
            await self._update_subscription_metadata_only(
                account_id, subscription, price_id, payment_status="past_due"
            )
        ...
        await self._handle_trial_status_transition(
            subscription, previous_attributes, current_account, account_id
        )

        is_renewal = self.lifecycle_service.is_renewal(
            subscription, current_account, billing_anchor
        )

        if is_renewal:
            logger.info(
                f"[RENEWAL BLOCK] Subscription {subscription['id']} identified as renewal - NO CREDITS will be granted"
            )
        ...
        await self._process_subscription_change(
            subscription, account_id, price_id, billing_anchor, current_account
        )

    async def _is_already_processed(self, account_id: str, period_start: int) -> bool:
        guard_check = await self.credit_repo.check_renewal_already_processed(
    ...
    async def _handle_trial_status_transition(
        self,
        subscription: Dict,
        previous_attributes: Dict,
        current_account: Optional[Dict],
        account_id: str,
    ):
        ...
        prev_subscription_status = (
            previous_attributes.get("status") if previous_attributes else None
        )

        if (
            current_trial_status == "active"
            and prev_subscription_status == "trialing"
            and current_subscription_status == "active"
        ):
            logger.info(
                f"[TRIAL END] Subscription transitioned from trialing to active - marking trial as converted"
            )
    ...
    async def _process_subscription_change(
        ...
        new_tier = {"name": tier_info.name, "credits": float(tier_info.monthly_credits)}

        if subscription.status == "trialing" and subscription.get("trial_end"):
            await self._handle_trial_subscription(
                subscription, account_id, new_tier, current_account
            )
            await self.lifecycle_service.invalidate_caches(account_id)
            return
        ...
        if current_account:
            ...
            await self._handle_new_account_subscription(
                account_id, new_tier, billing_anchor, subscription
            )

        await self.lifecycle_service.invalidate_caches(account_id)

    async def _handle_trial_subscription(
        self, subscription: Dict, account_id: str, new_tier: Dict, current_account: Optional[Dict]
    ):
        existing_trial = current_account.get("trial_status") if current_account else None

        if existing_trial in ["converted"]:
            logger.info(
                f"[SUBSCRIPTION] Trial already converted for {account_id}, processing as regular subscription"
            )
            return

        await self.trial_service.activate_trial(account_id, subscription, new_tier)

    async def _handle_existing_account_subscription(
        self,
        existing_data: Dict,
        account_id: str,
        new_tier: Dict,
        ...
        if trial_status == "cancelled" and subscription.status == "active":
            logger.info(
                f"[SUBSCRIPTION] User {account_id} with cancelled trial is subscribing - treating as new subscription"
            )
            await self._handle_new_account_subscription(
                account_id, new_tier, billing_anchor, subscription
            )
        ...
        if not is_tier_change and self.lifecycle_service.is_duplicate_credit_grant(
            last_grant_date, billing_anchor, current_tier_name, new_tier
        ):
            logger.warning(
                f"[DOUBLE CREDIT BLOCK] Duplicate credit grant detected for SAME tier - updating metadata only"
            )
            await self._update_subscription_metadata_only(
                account_id, subscription, subscription["items"]["data"][0]["price"]["id"]
            )
            return
        elif is_tier_change:
            logger.info(
                f"[TIER CHANGE DETECTED] {current_tier_name} -> {new_tier['name']} - proceeding with credit processing"
            )

        current_tier = get_tier_by_name(current_tier_name) if current_tier_name else None
        current_tier_data = (
            {"name": current_tier.name, "credits": float(current_tier.monthly_credits)}
            if current_tier
            else {"name": "none", "credits": 0}
        )

        should_grant_credits = self.lifecycle_service.should_grant_credits(
            current_tier_data, new_tier, subscription
        )

        if should_grant_credits:
            original_tier = subscription.get("metadata", {}).get("previous_tier", current_tier_name)

            is_free_to_paid_upgrade = original_tier in ["free", "none"] and new_tier[
                "name"
            ] not in ["free", "none"]
            is_paid_to_paid_upgrade = (
                original_tier not in ["free", "none"] and original_tier != new_tier["name"]
            )

            is_tier_upgrade = is_free_to_paid_upgrade or is_paid_to_paid_upgrade

            logger.info(
                f"[TIER UPGRADE DETECTION] Original: {original_tier}, New: {new_tier['name']}, Is upgrade: {is_tier_upgrade}"
            )

            if is_tier_upgrade:
                logger.info(
                    f"[TIER UPGRADE] Processing tier upgrade: {current_tier_name} -> {new_tier['name']}"
                )
                await self.lifecycle_service.grant_subscription_credits(
                    account_id, new_tier, billing_anchor, True
                )
            else:
                logger.info(
                    f"[NEW SUBSCRIPTION] Processing brand new subscription for {new_tier['name']}"
                )
                await self.lifecycle_service.grant_subscription_credits(
                    account_id, new_tier, billing_anchor, False
                )

            await self.credit_repo.update_credit_account(
                account_id, {"last_grant_date": billing_anchor.isoformat()}
            )
    ...
    async def _handle_new_account_subscription(
        self, account_id: str, new_tier: Dict, billing_anchor: datetime, subscription: Dict
    ):
    ...
    async def _update_subscription_metadata_only(
        self, account_id: str, subscription: Dict, price_id: str, payment_status: str = "active"
    ):
    ...
    async def _track_commitment_if_needed(self, account_id: str, price_id: str, subscription: Dict):
        ...
        if existing_commitment:
            logger.info(
                f"[COMMITMENT] Commitment already tracked for subscription {subscription['id']}, skipping"
            )
            return
        ...
        end_date = (
            ...
            if commitment_duration == 12
            else start_date + timedelta(days=commitment_duration * 30)
        )
        ...
        logger.info(
            f"[COMMITMENT] Tracked yearly commitment for account {account_id}, subscription {subscription['id']}, ends {end_date.date()}"
        )
