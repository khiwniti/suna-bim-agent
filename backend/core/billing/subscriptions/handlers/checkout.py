...
class SubscriptionCheckoutHandler:
    ...
    async def _create_checkout_session(
        ...
        subscription_status = await self.checkout_service.get_current_subscription_status(
            account_id
        )

        logger.debug(
            f"[CHECKOUT ROUTING] account_id={account_id}, subscription_status={subscription_status}"
        )

        idempotency_key = self.checkout_service.generate_idempotency_key(
            account_id, price_id, commitment_type
        )
    ...
    async def _get_customer_email(self, account_id: str) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(
                f"[CHECKOUT] Could not retrieve customer email for account {account_id}: {e}"
            )
            return None
    ...
    async def _handle_trial_conversion(
        ...
        metadata = self.checkout_service.build_subscription_metadata(
            ...
            commitment_type,
            "trial_conversion",
            subscription_status["current_tier"],
    ...
    async def _cancel_existing_subscription(self, subscription_id: str, reason: str) -> None:
        ...
        try:
            ...
            logger.warning(
                f"[{reason.upper()}] Could not cancel existing subscription {subscription_id}: {e}"
            )
    ...
    async def _create_stripe_checkout_session(
        ...
        logger.debug(
            f"[CHECKOUT] Adaptive pricing enabled - Stripe will auto-detect currency from customer IP/location"
        )
    ...
    async def _handle_existing_subscription_upgrade(
        ...
        if upgrade_type == "yearly_to_monthly":
            ...
            logger.debug(
                f"[STANDARD UPGRADE] Modifying subscription in-place from {current_tier} to {price_id}"
            )
