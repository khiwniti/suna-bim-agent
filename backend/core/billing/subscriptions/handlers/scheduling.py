...
from core.billing.shared.config import get_tier_by_name, get_tier_by_price_id, get_price_type
...
class SchedulingHandler:
    ...
    async def schedule_tier_downgrade(
        account_id: str, target_tier_key: str, commitment_type: Optional[str] = None
    ) -> Dict:
        ...
        try:
            ...
            current_period_end, current_period_end_date = (
                SchedulingHandler._determine_downgrade_timing(
                    subscription, user_commitment_type, commitment_end_date
                )
            )

            target_price_id = SchedulingHandler._determine_target_price_id(
    ...
    @staticmethod
    def _determine_downgrade_timing(
        subscription: Dict, user_commitment_type: str, commitment_end_date: str
    ):
        if user_commitment_type == "yearly_commitment" and commitment_end_date:
            ...
            if datetime.now(timezone.utc) < commitment_end_dt:
                logger.info(
                    f"[DOWNGRADE] User has active commitment until {commitment_end_dt.date()}"
                )
                current_period_end = int(commitment_end_dt.timestamp())
                current_period_end_date = commitment_end_dt
                logger.info(
                    f"[DOWNGRADE] Scheduling downgrade for commitment end date: {commitment_end_date}"
                )
    ...
    def _determine_target_price_id(target_tier, commitment_type: Optional[str]) -> str:
        if commitment_type and len(target_tier.price_ids) >= 3:
            return (
                target_tier.price_ids[2]
                if commitment_type == "yearly_commitment"
                else target_tier.price_ids[0]
            )
    ...
    async def _handle_existing_schedule(
        ...
        try:
            existing_schedule = await StripeAPIWrapper.safe_stripe_call(
                stripe.SubscriptionSchedule.retrieve_async, existing_schedule_id
            )
            ...
            if schedule_status in ["active", "not_started"]:
                ...
                if phases and len(phases) > 0:
                    ...
                    if first_phase_end and first_phase_end < now:
                        logger.info(
                            f"[DOWNGRADE] Schedule {existing_schedule_id} phase 0 has ended (ended at {first_phase_end}, now is {now}), releasing and creating new"
                        )
                        try:
                            await StripeAPIWrapper.safe_stripe_call(
                                stripe.SubscriptionSchedule.release_async, existing_schedule_id
                            )
                            logger.info(
                                f"[DOWNGRADE] Released stale schedule {existing_schedule_id}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"[DOWNGRADE] Could not release schedule {existing_schedule_id}: {e}"
                            )
                ...
                logger.info(
                    f"[DOWNGRADE] Schedule {existing_schedule_id} is {schedule_status}, releasing and creating new"
                )
                try:
                    await StripeAPIWrapper.safe_stripe_call(
                        stripe.SubscriptionSchedule.release_async, existing_schedule_id
                    ...
                    logger.info(
                        f"[DOWNGRADE] Released completed/canceled schedule {existing_schedule_id}"
                    )
                except Exception as e:
                    logger.warning(
                        f"[DOWNGRADE] Could not release schedule {existing_schedule_id}: {e}"
            ...
            if "No such subscription_schedule" in error_str:
                logger.info(
                    f"[DOWNGRADE] Schedule {existing_schedule_id} no longer exists, will create new"
                )
                return None
            elif "phase that has already ended" in error_str:
                logger.info(
                    f"[DOWNGRADE] Schedule {existing_schedule_id} has ended phase, releasing and creating new"
                )
                try:
                    await StripeAPIWrapper.safe_stripe_call(
                        stripe.SubscriptionSchedule.release_async, existing_schedule_id
    ...
    def _build_downgrade_response(
        ...
        if current_price_type != target_price_type:
            if current_price_type == "yearly" or current_price_type == "yearly_commitment":
                ...
                change_description += (
                    f" (switching from {current_price_type} to {target_price_type} billing)"
                )
        ...
        return {
            ...
            "is_commitment": user_commitment_type == "yearly_commitment"
            and commitment_end_date is not None,
        }
    ...
    async def get_scheduled_changes(
        account_id: str, subscription_data: Optional[Dict] = None
    ) -> Dict:
        ...
        if scheduled_tier and current_tier_name == scheduled_tier:
            logger.info(
                f"[SCHEDULED_CHANGES] Scheduled tier {scheduled_tier} matches current tier - downgrade already completed, clearing fields"
            )
    ...
    @staticmethod
    async def _check_revenuecat_pending_changes(
        data: Dict, current_tier_name: str, account_id: str
    ) -> Dict:
    ...
    async def _check_stripe_metadata_for_scheduled_changes(
        data: Dict, current_tier_name: str, subscription_data: Optional[Dict] = None
    ) -> Dict:
            ...
            try:
                ...
                if subscription.get("metadata", {}).get("downgrade") == "true":
                    ...
                    if target_tier_name:
                        if current_tier_name == target_tier_name:
                            logger.info(
                                f"[SCHEDULED_CHANGES] Stripe metadata tier {target_tier_name} matches current tier - downgrade already completed"
                            )
                            return {"has_scheduled_change": False, "scheduled_change": None}
    ...
    async def cancel_scheduled_change(account_id: str) -> Dict:
        ...
        if provider == "revenuecat" and revenuecat_pending:
            ...
            logger.info(
                f"[CANCEL_SCHEDULE] Successfully cancelled RevenueCat scheduled change for {account_id}"
            )
            return {"success": True, "message": "Scheduled change cancelled successfully"}
        ...
        if not scheduled_tier:
                    ...
                    if subscription.get("metadata", {}).get("downgrade") == "true":
                        ...
                        if schedule_id:
                            try:
                                ...
                                logger.info(
                                    f"[CANCEL_SCHEDULE] Released Stripe schedule {schedule_id}"
                                )
                            except Exception as e:
            ...
            return {"success": False, "message": "No scheduled change found"}
