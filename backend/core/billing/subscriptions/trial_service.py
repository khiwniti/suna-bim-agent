...
class TrialService:
    ...
    async def get_trial_status(self, account_id: str) -> Dict:
        if not TRIAL_ENABLED:
            return {"has_trial": False, "message": "Trials are not enabled"}
    ...
    async def cancel_trial(self, account_id: str) -> Dict:
        ...
        if trial_status != "active":
            raise HTTPException(
                status_code=400, detail=f"No active trial to cancel. Current status: {trial_status}"
            )

        if not stripe_subscription_id:
            raise HTTPException(
                status_code=400, detail="No Stripe subscription found for this trial"
            )
        ...
        try:
            ...
            logger.info(
                f"[TRIAL CANCEL] Cancelled Stripe subscription {stripe_subscription_id} for account {account_id}"
            )
    ...
    async def start_trial(self, account_id: str, success_url: str, cancel_url: str) -> Dict:
        ...
        if not TRIAL_ENABLED:
            logger.warning(
                f"[TRIAL SECURITY] Trial attempt rejected - trials disabled for account {account_id}"
            )
        ...
        if history:
            ...
            if existing_status in retryable_statuses:
                logger.info(
                    f"[TRIAL RETRY] Allowing retry for account {account_id} with status: {existing_status}"
                )
                await billing_repo.update_trial_history(
                    account_id,
                    {
                ...
                logger.info(
                    f"[TRIAL SECURITY] Updated trial history record for retry (checkout_pending) for {account_id}"
                )
            else:
                logger.warning(
                    f"[TRIAL SECURITY] Trial attempt rejected - account {account_id} has completed trial"
                )
                logger.warning(
                    f"[TRIAL SECURITY] Existing trial found: "
            ...
            try:
                ...
                logger.info(
                    f"[TRIAL SECURITY] Created trial history record (checkout_pending) for {account_id}"
                )
            ...
            if existing_stripe_sub:
                try:
                    ...
                    if existing_sub and existing_sub.status in ["trialing", "active"]:
                        logger.warning(
                            f"[TRIAL SECURITY] Trial attempt rejected - account {account_id} has existing Stripe subscription {existing_stripe_sub}"
                        )
                        await billing_repo.delete_trial_history_by_status(
                            account_id, "checkout_pending"
                        )
                        raise HTTPException(
                            status_code=403,
        ...
        try:
            ...
            logger.info(
                f"[TRIAL] Creating checkout session for account {account_id} - all security checks passed"
            )
            ...
            await billing_repo.update_trial_history(
                account_id,
                {"status": "checkout_created", "stripe_checkout_session_id": session.id},
                status_filter="checkout_pending",
            )

            logger.info(
                f"[TRIAL SUCCESS] Checkout session created for account {account_id}: {session.id}"
            )
            ...
            logger.error(
                f"[TRIAL ERROR] Failed to create checkout session for account {account_id}: {e}"
            )
            try:
                await billing_repo.update_trial_history(
                    account_id,
