...
class LifecycleService:
    ...
    async def get_account_id_from_subscription(
        self, subscription: Dict, customer_id: str
    ) -> Optional[str]:
    ...
    def is_renewal(
        self, subscription: Dict, current_account_data: Optional[Dict], billing_anchor
    ) -> bool:
        ...
        is_new_subscription = (
            old_subscription_id is None
            or old_subscription_id == ""
            or old_subscription_id != subscription.get("id")
        )

        is_free_to_paid_upgrade = (
            current_tier_name in ["free", "none"]
            and new_tier_info
            and new_tier_info.name not in ["free", "none"]
        )

        is_tier_change = (
            current_tier_name not in ["free", "none"]
            and new_tier_info
            and new_tier_info.name != current_tier_name
        )

        if is_new_subscription or is_free_to_paid_upgrade or is_tier_change:
            logger.info(
                f"[RENEWAL DETECTION] New subscription, free-to-paid upgrade, or tier change ({current_tier_name} -> {new_tier_info.name if new_tier_info else 'unknown'}) - NOT blocking"
            )
            return False
            ...
            if isinstance(billing_anchor, datetime):
                ...
                if hasattr(billing_anchor, "timestamp"):
                    billing_anchor = datetime.fromtimestamp(
                        billing_anchor.timestamp(), tz=timezone.utc
                    )
                else:
        ...
        if 0 <= seconds_since_period_start < 1800:
            logger.warning(
                f"[RENEWAL DETECTION] Within 30min of period start - likely renewal - BLOCKING"
            )
            return True

        return False

    def should_grant_credits(
        self,
        current_tier_data: Optional[Dict],
        new_tier: Dict,
        subscription: Dict,
        ...
        if current_tier_name != new_tier["name"]:
            if new_tier["credits"] > current_credits:
                logger.info(
                    f"Tier upgrade detected: {current_tier_name} -> {new_tier['name']} ({current_credits} -> {new_tier['credits']} credits)"
                )
                return True
            else:
                logger.info(
                    f"Tier change to lower/same credits: {current_tier_name} -> {new_tier['name']} ({current_credits} -> {new_tier['credits']} credits) - no additional credits granted"
                )
                return False

        return False

    def is_duplicate_credit_grant(
        self, last_grant_date: Optional[str], billing_anchor, current_tier_name: str, new_tier: Dict
    ) -> bool:
        ...
        try:
            if isinstance(last_grant_date, datetime):
                last_grant_dt = (
                    last_grant_date
                    if last_grant_date.tzinfo
                    else last_grant_date.replace(tzinfo=timezone.utc)
                )
            else:
            ...
            is_free_to_paid_upgrade = current_tier_name in ["free", "none"] and new_tier[
                "name"
            ] not in ["free", "none"]

            is_tier_upgrade = (
                current_tier_name not in ["free", "none"]
                and new_tier["name"] not in ["free", "none"]
                and current_tier_name != new_tier["name"]
            )

            if (
                time_since_last_grant < 900
                and current_tier_name == new_tier["name"]
                and not is_free_to_paid_upgrade
            ):
                logger.warning(
                    f"[DOUBLE CREDIT BLOCK] Credits granted {time_since_last_grant:.0f}s ago for SAME tier {new_tier['name']}"
                )
                return True

            if is_free_to_paid_upgrade or is_tier_upgrade:
                logger.info(
                    f"[TIER CHANGE ALLOWED] Allowing credit grant for tier change: {current_tier_name} -> {new_tier['name']}"
                )
                return False

            if (
                abs((billing_anchor - last_grant_dt).total_seconds()) < 900
                and current_tier_name == new_tier["name"]
            ):
                logger.warning(
                    f"[DOUBLE CREDIT BLOCK] Credits already granted near billing period start for SAME tier"
                )
                return True
    ...
    async def grant_subscription_credits(
        self, account_id: str, tier: Dict, billing_anchor: datetime, is_tier_upgrade: bool = False
    ) -> None:
        ...
        if not acquired:
            logger.error(
                f"[CREDIT GRANT] Failed to acquire lock for subscription grant to {account_id}"
            )
            raise Exception(
                f"Failed to acquire lock for credit grant - possible concurrent processing"
            )

        try:
            logger.info(
                f"[CREDIT GRANT] 🔒 Acquired lock for granting ${full_amount} credits to {account_id}"
            )
            ...
            if is_tier_upgrade:
                logger.info(
                    f"[TIER UPGRADE] Replacing existing credits with ${full_amount} for {tier['name']} (Stripe handled payment proration)"
                )
                ...
                logger.info(
                    f"[NEW SUBSCRIPTION] Adding ${full_amount} credits for new {tier['name']} subscription"
                )
            ...
            logger.info(
                f"[CREDIT GRANT] ✅ Successfully processed ${full_amount} credits for {tier['name']}"
            )
        finally:
            await lock.release()

    async def _check_for_recent_duplicate_credits(
        self, account_id: str, tier_name: str, amount: Decimal
    ) -> None:
        ...
        if recent_credits:
            for credit in recent_credits:
                ...
                if (
                    f"to {tier_name}" in credit_description
                    and abs(float(credit_amount) - float(amount)) < 0.01
                ):
                    logger.warning(
                        f"[DUPLICATE PREVENTION] Credits for {tier_name} (${amount}) already granted recently - BLOCKING duplicate"
                    )
    ...
    async def invalidate_caches(self, account_id: str) -> None:
        ...
        try:
            ...
            logger.warning(
                f"[CACHE] Failed to invalidate slot_manager tier cache for {account_id}: {e}"
            )

        logger.info(f"[SUBSCRIPTION] Invalidated all caches for {account_id}")

    def calculate_next_credit_grant(
        self, plan_type: str, billing_anchor: datetime, period_end_timestamp: int
    ) -> datetime:
        if plan_type == "yearly":
            from dateutil.relativedelta import relativedelta  # type: ignore
