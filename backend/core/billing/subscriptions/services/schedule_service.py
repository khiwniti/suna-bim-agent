...
class ScheduleService:
    ...
    async def schedule_yearly_plan_change(
        ...
        current_tier_name = get_tier_by_price_id(
            current_subscription["items"]["data"][0]["price"]["id"]
        ).name

        logger.info(
            f"[YEARLY SCHEDULE] Scheduling change from {current_tier_name} to {target_tier.name} at period end: {current_period_end_date}"
        )
    ...
    async def _create_subscription_schedule(
        ...
        schedule = await StripeAPIWrapper.safe_stripe_call(
            stripe.SubscriptionSchedule.create_async, from_subscription=current_subscription["id"]
        )
        ...
        logger.info(
            f"[YEARLY SCHEDULE] Created subscription schedule {schedule.id} for {account_id}"
        )
        return schedule
        ...
        if current_credit_result:
            ...
            if current_plan_type == "yearly":
                ...
                if current_anchor:
                    ...
                    logger.info(
                        f"[YEARLY SCHEDULE] Preserving yearly next_credit_grant: {correct_next_grant}"
                    )
