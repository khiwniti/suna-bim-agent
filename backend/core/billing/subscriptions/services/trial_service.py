    ...
    async def activate_trial(self, account_id: str, subscription: Dict, tier: Dict) -> None:
        ...
        try:
            ...
            if recent_trial_credits:
                logger.warning(
                    f"[TRIAL] Trial credits already granted for account {account_id}, skipping"
                )
                return
            ...
            await self.trial_repo.create_trial_history(account_id, datetime.now(timezone.utc))

            await self._invalidate_trial_caches(account_id)

            logger.info(
                f"[TRIAL] ✅ Started trial for user {account_id} - granted ${TRIAL_CREDITS} credits"
            )
        finally:
            await lock.release()
