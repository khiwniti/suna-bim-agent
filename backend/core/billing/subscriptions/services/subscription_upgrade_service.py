...
class SubscriptionUpgradeService:
    ...
    async def perform_standard_upgrade(
        self, subscription_id: str, subscription: Dict, target_price_id: str, account_id: str
    ) -> Dict:
        ...
        return await self._process_subscription_change(
            updated_subscription, account_id, target_price_id
        )

    async def perform_yearly_to_monthly_upgrade(
        self, account_id: str, current_subscription: Dict, target_price_id: str
    ) -> Dict:
    ...
    async def perform_yearly_to_yearly_upgrade(
        ...
        modify_key = generate_subscription_modify_idempotency_key(
            current_subscription["id"], target_price_id
        )
    ...
    async def _process_subscription_change(
        self, updated_subscription: Dict, account_id: str, price_id: str
    ) -> Dict:
        ...
        try:
            ...
            if "already granted recently" in str(e):
                logger.info(
                    f"[UPGRADE] Credits already granted for {account_id} - continuing with upgrade"
                )
            else:
                raise
        ...
        try:
            ...
            logger.info(
                f"[UPGRADE] Cleared any scheduled downgrades for {account_id} after plan change"
            )
        except Exception as e:
    ...
    async def _cleanup_duplicate_subscriptions(
        self, customer_id: str, keep_subscription_id: str, account_id: str
    ):
        try:
            ...
            for sub in all_subscriptions:
                if sub.id != keep_subscription_id and sub.status in [
                    "active",
                    "trialing",
                    "past_due",
                    "unpaid",
                    ...
                    try:
                        ...
                        logger.error(
                            f"[CLEANUP] Failed to cancel duplicate subscription {sub.id}: {e}"
                        )

            if duplicates_found:
                logger.info(
                    f"[CLEANUP] ✅ Cleaned up {len(duplicates_found)} duplicate subscriptions for {account_id}"
                )
