...
class SchedulingService:
    ...
    def determine_downgrade_timing(
        self, subscription: Dict, user_commitment_type: str, commitment_end_date: str
    ) -> Tuple[int, datetime]:
    ...
    async def create_or_update_subscription_schedule(
        ...
        await self.credit_repo.update_credit_account(
            account_id,
            {
                ...
                "scheduled_tier_change_date": datetime.fromtimestamp(
    ...
    async def _release_completed_schedule(self, schedule_id: str):
        logger.info(
            f"[DOWNGRADE] Schedule {schedule_id} is completed/canceled, releasing and creating new"
        )
        await StripeAPIWrapper.safe_stripe_call(
