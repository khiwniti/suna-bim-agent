...
def cap_quality_for_tier(tier_name: str, requested_quality: str) -> str:
    ...
    if tier_name in FREE_TIERS:
        ...
        if requested_quality in ("high", "auto"):
            logger.info(
                f"[MEDIA_BILLING] Capping quality from '{requested_quality}' to 'medium' for tier '{tier_name}'"
            )
            return "medium"
...
def calculate_replicate_image_cost(
    model: str, count: int = 1, variant: Optional[str] = None
) -> Decimal:
    """
    ...
    try:
        ...
        if pricing and pricing.get("type") == "per_image":
            ...
            logger.warning(
                f"[MEDIA_BILLING] No pricing found for Replicate model '{model}', using default"
            )
        ...
        logger.debug(
            f"[MEDIA_BILLING] Replicate image cost: model={model}, count={count}{variant_str}, base=${base_cost:.4f}, total=${total_cost:.4f}"
        )
        return total_cost
...
def calculate_replicate_video_cost(
    model: str, duration_seconds: int = 5, with_audio: bool = False
) -> Decimal:
    """
    ...
    try:
        ...
        if pricing and pricing.get("type") == "per_second":
            ...
            if with_audio and "cost_usd_per_second_with_audio" in pricing:
                ...
                cost_per_second = pricing.get(
                    "cost_usd_per_second", DEFAULT_VIDEO_COST_PER_SECOND_USD
                )

            base_cost = cost_per_second * Decimal(clamped_duration)
        else:
            logger.warning(
                f"[MEDIA_BILLING] No video pricing found for model '{model}', using default"
            )
            base_cost = DEFAULT_VIDEO_COST_PER_SECOND_USD * Decimal(duration_seconds)

        total_cost = base_cost * TOKEN_PRICE_MULTIPLIER
        logger.debug(
            f"[MEDIA_BILLING] Replicate video cost: model={model}, duration={duration_seconds}s, audio={with_audio}, base=${base_cost:.4f}, total=${total_cost:.4f}"
        )
        return total_cost
        ...
        return (
            DEFAULT_VIDEO_COST_PER_SECOND_USD * Decimal(duration_seconds) * TOKEN_PRICE_MULTIPLIER
        )
...
def calculate_replicate_voice_cost(model: str, char_count: int) -> Decimal:
    ...
    try:
        ...
        if pricing and pricing.get("type") == "per_characters":
            ...
            logger.warning(
                f"[MEDIA_BILLING] No voice pricing found for model '{model}', using default"
            )
        ...
        logger.debug(
            f"[MEDIA_BILLING] Replicate voice cost: model={model}, chars={char_count}, base=${base_cost:.4f}, total=${total_cost:.4f}"
        )
        return total_cost
        ...
        base = (
            (Decimal(char_count) / Decimal(1000))
            * DEFAULT_VOICE_COST_PER_1K_CHARS_USD
...
def calculate_openrouter_image_cost(model: str, count: int = 1) -> Decimal:
    ...
    try:
        ...
        if pricing:
            ...
            logger.warning(
                f"[MEDIA_BILLING] No pricing found for OpenRouter model '{model}', using default"
            )
            base_cost = DEFAULT_IMAGE_COST_USD * Decimal(count)

        total_cost = base_cost * TOKEN_PRICE_MULTIPLIER
        logger.debug(
            f"[MEDIA_BILLING] OpenRouter image cost: model={model}, count={count}, base=${base_cost:.4f}, total=${total_cost:.4f}"
        )
        return total_cost
