...
def calculate_token_cost(prompt_tokens: int, completion_tokens: int, model: str) -> Decimal:
    ...
    try:
        logger.debug(
            f"[COST_CALC] Calculating cost for model '{model}' with {prompt_tokens} prompt + {completion_tokens} completion tokens"
        )
        ...
        if pricing:
            input_cost = (
                ...
                / Decimal("1000000")
                * Decimal(str(pricing.input_cost_per_million_tokens))
            )
            output_cost = (
                Decimal(completion_tokens)
                / Decimal("1000000")
                * Decimal(str(pricing.output_cost_per_million_tokens))
            )
            total_cost = (input_cost + output_cost) * TOKEN_PRICE_MULTIPLIER

            logger.debug(
                f"[COST_CALC] Model '{model}' pricing: input=${pricing.input_cost_per_million_tokens}/M, output=${pricing.output_cost_per_million_tokens}/M"
            )
            logger.debug(
                f"[COST_CALC] Calculated: input=${input_cost:.6f}, output=${output_cost:.6f}, total with {TOKEN_PRICE_MULTIPLIER}x markup=${total_cost:.6f}"
            )
        ...
        logger.warning(
            f"[COST_CALC] No pricing found for model '{model}'. Check if model is registered in ai_models/registry.py"
        )
        return Decimal("0.01")
    ...
    try:
        ...
        if pricing:
            ...
            cost = (
                ...
                * Decimal(str(cached_read_cost_per_token))
                * TOKEN_PRICE_MULTIPLIER
            )
            logger.debug(
                f"[COST_CALC] Cached read cost for {cached_tokens} tokens @ ${pricing.cached_read_cost_per_million_tokens}/M: ${cost:.6f}"
            )
            return cost

        # Fall back to regular input pricing if no cached pricing available
        logger.debug(
            f"[COST_CALC] No cached read pricing for model '{model}', using regular input pricing"
        )
...
def calculate_cache_write_cost(
    cache_creation_tokens: int, model: str, cache_ttl: str = "5m"
) -> Decimal:
    ...
    try:
        ...
        if pricing:
            ...
            cost = (
                ...
                * Decimal(str(cache_write_cost_per_token))
                * TOKEN_PRICE_MULTIPLIER
            )
            logger.debug(
                f"[COST_CALC] Cache write cost for {cache_creation_tokens} tokens @ ${cost_per_million}/M (TTL: {cache_ttl}): ${cost:.6f}"
            )
            return cost

        # Fall back to regular input pricing if no cache write pricing available
        logger.debug(
            f"[COST_CALC] No cache write pricing for model '{model}', using regular input pricing"
        )
