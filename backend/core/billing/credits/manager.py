...
class CreditManager:
        ...
        if self.use_atomic_functions:
            try:
                ...
                if result:
                    ...
                    balance_data = {"total": float(new_balance), "account_id": account_id}
                ...
                if "duplicate key" in str(e).lower():
                    logger.info(
                        f"[ATOMIC] Duplicate credit addition detected for {account_id}, returning success"
                    )
    ...
    async def reset_expiring_credits(
            ...
            try:
                ...
                if result:
                    if result.get("success"):
                        logger.info(
                            f"[ATOMIC] Reset expiring credits to ${new_credits} for {account_id} atomically"
                        )

                        total_balance = result.get("total_balance", 0)
                        # Update cache with new balance instead of invalidating
                        balance_data = {"total": float(total_balance), "account_id": account_id}
                        await Cache.set(f"credit_balance:{account_id}", balance_data, ttl=300)
                        await Cache.invalidate(f"credit_summary:{account_id}")
        ...
        # Update cache with new balance instead of invalidating
        balance_data = {"total": float(new_total), "account_id": account_id}
        await Cache.set(f"credit_balance:{account_id}", balance_data, ttl=300)
        await Cache.invalidate(f"credit_summary:{account_id}")
    ...
    async def _add_credits_manual(
        ...
        logger.info(
            f"[MANUAL] Successfully added ${amount} credits to {account_id}. New balance: ${new_balance}"
        )
    ...
    async def _reset_expiring_credits_manual(
        ...
        logger.info(
            f"[MANUAL RESET] Replacing expiring credits with ${new_credits} for {account_id}"
        )
        ...
        logger.info(
            f"[MANUAL RESET] Successfully reset expiring credits to ${new_credits} for {account_id}. New balance: ${new_balance}"
        )
    ...
    async def deduct_credits(
        ...
        if amount <= 0:
            logger.warning(
                f"[DEDUCTION] Zero or negative amount {amount} for {account_id}, skipping"
            )
            balance_info = await self.get_balance(account_id)
            return {
                "success": True,
        ...
        if self.use_atomic_functions:
            try:
                ...
                if result:
                    ...
                    logger.info(
                        f"[ATOMIC] Deducted ${amount_deducted} from {account_id}. New balance: ${new_balance}"
                    )
    ...
    async def _deduct_credits_manual(
        ...
        logger.info(
            f"[MANUAL] Successfully deducted ${amount} from {account_id}. New balance: ${new_balance}"
        )
    ...
    async def get_balance(self, account_id: str, use_cache: bool = True) -> Dict:
        ...
        if result:
            balance_data = {"total": result.get("balance", 0), "account_id": account_id}
        else:
            balance_data = {"total": 0, "account_id": account_id}
