...
class BillingIntegration:
    ...
    async def check_and_reserve_credits(
        account_id: str, estimated_tokens: int = 10000, wait_for_cache_ms: int = 0
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"[CREDIT_CACHE] Redis timeout checking cache for {account_id} - proceeding without cache"
                )
                cached_check = None

            if cached_check:
                logger.debug(
                    f"⚡ [CREDIT_CACHE] Daily refresh already checked for {account_id} today (cached)"
                )
            else:
                ...
                try:
                    ...
                    logger.debug(
                        f"✅ [CREDIT_CACHE] Daily refresh check completed and cached for {account_id} (TTL: 1h)"
                    )
                except asyncio.TimeoutError:
                    logger.warning(
                        f"[CREDIT_CACHE] Redis timeout caching check for {account_id} - continuing without"
                    )
        except Exception as e:
            logger.warning(
                f"[DAILY_CREDITS] Failed to check/refresh daily credits for {account_id}: {e}"
            )
        ...
        if wait_for_cache_ms > 0:
            ...
            for _ in range(max_polls):
                ...
                if cached is not None:
                    logger.debug(
                        f"⚡ [BILLING] Cache hit after waiting (waited {(time.time() - balance_start) * 1000:.0f}ms)"
                    )
                    balance_info = cached
                    break
        ...
        if balance < 0:
            return (
                False,
                f"Insufficient credits. Your balance is {int(balance * 100)} credits. Please add credits to continue.",
                None,
            )
    ...
    async def check_model_and_billing_access(
        account_id: str, model_name: Optional[str], client=None
    ) -> Tuple[bool, str, Dict]:
        if config.ENV_MODE == EnvMode.LOCAL:
            logger.debug(
                "Running in local development mode - skipping all billing and model access checks"
            )
        ...
        try:
            ...
            return (
                True,
                "Access granted",
                {"tier_info": tier_info, "reservation_id": reservation_id},
            )
    ...
    async def deduct_usage(
        ...
        if cache_read_tokens > 0 or cache_creation_tokens > 0:
            ...
            if cache_creation_tokens > 0:
                ...
                cache_write_cost = calculate_cache_write_cost(
                    cache_creation_tokens, model, cache_ttl="5m"
                )

            # Regular non-cached tokens
            non_cached_cost = calculate_token_cost(
                non_cached_prompt_tokens, completion_tokens, model
            )

            cost = cached_read_cost + cache_write_cost + non_cached_cost

            logger.debug(
                f"[BILLING] Cost breakdown: cached_read=${cached_read_cost:.6f} + cache_write=${cache_write_cost:.6f} + regular=${non_cached_cost:.6f} = total=${cost:.6f}"
            )
        else:
        ...
        if cost <= 0:
            logger.warning(
                f"Zero cost calculated for {model} with {prompt_tokens}+{completion_tokens} tokens"
            )
        ...
        if result.get("success"):
            logger.debug(
                f"[BILLING] Successfully deducted ${cost:.6f} from user {account_id}. New balance: ${result.get('new_total', result.get('new_balance', 0)):.2f} (expiring: ${result.get('from_expiring', 0):.2f}, non-expiring: ${result.get('from_non_expiring', 0):.2f})"
            )
            await invalidate_account_state_cache(account_id)
        else:
            logger.error(
                f"[BILLING] Failed to deduct credits for user {account_id}: {result.get('error')}"
            )
