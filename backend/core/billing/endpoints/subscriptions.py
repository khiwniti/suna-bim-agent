    ...
    try:
        ...
        if tier.name == "free":
            ...
            if result.get("success"):
                ...
                raise HTTPException(
                    status_code=400,
                    detail=result.get("message", "Failed to subscribe to free tier"),
                )
        ...
        if request.commitment_type == "yearly_commitment":
            ...
            if not price_ids:
                raise HTTPException(
                    status_code=400, detail="Yearly commitment not available for this tier"
                )
            price_id = price_ids[0]
        elif request.commitment_type == "yearly":
            logger.debug(
                f"[YEARLY-BILLING] Selecting yearly price for tier: {tier.name}, available price_ids: {tier.price_ids}"
            )
            price_id = None
            if tier.name == "tier_2_20":
                ...
                logger.debug(
                    f"[YEARLY-BILLING] Using fallback string matching for tier: {tier.name}"
                )
                price_ids = [
                    pid
                    for pid in tier.price_ids
                    if "yearly" in pid.lower() and "commitment" not in pid.lower()
                ]
        ...
        logger.debug(
            f"[BILLING] Creating checkout session: account_id={account_id}, tier={tier.name}, commitment_type={request.commitment_type}, selected_price_id={price_id}"
        )
...
async def create_inline_checkout(
    request: CreateInlineCheckoutRequest, account_id: str = Depends(verify_and_get_user_id_from_jwt)
) -> Dict:
    """
    ...
    try:
        ...
        if existing_subs.data:
            ...
            if existing_items:
                ...
                if unit_amount == 0 or unit_amount is None:
                    ...
                    logger.info(
                        f"[INLINE CHECKOUT] Cancelled free tier subscription {existing_sub['id']}"
                    )
                else:
                    ...
                    if not current_tier_key:
                        ...
                        from ..subscriptions.repositories.credit_account import (
                        ...
                        logger.info(
                            f"[INLINE CHECKOUT] Got current tier from credit_account: {current_tier_key}"
                        )
                    ...
                    logger.info(
                        f"[INLINE CHECKOUT] Comparing tiers: current={current_tier_key} ({current_credits} credits) vs new={request.tier_key} ({new_credits} credits)"
                    )
                    ...
                    logger.info(
                        f"[INLINE CHECKOUT] Upgrading subscription {existing_sub['id']} from {current_tier_key} to {request.tier_key}"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"[INLINE CHECKOUT] Subscription upgraded: {updated_sub['id']} status={updated_sub['status']}"
                        )

                        # Verify subscription is active before granting credits
                        if updated_sub["status"] != "active":
                            logger.error(
                                f"[INLINE CHECKOUT] Upgrade failed - subscription status: {updated_sub['status']}"
                            )
                            raise HTTPException(
                                status_code=402,
                        ...
                        new_tier_info = {"name": tier.name, "credits": float(tier.monthly_credits)}
                        billing_anchor = datetime.fromtimestamp(
                            updated_sub["current_period_start"], tz=timezone.utc
                        )
                        await lifecycle_service.grant_subscription_credits(
                            account_id, new_tier_info, billing_anchor, is_tier_upgrade=True
                        )
                        ...
                        credit_repo = CreditAccountRepository()
                        await credit_repo.update_credit_account(
                            account_id,
                            {
                        ...
                        raise HTTPException(
                            status_code=402, detail=f"Payment failed: {e.user_message}"
                        )
                    except Exception as e:
        ...
        try:
            incomplete_subs = await stripe.Subscription.list_async(
                customer=customer_id, status="incomplete", limit=10
            )
            for sub in incomplete_subs.data:
                try:
                    ...
                    logger.warning(
                        f"[INLINE CHECKOUT] Could not cancel incomplete sub {sub['id']}: {e}"
                    )
        except Exception as e:
        ...
        if request.promo_code:
            try:
                promo_codes = await stripe.PromotionCode.list_async(
                    code=request.promo_code, active=True, limit=1
                )
                if promo_codes.data:
                    promotion_code_id = promo_codes.data[0]["id"]
                    logger.info(
                        f"[INLINE CHECKOUT] Found promo code {request.promo_code} -> {promotion_code_id}"
                    )
                else:
                    logger.warning(
                        f"[INLINE CHECKOUT] Promo code {request.promo_code} not found or inactive"
                    )
                    raise HTTPException(
                        status_code=400,
                        detail=f"Promo code '{request.promo_code}' is invalid or expired",
                    )
            except HTTPException:
                raise
            except Exception as e:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Could not validate promo code '{request.promo_code}'"
                )
        ...
        if payment_intent is None:
            ...
            logger.info(
                f"[INLINE CHECKOUT] No payment required (100% discount) - subscription {subscription['id']} is active"
            )
        ...
        logger.info(
            f"[INLINE CHECKOUT] Created subscription {subscription['id']} for account {account_id}"
        )
...
async def confirm_inline_checkout(
    request: dict, account_id: str = Depends(verify_and_get_user_id_from_jwt)
) -> Dict:
    """
    ...
    try:
        ...
        logger.info(
            f"[INLINE CHECKOUT] Parsed: subscription_id={subscription_id}, tier_key={tier_key}, payment_intent_id={payment_intent_id}"
        )
        ...
        if payment_intent_id:
            try:
                ...
                logger.info(
                    f"[INLINE CHECKOUT] Direct PaymentIntent check: {pi['id']} status={pi['status']}"
                )
                if pi["status"] == "succeeded":
                    payment_succeeded = True
            except Exception as e:
                logger.warning(
                    f"[INLINE CHECKOUT] Could not verify PaymentIntent {payment_intent_id}: {e}"
                )
        ...
        if sub_account_id != account_id:
            logger.warning(
                f"[INLINE CHECKOUT] Account mismatch: subscription has {sub_account_id}, request from {account_id}"
            )
            raise HTTPException(
                status_code=403, detail="Subscription does not belong to this account"
            )
        ...
        if not payment_succeeded:
            ...
            payment_intent = latest_invoice.get("payment_intent") if latest_invoice else None
            payment_succeeded = subscription["status"] == "active" or (
                payment_intent and payment_intent.get("status") == "succeeded"
            )
            logger.info(
                f"[INLINE CHECKOUT] Subscription check: status={subscription['status']}, invoice PI status={payment_intent.get('status') if payment_intent else 'N/A'}"
            )

        if not payment_succeeded:
            raise HTTPException(
                status_code=400,
                detail=f"Payment not confirmed. Subscription status: {subscription['status']}",
            )
        ...
        if previous_subscription_id:
            try:
                ...
                logger.info(
                    f"[INLINE CHECKOUT] Cancelled previous subscription {previous_subscription_id} after upgrade payment confirmed"
                )
            except Exception as e:
                # Log but don't fail - old subscription might already be cancelled
                logger.warning(
                    f"[INLINE CHECKOUT] Could not cancel previous subscription {previous_subscription_id}: {e}"
                )
        ...
        lifecycle_service = LifecycleService()
        new_tier = {"name": tier.name, "credits": float(tier.monthly_credits)}

        logger.info(
            f"[INLINE CHECKOUT] Granting {tier.monthly_credits} credits for {tier.display_name}"
        )
        await lifecycle_service.grant_subscription_credits(
            account_id, new_tier, billing_anchor, is_tier_upgrade=False
        )
        ...
        logger.info(
            f"[INLINE CHECKOUT] Confirmed subscription {subscription_id} for account {account_id}, tier={tier_key}, credits granted"
        )
...
async def create_portal_session(
    request: CreatePortalSessionRequest, account_id: str = Depends(verify_and_get_user_id_from_jwt)
) -> Dict:
...
async def sync_subscription(account_id: str = Depends(verify_and_get_user_id_from_jwt)) -> Dict:
...
async def cancel_subscription(
    request: CancelSubscriptionRequest, account_id: str = Depends(verify_and_get_user_id_from_jwt)
) -> Dict:
...
async def schedule_downgrade(
    ...
    try:
        ...
        result = await subscription_service.schedule_tier_downgrade(
            account_id=account_id, target_tier_key=target_tier_key, commitment_type=commitment_type
        )
...
async def preview_proration(
    ...
    try:
        return {
            "preview": {"amount_due": 0, "proration_amount": 0, "next_invoice_total": 0},
...
async def get_checkout_session(
    session_id: str, account_id: str = Depends(verify_and_get_user_id_from_jwt)
) -> Dict:
    """
        ...
        try:
            if session.subscription:
                ...
                sub = await stripe.Subscription.retrieve_async(
                    session.subscription, expand=["latest_invoice.charge.balance_transaction"]
                )
                if (
                    sub.latest_invoice
                    and hasattr(sub.latest_invoice, "charge")
                    and sub.latest_invoice.charge
                    and hasattr(sub.latest_invoice.charge, "balance_transaction")
                    and sub.latest_invoice.charge.balance_transaction
                ):
                ...
                pi = await stripe.PaymentIntent.retrieve_async(
                    session.payment_intent, expand=["latest_charge.balance_transaction"]
                )
                if (
                    pi.latest_charge
                    and hasattr(pi.latest_charge, "balance_transaction")
                    and pi.latest_charge.balance_transaction
                ):
