...
def _extract_commitment_from_credit_account(credit_account: Dict) -> Dict:
    ...
    try:
        ...
        return {
            ...
            if isinstance(commitment_end_date, str)
            else commitment_end_date.isoformat(),
        }
    except Exception:
        return {
            "has_commitment": False,
            ...
            if pending_product and pending_date:
                ...
                if target_tier_info and current_tier_name != target_tier_name:
                    ...
                    return {
                        ...
                        "scheduled_change": {
                            "type": "downgrade"
                            if target_tier_info.monthly_credits
                            < (current_tier.monthly_credits if current_tier else 0)
                            else "change",
                            "current_tier": {
...
async def _refresh_daily_credits_background(account_id: str) -> None:
    ...
    try:
        ...
        logger.debug(
            f"[ACCOUNT_STATE] Background daily credit refresh failed for {account_id}: {e}"
        )
...
async def _build_minimal_account_state(account_id: str) -> Dict:
    ...
    try:
        await Cache.set(
            ...
            {"total": total_balance_dollars, "account_id": account_id},
            ttl=300,
        )
    except Exception:
        pass
    ...
    has_daily_credits = tier_info.daily_credit_config and tier_info.daily_credit_config.get(
        "enabled"
    )
    ...
    logger.info(
        f"[ACCOUNT_STATE_MINIMAL] Built in {(time.time() - t_start) * 1000:.1f}ms for {account_id[:8]}..."
    )
    ...
    return {
        ...
        "tier": {"name": tier_name, "display_name": tier_info.display_name},
    }


async def _get_cached_stripe_subscription(
    subscription_id: str, timeout: float = STRIPE_FETCH_TIMEOUT
) -> Optional[Dict]:
    ...
    try:
        subscription_data = await asyncio.wait_for(
            StripeAPIWrapper.retrieve_subscription(subscription_id), timeout=timeout
        )
        ...
        logger.warning(
            f"[ACCOUNT_STATE] Stripe subscription fetch timed out after {timeout}s: {subscription_id[:8]}..."
        )
        return None
    except Exception as e:
...
async def _build_account_state(account_id: str, skip_cache: bool = False) -> Dict:
    ...
    tier_info_task = subscription_service.get_user_subscription_tier(
        account_id, skip_cache=skip_cache
    )

    credit_account, subscription_tier_info = await asyncio.gather(
        credit_account_task, tier_info_task
    )
    credit_account = credit_account or {}
    ...
    logger.info(
        f"[ACCOUNT_STATE] {account_id[:8]}... tier={tier_name} thread_limit={subscription_tier_info.get('thread_limit')} project_limit={subscription_tier_info.get('project_limit')} skip_cache={skip_cache}"
    )
    logger.info(
        f"[ACCOUNT_STATE] Fetched credit account + tier in {(time.time() - t_start) * 1000:.1f}ms"
    )
    ...
    try:
        balance_data = {"total": total_balance_dollars, "account_id": account_id}
...
async def get_minimal_account_state(
    account_id: str = Depends(verify_and_get_user_id_from_jwt), skip_cache: bool = False
) -> Dict:
    """
    ...
    if config.ENV_MODE == EnvMode.LOCAL:
        return {
            ...
            "tier": {"name": "tier_25_200", "display_name": "Local Development"},
    ...
    if not skip_cache:
        try:
            ...
            if cached_data:
                cached_data["_cache"] = {
                    "cached": True,
                    "ttl_seconds": MINIMAL_ACCOUNT_STATE_CACHE_TTL,
                }
                return cached_data
        except Exception:
            pass
...
async def get_account_state(
    ...
    if config.ENV_MODE == EnvMode.LOCAL:
        ...
        return {
            ...
            "limits": {
                ...
                "trigger_count": {
                    "scheduled": {"current_count": 0, "limit": 99999, "can_create": True},
                    "app": {"current_count": 0, "limit": 99999, "can_create": True},
                    "tier_name": "tier_25_200",
                },
            },
            "tier": {
    ...
    if not skip_cache:
        try:
            cached_data = await Cache.get(cache_key)
            if cached_data:
                cached_data["_cache"] = {"cached": True, "ttl_seconds": ACCOUNT_STATE_CACHE_TTL}
                return cached_data
    ...
    for attempt in range(max_retries):
        try:
            ...
            try:
                ...
                logger.debug(
                    f"[ACCOUNT_STATE] Cache write failed for {account_id}: {cache_write_err}"
                )
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(
                    f"[ACCOUNT_STATE] Failed after {max_retries} attempts for {account_id}: {e}"
                )
                raise HTTPException(
