...
async def purchase_credits_checkout(
    request: PurchaseCreditsRequest, account_id: str = Depends(verify_and_get_user_id_from_jwt)
) -> Dict:
    try:
    ...
    try:
        ...
        if start_date and end_date:
            try:
                ...
                logger.info(
                    f"[BILLING] Filtering credit usage by date range: {start_dt.isoformat()} to {end_dt.isoformat()}"
                )
        ...
        (
            ...
            total_threads,
            total_usage,
        ) = await billing_repo.get_credit_usage_by_thread_with_dates(
            account_id=account_id, limit=limit, offset=offset, start_date=start_dt, end_date=end_dt
        )

        logger.info(
            f"[BILLING] Found {total_threads} threads with credit usage for account {account_id}"
        )
