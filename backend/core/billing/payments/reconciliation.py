...
class ReconciliationService(ReconciliationManagerInterface):
    ...
    async def reconcile_failed_payments(self) -> Dict:
        results = {"checked": 0, "fixed": 0, "failed": 0, "errors": []}
        ...
        try:
            ...
            for purchase in failed_purchases:
                try:
                        ...
                        if not ledger_exists:
                            ...
                            if result.get("success"):
                                ...
                                results["errors"].append(
                                    f"Failed to add credits for {purchase['id']}"
                                )
                        else:
                            await billing_repo.update_purchase_by_id(
                                purchase_id=purchase["id"],
                            ...
                            logger.info(
                                f"[RECONCILIATION] Purchase {purchase['id']} already processed"
                            )
                    ...
                    logger.error(
                        f"[RECONCILIATION] Error processing purchase {purchase['id']}: {e}"
                    )
        ...
        logger.info(
            f"[RECONCILIATION] Complete: checked={results['checked']}, fixed={results['fixed']}, failed={results['failed']}"
        )
        return results

    async def verify_balance_consistency(self) -> Dict:
        results = {"checked": 0, "fixed": 0, "discrepancies_found": []}
        ...
        try:
            ...
            for account in accounts or []:
                expected = Decimal(str(account["expiring_credits"])) + Decimal(
                    str(account["non_expiring_credits"])
                )
                actual = Decimal(str(account["balance"]))

                if abs(expected - actual) > Decimal("0.01"):
                    logger.warning(
                        f"[BALANCE CHECK] Discrepancy found for {account['account_id']}: "
    ...
    async def detect_double_charges(self) -> Dict:
        results = {"duplicates_found": [], "total_checked": 0}

        try:
            since = datetime.now(timezone.utc) - timedelta(days=7)

            ledger_entries = await billing_repo.get_recent_ledger_entries_for_duplicate_check(
                since.isoformat()
            )
                ...
                if key in seen:
                    time_diff = abs(
                        (
                            ...
                            - datetime.fromisoformat(seen[key]["created_at"].replace("Z", "+00:00"))
                        ).total_seconds()
                    )
                    ...
                    if time_diff < 60:
                        ...
                        logger.warning(
                            f"[DUPLICATE CHECK] Potential duplicate found for {entry['account_id']}: "
    ...
    async def cleanup_expired_credits(self) -> Dict:
        results = {"accounts_cleaned": 0, "credits_removed": 0.0}
        ...
        try:
            ...
            if result:
                for row in result:
                    ...
                    logger.info(
                        f"[CLEANUP] Removed ${row['credits_removed']:.2f} expired credits from {row['account_id']}"
                    )
    ...
    async def retry_failed_payment(self, payment_id: str) -> Dict:
        try:
            ...
            if payment_intent.status == "succeeded":
                ...
                logger.info(
                    f"[RETRY] Payment {payment_id} still pending in Stripe: {payment_intent.status}"
                )
                return {
                    "success": False,
                    "error": f"Stripe payment status: {payment_intent.status}",
                }
