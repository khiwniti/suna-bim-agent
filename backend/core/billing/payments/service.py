...
class PaymentService(PaymentProcessorInterface):
    ...
    async def create_checkout_session(
        self, account_id: str, amount: Decimal, success_url: str, cancel_url: str
    ) -> Dict:
        return await self.create_credit_purchase_checkout(
            account_id, amount, success_url, cancel_url, self._get_user_subscription_tier
        )
    ...
    async def create_credit_purchase_checkout(
        ...
        if not tier.get("can_purchase_credits", False):
            raise HTTPException(
                status_code=403, detail="Credit purchases not available for your tier"
            )
        ...
        try:
            await StripeAPIWrapper.safe_stripe_call(stripe.Customer.retrieve_async, customer_id)
            logger.info(
                f"[PAYMENT] Verified Stripe customer {customer_id} for account {account_id}"
            )
        except stripe.error.InvalidRequestError as e:
            if "No such customer" in str(e):
                logger.error(
                    f"[PAYMENT] Customer {customer_id} not found in Stripe for account {account_id}"
                )
                raise HTTPException(
                    status_code=400,
        ...
        try:
            ...
            if purchase_record:
                ...
                logger.info(
                    f"[PAYMENT] Created purchase record {purchase_id} for account {account_id}"
                )
        ...
        try:
            ...
            if not payment_intent_id:
                logger.warning(
                    f"[PAYMENT] No payment_intent in session {session.id} for account {account_id} - will track by session_id"
                )
            ...
            logger.info(
                f"[PAYMENT SUCCESS] Created checkout session {session.id} for purchase {purchase_id}"
            )
