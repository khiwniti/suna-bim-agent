...
class ConvertToDocsResponse(BaseModel):
    ...
    docx_url: Optional[str] = Field(
        None, description="URL to the generated DOCX file (if stored locally)"
    )
...
async def convert_and_upload_to_google_docs(
    ...
    try:
        ...
        async with get_http_client() as client:
            convert_response = await client.post(convert_url, json=convert_payload, timeout=120.0)
        ...
        try:
            ...
            upload_result = await google_service.upload_docx_to_docs(
                docx_file_path=temp_docx_path, user_id=user_id, document_name=document_name
            )
