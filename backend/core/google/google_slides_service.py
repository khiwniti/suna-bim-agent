...
class OAuthTokenService:
    ...
    async def store_token(
        self, user_id: str, token_data: Dict[str, Any], expires_at: Optional[datetime] = None
    ) -> str:
        """
        ...
        await client.table("google_oauth_tokens").delete().eq("user_id", user_id).execute()
    ...
    async def get_token(self, user_id: str) -> Optional[OAuthToken]:
        ...
        client = await self._db.client
        result = (
            await client.table("google_oauth_tokens")
            .select("*")
            .eq("user_id", user_id)
    ...
    async def delete_token(self, user_id: str) -> bool:
        ...
        client = await self._db.client
        result = await client.table("google_oauth_tokens").delete().eq("user_id", user_id).execute()
...
class GoogleSlidesService:
    ...
    def __init__(self, oauth_token_service: OAuthTokenService):
        ...
        self.redirect_uri = os.getenv(
            "GOOGLE_REDIRECT_URI", "http://localhost:8000/v1/google/callback"
        )

        # Validate required credentials
        if not self.client_id or not self.client_secret:
            logger.warning(
                "Google OAuth credentials not configured. Set GOOGLE_OAUTH_CLIENT_ID and GOOGLE_OAUTH_CLIENT_SECRET environment variables."
            )
            logger.warning(
                "Google Slides integration will not work until credentials are properly configured."
            )
    ...
    async def handle_oauth_callback(
        ...
        try:
            ...
            await self.oauth_service.store_token(
                user_id=user_id, token_data=tokens, expires_at=expires_at
            )
    ...
    async def _refresh_token(self, user_id: str) -> bool:
        ...
        try:
            ...
            await self.oauth_service.store_token(
                user_id=user_id, token_data=new_tokens, expires_at=expires_at
            )
    ...
    async def upload_pptx_to_slides(
        self, pptx_file_path: Path, user_id: str, presentation_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if oauth_token.expires_at and oauth_token.expires_at <= datetime.now(timezone.utc):
            ...
            if not oauth_token:
                raise HTTPException(
                    status_code=401, detail="No authentication token found after refresh"
                )
        ...
        try:
            ...
            file = (
                service.files()
                .create(body=metadata, media_body=media, fields="id,name,webViewLink,mimeType")
                .execute()
            )

            logger.info(
                f"Successfully uploaded and converted PPTX to Google Slides: {file.get('id')}"
            )
            ...
            if error.resp.status == 401:
                ...
                if await self._refresh_token(user_id):
                    ...
                    return await self.upload_pptx_to_slides(
                        pptx_file_path, user_id, presentation_name
                    )
                else:
                    raise HTTPException(
                        status_code=401, detail="Authentication failed - please re-authenticate"
                    )
            else:
