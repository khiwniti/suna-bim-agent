...
class GoogleDocsService:
    def __init__(self, oauth_token_service: OAuthTokenService):
        ...
        if not self.client_id or not self.client_secret:
            ...
            logger.warning(
                "Google Docs integration will not work until credentials are properly configured."
            )
    ...
    async def upload_docx_to_docs(
        self, docx_file_path: Path, user_id: str, document_name: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully uploaded and converted DOCX to Google Docs: {file.get('id')}"
            )
