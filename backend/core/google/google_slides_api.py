...
class ConvertToSlidesRequest(BaseModel):
    presentation_path: str = Field(
        ...,
        description="Path to the presentation in sandbox (e.g., /workspace/presentations/my-pres)",
    )
...
async def get_google_slides_service(
    oauth_service: OAuthTokenService = Depends(get_oauth_service),
) -> GoogleSlidesService:
...
async def get_google_auth_url(
    ...
    try:
        ...
        return AuthURLResponse(
            auth_url=auth_url, message="Visit this URL to authenticate with Google"
        )
    ...
    try:
        ...
        if return_url:
            ...
            if return_url.startswith(frontend_url):
                ...
                logger.warning(
                    f"Return URL {return_url} not from same domain as {frontend_url}, using default"
                )
...
async def convert_and_upload_to_google_slides(
    ...
    try:
        ...
        logger.debug(
            f"Saved PPTX to temporary file: {temp_pptx_path} (size: {len(pptx_content)} bytes)"
        )
        ...
        try:
            ...
            presentation_name = (
                filename.replace(".pptx", "").split("_")[0]
                if "_" in filename
                else filename.replace(".pptx", "")
            )

            upload_result = await google_service.upload_pptx_to_slides(
                pptx_file_path=temp_pptx_path, user_id=user_id, presentation_name=presentation_name
            )
