...
async def _invalidate_kb_cache_for_entry(client, entry_id: str) -> None:
    ...
    try:
        ...
        if agent_ids:
            logger.debug(
                f"🗑️ Invalidated KB cache for {len(agent_ids)} agents after entry summary update"
            )
...
class FileProcessor:
    ...
    def _is_likely_text_file(self, file_content: bytes) -> bool:
        ...
        try:
            ...
            if detected.get("confidence", 0) > 0.7:
                ...
                printable_ratio = len([c for c in decoded if c.isprintable() or c.isspace()]) / len(
                    decoded
                )
    ...
    async def process_file_fast(
        ...
        try:
            ...
            background_tasks.add_task(
                self._generate_and_update_summary, entry_id, file_content, filename, mime_type
            )
            ...
            return {"success": True, "entry_id": entry_id, "filename": filename, "processing": True}
    ...
    async def _generate_and_update_summary(
        self, entry_id: str, file_content: bytes, filename: str, mime_type: str
    ):
        ...
        try:
            ...
            try:
                ...
                await (
                    client.table("knowledge_base_entries")
                    .update({"summary": f"Error generating summary: {str(e)}"})
                    .eq("entry_id", entry_id)
                    .execute()
                )
                await _invalidate_kb_cache_for_entry(client, entry_id)
    ...
    async def process_file(
        self, account_id: str, folder_id: str, file_content: bytes, filename: str, mime_type: str
    ) -> Dict[str, Any]:
            ...
            for line in lines:
                ...
                is_important = (
                    ...
                    or any(
                        ...
                        for keyword in ["summary", "conclusion", "important", "key", "main"]
                    )
