    ...
    try:
        ...
        for folder_data in result.data:
            ...
            count_result = (
                ...
                .eq("folder_id", folder_data["folder_id"])
                .execute()
            )
...
async def create_folder(
    folder_data: FolderRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
...
async def delete_folder(folder_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    if not config.ENABLE_KNOWLEDGE_BASE:
        raise HTTPException(status_code=503, detail="Knowledge base feature is currently disabled")
    ...
    try:
        ...
        try:
            ...
            if agent_ids_to_invalidate:
                logger.debug(
                    f"🗑️ Invalidated KB cache for {len(agent_ids_to_invalidate)} agents after folder deletion"
                )
        except Exception as e:
...
async def get_folder_entries(
    folder_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
...
async def delete_entry(entry_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    try:
        ...
        try:
            ...
            agent_ids = (
                set(row["agent_id"] for row in assignments_result.data)
...
async def update_entry(
    ...
    try:
        ...
        entry_result = (
            await client.table("knowledge_base_entries")
            .select("entry_id, filename, summary, file_size, created_at, account_id")
            .eq("entry_id", entry_id)
            .eq("account_id", account_id)
            .execute()
        )
...
async def get_agent_assignments(
    agent_id: str, auth: AuthorizedAgentAccess = Depends(require_agent_access)
):
...
async def get_entry_content(entry_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    try:
        ...
        entry_result = (
            await client.table("knowledge_base_entries")
            .select("entry_id, filename, file_path, mime_type, file_size")
            .eq("entry_id", entry_id)
            .eq("account_id", account_id)
            .eq("is_active", True)
            .execute()
...
async def move_file(
    ...
    try:
        ...
        entry_result = (
            await client.table("knowledge_base_entries")
            .select("entry_id, folder_id, file_path, filename")
            .eq("entry_id", entry_id)
            .execute()
        )
        ...
        await (
            ...
            .update({"folder_id": request.folder_id, "file_path": new_file_path})
            .eq("entry_id", entry_id)
            .execute()
        )
