...
CARBON_BIM_HELLO_EMAIL = "hello@carbon-bim.com"
...
class NotificationService:
    ...
    async def send_referral_code_notification(
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Referral code email sent to {recipient_email_clean} from {inviter_name}"
                )
                return {"success": True}
            else:
    ...
    async def send_task_completion_notification(
        ...
        try:
            ...
            should_send = await presence_service.should_send_notification(
                account_id=account_id, thread_id=thread_id, channel="email"
            )

            if not should_send:
                logger.info(
                    f"Suppressing task completion notification for account {account_id} (actively viewing thread {thread_id})"
                )
                return {
                    "success": False,
                    "reason": "Account is actively viewing thread",
                }
            ...
            project_id = (
                thread_result.data.get("project_id")
                if thread_result and thread_result.data
                else None
            )

            task_url = (
                f"http://20.55.21.69:30/projects/{project_id}/thread/{thread_id}"
                if project_id
                else f"http://20.55.21.69:30/thread/{thread_id}"
            )
    ...
    async def send_task_failed_notification(
        ...
        try:
            ...
            if thread_id:
                ...
                if not should_send:
                    logger.info(
                        f"Suppressing task failed notification for account {account_id} (actively viewing thread {thread_id})"
                    )
                    return {
                        "success": False,
                        "reason": "Account is actively viewing thread",
    ...
    async def send_credits_low_notification(
        self, account_id: str, remaining_credits: float, threshold_percentage: int = 20
    ) -> Dict[str, Any]:
        try:
            account_info = await self._get_account_info(account_id)
    ...
    async def trigger_workflow_admin(
        ...
        try:
            if broadcast:
                ...
                return await self._trigger_workflow_by_email(
                    workflow_id, payload_template, subscriber_email
                )
            elif subscriber_id:
                return await self._trigger_workflow_for_user(
                    workflow_id, payload_template, subscriber_id
                )
            else:
                raise ValueError(
                    "Either subscriber_id, subscriber_email, or broadcast=True must be provided"
                )
        except Exception as e:
    ...
    async def _trigger_workflow_for_user(
        self, workflow_id: str, payload_template: Dict[str, Any], subscriber_id: str
    ) -> Dict[str, Any]:
        ...
        if not account_info or not account_info.get("email"):
            return {
                "success": False,
                "error": f"No email found for subscriber {subscriber_id}",
            }
    ...
    async def _trigger_workflow_by_email(
        ...
        try:
            user_response = await client.rpc(
                "get_user_account_by_email", {"email_input": email}
            ).execute()
        ...
        await self.novu.upsert_subscriber(user_id=subscriber_id, email=email, name=name)
    ...
    async def _broadcast_workflow(
        self, workflow_id: str, payload_template: Dict[str, Any]
    ) -> Dict[str, Any]:
        try:
            result = await self.novu.trigger_broadcast(
                workflow_id=workflow_id, payload=payload_template
            )
    ...
    def _replace_template_variables(
        self, payload_template: Dict[str, Any], account_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        import re
        import json
    ...
    async def send_welcome_email(self, account_id: str) -> Dict[str, Any]:
        try:
            logger.info(
                f"[WELCOME_EMAIL] ENV_MODE={config.ENV_MODE.value if config.ENV_MODE else 'None'}, Novu enabled={self.novu.enabled}, API key configured={bool(self.novu.api_key)}"
            )
            ...
            logger.info(
                f"[WELCOME_EMAIL] Triggering for {account_id}: email={email}, name={name}, phone={phone}, avatar={avatar}"
            )
            ...
            if not result:
                logger.error(
                    f"Failed to trigger welcome email workflow for account {account_id} - Novu returned False (ENV_MODE={config.ENV_MODE.value if config.ENV_MODE else 'None'})"
                )
                return {"success": False, "error": "Failed to trigger workflow"}

            logger.info(
                f"Welcome email workflow triggered for account {account_id} (ENV_MODE: {config.ENV_MODE.value if config.ENV_MODE else 'None'})"
            )
            return {"success": True, "result": result}
    ...
    async def register_device_token(
        ...
        try:
            ...
            if result:
                ...
                logger.info(
                    f"✅ Device token registered with Novu for account {account_id}: {device_token[:20]}..."
                )
                return True
            else:
                logger.error(
                    f"❌ Failed to register device token with Novu for account {account_id}"
                )
                return False
    ...
    async def unregister_device_token(
        self, account_id: str, device_token: str, provider: str = "expo"
    ) -> bool:
            ...
            if result:
                if account_id in self._device_tokens:
                    if device_token in self._device_tokens[account_id]:
                        ...
                        logger.info(
                            f"✅ Device token unregistered from Novu for account {account_id}"
                        )
                        return True
                ...
                logger.error(
                    f"❌ Failed to unregister device token from Novu for account {account_id}"
                )
                return False
