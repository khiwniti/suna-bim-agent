...
class NovuService:
    def __init__(self):
        ...
        if not self.enabled:
            if self.api_key:
                logger.warning(
                    f"Novu service disabled (ENV_MODE: {config.ENV_MODE.value}) but NOVU_SECRET_KEY is present. Set ENV_MODE to 'staging' or 'production' to enable."
                )
            else:
    ...
    async def trigger_notification(
        ...
        if not self.enabled:
            logger.warning(
                f"Notification skipped (Novu disabled in {config.ENV_MODE.value} mode): {event_name}"
            )
            return {
                "success": False,
                "error": "Novu notifications only enabled in staging and production modes",
            }
        ...
        try:
            await self.upsert_subscriber(
                user_id=user_id, email=subscriber_email, name=subscriber_name
            )
            ...
            return {"success": True, "transaction_id": transaction_id, "response": response}
            ...
            with Novu(
                ...
                try:
                    ...
                    novu.subscribers.patch(
                        subscriber_id=user_id, patch_subscriber_request_dto=patch_dto
                    )
    ...
    async def update_subscriber_credentials(
        ...
        if not self.enabled:
            logger.debug(
                f"Skipping credential update (Novu disabled in {config.ENV_MODE.value} mode)"
            )
            return True
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.put(url, json=payload, headers=headers) as response:
                    ...
                    logger.info(
                        f"✅ Set push credentials for subscriber {user_id} with provider {provider_id}"
                    )
                    return True
    ...
    async def set_subscriber_preference(
        self, user_id: str, template_id: str, channel: Optional[str] = None, enabled: bool = True
    ) -> bool:
        if not self.enabled:
            return True
    ...
    async def trigger_workflow(
        ...
        if not self.enabled:
            logger.error(
                f"❌ Workflow skipped (Novu disabled in {config.ENV_MODE.value} mode): {workflow_id} - Set ENV_MODE to 'staging' or 'production' to enable"
            )
            return False
    ...
    async def register_push_token(
        ...
        if not self.enabled:
            logger.debug(
                f"Skipping push token registration (Novu disabled in {config.ENV_MODE.value} mode)"
            )
            return True
        ...
        try:
            ...
            logger.info(
                f"📱 Registering push token for user {user_id[:8]}... with provider {provider_id}"
            )
    ...
    async def unregister_push_token(
        self, user_id: str, provider_id: str, integration_identifier: Optional[str] = None
    ) -> bool:
        if not self.enabled:
            logger.debug(
                f"Skipping push token unregistration (Novu disabled in {config.ENV_MODE.value} mode)"
            )
            return True
        ...
        try:
            ...
            logger.info(
                f"🗑️  Unregistering push token for user {user_id[:8]}... with provider {provider_id}"
            )
    ...
    async def trigger_broadcast(self, workflow_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        if not self.enabled:
            logger.warning(
                f"Broadcast skipped (Novu disabled in {config.ENV_MODE.value} mode): {workflow_id}"
            )
            return {
                "success": False,
                "error": "Novu notifications only enabled in staging and production modes",
        ...
        try:
            ...
            data = {"name": workflow_id, "payload": payload}
    ...
    async def list_workflows(self) -> Dict[str, Any]:
        if not self.enabled:
            logger.warning(
                f"Workflows list skipped (Novu disabled in {config.ENV_MODE.value} mode)"
            )
            return {
                "success": False,
                "error": "Novu notifications only enabled in staging and production modes",
