...
async def update_presence(
    payload: UpdatePresenceRequest, request: Request, token: Optional[str] = None
):
    ...
    try:
        ...
        if not success:
            ...
            logger.warning(
                f"Presence update failed for session {payload.session_id}, account {account_id}"
            )
...
async def clear_presence(
    request: Request, session_id: Optional[str] = None, token: Optional[str] = None
):
    if config.DISABLE_PRESENCE:
...
async def get_thread_viewers(thread_id: str, request: Request, token: Optional[str] = None):
    if config.DISABLE_PRESENCE:
...
async def get_account_active_threads(request: Request, token: Optional[str] = None):
    if config.DISABLE_PRESENCE:
...
@router.post("/cleanup")
async def cleanup_stale_sessions(request: Request, token: Optional[str] = None):
    if config.DISABLE_PRESENCE:
