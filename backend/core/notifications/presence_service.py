...
class PresenceService:
    ...
    def _normalize_session_id(self, session_id: str) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Converted non-UUID session_id '{session_id}' to UUID '{deterministic_uuid}'"
            )
    ...
    async def _upsert_session(
        ...
        try:
            ...
            if "permission denied" in error_str or "policy" in error_str:
                raise ValueError(
                    f"Permission denied: Unable to update presence for account {account_id}. This may indicate an account membership issue."
                )
            raise
    ...
    async def update_presence(
        ...
        try:
            ...
            existing_data = (
                existing.data if existing and hasattr(existing, "data") and existing.data else None
            )

            is_new_session = not existing_data

            if existing_data and self._is_stale(
                client_timestamp, existing_data.get("client_timestamp")
            ):
                logger.warning(
            ...
            if (
                ...
                or "invalid or non-existent account_id" in error_str
            ):
                logger.error(
                    f"Permission/validation error updating presence for session {session_id}, account {account_id}: {str(e)}"
                )
            else:
                logger.error(
                    f"Error updating presence for session {session_id}: {str(e)}", exc_info=True
                )
            return False
    ...
    async def cleanup_stale_sessions(self, account_id: Optional[str] = None) -> int:
        ...
        try:
            ...
            count = await presence_repo.delete_stale_sessions(
                self.stale_session_threshold_minutes, account_id
            )
    ...
    async def is_account_viewing_thread(self, account_id: str, thread_id: str) -> bool:
        ...
        try:
            ...
            threshold = datetime.now(timezone.utc) - timedelta(
                minutes=self.activity_threshold_minutes
            )
            return any(
    ...
    async def get_thread_viewers(self, thread_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = await client.rpc(
                "get_thread_viewers", {"thread_id_param": thread_id}
            ).execute()
    ...
    async def get_account_active_threads(self, account_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            client = await self.db.client
            result = await client.rpc(
                "get_account_active_threads", {"account_id_param": account_id}
            ).execute()
            return result.data if result.data else []
        except Exception as e:
    ...
    async def should_send_notification(
        self, account_id: str, thread_id: Optional[str] = None, channel: str = "email"
    ) -> bool:
        if config.DISABLE_PRESENCE:
            return True
