...
@router.post(
    "/apify/approvals/{approval_id}/approve",
    summary="Approve Apify Request",
    operation_id="approve_apify_request",
)
async def approve_apify_request(
    ...
    try:
        ...
        if approval.get("account_id") != user_id:
            raise HTTPException(
                status_code=403, detail="You can only approve your own approval requests"
            )
        ...
        try:
            ...
            if current_status != "pending":
                raise HTTPException(
                    status_code=400,
                    detail=f"Approval request cannot be approved (status: {current_status}). Only pending approvals can be approved.",
                )
            ...
            if expires_at_str:
                try:
                    ...
                    if datetime.now(timezone.utc) > expires_at:
                        ...
                        raise HTTPException(
                            status_code=400,
                            detail=f"Approval request {approval_id} has expired. Please create a new approval request.",
                        )
            ...
            max_cost_with_markup = (
                max_cost_usd * TOKEN_PRICE_MULTIPLIER
            )  # Apply markup (deduct_credits expects USD with markup)
            ...
            try:
                ...
                logger.error(
                    f"Failed to update approval status in Redis: {redis_err}", exc_info=True
                )
                # If Redis write fails here, we haven't deducted credits yet, so it's safe to raise
                raise HTTPException(
                    status_code=500, detail="Failed to update approval status. Please try again."
                )
            ...
            if max_cost_usd > 0:
                try:
                    ...
                    if result.get("success"):
                        ...
                        logger.info(
                            f"✅ Deducted ${max_cost_with_markup:.6f} USD (with markup) on approve for {approval_id} (max cost hold: ${max_cost_usd:.6f} USD)"
                        )
                    else:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to rollback approval status in Redis: {rollback_err}",
                                exc_info=True,
                            )
            ...
            try:
                ...
                logger.warning(
                    f"Failed final status update in Redis (non-critical): {final_update_err}"
                )
...
@router.get(
    "/apify/approvals/{approval_id}",
    summary="Get Approval Status",
    operation_id="get_apify_approval_status",
)
async def get_apify_approval_status(
    ...
    try:
        ...
        if approval.get("account_id") != user_id:
            raise HTTPException(
                status_code=403, detail="You can only view your own approval requests"
            )
