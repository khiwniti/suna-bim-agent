...
async def regenerate_signed_link(
    request: RegenerateLinkRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    try:
        db = DBConnection()
        ...
        if request.file_upload_id:
            ...
            file_id_to_update = (
                file_upload_result.data[0]["id"] if file_upload_result.data else None
            )
        else:
            raise HTTPException(
                status_code=400, detail="Either file_upload_id or storage_path must be provided"
            )
        ...
        if file_id_to_update:
            await (
                client.table("file_uploads")
                .update({"signed_url": signed_url, "url_expires_at": url_expires_at.isoformat()})
                .eq("id", file_id_to_update)
                .execute()
            )
...
async def get_file_upload(
    file_upload_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    try:
        db = DBConnection()
        client = await db.client
        ...
        file_upload_result = (
            await client.table("file_uploads").select("*").eq("id", file_upload_id).execute()
        )
