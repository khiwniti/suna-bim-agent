...
async def request_account_deletion(
    body: AccountDeletionRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    try:
        db = DBConnection()
        ...
        existing_request = (
            await client.table("account_deletion_requests")
            .select("*")
        ...
        if existing_request.data and len(existing_request.data) > 0:
            ...
            return AccountDeletionResponse(
                ...
                deletion_scheduled_for=datetime.fromisoformat(
                    existing["deletion_scheduled_for"].replace("Z", "+00:00")
                ),
                can_cancel=True,
            )
        ...
        logger.info(
            f"Account deletion requested for user {user_id}, scheduled for {deletion_date} (will be processed by daily check)"
        )
...
async def cancel_account_deletion(user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    try:
        db = DBConnection()
        client = await db.client
        ...
        await (
            ...
            .update({"is_cancelled": True, "cancelled_at": datetime.now(timezone.utc).isoformat()})
            .eq("id", request_id)
            .execute()
        )
...
async def get_account_deletion_status(user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    try:
        db = DBConnection()
        client = await db.client
        ...
        return AccountDeletionStatusResponse(
            ...
            deletion_scheduled_for=datetime.fromisoformat(
                request["deletion_scheduled_for"].replace("Z", "+00:00")
            ),
...
async def delete_account_sandboxes_endpoint(
    request: DeleteSandboxesRequest, _: bool = Depends(verify_admin_api_key)
):
    ...
    try:
        ...
        return {"success": True, "deleted_count": deleted_count, "account_id": request.account_id}
    except Exception as e:
...
async def check_and_schedule_subscriptions(account_id: str, cancel_at: datetime, client) -> None:
    try:
        credit_account = (
            ...
            .select("stripe_subscription_id, revenuecat_subscription_id, tier, provider")
            .eq("account_id", account_id)
            .execute()
        )
        ...
        if revenuecat_sub_id and tier not in ["none", "free"]:
            logger.warning(
                f"[ACCOUNT_DELETION] User {account_id} has active RevenueCat subscription - must cancel through app store first"
            )
            raise HTTPException(
                status_code=400,
        ...
        if stripe_sub_id:
            await schedule_stripe_subscription_cancellation(
                account_id, stripe_sub_id, cancel_at, tier
            )
        else:
            logger.info(
                f"[ACCOUNT_DELETION] No Stripe subscription to schedule for account {account_id}"
            )
        ...
        logger.error(
            f"[ACCOUNT_DELETION] Error checking subscriptions for account {account_id}: {str(e)}"
        )
        raise


async def schedule_stripe_subscription_cancellation(
    account_id: str, stripe_sub_id: str, cancel_at: datetime, tier: str
) -> None:
    try:
        cancel_at_timestamp = int(cancel_at.timestamp())

        logger.info(
            f"[ACCOUNT_DELETION] Scheduling Stripe subscription {stripe_sub_id} to cancel at {cancel_at.date()} for account {account_id} (tier: {tier})"
        )

        await StripeAPIWrapper.modify_subscription(stripe_sub_id, cancel_at=cancel_at_timestamp)

        logger.info(
            f"[ACCOUNT_DELETION] ✅ Successfully scheduled subscription {stripe_sub_id} to cancel in 30 days"
        )

    except Exception as e:
        logger.error(
            f"[ACCOUNT_DELETION] Error scheduling Stripe subscription cancellation for account {account_id}: {str(e)}"
        )
        raise
...
async def unschedule_subscription_cancellation(account_id: str, client) -> None:
    try:
        ...
        logger.info(
            f"[ACCOUNT_DELETION] Removing scheduled cancellation for Stripe subscription {stripe_sub_id}"
        )

        await StripeAPIWrapper.modify_subscription(
            stripe_sub_id, cancel_at=None, cancel_at_period_end=False
        )

        logger.info(
            f"[ACCOUNT_DELETION] ✅ Successfully removed scheduled cancellation for {stripe_sub_id}"
        )

    except Exception as e:
        logger.error(
            f"[ACCOUNT_DELETION] Error removing scheduled subscription cancellation for account {account_id}: {str(e)}"
        )
        raise
...
async def cancel_account_subscriptions_immediately(account_id: str, client) -> dict:
    ...
    try:
        ...
        if not credit_account.data or len(credit_account.data) == 0:
            logger.info(
                f"[ACCOUNT_DELETION] No credit account found for {account_id}, nothing to cancel"
            )
            return result
        ...
        if revenuecat_sub_id and tier not in ["none", "free"]:
            logger.warning(
                f"[ACCOUNT_DELETION] User {account_id} has active RevenueCat subscription - cannot delete immediately"
            )
            raise HTTPException(
                status_code=400,
                detail="You have an active subscription through the App Store or Google Play. Please cancel your subscription through your device's subscription settings before deleting your account. Once cancelled, wait for your subscription to expire, then you can delete your account.",
        ...
        if stripe_sub_id:
            try:
                logger.info(
                    f"[ACCOUNT_DELETION] Cancelling Stripe subscription {stripe_sub_id} for account {account_id} (tier: {tier})"
                )
                ...
                logger.info(
                    f"[ACCOUNT_DELETION] ✅ Successfully cancelled Stripe subscription {stripe_sub_id}"
                )
            except Exception as e:
                logger.error(
                    f"[ACCOUNT_DELETION] Failed to cancel Stripe subscription {stripe_sub_id}: {str(e)}"
                )
                raise
        else:
            logger.info(
                f"[ACCOUNT_DELETION] No Stripe subscription to cancel for account {account_id}"
            )
        ...
        logger.error(
            f"[ACCOUNT_DELETION] Error cancelling subscriptions for account {account_id}: {str(e)}"
        )
        raise
...
async def delete_account_sandboxes(account_id: str, client) -> int:
    ...
    try:
        ...
        resources = await resource_service.get_account_resources(
            account_id=account_id, resource_type=ResourceType.SANDBOX, status=ResourceStatus.ACTIVE
        )
        ...
        for resource in resources:
            ...
            try:
                ...
                deleted_count += 1
                logger.info(
                    f"Deleted sandbox {sandbox_id} (resource {resource_id}) for account {account_id}"
                )
            except Exception as e:
                # Log but don't fail - sandbox might already be deleted or not exist
                logger.warning(
                    f"Failed to delete sandbox {sandbox_id} (resource {resource_id}): {str(e)}"
                )
...
async def delete_account_immediately(user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    try:
        db = DBConnection()
        client = await db.client
        ...
        result = await client.rpc(
            "delete_user_immediately", {"p_account_id": account_id, "p_user_id": user_id}
        ).execute()
