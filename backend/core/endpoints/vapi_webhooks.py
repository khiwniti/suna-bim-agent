...
class VapiWebhookHandler:
    ...
    async def _handle_conversation_update(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        transcript_data = self._process_messages(messages) or self._process_conversation(
            conversation
        )

        status = "in-progress" if transcript_data else call.get("status", "in-progress")

        update_data = {"transcript": transcript_data, "status": status, "updated_at": "now()"}

        try:
            result = await client.table("vapi_calls").select("*").eq("call_id", call_id).execute()

            if result.data:
                await (
                    client.table("vapi_calls").update(update_data).eq("call_id", call_id).execute()
                )
            else:
                new_call = {
                    "call_id": call_id,
    ...
    async def _handle_status_update(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if status in ["completed", "ended", "failed", "no-answer", "busy", "cancelled"]:
            update_data["ended_at"] = (
                message_data.get("timestamp") or call.get("endedAt") or "now()"
            )
    ...
    async def _handle_end_of_call_report(self, payload: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if user_id and cost > 0:
                try:
                    ...
                    if deduct_result.get("success"):
                        ...
                        logger.info(
                            f"Successfully deducted {cost_in_credits} credits for call {call_id}"
                        )
                    else:
                        logger.warning(
                            f"Failed to deduct credits for call {call_id}: {deduct_result.get('error', 'Unknown error')}"
                        )
                except Exception as e:
        ...
        await self._notify_call_completion(
            call_id, duration, actual_cost_deducted if credit_deducted else cost
        )
        ...
        for line in transcript_text.split("\n"):
            if line.startswith("AI:"):
                transcript_data.append(
                    {"role": "assistant", "message": line.replace("AI:", "").strip()}
                )
            elif line.startswith("User:"):
                transcript_data.append(
                    {"role": "user", "message": line.replace("User:", "").strip()}
                )
    ...
    async def _handle_transcript_updated(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        await (
            ...
            .update({"transcript": json.dumps(transcript)})
            .eq("call_id", call_id)
            .execute()
        )
    ...
    async def _handle_assistant_request(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        return {"assistant": vapi_config.get_assistant_config()}
    ...
    async def _stream_transcript_to_thread(
        self, call_id: str, transcript_data: List[Dict[str, Any]]
    ) -> None:
        try:
            thread_id = await self._get_thread_id_for_call(call_id)
            if not thread_id:
            ...
            if existing_result.data:
                for msg in existing_result.data:
                    ...
                    if (
                        ...
                        and metadata.get("message_index") is not None
                    ):
    ...
    async def _save_transcript_to_thread(
        self, thread_id: str, call_id: str, transcript: list
    ) -> None:
        try:
            client = await self.db.client

            conversation_text = "\n".join(
                [f"{msg.get('role', 'unknown')}: {msg.get('message', '')}" for msg in transcript]
            )
