...
class MarketplaceFilters:
    def __init__(
        ...
        self.search = search
        self.tags = tags or []
        self.is_carbon_bim_team = is_carbon_bim_team
...
class MarketplaceService:
    ...
    async def get_marketplace_templates_paginated(
        self, pagination_params: PaginationParams, filters: MarketplaceFilters
        ...
        try:
            ...
            templates = await template_service.get_public_templates(
                is_carbon_bim_team=filters.is_carbon_bim_team,
                limit=limit,
                offset=offset,
            ...
            if filters.creator_id is not None:
                ...
                if filters.creator_id:
                    ...
                    filtered_templates = [
                        t for t in all_templates if t.creator_id == filters.creator_id
                    ]
    ...
    async def get_user_templates_paginated(
        ...
        try:
            ...
            if filters.tags:
                ...
                filtered_templates = [
                    t for t in filtered_templates if tag_set.intersection(set(t.tags or []))
                ]

            if filters.sort_by == "name":
                filtered_templates.sort(
                    key=lambda t: t.name.lower() if t.name else "",
                    reverse=(filters.sort_order == "desc"),
                )
            elif filters.sort_by == "download_count":
                filtered_templates.sort(
                    key=lambda t: t.download_count, reverse=(filters.sort_order == "desc")
                )
            else:
                filtered_templates.sort(
                    key=lambda t: t.created_at, reverse=(filters.sort_order == "desc")
                )
    ...
    def _build_marketplace_base_query(self, filters: MarketplaceFilters):
        ...
        if filters.is_carbon_bim_team is not None:
            query = query.eq("is_carbon_bim_team", filters.is_carbon_bim_team)
