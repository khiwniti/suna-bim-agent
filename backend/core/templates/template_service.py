...
class AgentTemplate:
    ...
    def with_public_status(
        ...
        return AgentTemplate(
            **{**self.__dict__, "is_public": is_public, "marketplace_published_at": published_at}
        )
...
class TemplateService:
    ...
    async def get_public_templates(
        ...
        query = query.order("download_count", desc=True).order(
            "marketplace_published_at", desc=True
        )
        ...
        for mcp in custom_mcps:
            if isinstance(mcp, dict):
                ...
                if mcp_type == "composio":
                    ...
                    qualified_name = (
                        mcp.get("mcp_qualified_name")
                        or original_config.get("mcp_qualified_name")
                        or mcp.get("qualifiedName")
                        or original_config.get("qualifiedName")
                    )
                    toolkit_slug = mcp.get("toolkit_slug") or original_config.get("toolkit_slug")
    ...
    def _map_to_template(self, data: Dict[str, Any]) -> AgentTemplate:
        ...
        logger.debug(
            f"Mapping template {data.get('template_id')}: usage_examples from DB = {usage_examples}"
        )
