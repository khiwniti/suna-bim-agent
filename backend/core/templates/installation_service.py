...
class InstallationService:
    ...
    async def install_template(
        ...
        logger.debug(
            f"Total requirements from template: {[r.qualified_name for r in all_requirements]}"
        )
        ...
        if not request.profile_mappings:
            request.profile_mappings = await self._auto_map_profiles(
                all_requirements, request.account_id
            )
        ...
        missing_profiles, missing_configs = await self._validate_installation_requirements(
            all_requirements, request.profile_mappings, request.custom_mcp_configs
        )
        ...
        if missing_profiles or missing_configs or missing_trigger_variables:
            return TemplateInstallationResult(
                ...
                missing_trigger_variables=missing_trigger_variables
                if missing_trigger_variables
                else None,
                template_info={"template_id": template.template_id, "name": template.name},
            )

        agent_config = await self._build_agent_config(template, request, all_requirements)
        ...
        return TemplateInstallationResult(status="installed", instance_id=agent_id, name=agent_name)
            ...
            if not req.is_custom():
                ...
                if default_profile:
                    if req.source == "trigger" and req.trigger_index is not None:
                        ...
                        logger.debug(
                            f"Auto-mapped {trigger_key} to profile {default_profile.profile_id} (trigger)"
                        )
                    else:
                ...
                if profile_id:
                    ...
                    if profile:
                        if (
                            ...
                            or "composio" in req.qualified_name
                        ):
                            toolkit_slug = req.toolkit_slug
                            if not toolkit_slug:
    ...
    async def _create_agent(
        ...
        if template.is_carbon_bim_team:
            metadata["is_carbon_bim_team"] = True
            metadata["carbon_bim_template_id"] = template.template_id
        ...
        logger.debug(
            f"Created agent {agent_id} from template {template.template_id}, is_carbon_bim_team: {template.is_carbon_bim_team}"
        )
        return agent_id

    async def _create_initial_version(
        self, agent_id: str, user_id: str, agent_config: Dict[str, Any], system_prompt: str
    ) -> None:
        try:
            ...
            logger.debug(
                f"Creating initial version for agent {agent_id} with system_prompt: {system_prompt[:100]}..."
            )
            logger.debug(
                f"Agent config tools: agentpress={len(agentpress_tools)}, mcp={len(configured_mcps)}, custom_mcp={len(custom_mcps)}"
            )
            ...
            logger.info(
                f"Successfully created initial version {version.version_id} for agent {agent_id}"
            )
            ...
            if agent_check.data and agent_check.data[0].get("current_version_id"):
                ...
                logger.error(
                    f"Agent {agent_id} current_version_id was not updated after version creation!"
                )
        ...
        for i, trigger in enumerate(triggers):
            ...
            if provider_id == "composio":
                ...
                if trigger_configs and trigger_profile_key in trigger_configs:
                    ...
                    logger.info(
                        f"Using user-provided trigger config for {trigger_profile_key}: {trigger_specific_config}"
                    )
                else:
                    logger.info(
                        f"No user trigger config found for key {trigger_profile_key}. Available keys: {list(trigger_configs.keys()) if trigger_configs else 'None'}"
                    )
                ...
                if result.data:
                    ...
                    failed_count += 1
                    logger.warning(
                        f"Failed to insert trigger '{trigger.get('name')}' for agent {agent_id}"
                    )

        logger.debug(
            f"Successfully restored {created_count}/{len(triggers)} triggers for agent {agent_id}"
        )
    ...
    async def _sync_triggers_to_version_config(self, agent_id: str) -> None:
        try:
            ...
            triggers_result = (
                await client.table("agent_triggers").select("*").eq("agent_id", agent_id).execute()
            )
            triggers = []
    ...
    async def _create_composio_trigger(
        ...
        try:
            ...
            keys_to_check.extend([qualified_name, f"composio.{app_name}", "composio"])
            ...
            if connected_account_id:
                ...
                logger.debug(
                    f"Adding connected_account_id to Composio trigger request: {connected_account_id}"
                )
            else:
                logger.warning(
                    "No connected_account_id found - trigger creation may fail for OAuth apps"
                )
