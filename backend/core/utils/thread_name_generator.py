...
Respond with JSON: {"title": "Your Title"}"""
        ...
        if response and response.get("choices") and response["choices"][0].get("message"):
            ...
            try:
                ...
                if isinstance(parsed_response, dict):
                    ...
                    if title:
                        ...
                        logger.debug(
                            f"LLM generated name for thread {thread_id}: '{generated_name}'"
                        )
                else:
                    logger.warning(
                        f"LLM returned non-dict JSON for thread {thread_id}: {parsed_response}"
                    )

            except json.JSONDecodeError as e:
                logger.warning(
                    f"Failed to parse LLM JSON response for thread {thread_id}: {e}. Raw content: {raw_content}"
                )
            ...
            logger.warning(
                f"Failed to get valid response from LLM for thread {thread_id} naming. Response: {response}"
            )
        ...
        if generated_name:
            ...
            if hasattr(update_result, "data") and update_result.data:
                ...
                logger.error(
                    f"Failed to update thread {thread_id} in database. Update result: {update_result}"
                )
        else:
        ...
        logger.error(
            f"Error in background naming task for thread {thread_id}: {str(e)}\n{traceback.format_exc()}"
        )
    finally:
