...
Respond with JSON: {"title": "Your Title Here"}"""
        ...
        user_message = (
            f'Generate a short descriptive title for this {file_type}: "{truncated_prompt}"'
        )
        messages = [
    ...
    prefix_map = {"image": "Image", "video": "Video", "upscaled": "Upscaled", "nobg": "Transparent"}
    prefix = prefix_map.get(file_type, "Media")
    return _generate_fallback_name(prefix, extension)


def generate_filename_sync(prompt: str, file_type: str = "image", extension: str = "png") -> str:
    """
...
async def rename_ugly_files(
    sandbox, workspace_path: str = "/workspace", dry_run: bool = True
) -> List[Tuple[str, str]]:
    """
