...
_tool_output_streaming_context: ContextVar[Optional[ToolOutputStreamingContext]] = ContextVar(
    "tool_output_streaming_context", default=None
)


def set_tool_output_streaming_context(
    agent_run_id: str, stream_key: str, tool_call_id: Optional[str] = None
) -> None:
    ctx = ToolOutputStreamingContext(
        agent_run_id=agent_run_id, stream_key=stream_key, tool_call_id=tool_call_id
    )
...
async def stream_tool_output(
    tool_call_id: str, output_chunk: str, is_final: bool = False, tool_name: str = "execute_command"
) -> None:
    ctx = get_tool_output_streaming_context()
    if not ctx:
    ...
    try:
        ...
        logger.debug(
            f"[TOOL OUTPUT] Writing to stream {ctx.stream_key}: tool_call_id={tool_call_id}, chunk_len={len(output_chunk)}, is_final={is_final}"
        )

        await redis.stream_add(ctx.stream_key, {"data": message_json}, maxlen=200, approximate=True)
