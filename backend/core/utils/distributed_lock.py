        ...
        while True:
            try:
                ...
                if elapsed >= wait_timeout:
                    logger.warning(
                        f"[LOCK] Lock acquisition timeout after {elapsed}s: {self.lock_key}"
                    )
                    return False
...
class RenewalLock:
    ...
    async def check_and_mark_renewal_processed(
        ...
        try:
            await (
                client.from_("renewal_processing")
                .insert(
                    {
...
class WebhookLock:
    ...
    async def check_and_mark_webhook_processing(
        event_id: str, event_type: str, payload: dict = None, force_reprocess: bool = False
        ...
        existing = (
            ...
            .select("id, status, processed_at, processing_started_at, retry_count")
        ...
        if existing.data:
            ...
            if event["status"] == "completed":
                if force_reprocess:
                    logger.warning(
                        f"[WEBHOOK] Event {event_id} marked as completed but force_reprocess=True - resetting to processing"
                    )
                    await (
                        client.from_("webhook_events")
                        .update(
                            {
                                "status": "processing",
                    ...
                    return True, None
                logger.info(
                    f"[WEBHOOK] Event {event_id} already completed at {event['processed_at']}"
                )
                ...
                if processing_started:
                    try:
                        started_at = datetime.fromisoformat(
                            processing_started.replace("Z", "+00:00")
                        )
            ...
            try:
                ...
                if "duplicate key" in str(e).lower() or "unique" in str(e).lower():
                    logger.warning(
                        f"[WEBHOOK] Race condition detected for event {event_id}, checking if existing event is stuck..."
                    )
                    ...
                    if existing_after_race.data:
                        ...
                        if race_event["status"] == "completed":
                            logger.info(
                                f"[WEBHOOK] Race condition resolved - event {event_id} already completed"
                            )
                            return False, "already_completed"
                        ...
                        if race_event["status"] == "failed":
                            logger.info(
                                f"[WEBHOOK] Race condition resolved - event {event_id} failed, retrying"
                            )
                            await (
                                client.from_("webhook_events")
                                .update(
                        ...
                        if race_event["status"] == "processing":
                            logger.info(
                                f"[WEBHOOK] Race condition - event {event_id} being processed, waiting to verify..."
                            )
                            ...
                            if recheck.data:
                                ...
                                if final_status == "completed":
                                    logger.info(
                                        f"[WEBHOOK] Race condition resolved - event {event_id} completed while waiting"
                                    )
                                    return False, "already_completed"
                                        ...
                                        .update(
                                            {
                                                ...
                                                "retry_count": recheck.data[0].get("retry_count", 0)
                                                + 1,
                                            }
                                        )
                                        .eq("id", recheck.data[0]["id"])
                                        .execute()
                                    )
                                    return True, None
                            ...
                            logger.warning(
                                f"[WEBHOOK] Event {event_id} still processing after wait, signaling retry needed"
                            )
                            return False, "processing_retry_later"

                    logger.warning(
                        f"[WEBHOOK] Race condition detected for event {event_id}, signaling retry needed"
                    )
                    return False, "processing_retry_later"
                raise
    ...
    async def mark_webhook_completed(event_id: str):
        ...
        await (
            client.from_("webhook_events")
            .update({"status": "completed", "processed_at": datetime.now(timezone.utc).isoformat()})
            .eq("event_id", event_id)
            .execute()
        )
    ...
    async def mark_webhook_failed(event_id: str, error_message: str):
        ...
        try:
            ...
            logger.error(
                f"[WEBHOOK] Failed to mark event {event_id} as failed in database: {db_error}"
            )
