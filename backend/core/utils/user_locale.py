...
SUPPORTED_LOCALES = ["en", "de", "it", "zh", "ja", "pt", "fr", "es", "th"]
DEFAULT_LOCALE = "en"
...
async def get_user_locale(user_id: str, client=None) -> str:
    ...
    Returns:
        Locale string ('en', 'de', 'it', 'zh', 'ja', 'pt', 'fr', 'es', 'th') or 'en' as default
    """
    try:
        if client is None:
        ...
        if result.data:
            if isinstance(result.data, dict):
                ...
                metadata = {}
                logger.warning(
                    f"⚠️ Unexpected result.data type for user {user_id}: {type(result.data)}"
                )
            ...
            if locale and locale in SUPPORTED_LOCALES:
                ...
                logger.warning(
                    f"⚠️ Invalid locale '{locale}' for user {user_id}, not in supported locales: {SUPPORTED_LOCALES}"
                )

        logger.debug(
            f"⚠️ No locale preference found for user {user_id}, using default: {DEFAULT_LOCALE}"
        )
        ...
        if "PGRST202" in error_msg or "Could not find the function" in error_msg:
            logger.debug(
                f"RPC function not yet available in PostgREST cache for user {user_id}. This is normal immediately after migration. PostgREST will auto-refresh its cache shortly."
            )
        else:
...
def get_locale_context_prompt(locale: str) -> str:
    ...
    Args:
        locale: User's preferred locale ('en', 'de', 'it', 'zh', 'ja', 'pt', 'fr', 'es', 'th')
