...
from core.utils.tool_discovery import (
    get_tool_group,
    get_tools_metadata,
    get_enabled_methods_for_tool,
)
        ...
        if isinstance(tool_value, bool):
            if len(methods) > 1:
                migrated_config[tool_name] = {
                    "enabled": tool_value,
                    "methods": {method["name"]: tool_value for method in methods},
                }
            else:
                migrated_config[tool_name] = tool_value
            ...
            if len(methods) > 1:
                ...
                migrated_config[tool_name] = {"enabled": enabled, "methods": complete_methods}
            else:
                migrated_config[tool_name] = enabled
        else:
            logger.warning(
                f"Invalid tool config format for {tool_name}: {tool_value}, defaulting to enabled"
            )
        ...
        if tool_name not in complete_config:
            ...
            if len(methods) > 1:
                complete_config[tool_name] = {
                    ...
                    "methods": {method["name"]: method.get("enabled", True) for method in methods},
                }
            else:
