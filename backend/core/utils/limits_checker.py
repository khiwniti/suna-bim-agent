...
async def _get_tier_info_if_needed(account_id: str, tier_info: Optional[Dict] = None) -> Dict:
    ...
    try:
        ...
        return {
            ...
            "custom_workers_limit": limits.get(
                "custom_workers_limit", defaults["custom_workers_limit"]
            ),
            "scheduled_triggers_limit": limits.get(
...
async def check_agent_run_limit(
    account_id: str, tier_info: Optional[Dict] = None, client=None
) -> Dict[str, Any]:
    try:
...
async def check_agent_count_limit(
    ...
    try:
        if config.ENV_MODE.value == "local":
            return {"can_create": True, "current_count": 0, "limit": 999999, "tier_name": "local"}
        ...
        logger.debug(
            f"Account {account_id} has {current_count}/{agent_limit} agents (tier: {tier_name})"
        )
        ...
        return {"can_create": True, "current_count": 0, "limit": 1, "tier_name": "free"}
...
async def check_project_count_limit(
    ...
    try:
        ...
        return {"can_create": True, "current_count": 0, "limit": 2, "tier_name": "free"}
...
async def check_custom_mcp_limit(
    account_id: str, tier_info: Optional[Dict] = None, client=None
) -> Dict[str, Any]:
    try:
        tier = await _get_tier_info_if_needed(account_id, tier_info)
        ...
        logger.debug(
            f"Account {account_id} has {total_custom_mcps}/{worker_limit} custom MCPs (tier: {tier_name})"
        )
        ...
        return {"can_create": True, "current_count": 0, "limit": 0, "tier_name": "free"}
...
async def get_all_limits_fast(account_id: str, tier_info: Dict) -> Dict[str, Any]:
    ...
    try:
        ...
        thread_val, project_val = await asyncio.gather(
            redis_service.get(thread_key), redis_service.get(project_key), return_exceptions=True
        )
    ...
    logger.info(
        f"[LIMITS] {account_id[:8]}... threads={counts['thread_count']}/{thread_limit} projects={counts['project_count']}/{project_limit} can_create_thread={counts['thread_count'] < thread_limit}"
    )
