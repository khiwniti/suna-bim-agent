...
def needs_migration(message: Dict[str, Any]) -> bool:
    ...
    if msg_type == "assistant":
        ...
        has_xml_tool_calls = (
            "<function_calls>" in content_str if isinstance(content_str, str) else False
        )
    ...
    if native_tool_calls and isinstance(native_tool_calls, list):
        for idx, tc in enumerate(native_tool_calls):
            if isinstance(tc, dict):
                ...
                # Parse arguments
                parsed_args = (
                    safe_json_parse(func_args, {}) if isinstance(func_args, str) else func_args
                )
...
def migrate_tool_message(
    message: Dict[str, Any], assistant_messages: Optional[List[Dict[str, Any]]] = None
) -> Dict[str, Any]:
    """
    ...
    if tool_execution and isinstance(tool_execution, dict):
        ...
        if "tool_call_id" in content or (
            content.get("role") == "tool" and "tool_call_id" in content
        ):
            ...
            if isinstance(content_value, str):
                try:
                    ...
                    if isinstance(parsed_content, dict) and "tool_execution" in parsed_content:
                        ...
                        result_data = {"success": True, "output": parsed_content, "error": None}
                except:
                    result_data = {"success": True, "output": content_value, "error": None}
            else:
                result_data = {"success": True, "output": content_value, "error": None}
            ...
            try:
                ...
                if isinstance(parsed, dict) and "tool_execution" in parsed:
                    ...
                    result_data = {"success": True, "output": parsed, "error": None}
            except:
                # Fallback: use content as-is
                function_name = "unknown"
                result_data = {"success": True, "output": content_str, "error": None}
    else:
        # Content is string or other type - treat as XML (old messages are XML)
        return_format = "xml"
        function_name = "unknown"
        result_data = {"success": True, "output": str(content), "error": None}
    ...
    if assistant_message_id and assistant_messages:
        ...
        assistant_msg = next(
            (m for m in assistant_messages if m.get("message_id") == assistant_message_id), None
        )
        if assistant_msg:
            ...
            if not metadata.get("tool_call_id") and function_name and function_name != "unknown":
                ...
                matching_tc = next(
                    (tc for tc in tool_calls if tc.get("function_name") == function_name), None
                )
                ...
                if not matching_tc and xml_tag_name:
                    matching_tc = next(
                        (
                            ...
                            if tc.get("xml_tag_name") == xml_tag_name
                            or tc.get("xml_tag_name") == xml_tag_name.replace("_", "-")
                            or tc.get("function_name") == xml_tag_name.replace("-", "_")
                        ),
                        None,
                    )
            ...
            if not metadata.get("tool_call_id") and function_name and function_name != "unknown":
                ...
                matching_tool_calls = [
                    ...
                    or (xml_tag_name and tc.get("xml_tag_name") == xml_tag_name)
                ]
    ...
    return {**message, "metadata": cleaned_metadata}
                ...
                if migrated:
                    if save:
                        await (
                            ...
                            .update({"metadata": migrated["metadata"]})
