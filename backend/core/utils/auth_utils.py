    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            response = await client.get(
                jwks_url, headers={"apikey": supabase_anon_key, "Accept": "application/json"}
            )
...
async def verify_admin_api_key(x_admin_api_key: Optional[str] = Header(None)):
    ...
    if not x_admin_api_key:
        raise HTTPException(
            status_code=401, detail="Admin API key required. Include X-Admin-Api-Key header."
        )
...
async def _decode_jwt_with_verification_async(token: str) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=401, detail="Invalid token format", headers={"WWW-Authenticate": "Bearer"}
        )
    ...
    if algorithm == "ES256" and kid:
        try:
            ...
            logger.warning(
                "JWT signature verification failed (ES256) - possible token forgery attempt"
            )
            raise HTTPException(
                status_code=401,
                detail="Invalid token signature",
    ...
    if algorithm == "HS256":
        ...
        try:
            ...
            logger.warning(
                "JWT signature verification failed (HS256) - possible token forgery attempt"
            )
            raise HTTPException(
                status_code=401,
                detail="Invalid token signature",
...
async def verify_and_get_user_id_from_jwt(request: Request) -> str:
    ...
    if config.ENV_MODE == EnvMode.LOCAL:
        if not auth_header or auth_header == f"Bearer {LOCAL_DEV_ACCESS_TOKEN}":
            structlog.contextvars.bind_contextvars(
                user_id=LOCAL_DEV_USER_ID, auth_method="local_dev"
            )
...
async def verify_and_authorize_thread_access(
    client, thread_id: str, user_id: Optional[str], require_write_access: bool = False
):
    """
    ...
    try:
        ...
        if result.get("project_is_public"):
            if require_write_access:
                ...
                structlog.get_logger().debug(
                    f"Public thread write access requested, checking ownership: {thread_id}"
                )
            else:
        ...
        if not user_id:
            if require_write_access:
                raise HTTPException(
                    status_code=403, detail="Authentication required to modify this thread"
                )
            raise HTTPException(
        ...
        if user_role in ("admin", "super_admin"):
            structlog.get_logger().debug(
                f"Admin access granted for thread {thread_id}", user_role=user_role
            )
            return True
...
async def get_authorized_user_for_thread(
    thread_id: str, request: Request, require_write_access: bool = False
) -> str:
    """
...
async def get_authorized_user_for_agent(agent_id: str, request: Request) -> tuple[str, dict]:
    """
...
async def require_thread_access(thread_id: str, request: Request) -> AuthorizedThreadAccess:
    """
...
async def require_thread_write_access(thread_id: str, request: Request) -> AuthorizedThreadAccess:
    """
...
async def require_agent_access(agent_id: str, request: Request) -> AuthorizedAgentAccess:
    """
    FastAPI dependency that verifies JWT and authorizes agent access.
...
async def verify_sandbox_access(client, sandbox_id: str, user_id: str):
    ...
    if not result:
        raise HTTPException(
            status_code=404, detail="Sandbox not found - no resource exists for this sandbox"
        )
    ...
    if not project_id:
        if is_resource_team_member:
            structlog.get_logger().debug(
                "User has access to resource via account membership",
            ...
            return {
                ...
                "account_id": resource_account_id,
                "is_public": False,
                "sandbox": {"id": sandbox_id, **(result.get("resource_config") or {})},
            }
        raise HTTPException(
            status_code=404, detail="Sandbox not found - no project uses this sandbox"
        )
    ...
    if is_public:
        structlog.get_logger().debug(
            "Allowing access to public project sandbox", project_id=project_id
        )
        return project_data
    ...
    if user_role in ("admin", "super_admin"):
        structlog.get_logger().debug(
            "Admin access granted for sandbox", sandbox_id=sandbox_id, user_role=user_role
        )
        return project_data
    ...
    if is_project_team_member:
        structlog.get_logger().debug(
            "User has access to private project sandbox via team membership", project_id=project_id
        )
        return project_data
