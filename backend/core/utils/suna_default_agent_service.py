...
class SunaDefaultAgentService:
    ...
    async def install_for_all_users(self) -> Dict[str, Any]:
        ...
        try:
            ...
            all_account_ids = (
                {row["id"] for row in accounts_result.data} if accounts_result.data else set()
            )
            ...
            existing_account_ids = (
                {row["account_id"] for row in existing_result.data}
            ...
            return {
                ...
                "details": errors
                if errors
                else [f"Successfully installed for {success_count} users"],
            }

        except Exception as e:
            error_msg = f"Installation operation failed: {str(e)}"
            logger.error(error_msg)
            return {"installed_count": 0, "failed_count": 0, "details": [error_msg]}
