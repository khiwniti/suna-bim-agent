...
async def generate_and_update_project_name(project_id: str, prompt: str):
    ...
    try:
        ...
        logger.debug(
            f"Calling LLM ({model_name}) for project {project_id} naming and icon selection."
        )
            ...
            try:
                ...
                if isinstance(parsed_response, dict):
                    ...
                    if icon and icon in relevant_icons:
                        selected_icon = icon
                        logger.debug(
                            f"LLM selected icon for project {project_id}: '{selected_icon}'"
                        )
                    else:
                        logger.warning(
                    ...
                    logger.warning(
                        f"LLM returned non-dict JSON for project {project_id}: {parsed_response}"
                    )
        ...
        if generated_name:
            ...
            logger.info(
                f"Storing project {project_id} with title: '{generated_name}', icon: '{selected_icon}'"
            )
            ...
            if hasattr(update_result, "data") and update_result.data:
                ...
                logger.error(
                    f"Failed to update project {project_id} in database. Update result: {update_result}"
                )
        else:
        ...
        logger.debug(
            f"Finished background naming and icon selection task for project: {project_id}"
        )
