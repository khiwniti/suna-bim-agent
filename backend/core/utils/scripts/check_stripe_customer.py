...
async def check_stripe_customer(customer_id: str):
    ...
    for idx, sub in enumerate(subscriptions.data, 1):
        ...
        logger.info(
            f"Current period: {datetime.fromtimestamp(sub.current_period_start).isoformat()} to {datetime.fromtimestamp(sub.current_period_end).isoformat()}"
        )
        ...
        if items_data:
            for item in items_data:
                ...
                logger.info(
                    f"  Interval: {price.recurring.interval if hasattr(price, 'recurring') and price.recurring else 'N/A'}"
                )
...
def main():
    parser = argparse.ArgumentParser(description="Check Stripe customer and their subscriptions")
    parser.add_argument("customer_id", type=str, help="Stripe customer ID (e.g., cus_xxxxx)")
