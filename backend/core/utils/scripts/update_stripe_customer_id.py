...
async def update_stripe_customer_id(user_email: str, new_customer_id: str):
    ...
    if billing_customer_result.data:
        ...
        await (
            client.schema("basejump")
            .from_("billing_customers")
            .insert({"id": new_customer_id, "account_id": account_id})
            .execute()
        )
...
def main():
    parser = argparse.ArgumentParser(description="Update Stripe customer ID for a user")
    parser.add_argument("email", type=str, help="Email address of the user")
    parser.add_argument("customer_id", type=str, help="New Stripe customer ID (e.g., cus_xxxxx)")
