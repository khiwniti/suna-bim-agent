        ...
        if result["failed_count"] > 0:
            ...
            for detail in result["details"]:
                if detail["status"] == "failed":
                    print(
                        f"   - User {detail['account_id']}: {detail.get('error', 'Unknown error')}"
                    )
...
async def main():
    ...
    install_user_parser = subparsers.add_parser(
        "install-user", help="Install Suna agent for specific user"
    )
    install_user_parser.add_argument("account_id", help="Account ID to install Suna for")

    replace_user_parser = subparsers.add_parser(
        "replace-user", help="Replace Suna agent for specific user (if corrupted)"
    )
