...
async def delete_sandbox_if_free_user(
    daytona_client, supabase_client, sandbox_id: str, dry_run: bool = False
) -> tuple[bool, str]:
    """
    ...
    try:
        ...
        if dry_run:
            return (
                True,
                f"would_delete (project: {project_id}, user: {account_id}, {subscription_info})",
            )
        else:
            ...
            try:
                ...
                return (
                    True,
                    f"deleted (project: {project_id}, user: {account_id}, {subscription_info})",
                )
...
async def delete_free_user_sandboxes(
    sandbox_ids: List[str], dry_run: bool = False
) -> Dict[str, int]:
    """
...
Examples:
  ...
  python delete_free_user_sandboxes.py --use-json raw_sandboxes_20250817_194448.json --limit 100
    ...
    parser.add_argument(
        "--sandbox-ids", type=str, help="Comma-separated list of sandbox IDs to process"
    )
    parser.add_argument(
        "--use-json",
        type=str,
        help="JSON file containing sandbox data (e.g., raw_sandboxes_20250817_194448.json)",
    )
    parser.add_argument(
        "--limit", type=int, help="Limit the number of sandboxes to process (for testing)"
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Required for processing more than 50 sandboxes without dry-run",
    )
    ...
    if not args.dry_run and len(sandbox_ids) > 50 and not args.force:
        logger.error(
            f"Safety check: Attempting to delete {len(sandbox_ids)} sandboxes without --dry-run"
        )
