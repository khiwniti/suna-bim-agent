...
class StripeSubscriptionSyncV2:
    ...
    async def sync_all_subscriptions(self):
        ...
        while True:
            ...
            print(
                f"  Processing batch {offset // batch_size + 1} ({len(customers_result.data)} customers)..."
            )
    ...
    async def sync_customer_subscription(self, customer: Dict) -> None:
        ...
        try:
            ...
            if not price_id:
                ...
                logger.warning(
                    f"Customer {stripe_customer_id}: Could not extract price_id from subscription"
                )
                logger.debug(
                    f"Subscription items: {sub.items if hasattr(sub, 'items') else 'No items'}"
                )
                return
            ...
            update_data = {
                ...
                "billing_cycle_anchor": datetime.fromtimestamp(sub.created).isoformat()
                if sub.created
                else None,
            ...
            await (
                self.client.from_("credit_accounts")
                .update(update_data)
                .eq("account_id", account_id)
                .execute()
            )
            ...
            current_balance = (
                Decimal(str(current_account.data.get("balance", 0)))
                if current_account.data
                else Decimal("0")
            )
            needs_credits = (current_tier == "none" and tier.monthly_credits > 0) or (
            ...
            if needs_credits:
                ...
                logger.info(
                    f"Granted {tier.monthly_credits} credits to {account_id} for tier {tier.name}"
                )
    ...
    def extract_price_id(self, subscription: Any, customer_id: str) -> Optional[str]:
        try:
            ...
            logger.warning(
                f"Customer {customer_id}: Unable to extract price_id, subscription structure: {type(subscription)}"
            )
    ...
    def print_stats(self):
        ...
        success_rate = (
            (self.stats["synced"] / self.stats["total_customers"] * 100)
