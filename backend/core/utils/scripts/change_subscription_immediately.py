...
def get_available_tiers() -> str:
    ...
    for tier_name, tier in TIERS.items():
        if (
            tier_name in ["none"]
            or tier_name.startswith("tier_12")
            or tier_name.startswith("tier_50")
            or tier_name.startswith("tier_125")
            or tier_name.startswith("tier_200")
            or tier_name.startswith("tier_150")
        ):
            continue
...
async def get_user_info(client, user_email: str) -> Optional[dict]:
    result = await client.rpc(
        "get_user_account_by_email", {"email_input": user_email.lower()}
    ).execute()
    ...
    for sub in subscriptions.data:
        if sub.status in ["active", "trialing", "past_due"]:
            return await stripe.Subscription.retrieve_async(
                sub.id, expand=["items.data.price", "schedule"]
            )
...
async def change_subscription_immediately(
    user_email: str, target_tier: str, billing_type: str = "monthly", dry_run: bool = False
):
    logger.info("=" * 80)
    ...
    if billing_type not in TIER_PRICE_MAPPING[target_tier]:
        ...
        logger.info(
            f"Available billing types for {target_tier}: {list(TIER_PRICE_MAPPING[target_tier].keys())}"
        )
        return False
    ...
    try:
        items_data = (
            current_subscription["items"]["data"]
            if isinstance(current_subscription, dict)
            else current_subscription.items.data
        )
    except (AttributeError, TypeError):
        items_data = (
            current_subscription.get("items", {}).get("data", [])
            if isinstance(current_subscription, dict)
            else []
        )
    ...
    current_price_id = (
        items_data[0].price.id if hasattr(items_data[0], "price") else items_data[0]["price"]["id"]
    )
    ...
    logger.info(
        f"  Tier: {current_tier_info.name if current_tier_info else 'unknown'} ({current_tier_info.display_name if current_tier_info else 'unknown'})"
    )
    logger.info(f"  Price ID: {current_price_id}")
    logger.info(
        f"  Period end: {datetime.fromtimestamp(current_subscription.current_period_end).isoformat()}"
    )
    ...
    if is_current_commitment:
        credit_account = (
            ...
            .select("commitment_end_date")
            .eq("account_id", account_id)
            .execute()
        )
        if credit_account.data and credit_account.data[0].get("commitment_end_date"):
            commitment_end = datetime.fromisoformat(
                credit_account.data[0]["commitment_end_date"].replace("Z", "+00:00")
            )
    ...
    try:
        ...
        logger.info(
            f"  New period: {datetime.fromtimestamp(updated_subscription.current_period_start).isoformat()} to {datetime.fromtimestamp(updated_subscription.current_period_end).isoformat()}"
        )
    except Exception as e:
    ...
    update_result = (
        await client.from_("credit_accounts")
        .update({k: v for k, v in update_data.items() if k != "account_id"})
        .eq("account_id", account_id)
        .execute()
    )
    ...
    if is_commitment and commitment_duration > 0:
        existing = (
            ...
            .select("id")
            .eq("stripe_subscription_id", updated_subscription.id)
            .execute()
        )
        ...
        if not existing.data:
            end_date = start_date + timedelta(days=365)
            await (
                client.from_("commitment_history")
                .insert(
                    {
                        "account_id": account_id,
                        "commitment_type": "yearly_commitment",
    ...
    balance = (
        Decimal(str(current_balance.data[0]["balance"])) if current_balance.data else Decimal("0")
    )
    ...
    if balance < Decimal("1.0"):
        ...
        if result.get("success"):
            logger.info(
                f"Granted ${credits_to_grant} credits, new balance: ${result.get('new_total', 0)}"
            )
        else:
    ...
    final_account = (
        await client.from_("credit_accounts").select("*").eq("account_id", account_id).execute()
    )
...
def main():
    ...
    asyncio.run(
        change_subscription_immediately(
