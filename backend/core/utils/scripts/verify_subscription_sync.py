...
class VerifySubscriptionSyncService:
    ...
    async def verify_active_subscriptions(self):
        ...
        for i in range(0, len(customers_result.data), batch_size):
            ...
            print(
                f"  Checking batch {i // batch_size + 1} ({i + 1}-{min(i + batch_size, len(customers_result.data))} of {len(customers_result.data)})..."
            )
    ...
    async def verify_customer(self, customer: Dict) -> None:
        ...
        try:
            subscriptions = await stripe.Subscription.list_async(
                customer=stripe_customer_id, status="all", limit=1, expand=["data.items.data.price"]
            )
            ...
            if not price_id:
                logger.warning(
                    f"Customer {stripe_customer_id}: Active subscription but no price_id"
                )
                return
            ...
            if not expected_tier:
                logger.warning(
                    f"Customer {stripe_customer_id}: price_id {price_id} doesn't match any tier"
                )
                return
            ...
            if db_sub_id != sub.id:
                logger.warning(
                    f"Customer {stripe_customer_id}: DB has subscription {db_sub_id} but Stripe has {sub.id}"
                )
    ...
    async def check_database_consistency(self):
        ...
        if orphaned.data:
            ...
            for user in orphaned.data[:5]:
                print(
                    f"    - Account {user['account_id']}: tier={user['tier']}, balance={user['balance']}"
                )
        ...
        if mismatched.data:
            print(
                f"  ⚠️  Found {len(mismatched.data)} users with tier='none' but have subscription ID"
            )
            for user in mismatched.data[:5]:
                print(
                    f"    - Account {user['account_id']}: sub_id={user['stripe_subscription_id']}"
                )
        ...
        if total_issues == 0:
            ...
            if self.issues["missing_in_db"]:
                ...
                for issue in self.issues["missing_in_db"][:5]:
                    print(
                        f"  - Customer {issue['customer_id']}: expected tier={issue['expected_tier']}"
                    )
            ...
            if self.issues["wrong_tier"]:
                ...
                for issue in self.issues["wrong_tier"][:5]:
                    print(
                        f"  - Customer {issue['customer_id']}: has {issue['current_tier']}, should be {issue['expected_tier']}"
                    )
            ...
            if self.issues["no_credits"]:
                ...
                for issue in self.issues["no_credits"][:5]:
                    print(
                        f"  - Customer {issue['customer_id']}: tier={issue['tier']}, balance={issue['balance']}, expected={issue['expected_credits']}"
                    )
