...
def archive_stopped_sandboxes(
    dry_run=False, save_json=False, json_filename=None, use_existing_json=None
):
...
def main():
    parser = argparse.ArgumentParser(
        description="Archive stopped Daytona sandboxes and optionally save list as JSON"
    )
    ...
    parser.add_argument(
        "--json-file",
        type=str,
        help="Custom filename for JSON output (default: sandboxes_TIMESTAMP.json)",
    )
    parser.add_argument(
        "--use-json",
        type=str,
        help="Use existing JSON file to get STOPPED sandbox IDs (e.g., raw_sandboxes_20250817_194448.json)",
    )
