...
class FastSyncService:
    ...
    async def sync_customer_subscription(self, customer: Dict) -> None:
        ...
        try:
            ...
            if sub.status not in ["active", "trialing"]:
                ...
                logger.debug(
                    f"Customer {stripe_customer_id}: subscription status is {sub.status}, skipping"
                )
                return
            ...
            if sub.status in ["active", "trialing"]:
                ...
                if not price_id:
                    ...
                    logger.debug(
                        f"Customer {stripe_customer_id}: no price_id found in subscription"
                    )
                    return
                ...
                if not tier:
                    ...
                    logger.debug(
                        f"Customer {stripe_customer_id}: price_id {price_id} doesn't match any configured tier"
                    )
                    return
    ...
    async def convert_free_users_batch(self):
        ...
        while True:
            ...
            print(
                f"  Converting batch {offset // batch_size + 1} ({len(free_users.data)} users)..."
            )
    ...
    async def fix_topup_credits_batch(self):
        ...
        while True:
            purchases = (
                await self.client.from_("credit_purchases")
