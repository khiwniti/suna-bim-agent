            ...
            try:
                ...
                if last_grant_date:
                    ...
                    if last_grant_dt >= start_of_day:
                        logger.info(
                            f"✅ Already processed: {account_id} (last_grant: {last_grant_date})"
                        )
                        skipped_count += 1
                        continue
                ...
                try:
                    ...
                    logger.warning(
                        f"⚠️  Could not fetch subscription {stripe_subscription_id} for {account_id}: {e}"
                    )
                    skipped_count += 1
                    continue
                ...
                if subscription.status not in ["active", "trialing", "past_due"]:
                    logger.info(
                        f"Skipping {account_id}: subscription status is {subscription.status}"
                    )
                    skipped_count += 1
                    continue
