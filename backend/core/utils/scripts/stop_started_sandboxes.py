...
def stop_started_sandboxes(
    dry_run: bool = False, save_json: bool = False, json_filename: Optional[str] = None
) -> Dict[str, int]:
    """
    ...
    try:
        ...
        if all_sandboxes:
            ...
            for i, sb in enumerate(all_sandboxes[1:6]):  # Show next 5 samples
                logger.info(
                    f"  Sample {i + 2}: State='{getattr(sb, 'state', 'N/A')}', ID={getattr(sb, 'id', 'N/A')[:20]}..."
                )
    ...
    for i, sandbox in enumerate(started_sandboxes):
        ...
        logger.info(
            f"[{i + 1}/{len(started_sandboxes)}] Processing sandbox: {sandbox_id} ({sandbox_name})"
        )
        ...
        try:
            if dry_run:
                ...
                try:
                    ...
                    logger.warning(
                        f"  ⚠ Sandbox {sandbox_id} stop command sent, but wait failed: {wait_error}"
                    )
...
Examples:
  ...
  python stop_started_sandboxes.py --save-json --json-file started_sandboxes.json
    ...
    parser.add_argument(
        "--save-json", action="store_true", help="Save list of started sandboxes as JSON file"
    )
    parser.add_argument(
        "--json-file",
        type=str,
        help="Custom filename for JSON output (default: started_sandboxes_TIMESTAMP.json)",
    )
    ...
    try:
        stats = stop_started_sandboxes(
            dry_run=args.dry_run, save_json=args.save_json, json_filename=args.json_file
        )
