...
async def fix_missing_subscription(user_email: str):
    ...
    logger.info(
        f"  Current period: {datetime.fromtimestamp(subscription.current_period_start).isoformat()} to {datetime.fromtimestamp(subscription.current_period_end).isoformat()}"
    )
    ...
    if hasattr(subscription, "schedule") and subscription.schedule:
        ...
        try:
            schedule = await stripe.SubscriptionSchedule.retrieve_async(
                subscription.schedule, expand=["phases.items.price"]
            )
            logger.info(f"\n  Schedule details:")
            ...
            for idx, phase in enumerate(schedule.phases):
                ...
                logger.info(
                    f"      Start: {datetime.fromtimestamp(phase['start_date']).isoformat()}"
                )
                logger.info(
                    f"      End: {datetime.fromtimestamp(phase['end_date']).isoformat() if phase.get('end_date') else 'ongoing'}"
                )
                if phase.get("items"):
    ...
    if is_commitment:
        ...
        if commitment_history.data:
            ...
            for record in commitment_history.data:
                logger.info(
                    f"     - Type: {record.get('commitment_type')}, ends: {record.get('end_date')}"
                )
...
def main():
    ...
    parser.add_argument("email", type=str, help="Email address of the user to fix subscription for")
