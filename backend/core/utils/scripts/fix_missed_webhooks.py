...
async def fix_new_signups(target_date: str, dry_run: bool = False):
    ...
    logger.info(
        "Searching database for users stuck on 'none' tier with active Stripe subscriptions..."
    )
    ...
    while True:
        ...
        affected_users = (
            ...
            .select("account_id, tier, stripe_subscription_id, stripe_subscription_status, balance")
            .eq("tier", "none")
            ...
            try:
                ...
                try:
                    subscription = await stripe.Subscription.retrieve_async(
                        current_sub_id, expand=["items.data.price"]
                    )
                except Exception as e:
                    logger.warning(
                        f"⚠️  Could not fetch subscription {current_sub_id} for {account_id}: {e}"
                    )
                    skipped_count += 1
                    continue
                ...
                if not tier_info:
                    logger.warning(
                        f"⚠️  Unknown price_id {price_id} for subscription {current_sub_id}"
                    )
                    skipped_count += 1
                    continue

                logger.info(
                    f"{'[DRY RUN] ' if dry_run else ''}Fixing {account_id}: tier=none -> {tier_info.name}"
                )
...
async def fix_renewals(target_date: str, dry_run: bool = False):
    ...
    logger.info(
        f"Searching for users whose next_credit_grant was on {target_date} but weren't processed..."
    )
    ...
    while True:
        ...
        affected_users = (
            ...
            .gte("next_credit_grant", start_of_day.isoformat())
            .lt("next_credit_grant", end_of_day.isoformat())
            .not_.is_("stripe_subscription_id", "null")
        ...
        for user_data in affected_users.data:
            ...
            try:
                ...
                logger.info(
                    f"{'[DRY RUN] ' if dry_run else ''}Granting missed renewal for {account_id}: {tier_info.name}, ${tier_info.monthly_credits}"
                )
                ...
                if not dry_run:
                    ...
                    if plan_type == "yearly":
                        ...
                        last_grant_dt = (
                            datetime.fromisoformat(last_grant.replace("Z", "+00:00"))
                            if last_grant
                            else billing_anchor
                        )
...
async def fix_upgrades(target_date: str, dry_run: bool = False):
    ...
    while True:
        ...
        users = (
            await client.from_("credit_accounts")
            .select("account_id, tier, stripe_subscription_id")
            .not_.is_("stripe_subscription_id", "null")
            .not_.in_("tier", ["none"])
            .range(offset, offset + batch_size - 1)
            .execute()
        ...
        for user_data in users.data:
            ...
            try:
                ...
                logger.info(
                    f"{'[DRY RUN] ' if dry_run else ''}Fixing tier mismatch for {account_id}: {db_tier} -> {tier_info.name}"
                )
                ...
                if not dry_run:
                    ...
                    if tier_info.monthly_refill_enabled:
                        ...
                        unique_id = (
                            f"upgrade_recovery_{account_id}_{tier_info.name}_{int(time.time())}"
                        )
