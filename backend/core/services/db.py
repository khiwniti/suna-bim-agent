...
def _get_dsn() -> str:
    ...
    return (
        f"postgresql+psycopg://postgres.{project_ref}:{encoded_password}.supabase.com:6543/postgres"
    )
...
async def init_db() -> None:
    ...
    if read_dsn:
        ...
        if not is_supavisor_read:
            connect_args_read["options"] = (
                f"-c statement_timeout={STATEMENT_TIMEOUT} -c lock_timeout=5000"
            )
        ...
        _read_session_factory = async_sessionmaker(
            _read_engine, class_=AsyncSession, expire_on_commit=False, autoflush=False
        )
        ...
        try:
            ...
            logger.info(
                f"✅ Database initialized | Primary: {pool_info} | Read Replica: {read_pool_info} ✓ | timeout={STATEMENT_TIMEOUT}ms"
            )
        except Exception as e:
            logger.warning(
                f"⚠️ Read replica connection test failed: {e}. Will fallback to primary for reads."
            )
            logger.info(
                f"✅ Database initialized | Primary: {pool_info} | Read Replica: CONFIGURED BUT UNREACHABLE | timeout={STATEMENT_TIMEOUT}ms"
            )
    else:
        _has_read_replica = False
        logger.info(
            f"✅ Database initialized | {pool_info} | timeout={STATEMENT_TIMEOUT}ms | No read replica configured"
        )
        ...
        try:
            ...
            async with _session_factory() as session:
                ...
                if init_ms > 50 or session_ms > 50:
                    logger.warning(
                        f"⏱️ [DB] get_session: init={init_ms:.0f}ms, session_acquire={session_ms:.0f}ms"
                    )
                try:
                    yield session
                    return
...
async def execute(sql: str, params: Optional[dict] = None) -> list[dict]:
    ...
    async with get_session() as session:
        ...
        try:
            ...
            logger.debug(
                f"⏱️ [DB] Raw connection acquired, backend_pid={raw_conn.driver_connection.info.backend_pid}"
            )
        except Exception as e:
        ...
        if conn_ms > 100 or query_ms > 100:
            ...
            logger.warning(
                f"⏱️ [DB] execute: conn={conn_ms:.0f}ms, raw_conn={conn_raw_ms:.0f}ms, query={query_ms:.0f}ms, fetch={fetch_ms:.0f}ms, rows={len(rows)} | SQL: {sql_preview}"
            )
        return rows
...
async def execute_read(sql: str, params: Optional[dict] = None) -> list[dict]:
    ...
    async with get_read_session() as session:
        ...
        if conn_ms > 100 or query_ms > 100:
            sql_preview = sql.strip().replace("\n", " ")[:200]
            logger.warning(
                f"⏱️ [DB] execute_read: conn={conn_ms:.0f}ms, query={query_ms:.0f}ms, fetch={fetch_ms:.0f}ms, rows={len(rows)} | SQL: {sql_preview}"
            )
        return rows


async def execute_one(
    sql: str, params: Optional[dict] = None, commit: bool = False
) -> Optional[dict]:
...
class Table:
    ...
    async def get(
        self, id_col: str, id_val, cols: str = "*", use_replica: Optional[bool] = None
    ) -> Optional[dict]:
        """
        ...
        async with session_ctx as s:
            r = await s.execute(
                text(f'SELECT {cols} FROM "{self.name}" WHERE "{id_col}" = :id'), {"id": id_val}
            )
            row = r.fetchone()
            return dict(row._mapping) if row else None

    async def get_by(
        self, filters: dict, cols: str = "*", use_replica: Optional[bool] = None
    ) -> Optional[dict]:
        """
        ...
        async with session_ctx as s:
            r = await s.execute(
                text(f'SELECT {cols} FROM "{self.name}" WHERE {where} LIMIT 1'), params
            )
            row = r.fetchone()
            return dict(row._mapping) if row else None
    ...
    async def list(
        ...
        limit: int = 100,
        offset: int = 0,
        use_replica: Optional[bool] = None,
    ) -> list[dict]:
        """
        ...
        async with session_ctx as s:
            r = await s.execute(
                text(
                    f'SELECT {cols} FROM "{self.name}" {where} {order} LIMIT :limit OFFSET :offset'
                ),
                params,
            )
    ...
    async def count(
        self, filters: Optional[dict] = None, use_replica: Optional[bool] = None
    ) -> int:
        """
    ...
    async def insert(self, data: dict) -> Optional[dict]:
        ...
        async with get_session() as s:
            r = await s.execute(
                text(f'INSERT INTO "{self.name}" ({col_list}) VALUES ({placeholders}) RETURNING *'),
                data,
            )
            await s.commit()
            row = r.fetchone()
    ...
    async def update(self, id_col: str, id_val, data: dict) -> Optional[dict]:
        ...
        data = self._prep_json(data)
        set_clause = ", ".join(
            f'"{k}" = :{k}::jsonb' if k in self._json_cols else f'"{k}" = :{k}' for k in data
        )
        async with get_session() as s:
            r = await s.execute(
                text(f'UPDATE "{self.name}" SET {set_clause} WHERE "{id_col}" = :id RETURNING *'),
                {**data, "id": id_val},
            )
            await s.commit()
            row = r.fetchone()
    ...
    async def delete(self, id_col: str, id_val) -> bool:
        ...
        async with get_session() as s:
            r = await s.execute(
                text(f'DELETE FROM "{self.name}" WHERE "{id_col}" = :id RETURNING "{id_col}"'),
                {"id": id_val},
            )
            await s.commit()
