...
class CreditService:
    ...
    async def check_and_refresh_daily_credits(self, user_id: str) -> tuple[bool, Decimal]:
        try:
            ...
            account_result = (
                ...
                .select("tier, last_daily_refresh")
            ...
            if not acquired:
                logger.info(
                    f"[DAILY REFRESH] Lock busy for {user_id} on {today}, skipping (another request is processing)"
                )
                return False, Decimal("0")
            ...
            try:
                ...
                if not result.data:
                    logger.error(
                        f"[DAILY REFRESH] No data returned from atomic function for {user_id}"
                    )
                    return False, Decimal("0")
    ...
    async def get_balance(self, user_id: str, use_cache: bool = True) -> Decimal:
        ...
        if use_cache and self.cache:
            ...
            if cached is not None:
                if isinstance(cached, (str, int, float)):
                    ...
                    logger.warning(
                        f"Invalid cache entry for {cache_key}: expected str/int/float, got {type(cached)}"
                    )
        ...
        if result.data and len(result.data) > 0:
            ...
            if config.ENV_MODE == EnvMode.LOCAL:
                logger.info(
                    f"LOCAL mode: Creating user {user_id} with tier='none' (no free tier in local mode)"
                )
                account_data = {
                    "account_id": user_id,
                    "balance": "0",
                ...
                try:
                    ...
                    logger.info(
                        f"Successfully created tier='none' account for user {user_id} in LOCAL mode"
                    )
                except Exception as e:
    ...
    async def grant_tier_credits(self, user_id: str, price_id: str, tier_name: str) -> bool:
        try:
            ...
            result = await client.rpc(
                "grant_tier_credits",
                {"p_user_id": user_id, "p_amount": str(amount), "p_tier": tier_name},
            ).execute()
    ...
    async def get_account_summary(self, user_id: str) -> dict[str, Any]:
        client = await self._get_client()
        account_result = (
            await client.from_("credit_accounts").select("*").eq("account_id", user_id).execute()
        )
