...
async def publish_to_cloudwatch(metrics: dict) -> bool:
    ...
    try:
        ...
        cloudwatch.put_metric_data(Namespace="Carbon BIM", MetricData=metric_data)
        logger.debug(
            f"Published metrics to CloudWatch: {metrics.get('active_agent_runs')} active runs, {metrics.get('orphaned_streams')} orphaned streams"
        )
        return True
    except Exception as e:
...
async def start_cloudwatch_publisher(interval_seconds: int = 60):
    ...
    """
    logger.info(
        f"Starting CloudWatch API instance metrics publisher (interval: {interval_seconds}s)"
    )
