...
async def cleanup_orphaned_agent_runs(db_client) -> int:
    ...
    try:
        ...
        stale_runs = (
            await db_client.table("agent_runs")
        ...
        if skipped_recent_legacy > 0:
            logger.info(
                f"⏭️ Skipped {skipped_recent_legacy} recent legacy runs (no instance_id, <1h old)"
            )
        ...
        logger.warning(
            f"🧹 Found {total_runs} orphaned agent runs for instance {current_instance_id}"
        )
        ...
        for i in range(0, total_runs, BATCH_SIZE):
            ...
            results = await asyncio.gather(
                *[cleanup_single_run(run) for run in batch], return_exceptions=True
            )
            batch_cleaned = sum(1 for r in results if r is True)
            cleaned_count += batch_cleaned

        logger.info(
            f"🧹 Cleaned up {cleaned_count}/{total_runs} orphaned agent runs for instance {current_instance_id}"
        )
