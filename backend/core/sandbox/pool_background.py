...
async def _pool_keepalive_loop(service: SandboxPoolService) -> None:
    logger.info(f"[SANDBOX_POOL] Starting keepalive loop (interval={KEEPALIVE_INTERVAL_SECONDS}s)")
...
async def _initial_pool_warmup(service: SandboxPoolService) -> None:
    ...
    try:
        ...
        if pool_size >= config.replenish_below:
            logger.info(
                f"[SANDBOX_POOL] Pool already at healthy size ({pool_size}), no warmup needed"
            )
            return

        logger.info(
            f"[SANDBOX_POOL] Pool below threshold ({pool_size} < {config.replenish_below}), starting warmup..."
        )
        ...
        while created_total < to_create:
            ...
            logger.info(
                f"[SANDBOX_POOL] Warmup progress: {created_total}/{to_create} sandboxes created"
            )

            if created_total < to_create:
                logger.info(
                    f"[SANDBOX_POOL] Waiting {config.batch_delay}s before next batch to avoid rate limits..."
                )
...
def is_pool_service_running() -> bool:
    return (
        _pool_task is not None
        and not _pool_task.done()
        and _cleanup_task is not None
        and not _cleanup_task.done()
        and _keepalive_task is not None
        and not _keepalive_task.done()
    )
