...
class ConvertRequest(BaseModel):
    presentation_path: str = Field(
        ..., description="Path to the presentation folder containing metadata.json"
    )
    download: bool = Field(
        False,
        description="If true, returns the PDF file directly. If false, returns JSON with download URL.",
    )
...
class PresentationToPDFAPI:
    ...
    async def render_slide_to_pdf(
        self, browser, slide_info: Dict, temp_dir: Path, max_retries: int = 3
    ) -> Path:
        ...
        raise RuntimeError(
            f"Error rendering slide {slide_num} after {max_retries} attempts: {last_error}"
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            async with async_playwright() as p:
                ...
                try:
                    ...
                    print(
                        f"📄 Processing {len(self.slides_info)} slides (max {max_concurrent} concurrent)..."
                    )

                    tasks = [render_with_limit(slide_info) for slide_info in self.slides_info]
...
async def convert_presentation_to_pdf(request: ConvertRequest):
    ...
    try:
        ...
        if request.download:
            pdf_content, total_slides, presentation_name = await converter.convert_to_pdf(
                store_locally=False
            )
...
class HtmlToPdfRequest(BaseModel):
    ...
    content: str = Field(
        None, description="HTML content to convert (used if file_path not provided)"
    )
    file_path: str = Field(
        None, description="Path to HTML file in workspace (takes precedence over content)"
    )
