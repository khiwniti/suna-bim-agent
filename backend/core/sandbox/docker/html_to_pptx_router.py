...
try:
    ...
    raise ImportError(
        f"python-pptx is not installed. Please install it with: pip install python-pptx. Error: {e}"
    )
...
class ConvertRequest(BaseModel):
    ...
    download: bool = Field(
        False,
        description="If true, returns the PPTX file directly. If false, returns JSON with download URL.",
    )
...
async def convert_presentation_to_pptx(request: ConvertRequest):
    ...
    try:
        ...
        if not presentation_path.exists():
            raise HTTPException(
                status_code=404, detail=f"Presentation path not found: {request.presentation_path}"
            )
        ...
        if not metadata_path.exists():
            raise HTTPException(
                status_code=400, detail=f"metadata.json not found in: {request.presentation_path}"
            )
        ...
        if request.download:
            pptx_content, total_slides, presentation_name = await converter.convert_to_pptx(
                store_locally=False
            )
        ...
        print(f"❌ Traceback:\n{error_traceback}")
        raise HTTPException(status_code=500, detail=f"PPTX conversion failed: {error_msg}")
