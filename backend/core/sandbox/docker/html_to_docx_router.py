...
try:
    ...
    raise ImportError(
        f"python-docx is not installed. Please install it with: pip install python-docx. Error: {e}"
    )
...
class ConvertRequest(BaseModel):
    ...
    download: bool = Field(
        False,
        description="If true, returns the DOCX file directly. If false, returns JSON with download URL.",
    )
...
class HTMLToDocxConverter:
    ...
    def load_document(self):
        ...
        try:
            ...
            self.doc_data = {"title": self.doc_path.stem, "content": content, "metadata": {}}
