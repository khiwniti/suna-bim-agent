...
class SandboxResolver:
    ...
    async def resolve(
        self, project_id: str, account_id: str, db_client=None, require_started: bool = True
    ) -> Optional[SandboxInfo]:
        lock = await self._get_project_lock(project_id)

        async with lock:
            return await self._resolve_internal(project_id, account_id, db_client, require_started)

    async def _resolve_internal(
        self, project_id: str, account_id: str, db_client, require_started: bool
    ) -> Optional[SandboxInfo]:
        ...
        if sandbox_id:
            try:
                ...
                logger.debug(
                    f"[RESOLVER] Cache hit for {project_id}: {sandbox_id} in {elapsed:.0f}ms"
                )
                return SandboxInfo(
        ...
        if db_client:
            ...
            if sandbox_info:
                elapsed = (time.time() - start_time) * 1000
                logger.debug(
                    f"[RESOLVER] DB hit for {project_id}: {sandbox_info.sandbox_id} in {elapsed:.0f}ms"
                )
                return sandbox_info
        ...
        if sandbox_info:
            elapsed = (time.time() - start_time) * 1000
            logger.info(
                f"[RESOLVER] New sandbox for {project_id}: {sandbox_info.sandbox_id} in {elapsed:.0f}ms"
            )

        return sandbox_info

    async def _resolve_from_db(
        self, project_id: str, account_id: str, db_client
    ) -> Optional[SandboxInfo]:
        try:
    ...
    async def _claim_or_create(
        ...
        if pool_config.enabled:
            ...
            logger.debug(
                f"[RESOLVER] Pool disabled (SANDBOX_POOL_ENABLED=false), creating new sandbox for {project_id}"
            )
...
async def resolve_sandbox(
    project_id: str, account_id: str, db_client=None, require_started: bool = True
) -> Optional[SandboxInfo]:
