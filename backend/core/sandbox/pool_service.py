...
class SandboxPoolService:
    ...
    async def create_pooled_sandbox(self) -> Optional[str]:
        ...
        if current_size >= self.config.max_size:
            logger.warning(
                f"[SANDBOX_POOL] Pool at max capacity ({current_size}/{self.config.max_size}), skipping creation"
            )
            return None
    ...
    async def claim_sandbox(
        ...
        try:
            ...
            logger.info(
                f"[SANDBOX_POOL] Claimed sandbox {sandbox_id} for project {project_id} in {claim_time_ms:.1f}ms"
            )
    ...
    async def ensure_pool_size(self) -> int:
        async with self._lock:
            ...
            if current_size >= self.config.replenish_below:
                logger.debug(
                    f"[SANDBOX_POOL] Pool size {current_size} >= threshold {self.config.replenish_below}, no replenishment needed"
                )
                return 0
            ...
            logger.info(
                f"[SANDBOX_POOL] Replenishing pool: {current_size} -> {current_size + to_create} (target: {target})"
            )
