...
def get_action_prompt(action: str, user_prompt: Optional[str] = None) -> str:
    ...
    prompts = {
        ...
        "edit_text": f"{user_prompt}"
        if user_prompt
        else "Edit the text in this image as requested.",
...
async def process_with_replicate_gpt(image_bytes: bytes, mime_type: str, prompt: str) -> str:
    ...
    logger.info(
        f"Calling Replicate openai/gpt-image-1.5 for editing with quality: low (image size: {len(image_bytes)} bytes)"
    )
...
async def process_with_gemini(
    image_bytes: bytes, mime_type: str, prompt: str, model_key: str = "gemini-flash"
) -> str:
    ...
    headers = {
        ...
        "HTTP-Referer": "https://carbon-bim.ai",
        "X-Title": "Carbon BIM Canvas AI",
    }
    ...
    async with get_http_client() as client:
        response = await client.post(
            f"{OPENROUTER_BASE_URL}/chat/completions", headers=headers, json=payload, timeout=120.0
        )
...
async def process_with_replicate_upscale(image_bytes: bytes, mime_type: str) -> str:
    ...
    try:
        # Wrap replicate.run() in thread pool to avoid blocking event loop
        output = await asyncio.to_thread(
            replicate.run, "recraft-ai/recraft-crisp-upscale", input={"image": image_data_url}
        )
...
async def process_image(
    request: ImageEditRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    """
    ...
    if not has_credits:
        ...
        return ImageEditResponse(success=False, error=f"Insufficient credits: {credit_msg}")
    ...
    try:
        ...
        if request.action not in valid_actions:
            return ImageEditResponse(
                success=False, error=f"Invalid action. Must be one of: {valid_actions}"
            )

        # Validate image
        if not request.image_base64:
            return ImageEditResponse(success=False, error="Image is required")
        ...
        try:
            ...
            return ImageEditResponse(success=False, error="Invalid base64 image data")
        ...
        if model_key == "replicate-remove-bg":
            ...
            result_base64 = await process_with_gemini(
                image_bytes, mime_type, prompt, "gemini-flash"
            )
        ...
        return ImageEditResponse(success=False, error=error_msg)


@router.post("/merge", response_model=ImageMergeResponse)
async def merge_images(
    request: ImageMergeRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    """
    ...
    """
    logger.info(
        f"Canvas AI: Merging {len(request.images)} images with GPT Image 1.5 for user {user_id}"
    )
    ...
    if not has_credits:
        logger.warning(f"[CANVAS_BILLING] Credit check failed for {user_id}: {credit_msg}")
        return ImageMergeResponse(success=False, error=f"Insufficient credits: {credit_msg}")

    try:
        if len(request.images) < 2:
            return ImageMergeResponse(
                success=False, error="At least 2 images are required for merging"
            )

        if not request.prompt.strip():
            return ImageMergeResponse(success=False, error="Merge prompt is required")
        ...
        if len(input_images) < 2:
            return ImageMergeResponse(success=False, error="Could not process images for merging")
...
The result should be a high-quality merged image."""
        ...
        logger.info(
            f"Calling Replicate openai/gpt-image-1.5 for merge with {len(input_images)} images"
        )
        ...
        return ImageMergeResponse(success=False, error=error_msg)


@router.post("/generate", response_model=ImageGenerateResponse)
async def generate_images(
    request: ImageGenerateRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    """
    ...
    if not has_credits:
        logger.warning(f"[CANVAS_BILLING] Credit check failed for {user_id}: {credit_msg}")
        return ImageGenerateResponse(success=False, error=f"Insufficient credits: {credit_msg}")

    try:
        if not request.prompt.strip():
            return ImageGenerateResponse(success=False, error="Prompt is required")
        ...
        if len(generated_images) == 0:
            return ImageGenerateResponse(success=False, error="Failed to generate any images")
        ...
        return ImageGenerateResponse(success=True, images=generated_images)
        ...
        return ImageGenerateResponse(success=False, error=error_msg)
...
async def _convert_with_recraft(image_data_url: str) -> Optional[str]:
    ...
    try:
        output = replicate.run("recraft-ai/recraft-vectorize", input={"image": image_data_url})
...
async def convert_to_svg(
    request: ConvertToSvgRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
    ...
    svg_str = _convert_with_vtracer(
        png_bytes, request.colormode or "color", request.mode or "spline"
    )
    if svg_str:
...
async def detect_text(
    request: OCRRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
) -> OCRResponse:
    """
    ...
    if not has_credits:
        logger.warning(f"[CANVAS_BILLING] Credit check failed for {user_id}: {credit_msg}")
        return OCRResponse(success=False, error=f"Insufficient credits: {credit_msg}")
    ...
    try:
        ...
        try:
            ...
            return OCRResponse(success=False, error=f"Invalid base64 image data: {str(e)}")
        ...
        try:
            ...
            return OCRResponse(success=False, error=f"OCR failed: {str(e)[:100]}")
