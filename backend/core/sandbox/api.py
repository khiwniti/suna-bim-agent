...
async def get_sandbox_by_id_safely(client, sandbox_id: str) -> AsyncSandbox:
    ...
    if not resource:
        ...
        raise HTTPException(
            status_code=404, detail="Sandbox not found - no resource exists for this sandbox ID"
        )
...
async def update_file_binary(
    ...
    logger.debug(
        f"Received binary file update request for sandbox {sandbox_id}, path: {path}, user_id: {user_id}"
    )
    client = await db.client
...
async def update_file(
    sandbox_id: str, request: Request = None, user_id: Optional[str] = Depends(get_optional_user_id)
):
    try:
        ...
        logger.debug(
            f"Received file update request for sandbox {sandbox_id}, path: {path}, user_id: {user_id}"
        )
        client = await db.client
...
async def list_files(
    ...
    try:
        ...
        logger.error(
            f"Error verifying sandbox access for {sandbox_id}: {str(access_err)}", exc_info=True
        )
        ...
        if "not found" in error_str or "404" in error_str or "no project owns" in error_str:
            ...
            raise HTTPException(
                status_code=401, detail="Authentication required for this private project"
            )
        elif "not authorized" in error_str or "forbidden" in error_str or "403" in error_str:
            raise HTTPException(
                status_code=403, detail=f"Access denied: Not authorized to access this sandbox"
            )
        else:
            # For other errors, return 500 but with a clear message
            raise HTTPException(
                status_code=500, detail=f"Error verifying sandbox access: {str(access_err)}"
            )
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Error retrieving sandbox {sandbox_id}: {str(sandbox_err)}", exc_info=True
            )
            raise HTTPException(
                status_code=500, detail=f"Failed to retrieve sandbox: {str(sandbox_err)}"
            )
        ...
        logger.error(
            f"Error listing files in sandbox {sandbox_id}, path {path}: {str(e)}", exc_info=True
        )
...
async def read_file(
    ...
    try:
        ...
        logger.error(
            f"Error reading file in sandbox {sandbox_id}, path {path}: {str(e)}", exc_info=True
        )
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
...
async def ensure_project_sandbox_active(
    project_id: str, request: Request = None, user_id: Optional[str] = Depends(get_optional_user_id)
):
    """
...
async def get_project_sandbox_details(
    ...
    """
    logger.debug(
        f"Received get sandbox details request for project {project_id}, user_id: {user_id}"
    )
    client = await db.client
    ...
    try:
        ...
        return {"status": "success", "sandbox": sandbox_details}
    except HTTPException:
        raise
    except Exception as e:
        ...
        if not sandbox_resource:
            ...
            async def create_sandbox_background():
                try:
                    ...
                    if sandbox_info:
                        logger.info(
                            f"Successfully created sandbox {sandbox_info.sandbox_id} for project {project_id}"
                        )
                    else:
...
async def get_sandbox_status_by_id(
    ...
    try:
        ...
        resource_result = (
            ...
            .select("*")
            .eq("external_id", sandbox_id)
            .eq("type", ResourceType.SANDBOX.value)
            .execute()
        )
...
async def file_upload_completed(
    project_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    from core.services import redis
...
async def get_file_upload_status(
    ...
    return {"uploading": pending is not None, "pending_files": int(pending) if pending else 0}
...
async def read_file_by_hash(
    ...
    try:
        ...
        try:
            await sandbox.commands.run(f"bash -lc {shlex.quote(git_cmd)}")
            ...
            raise HTTPException(
                status_code=404, detail=f"File not found at commit {commit}: {str(git_err)}"
            )
...
async def list_file_history(
    ...
    try:
        ...
        logger.debug(f"Found {len(versions)} versions for file {path} in sandbox {sandbox_id}")
        ...
        logger.error(f"Error listing file history in sandbox {sandbox_id}, path {path}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_commit_info(
    ...
    try:
        ...
        try:
            await sandbox.commands.run(f"bash -lc {shlex.quote(git_header_cmd)}")
            await sandbox.commands.run(f"bash -lc {shlex.quote(git_files_cmd)}")
            await sandbox.commands.run(f"bash -lc {shlex.quote(git_diff_cmd)}")
        except Exception as git_err:
            logger.error(
...
async def list_files_at_commit(
    ...
    try:
        ...
        git_path = (
            f"{shlex.quote(commit)}:{shlex.quote(rel_path)}" if rel_path else shlex.quote(commit)
        )
        git_cmd = f"cd /workspace && git ls-tree {git_path} > {shlex.quote(tmp_path)}"
    ...
    try:
        ...
        if not paths:
            ...
            commands = [
                ...
                'git checkout "$TARGET" -- .',
                "if [ -s /tmp/git_to_delete ]; then xargs -a /tmp/git_to_delete git rm -f --; fi",
                'if [ -n "$(git status --porcelain)" ]; then git commit -m "$snapshot_msg"; fi',
            ]

            full_cmd = " && ".join(commands)

            try:
                await sandbox.commands.run(f"bash -lc {shlex.quote(full_cmd)}")
            except Exception as e:
                logger.error(
                    f"Snapshot revert failed for commit {commit} in sandbox {sandbox_id}: {str(e)}"
                )
                raise HTTPException(status_code=400, detail=f"Snapshot revert failed: {str(e)}")
        ...
        for i, rel in enumerate(safe_paths):
            ...
            commands.append(
                ...
                "else "
                "if git ls-files --error-unmatch " + shlex.quote(rel) + " >/dev/null 2>&1; then "
                "git rm -f -- " + shlex.quote(rel) + "; "
                "fi; "
                "fi"
            )
        ...
        logger.info(
            f"Executing single-file revert for paths {safe_paths} to commit {commit} in sandbox {sandbox_id}"
        )
        ...
        try:
            await sandbox.commands.run(f"bash -lc {shlex.quote(full_cmd)}")
            logger.info(
                f"Successfully reverted files {safe_paths} to commit {commit} in sandbox {sandbox_id}"
            )
        except Exception as e:
            logger.error(
                f"Snapshot revert of files {safe_paths} to commit {commit} in sandbox {sandbox_id} failed: {str(e)}"
            )
            raise HTTPException(status_code=400, detail=f"Snapshot file revert failed: {str(e)}")
        ...
        logger.error(f"Error handling snapshot revert in sandbox {sandbox_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def execute_terminal_command(
    ...
    try:
        ...
        try:
            result = await sandbox.commands.run(f"bash -lc {shlex.quote(wrapped_command)}")
            ...
            return {"output": str(exec_err), "exit_code": 1, "success": False}
...
async def create_ssh_access_token(
    ...
    try:
        ...
        raise HTTPException(
            status_code=501, detail="SSH access is not supported with the E2B sandbox provider"
        )
...
async def websocket_pty_terminal(
    ...
    try:
            ...
            if message.get("type") == "websocket.receive":
                if "text" in message:
                    ...
                    await websocket.send_json(
                        {"type": "error", "message": "Unknown message format"}
                    )
        ...
        try:
            ...
            await websocket.send_json(
                {"type": "error", "message": f"Failed to create terminal: {str(e)}"}
            )
            await websocket.close()
            return
...
async def smart_rename_files(
    ...
    try:
        ...
        logger.info(
            f"Starting smart rename for sandbox {sandbox_id}, dry_run={request_body.dry_run}"
        )

        renames = await rename_ugly_files(
            sandbox=sandbox, workspace_path=request_body.path, dry_run=request_body.dry_run
        )
...
async def get_sandbox_pool_stats(user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    """
...
async def trigger_pool_replenish(user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    """
