...
async def search_threads_endpoint(
    ...
    try:
        ...
        if not service.is_configured:
            ...
            return {"results": [], "total": 0, "configured": False}
        ...
        return {"results": enriched, "total": len(enriched), "configured": True}
...
async def get_project(project_id: str, request: Request):
    ...
    try:
        ...
        if not is_public:
            if not user_id:
                raise HTTPException(
                    status_code=401, detail="Authentication required for private projects"
                )
...
@router.get(
    ...
    operation_id="list_project_threads",
)
...
@router.post(
    ...
    summary="Create Thread in Project",
    operation_id="create_thread_in_project",
)
    ...
    try:
        ...
        if config.ENV_MODE != EnvMode.LOCAL:
            ...
            if not thread_check.allowed:
                ...
                logger.warning(
                    f"Thread limit exceeded for account {account_id}: {thread_check.current_count}/{thread_check.limit}"
                )
        ...
        thread_result = await repo_create_thread(
            thread_id=thread_id, project_id=project_id, account_id=account_id, name="New Chat"
        )
        ...
        logger.error(
            f"Error creating thread in project {project_id}: {str(e)}\n{traceback.format_exc()}"
        )
        raise HTTPException(status_code=500, detail=f"Failed to create thread: {str(e)}")


@router.delete("/projects/{project_id}", summary="Delete Project", operation_id="delete_project")
async def delete_project(project_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    try:
        ...
        threads_result = (
            await client.table("threads").select("thread_id").eq("project_id", project_id).execute()
        )
        ...
        return {
            ...
            "project_id": project_id,
            "threads_deleted": threads_deleted_count,
        }
    ...
    try:
        ...
        if thread.get("project_id"):
            ...
            if sandbox_info and sandbox_info.get("id"):
                ...
                logger.info(
                    f"Thread {thread_id} has existing sandbox {sandbox_id}, starting it in background..."
                )
                ...
                async def start_sandbox_background():
                    try:
                        ...
                        logger.info(
                            f"Successfully started sandbox {sandbox_id} for thread {thread_id}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Failed to start sandbox {sandbox_id} for thread {thread_id}: {str(e)}"
                        )
...
async def create_thread(
    name: Optional[str] = Form(None), user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    if not name:
        name = "New Project"
    ...
    try:
        if config.ENV_MODE != EnvMode.LOCAL:
            ...
            if not project_check.allowed:
                ...
                logger.warning(
                    f"Project limit exceeded for account {account_id}: {project_check.current_count}/{project_check.limit}"
                )
                raise HTTPException(status_code=402, detail=error_detail)
...
async def get_thread_messages(
    ...
    optimized: bool = Query(
        True,
        description="Return optimized messages (filtered types, minimal fields) or full messages (all types, all fields)",
    ),
):
    from core.threads import repo as threads_repo
    from core.cache.runtime_cache import get_pending_thread
    ...
    try:
        ...
        raw_messages = await threads_repo.get_thread_messages(
            thread_id=thread_id, order=order, optimized=optimized
        )
        ...
        migration_needed = any(
            needs_migration(msg) for msg in raw_messages if msg.get("type") in ["assistant", "tool"]
        )
...
async def add_message_to_thread(
    ...
    try:
        ...
        if thread_name in ("New Chat", None):
            ...
            asyncio.create_task(
                generate_and_update_thread_name(thread_id=thread_id, prompt=message)
            )
...
async def create_message_endpoint(
    ...
    if message_data.type == "user" and (
        not message_data.content or not message_data.content.strip()
    ):
        raise HTTPException(status_code=400, detail="Message content cannot be empty")
...
async def delete_message_endpoint(
    thread_id: str, message_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    from core.threads import repo as threads_repo
...
async def update_thread(
    ...
    try:
        ...
        updated_thread = await threads_repo.update_thread(
            thread_id=thread_id, metadata=thread_metadata, is_public=is_public
        )
...
async def delete_thread(
    thread_id: str, auth: AuthorizedThreadAccess = Depends(require_thread_write_access)
):
    ...
    try:
        ...
        if project_id:
            ...
            if remaining_thread_count == 0:
                ...
                logger.debug(
                    f"Project {project_id} has {remaining_thread_count} remaining threads, keeping project"
                )
