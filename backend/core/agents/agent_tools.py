...
@router.get(
    "/agents/{agent_id}/custom-mcp-tools",
    summary="Get Agent Custom MCP Tools",
    operation_id="get_agent_custom_mcp_tools",
)
async def get_custom_mcp_tools_for_agent(
    agent_id: str, request: Request, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    ...
    try:
        ...
        if agent.get("current_version_id"):
            version_result = await versioning_repo.get_agent_version_by_id(
...
@router.post(
    "/agents/{agent_id}/custom-mcp-tools",
    summary="Update Agent Custom MCP Tools",
    operation_id="update_agent_custom_mcp_tools",
)
async def update_custom_mcp_tools_for_agent(
    agent_id: str, request: dict, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    if not config.ACTIVATE_MCPS_TRIG:
        raise HTTPException(status_code=403, detail="MCP integrations are not enabled")
    ...
    try:
        ...
        if not updated:
            if config.ENV_MODE != EnvMode.LOCAL:
                ...
                if not limit_check["can_create"]:
                    ...
                    logger.warning(
                        f"Custom worker limit exceeded for account {user_id}: {limit_check['current_count']}/{limit_check['limit']}"
                    )
        ...
        return {"success": True, "enabled_tools": enabled_tools, "total_tools": len(enabled_tools)}
...
@router.put(
    "/agents/{agent_id}/custom-mcp-tools",
    summary="Update Agent Custom MCPs",
    operation_id="update_agent_custom_mcps",
)
        ...
        if config.ENV_MODE != EnvMode.LOCAL:
            ...
            if new_count > existing_count:
                ...
                if total_after_adding > limit_check["limit"]:
                    ...
                    logger.warning(
                        f"Custom worker limit would be exceeded for account {user_id}: {limit_check['current_count']} + {additional_workers_needed} > {limit_check['limit']}"
                    )
                    raise HTTPException(status_code=402, detail=error_detail)
            ...
            if mcp_type == "composio":
                ...
                for i, existing_mcp in enumerate(existing_custom_mcps):
                    if (
                        ...
                        and existing_mcp.get("config", {}).get("profile_id") == profile_id
                    ):
...
async def get_agent_tools(agent_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    for name, enabled in agentpress_tools_config.items():
        is_enabled_tool = (
            bool(enabled.get("enabled", False)) if isinstance(enabled, dict) else bool(enabled)
        )
