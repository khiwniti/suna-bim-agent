...
class TimeEstimator:
    ...
    CONFIDENCE_THRESHOLDS = {"high": 0.2, "medium": 0.4, "low": 0.6}
    ...
    def estimate(
        ...
        if enabled_tools:
            avg_tool_time = sum(
                self.TOOL_TIME_ESTIMATES.get(t, 1.5) for t in enabled_tools[:5]
        ...
        return EstimateResult(
            estimated_seconds=breakdown.total, confidence=confidence, breakdown=breakdown
        )

    def record_actual(
        self, model_name: str, actual_seconds: float, was_continuation: bool = False
    ) -> None:
    ...
    def get_historical_average(
        self, model_name: str, is_continuation: bool = False
    ) -> Optional[float]:
    ...
    def _calculate_confidence(self, model_key: str, is_continuation: bool) -> str:
