...
class TaskRegistry:
    ...
    async def register(
        self, run_id: str, task: asyncio.Task, name: str, critical: bool = False
    ) -> None:
        async with self._lock:
            ...
            self._runs[run_id][task_id] = TaskInfo(task=task, name=name, critical=critical)

            task.add_done_callback(
                lambda t, rid=run_id, tid=task_id: asyncio.create_task(self._on_task_done(rid, tid))
            )
    ...
    async def cancel_all(self, run_id: str, reason: str = "cleanup") -> int:
        ...
        for task_info in tasks_to_cancel:
            if not task_info.task.done():
                ...
                cancelled_count += 1
                try:
                    await asyncio.wait_for(asyncio.shield(task_info.task), timeout=5.0)
    ...
    async def get_active_count(self, run_id: str) -> int:
        async with self._lock:
            if run_id not in self._runs:
                return 0
            return sum(1 for info in self._runs[run_id].values() if not info.task.done())
