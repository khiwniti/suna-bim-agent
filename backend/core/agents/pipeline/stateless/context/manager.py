...
class ContextManager:
    ...
    def extract_layers(messages: List[Dict[str, Any]]) -> ContextLayers:
        ...
        return ContextLayers(
            facts_message=None, summary_message=None, working_memory=working_memory
        )
...
*(Compressed summary of {compressed_count} messages)*"""
    ...
    @staticmethod
    def should_compress(
        messages: List[Dict[str, Any]], working_memory_size: int = DEFAULT_WORKING_MEMORY_SIZE
    ) -> bool:
    ...
    async def compress_history(
        ...
        if len(messages) <= working_memory_size:
            raise ValueError(
                f"Not enough messages to compress (need >{working_memory_size}, got {len(messages)})"
            )
        ...
        result = await ContextManager._summarize_with_facts(
            old_messages=to_compress, recent_context=recent_context, model=model
        )
