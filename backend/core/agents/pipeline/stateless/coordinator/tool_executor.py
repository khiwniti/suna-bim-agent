...
class ToolExecutor:
    ...
    def start_tool_execution(
        self, tool_call: Dict[str, Any], tool_index: int, assistant_message_id: Optional[str] = None
        ...
        task = asyncio.create_task(self._execute_single_tool(name, args, available_functions))
    ...
    async def process_completed_execution(
        self, execution: PendingToolExecution, stream_start: str, defer_message: bool = False
        ...
        logger.debug(
            f"[ToolExecutor] Tool {execution.function_name} completed in {exec_time:.1f}ms, success={success}"
        )
        ...
        logger.debug(
            f"[ToolExecutor] Yielding tool result: {execution.tool_call_id}, message_id={tool_result_message_id}"
        )
        ...
        if is_terminating and success:
            async for msg in self._handle_terminating_tool(
                execution.tool_call_id, execution.function_name
            ):
                yield msg

    def yield_tool_started(
        self, tool_call: Dict[str, Any], tool_index: int, stream_start: str
    ) -> Dict[str, Any]:
        ...
        return self._message_builder.build_tool_started(
            tc_id, display_name, tool_index, stream_start, args
        )

    async def execute_tools(
        self, stream_start: str, assistant_message_id: Optional[str] = None
    ) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        logger.debug(
            f"[ToolExecutor] Executing {len(pending)} tools sequentially, assistant_message_id={assistant_message_id}"
        )
        ...
        for tool_index, tc in enumerate(pending):
            ...
            yield self._message_builder.build_tool_started(
                tc_id, display_name, tool_index, stream_start, args
            )

            start = time.time()
            output, success, error = await self._execute_single_tool(
                name, args, available_functions
            )
            exec_time = (time.time() - start) * 1000

            logger.debug(
                f"[ToolExecutor] Tool {name} completed in {exec_time:.1f}ms, success={success}"
            )
            ...
            tool_result_message_id = tool_result_msg.get("message_id")
            logger.debug(
                f"[ToolExecutor] Yielding tool result: {tc_id}, message_id={tool_result_message_id}"
            )
            yield tool_result_msg
    ...
    async def _execute_single_tool(
        self, name: str, args: str, available_functions: Dict
        ...
        try:
            ...
            if name == "create_slide":
                ...
                logger.info(
                    f"[ToolExecutor] DEBUG: tool_fn={tool_fn}, module={getattr(tool_fn, '__module__', 'unknown')}, qualname={getattr(tool_fn, '__qualname__', 'unknown')}"
                )
            ...
            if tool_fn:
                ...
                if name == "create_slide":
                    logger.info(
                        f"[ToolExecutor] create_slide returned: success={getattr(result, 'success', 'N/A')}, output={str(getattr(result, 'output', 'N/A'))[:200]}"
                    )
                if hasattr(result, "success") and hasattr(result, "output"):
                    ...
                    if not success:
                        logger.error(
                            f"[ToolExecutor] Tool {name} returned success=False, output: {str(output)[:500]}"
                        )
                else:
                    success = True
                    output = result
    ...
    async def _handle_deferred_image_context(
        self, output: Any, stream_start: str
    ) -> AsyncGenerator[Dict[str, Any], None]:
        try:
            ...
            self._state.add_message(message_content, {"type": "image_context", **metadata})
            ...
            try:
                ...
                thread_id = (
                    image_context_data.get("thread_id") or self._message_builder._get_thread_id()
                )
                ...
                logger.warning(
                    f"[DeferredImageContext] Failed to set has_images flag: {flag_error}"
                )
            ...
            logger.error(
                f"[DeferredImageContext] Error processing image context: {e}", exc_info=True
            )

    async def _handle_terminating_tool(
        self, tc_id: str, name: str
    ) -> AsyncGenerator[Dict[str, Any], None]:
