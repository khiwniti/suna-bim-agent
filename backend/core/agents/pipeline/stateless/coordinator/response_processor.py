...
class ResponseProcessor:
    ...
    async def process_response(self, response) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        async for chunk in response:
            if isinstance(chunk, dict):
                if chunk.get("__llm_ttft_seconds__"):
                    yield self._message_builder.build_llm_ttft(
                        chunk["__llm_ttft_seconds__"], self._state.model_name, self._state.thread_id
                    )
                    continue
            ...
            if hasattr(chunk, "usage") and chunk.usage and final_llm_response is None:
                ...
                logger.debug(
                    f"📊 [ResponseProcessor] Captured usage: prompt={getattr(chunk.usage, 'prompt_tokens', 0)}, completion={getattr(chunk.usage, 'completion_tokens', 0)}"
                )
                ...
                if isinstance(delta.content, list):
                    for block in delta.content:
                        ...
                        if block_type == "thinking" and block_text:
                            ...
                            yield self._message_builder.build_reasoning_chunk(
                                block_text, stream_start
                            )
                        elif block_type == "text" and block_text:
                            self._state.append_content(block_text)
                            yield self._message_builder.build_content_chunk(
                                block_text, stream_start
                            )
                        elif block_text:
                    ...
                    for idx in sorted(tool_call_buffer.keys()):
                        ...
                        if is_tool_call_complete(tool_call_buffer[idx]):
                            ...
                            if assistant_message_id is None:
                                ...
                                logger.debug(
                                    f"[ResponseProcessor] Reserved assistant message ID: {assistant_message_id}"
                                )
                            ...
                            execution = self._tool_executor.start_tool_execution(
                                tool_call_data, tool_index_counter, assistant_message_id
                            )
                            pending_executions.append(execution)
                            tool_index_counter += 1

                            logger.debug(
                                f"[ResponseProcessor] Started async execution for {tc_buf.get('function', {}).get('name')}"
                            )
            ...
            if execute_on_stream and pending_executions:
                async for msg in self._process_completed_executions(
                    ...
                    yield msg
                    if msg.get("type") == "status" and _parse_metadata(msg).get(
            ...
            if finish_reason and not finish_processed:
                ...
                if execute_on_stream and has_tool_calls:
                    ...
                    if assistant_message_id:
                        ...
                        logger.debug(
                            f"[ResponseProcessor] Finalized assistant message with {len(complete_tool_calls)} tool calls: {finalized_id}, committed {committed_count} tool results"
                        )
                    ...
                    yield self._message_builder.build_finish_message(
                        finish_reason, tools_executed=True
                    )
                else:
        ...
        if (
            self._state._accumulated_content
            and not self._state._terminated
            ...
            assistant_message_id = self._state.finalize_assistant_message(
                tool_calls if tool_calls else None, self._message_builder._get_thread_run_id()
            )
    ...
    async def _wait_and_process_remaining_executions(
        ...
        for idx in sorted(tool_call_buffer.keys()):
            ...
            if is_tool_call_complete(tool_call_buffer[idx]):
                ...
                if assistant_message_id is None:
                    assistant_message_id = self._state.reserve_assistant_message_id()
                    logger.debug(
                        f"[ResponseProcessor] Reserved assistant message ID in remaining: {assistant_message_id}"
                    )
        ...
        if pending_executions:
            logger.debug(
                f"[ResponseProcessor] Waiting for {len(pending_executions)} remaining tool executions"
            )
            ...
            if pending_tasks:
                ...
                for task in done:
                    if task.exception():
                        exc = task.exception()
                        logger.error(
                            f"[ResponseProcessor] Tool execution task failed: {exc}", exc_info=exc
                        )
    ...
    def _build_complete_tool_calls(
    ...
    def _extract_usage_data(self, final_llm_response, llm_response_id: str) -> Dict[str, Any]:
        ...
        if final_llm_response and hasattr(final_llm_response, "usage") and final_llm_response.usage:
            ...
            if hasattr(usage, "cache_read_input_tokens"):
                response_data["usage"]["cache_read_input_tokens"] = getattr(
                    usage, "cache_read_input_tokens", 0
                )
            if hasattr(usage, "cache_creation_input_tokens"):
                response_data["usage"]["cache_creation_input_tokens"] = getattr(
    ...
    async def _handle_finish_reason(
        ...
        if finish_reason == "tool_calls":
            async for msg in self._handle_tool_calls_finish(
                tool_calls, tool_call_buffer, stream_start, llm_response_id, final_llm_response
            ):
                yield msg
        elif finish_reason in ("stop", "end_turn"):
            async for msg in self._handle_stop_finish(
                tool_calls, stream_start, llm_response_id, final_llm_response
            ):
                yield msg
        elif finish_reason == "length":
            # Handle max tokens reached - save content and emit finish message for auto-continue
            async for msg in self._handle_length_finish(
                tool_calls, stream_start, llm_response_id, final_llm_response
            ):
                yield msg
    ...
    async def _handle_tool_calls_finish(
        ...
        accumulated_content = self._state._accumulated_content or ""
        accumulated_reasoning = self._state._accumulated_reasoning or ""
        assistant_message_id = self._state.finalize_assistant_message(tool_calls, thread_run_id)
    ...
    async def _handle_stop_finish(
        self, tool_calls: list, stream_start: str, llm_response_id: str, final_llm_response=None
    ) -> AsyncGenerator[Dict[str, Any], None]:
        thread_run_id = self._message_builder._get_thread_run_id()
        ...
        accumulated_reasoning = self._state._accumulated_reasoning or ""
        assistant_message_id = self._state.finalize_assistant_message(
            tool_calls if tool_calls else None, thread_run_id
        )
        ...
        self._state.add_status_message(
            {"status_type": "finish", "finish_reason": "stop"}, {"thread_run_id": thread_run_id}
        )
    ...
    async def _handle_length_finish(
        ...
        self._state.add_status_message(
            {"status_type": "finish", "finish_reason": "length"}, {"thread_run_id": thread_run_id}
        )
