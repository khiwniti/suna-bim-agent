...
class LimitEnforcer:
    ...
    async def check_all(
        ...
        if config.ENV_MODE == EnvMode.LOCAL:
            return AllLimitsResult(can_run=True, message="Local mode - all limits bypassed")
        ...
        try:
            results = await asyncio.gather(
                check_concurrent(), check_credits(), check_model_access(), return_exceptions=True
            )
        ...
        return AllLimitsResult(can_run=True, message="All limits passed", check_time_ms=elapsed_ms)
