...
async def prep_limits(account_id: str, skip_check: bool = False) -> LimitsResult:
    ...
    try:
        ...
        return LimitsResult(can_run=True, message=f"Limits check failed (allowing): {str(e)[:100]}")


async def prep_messages(
    thread_id: str, prefetch_task: Optional[asyncio.Task] = None
) -> MessagesResult:
    start = time.time()
    from_cache = False
    ...
    try:
        ...
        elapsed_ms = (time.time() - start) * 1000
        logger.debug(
            f"⏱️ [PREP] Messages fetch: {elapsed_ms:.1f}ms ({len(messages)} messages, cache={from_cache})"
        )

        return MessagesResult(
            messages=messages, count=len(messages), from_cache=from_cache, fetch_time_ms=elapsed_ms
        )
    except Exception as e:
        logger.error(f"Message fetch failed: {e}")
        return MessagesResult(
            messages=[], count=0, from_cache=False, fetch_time_ms=(time.time() - start) * 1000
        )
...
async def prep_prompt(
    ...
    try:
        ...
        return PromptResult(
            system_prompt=system_prompt, memory_context=memory_context, build_time_ms=elapsed_ms
        )
    except Exception as e:
...
async def prep_tools(tool_registry) -> ToolsResult:
    ...
    try:
        ...
        return ToolsResult(schemas=schemas, count=count, fetch_time_ms=elapsed_ms)
    except Exception as e:
...
async def prep_mcp(
    agent_config: Optional[Dict[str, Any]], account_id: str, thread_manager
) -> MCPResult:
    if not agent_config:
    ...
    try:
        ...
        return MCPResult(initialized=True, tool_count=tool_count, init_time_ms=elapsed_ms)
    except Exception as e:
