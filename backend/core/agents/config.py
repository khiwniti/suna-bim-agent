...
async def load_agent_config_fast(
    agent_id: Optional[str], account_id: Optional[str], user_id: Optional[str] = None
    ...
    try:
        ...
        if agent_type == "suna":
            ...
            if static_config:
                ...
                logger.info(
                    f"⏱️ [AGENT CONFIG FAST] Suna (cached type): {(time.time() - t) * 1000:.1f}ms"
                )
                return agent_config
            ...
            if cached_config:
                logger.info(
                    f"⏱️ [AGENT CONFIG FAST] Custom (cached): {(time.time() - t) * 1000:.1f}ms"
                )
                return cached_config

        # Cache miss - fall back to full DB load (will also populate cache)
        logger.info(
            f"⏱️ [AGENT CONFIG FAST] Cache miss (type={agent_type}), falling back to full load"
        )
    ...
    try:
        if not agent_id:
            ...
            if default_agent_id:
                ...
                logger.warning(
                    f"[AGENT LOAD] No default agent found for account {account_id}, searching for shared Suna"
                )
                ...
                if not agent_data:
                    ...
                    if any_agent_id:
                        agent_data = await loader.load_agent(
                            any_agent_id, user_id, load_config=True
                        )
                        logger.info(
                            f"[AGENT LOAD] Using fallback agent: {agent_data.name} ({agent_data.agent_id})"
                        )
                        return agent_data.to_dict()
                    else:
        ...
        if cached_config:
            ...
            logger.info(
                f"⏱️ [AGENT CONFIG] get_cached_agent_config: {(time.time() - t_cache) * 1000:.1f}ms (CACHE HIT)"
            )
        elif account_id:
            ...
            logger.info(
                f"⏱️ [AGENT CONFIG] DB load: {(time.time() - t_db) * 1000:.1f}ms (CACHE MISS)"
            )
        else:
            t_db = time.time()
            from core.agents.agent_loader import get_agent_loader
            ...
            agent_config = agent_data.to_dict()
            logger.info(
                f"⏱️ [AGENT CONFIG] DB load (no account): {(time.time() - t_db) * 1000:.1f}ms"
            )
...
async def _find_shared_suna_agent():
    ...
    if shared_agent:
        loader = await get_agent_loader()
        agent_data = await loader.load_agent(
            shared_agent["agent_id"], shared_agent["account_id"], load_config=True
        )
        if admin_user_id and shared_agent["account_id"] == admin_user_id:
            logger.info(
                f"✅ Using system Suna agent from admin user: {agent_data.name} ({agent_data.agent_id})"
            )
        else:
    ...
    if admin_user_id:
        logger.warning(
            f"⚠️ SYSTEM_ADMIN_USER_ID configured but no Suna agent found for user {admin_user_id}"
        )
