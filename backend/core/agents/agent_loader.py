...
class AgentLoader:
    ...
    async def load_agent(
        ...
        if load_config and not skip_cache:
            ...
            if cached:
                logger.debug(
                    f"⚡ Using cached config for agent {agent_id} ({(time.time() - t_start) * 1000:.1f}ms)"
                )
    ...
    async def _load_suna_config(self, agent: AgentData, user_id: Optional[str] = None):
        ...
        if agent.current_version_id and user_id:
            ...
            if cached_mcps:
                ...
                logger.debug(
                    f"⚡ Suna config loaded in {(time.time() - t_start) * 1000:.1f}ms (MCPs from cache)"
                )
                return
            ...
            try:
                ...
                version = await version_service.get_version(
                    agent_id=agent.agent_id, version_id=agent.current_version_id, user_id=user_id
                )
                ...
                await set_cached_user_mcps(
                    agent.agent_id, agent.configured_mcps, agent.custom_mcps, agent.triggers
                )

                logger.debug(
                    f"Suna config loaded in {(time.time() - t_start) * 1000:.1f}ms (MCPs from DB, now cached)"
                )
            except Exception as e:
            ...
            agent.triggers = []
            logger.debug(
                f"⚡ Suna config loaded in {(time.time() - t_start) * 1000:.1f}ms (no MCPs)"
            )
    ...
    def _load_fallback_config(self, agent: AgentData):
        ...
        from core.config.config_helper import (
            _get_default_agentpress_tools,
            _extract_agentpress_tools_for_run,
        )
            ...
            for agent in agents:
                if agent.current_version_id and not agent.is_suna_default:
                    try:
                        ...
                        logger.warning(
                            f"Failed to load version {agent.current_version_id} for agent {agent.agent_id}: {e}"
                        )
                        continue
