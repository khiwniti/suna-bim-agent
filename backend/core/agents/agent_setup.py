...
async def generate_agent_config_from_description(description: str) -> dict:
    ...
    try:
        ...
        name_prompt_result, icon_result = await asyncio.gather(
            name_prompt_task, icon_task, return_exceptions=True
        )
        ...
        result = {
            ...
            "system_prompt": name_prompt_result.get(
                "system_prompt", f"Act as a helpful AI assistant. {description}"
            ),
...
@router.post(
    ...
    response_model=AgentSetupFromChatResponse,
    summary="Setup Agent from Chat Description",
    operation_id="setup_agent_from_chat",
)
async def setup_agent_from_chat(
    request: AgentSetupFromChatRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    ...
    try:
        ...
        await agents_repo.update_agent_fields(
            agent_id, user_id, current_version_id=version.version_id, version_count=1
        )
        ...
        logger.info(
            f"Successfully created agent '{agent_name}' (ID: {agent_id}) from chat description"
        )
