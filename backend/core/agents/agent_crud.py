...
async def update_agent(
    ...
    try:
        ...
        if current_version_data is None:
            ...
            try:
                ...
                await agents_repo.update_agent_fields(
                    agent_id, user_id, current_version_id=version_id, version_count=1
                )
        ...
        if values_different(
            agent_data.agentpress_tools, current_version_data.get("agentpress_tools", {})
        ):
            needs_new_version = True
        ...
        current_model = (
            agent_data.model if agent_data.model is not None else current_version_data.get("model")
        )
        ...
        if agent_data.configured_mcps is not None:
            if agent_data.replace_mcps:
                ...
                logger.debug(
                    f"Replacing configured MCPs for agent {agent_id}: {current_configured_mcps}"
                )
            else:
                current_configured_mcps = agent_data.configured_mcps
        else:
        ...
        if update_data:
            try:
                print(
                    f"[DEBUG] update_agent DB UPDATE: About to update agent {agent_id} with data: {update_data}"
                )

                success = await agents_repo.update_agent(agent_id, user_id, update_data)
        ...
        if agent.get("current_version_id"):
            try:
                version_service = await _get_version_service()
                current_version_obj = await version_service.get_version(
                    agent_id=agent_id, version_id=agent["current_version_id"], user_id=user_id
                )
                current_version_data = current_version_obj.to_dict()
...
async def delete_agent(agent_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Agent config cache invalidation failed for {agent_id}: {str(cache_error)}"
            )
...
@router.get(
    "/agents", response_model=AgentsResponse, summary="List Agents", operation_id="list_agents"
)
async def get_agents(
    ...
    content_type: Optional[str] = Query(
        None, description="Content type filter: 'agents', 'templates', or None for agents only"
    ),
):
    try:
...
@router.post(
    "/agents", response_model=AgentResponse, summary="Create Agent", operation_id="create_agent"
)
async def create_agent(
    agent_data: AgentCreateRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    from core.agents import repo as agents_repo
    ...
    try:
        ...
        if config.ENV_MODE == EnvMode.STAGING:
            print(
                f"[DEBUG] create_agent: Creating with icon_name={agent_data.icon_name or 'bot'}, icon_color={agent_data.icon_color or '#000000'}, icon_background={agent_data.icon_background or '#F3F4F6'}"
            )
        ...
        try:
            ...
            agentpress_tools = (
                agent_data.agentpress_tools
                if agent_data.agentpress_tools
                else _get_default_agentpress_tools()
            )
...
async def generate_agent_icon(
    request: AgentIconGenerationRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
    ...
    try:
        from core.utils.icon_generator import (
            generate_icon_and_colors as generate_agent_icon_and_colors,
        )

        result = await generate_agent_icon_and_colors(name=request.name)
