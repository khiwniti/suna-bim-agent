...
from core.templates.template_service import (
    MCPRequirementValue,
    ConfigType,
    ProfileId,
    QualifiedName,
...
class JsonImportService:
    ...
    async def analyze_json(
        self, json_data: Dict[str, Any], account_id: str
    ) -> JsonAnalysisResponse:
        ...
        missing_profiles, missing_configs = await self._validate_requirements(
            mcp_requirements, account_id, profile_mappings=None, custom_configs=None
        )
    ...
    async def import_json(
        self, request: JsonImportRequestModel, account_id: str
    ) -> JsonImportResponse:
        ...
        missing_profiles, missing_configs = await self._validate_requirements(
            mcp_requirements, account_id, request.profile_mappings, request.custom_mcp_configs
        )
        ...
        agent_config = await self._build_agent_config_from_json(
            json_data, request, account_id, mcp_requirements
        )

        agent_id = await self._create_agent_from_json(json_data, request, account_id, agent_config)
    ...
    def _extract_mcp_requirements_from_json(
    ...
    async def _validate_requirements(
        ...
        return await installation_service._validate_installation_requirements(
            requirements, profile_mappings, custom_configs
        )
    ...
    async def _build_agent_config_from_json(
        ...
        for req in requirements:
            if req.custom_type == "composio":
                profile_id = (
                    request.profile_mappings.get(req.qualified_name)
                    if request.profile_mappings
                    else None
                )
                if profile_id:
                    from core.composio_integration.composio_profile_service import (
                        ComposioProfileService,
                    )

                    composio_service = ComposioProfileService(self._db)
                    mcp_config = await composio_service.get_mcp_config_for_agent(
                ...
                custom_config = (
                    request.custom_mcp_configs.get(req.qualified_name)
                    if request.custom_mcp_configs
                    else None
                )
                if custom_config:
                    mcp_config = {
    ...
    async def _create_initial_version(
        self, agent_id: str, account_id: str, agent_config: Dict[str, Any], system_prompt: str
    ) -> None:
        try:
            logger.debug(
            ...
            logger.info(
                f"Successfully created initial version {version.version_id} for JSON imported agent {agent_id}"
            )
            ...
            if agent_check and agent_check.get("current_version_id"):
                logger.debug(
                    f"Agent {agent_id} current_version_id updated to: {agent_check['current_version_id']}"
                )
            else:
                logger.error(
            ...
            logger.error(
                f"Failed to create initial version for JSON imported agent {agent_id}: {e}",
                exc_info=True,
            )
...
@router.get(
    "/agents/{agent_id}/export", summary="Export Agent as JSON", operation_id="export_agent_json"
)
async def export_agent(agent_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    try:
        ...
        if agent.get("current_version_id"):
            current_version = await versioning_repo.get_agent_version_by_id(
                agent_id, agent["current_version_id"]
            )
...
@router.post(
    "/agents/json/analyze",
    response_model=JsonAnalysisResponse,
    summary="Analyze Agent JSON",
    operation_id="analyze_agent_json",
)
async def analyze_json_for_import(
    request: JsonAnalysisRequest, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
...
async def import_agent_from_json(
    request: JsonImportRequestModel, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
