...
async def _get_agent_run_with_access_check(
    agent_run_id: str, user_id: str, require_write_access: bool = False
):
    ...
    await verify_and_authorize_thread_access(
        client, thread_id, user_id, require_write_access=require_write_access
    )
    return agent_run_data
...
async def _check_billing_and_limits(
    ...
    async def check_billing():
        ...
        if not is_model_allowed(tier_name, model_name):
            ...
            return (
                False,
                f"Your current subscription plan does not include access to {model_name}. Please upgrade your subscription.",
                {
        ...
        # Check credits
        can_run, message, reservation_id = await billing_integration.check_and_reserve_credits(
            account_id
        )
        if not can_run:
            return (
                False,
                f"Billing check failed: {message}",
                {"tier_info": tier_info, "error_type": "insufficient_credits"},
            )
    ...
    billing_result, limits_result = await asyncio.gather(check_billing(), check_all_limits())
    ...
    if not can_proceed:
        ...
        if error_type == "model_access_denied":
            ...
            raise HTTPException(
                status_code=402,
                detail={"message": error_message, "error_code": "INSUFFICIENT_CREDITS"},
            )
        else:
...
async def _get_effective_model(
    model_name: Optional[str], agent_config: Optional[dict], client, account_id: str
) -> str:
    if model_name:
        return model_name
...
async def start_agent_run(
    ...
    if model_name:
        ...
        effective_model = "carbon-bim/basic"
    ...
    if is_new_thread and not skip_limits_check:
        ...
        if not thread_check.allowed:
            logger.warning(
                f"⚠️ Thread limit exceeded for {account_id}: {thread_check.current_count}/{thread_check.limit}"
            )
            raise HTTPException(
                status_code=402,
                detail={
        ...
        if not project_check.allowed:
            logger.warning(
                f"⚠️ Project limit exceeded for {account_id}: {project_check.current_count}/{project_check.limit}"
            )
            raise HTTPException(
                status_code=402,
                detail={
    ...
    if has_files:
        ...
        if is_new_thread:
            placeholder_name = (
                f"{prompt[:30]}..." if len(prompt) > 30 else prompt if prompt else "Untitled"
            )
            try:
        ...
        if sandbox_id:
            logger.info(
                f"✅ [AGENT_START] Resolved sandbox {sandbox_id} for file uploads (project {project_id})"
            )
            ...
            logger.info(
                f"✅ [AGENT_START] Uploaded {len(files_data)} files to sandbox {sandbox_id}"
            )
        else:
            logger.warning(
                f"⚠️ [AGENT_START] Failed to resolve sandbox for file uploads (project {project_id})"
            )
        ...
        async def create_sandbox_background(proj_id: str, acc_id: str, db_client):
            try:
                ...
                sandbox_info = await resolve_sandbox(
                    proj_id, acc_id, db_client, require_started=True
                )
                if sandbox_info:
                    logger.info(
                        f"✅ [BACKGROUND] Created sandbox {sandbox_info.sandbox_id} for project {proj_id}"
                    )
                else:
        ...
        logger.info(
            f"🚀 [AGENT_START] Started background sandbox creation for new project {project_id}"
        )

    slot_reservation = await reserve_slot(
        account_id=account_id, agent_run_id=agent_run_id, skip=skip_limits_check
    )
    ...
    try:
        ...
        estimate = time_estimator.estimate(
            model_name=effective_model, has_mcp=has_mcp, is_continuation=False
        )
        asyncio.create_task(
            stream_estimate(
                stream_key,
    ...
    logger.info(
        f"⚡ [FAST RESPONSE] Returning in {setup_time_ms}ms (thread={thread_id}, run={agent_run_id})"
    )

    logger.info(
        f"[START_AGENT] Creating background task for agent_run_id={agent_run_id}, thread_id={thread_id}"
    )
    asyncio.create_task(
...
async def _background_setup_and_execute(
    ...
    structlog.contextvars.bind_contextvars(
        agent_run_id=agent_run_id, thread_id=thread_id, project_id=project_id, account_id=account_id
    )
    ...
    try:
        ...
        async def do_db_writes():
            ...
            try:
                if is_new_thread:
                    ...
                    if cache_updated:
                        await write_user_message_for_existing_thread(
                            thread_id, final_message_content
                        )
        ...
        run_timeout_seconds = int(
            os.getenv("AGENT_RUN_TIMEOUT_SECONDS", "3000")
        )  # 65min, > MAX_DURATION so cleanup always runs
        ...
        try:
            ...
            logger.error(
                f"[LIFECYCLE] RUN TIMEOUT agent_run={agent_run_id} after {run_timeout_seconds}s"
            )
            try:
                from core.agents.runner import update_agent_run_status

                await update_agent_run_status(
                    agent_run_id, "failed", error=cleanup_reason, account_id=account_id
                )
            except Exception as e:
...
async def unified_agent_start(
    ...
    if is_optimistic:
        if not thread_id or not project_id:
            raise HTTPException(
                status_code=400, detail="thread_id and project_id required for optimistic mode"
            )
    ...
    if model_name in ("carbon-bim/basic", "carbon-bim-basic") and config.ENV_MODE != EnvMode.LOCAL:
        from core.billing.subscriptions import subscription_service
        from core.cache.runtime_cache import get_cached_tier_info

        tier_info = await get_cached_tier_info(
            account_id
        ) or await subscription_service.get_user_subscription_tier(account_id)
        if tier_info.get("name") in ("free", "none"):
            logger.info(
                f"⚡ [MODEL_OVERRIDE] Free tier user {account_id} - using minimax instead of basic"
            )
            model_name = "carbon-bim/minimax"
    ...
    try:
        if thread_id and not is_optimistic:
            ...
            if thread_data["account_id"] != user_id:
                await verify_and_authorize_thread_access(
                    client, thread_id, user_id, require_write_access=True
                )
        ...
        if files:
            ...
            logger.info(
                f"[AGENT_START] Received {len(files)} files, parsed {len(files_data)} for upload"
            )
...
@router.post(
    "/agent-run/{agent_run_id}/stop", summary="Stop Agent Run", operation_id="stop_agent_run"
)
@router.post(
    "/agent-runs/{agent_run_id}/stop", summary="Stop Agent Run (plural)", include_in_schema=False
)
...
@router.get(
    "/agent-runs/{agent_run_id}/status",
...
@router.get(
    "/agent-runs/active", summary="List Active Agent Runs", operation_id="list_active_agent_runs"
)
...
@router.get(
    "/agent-run/{agent_run_id}/stream", summary="Stream Agent Run", operation_id="stream_agent_run"
)
        ...
        for i, (_, fields) in enumerate(entries):
            try:
                ...
                if msg_type == "llm_response_start":
                    ...
                    if open_responses == 0:
                        last_safe = i
                elif msg_type == "status" and data.get("status") in [
                    "completed",
                    "failed",
                    "stopped",
                    "error",
            ...
            try:
                ...
                async with redis.redis.hub.subscription(stream_key, last_id) as queue:
                    ...
                    if not skip_catchup:
                        ...
                        logger.debug(
                            f"[STREAM] Catch-up found {len(entries)} entries for {stream_key}"
                        )
                        if entries:
                                    ...
                                    if safe_idx >= 0:
                                        ...
                                        if "-" in safe_id:
                                            ...
                                            if len(parts) == 2:
                                                ...
                                                await redis.xtrim_minid(
                                                    stream_key, next_id, approximate=True
                                                )
                                except Exception as e:
                    ...
                    logger.debug(
                        f"[STREAM] Starting queue processing for {stream_key}, catchup_ids={len(catchup_ids)}"
                    )
                    ...
                    async for msg in redis.redis.hub.iter_queue(queue, timeout=0.5):
                        ...
                        if msg is not None:
                            ...
                            if (
                                entry_id in catchup_ids
                                or compare_stream_ids(entry_id, last_id) <= 0
                            ):
                                continue
                            received_data = True
                            timeout_count = 0
