...
class AgentService:
    ...
    async def _get_user_templates_paginated(
        ...
        try:
            ...
            count_query = (
                client.table("agent_templates").select("*", count="exact").eq("creator_id", user_id)
            )
            ...
            paginated_result = await PaginationService.paginate_database_query(
                base_query=base_query, params=pagination_params, count_query=count_query
            )
            ...
            return PaginatedResponse(
                data=template_responses, pagination=paginated_result.pagination
            )
    ...
    async def _get_agents_database_paginated(
        ...
        if user_id:
            ...
            agent_responses = [
                await self._transform_agent_data(row, load_config=False) for row in agents
            ]

            total_pages = (
                (total_count + pagination_params.page_size - 1) // pagination_params.page_size
                if total_count
                else 0
            )
        ...
        return PaginatedResponse(data=agent_responses, pagination=paginated_result.pagination)

    async def _get_agents_with_complex_filtering(
        self, user_id: str, pagination_params: PaginationParams, filters: AgentFilters, base_query
    ) -> PaginatedResponse[Dict[str, Any]]:
        ...
        return await PaginationService.paginate_filtered_dataset(
            all_items=agent_responses, params=pagination_params
        )

    async def _load_agent_versions_batch(
        self, agents: List[Dict[str, Any]]
    ) -> Dict[str, Dict[str, Any]]:
        version_map = {}
        version_ids = list(
            {agent["current_version_id"] for agent in agents if agent.get("current_version_id")}
        )
                ...
                for agent in agents:
                    ...
                    if version_id:
                        try:
                            ...
                            logger.warning(
                                f"Failed to load version {version_id} for agent {agent['agent_id']}: {e}"
                            )
                            continue
    ...
    async def _sort_by_tools_count(
        self, agents: List[Dict[str, Any]], version_map: Dict[str, Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        agent_tool_counts = []
    ...
    async def _load_agents_with_configs(
        self, agent_rows: List[Dict[str, Any]], version_map: Dict[str, Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def _transform_agent_data(
        self, agent_row: Dict[str, Any], load_config: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    async def _transform_template_to_agent_format(
