...
class ToolManager:
    ...
    def _register_enabled_core_tools(self):
        ...
        self.thread_manager.add_tool(
            ExpandMessageTool, thread_id=self.thread_id, thread_manager=self.thread_manager
        )
        ...
        if (config.TAVILY_API_KEY or config.FIRECRAWL_API_KEY) and self._is_tool_enabled(
            "web_search_tool"
        ):
        ...
        for tool_name in core_sandbox_tools:
            ...
            if tool_info:
                ...
                try:
                    ...
                    kwargs = {"project_id": self.project_id, "thread_manager": self.thread_manager}
                    ...
                    self.thread_manager.add_tool(
                        tool_class, function_names=enabled_methods, **kwargs
                    )
    ...
    def _register_agent_builder_tools(self):
        ...
        logger.info(
            f"🔧 Registering agent builder tools (agent_id={agent_id}, account_id={account_id}, has_db={db_connection is not None})"
        )
