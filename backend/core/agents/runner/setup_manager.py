...
async def prepopulate_caches_for_new_thread(
    ...
    if message_content and message_content.strip():
        initial_messages = [
            {"role": "user", "content": message_content, "message_id": str(uuid.uuid4())}
        ]
...
async def append_user_message_to_cache(thread_id: str, message_content: str) -> bool:
    ...
    message = {"role": "user", "content": message_content, "message_id": str(uuid.uuid4())}
...
async def prewarm_user_context(account_id: str) -> None:
    try:
        ...
        logger.debug(
            f"⚡ Pre-warmed user context for {account_id} (subscription={'yes' if subscription else 'no'})"
        )
...
async def create_agent_run_record(
    ...
    if agent_config:
        run_metadata["agent_config"] = {
            k: v for k, v in agent_config.items() if k not in ("system_prompt",)
        }
