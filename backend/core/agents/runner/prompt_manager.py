...
from core.tools.tool_guide_registry import (
    get_minimal_tool_index,
    get_minimal_tool_index_filtered,
    get_tool_guide,
)
...
class PromptManager:
    ...
    async def build_system_prompt(
        ...
        kb_task = PromptManager._with_timeout(
            PromptManager._fetch_knowledge_base(agent_config, client), 2.0, "KB fetch"
        )
        user_context_task = PromptManager._with_timeout(
            PromptManager._fetch_user_context_data(user_id, client), 2.0, "User context"
        )
        ...
        if agent_config and (
            agent_config.get("custom_mcps") or agent_config.get("configured_mcps")
        ):
            fresh_mcp_config = {
        ...
        t3 = time.time()
        system_content = await PromptManager._append_mcp_tools_info(
            system_content, agent_config, mcp_wrapper_instance, fresh_mcp_config, xml_tool_calling
        )
        logger.debug(f"⏱️ [PROMPT TIMING] _append_mcp_tools_info: {(time.time() - t3) * 1000:.1f}ms")

        t4 = time.time()
        system_content = await PromptManager._append_jit_mcp_info(
            system_content, mcp_loader, fresh_mcp_config
        )
        logger.debug(f"⏱️ [PROMPT TIMING] _append_jit_mcp_info: {(time.time() - t4) * 1000:.1f}ms")

        system_content = PromptManager._append_xml_tool_calling_instructions(
            system_content, xml_tool_calling, tool_registry
        )
        system_content = PromptManager._append_datetime_info(system_content)

        t5 = time.time()
        kb_data, user_context_data, memory_data, file_data = await asyncio.gather(
            kb_task, user_context_task, memory_task, file_task
        )
        logger.debug(
            f"⏱️ [PROMPT TIMING] parallel fetches (kb/user_context/memory/file): {(time.time() - t5) * 1000:.1f}ms"
        )

        logger.info(
            f"⏱️ [PROMPT TIMING] Total build_system_prompt: {(time.time() - build_start) * 1000:.1f}ms"
        )
    ...
    def _build_base_prompt(system_content: str, disabled_tools: list = None) -> str:
        ...
        system_content += "\n\n" + minimal_index
        logger.info(
            f"📊 [DYNAMIC TOOLS] Core prompt + minimal index: {len(system_content):,} chars"
        )
        ...
        if preloaded_guides:
            ...
            logger.info(
                f"📖 [DYNAMIC TOOLS] Added preloaded tool guides: {len(preloaded_guides):,} chars"
            )
...
3. The summaries above are just previews - sync to read full content"""
            ...
            kb_result = await client.rpc(
                "get_agent_knowledge_base_context", {"p_agent_id": agent_id}
            ).execute()
            ...
            if kb_data and kb_data.strip():
                ...
                elapsed = (time.time() - fetch_start) * 1000
                logger.debug(
                    f"⏱️ [TIMING] KB fetch: {elapsed:.1f}ms (cache: miss, found {len(kb_data)} chars)"
                )
...
3. The summaries above are just previews - sync to read full content"""
            ...
            logger.error(
                f"Error retrieving knowledge base context for agent {agent_config.get('agent_id', 'unknown')}: {e}"
            )
            return None

    @staticmethod
    async def _append_mcp_tools_info(
        system_content: str,
        agent_config: Optional[dict],
        mcp_wrapper_instance: Optional[MCPToolWrapper],
        fresh_mcp_config: Optional[dict] = None,
        xml_tool_calling: bool = False,
    ) -> str:
        if fresh_mcp_config:
            logger.debug(
                f"🔄 [MCP PROMPT] Using fresh MCP config: {len(fresh_mcp_config.get('configured_mcps', []))} configured, {len(fresh_mcp_config.get('custom_mcp', []))} custom"
            )
            agent_config = {
        ...
        mcp_info += (
            "6. If the tool doesn't return enough information, explicitly state this limitation\n"
        )
    ...
    @staticmethod
    async def _append_jit_mcp_info(
        system_content: str, mcp_loader, fresh_mcp_config: Optional[dict] = None
    ) -> str:
        toolkit_tools = {}
        ...
        if fresh_mcp_config:
            ...
            for mcp in configured_mcps:
                ...
                if enabled_tools:
                    display_name = (
                        toolkit_slug.upper() if toolkit_slug else mcp_name.upper().replace(" ", "_")
                    )
        ...
        mcp_jit_info += (
            'CORRECT: discover_mcp_tools(filter="TOOL1,TOOL2,TOOL3") - ONE call for all\n'
        )
        mcp_jit_info += "WRONG: Three separate discover calls - never do this!\n\n"
        mcp_jit_info += (
            'After discovery, execute with: execute_mcp_tool(tool_name="TOOL1", args={...})\n'
        )
        mcp_jit_info += "</batch_discovery>\n\n"
        ...
        logger.info(
            f"⚡ [MCP PROMPT] Appended MCP info ({len(mcp_jit_info)} chars) for {len(toolkit_tools)} toolkits, {total_tools} total tools"
        )
        return system_content + mcp_jit_info

    @staticmethod
    def _append_xml_tool_calling_instructions(
        system_content: str, xml_tool_calling: bool, tool_registry
    ) -> str:
...
"""
    ...
    async def _fetch_user_context_data(user_id: Optional[str], client) -> Optional[str]:
        ...
        if cached is not None:  # None = miss, empty string = no context (cached)
            ...
            logger.info(
                f"[USER_CONTEXT] Cache hit for {user_id}, len={len(cached)}, has_subscription={'<user_subscription>' in cached}"
            )
        ...
        locale, username, subscription = await asyncio.gather(
            fetch_locale(), fetch_username(), fetch_subscription()
        )

        logger.info(
            f"[USER_CONTEXT] Fetched for {user_id}: locale={locale}, username={username is not None}, subscription={subscription is not None}"
        )
        if subscription:
        ...
        if username:
            ...
            username_info += (
                "Use this to personalize responses and address the user appropriately.\n"
            )
        ...
        if subscription:
            ...
            logger.debug(
                f"Added subscription ({sub['display_name']}) to system prompt for user {user_id}"
            )
    ...
    async def _fetch_user_memories(user_id: Optional[str], thread_id: str, client) -> Optional[str]:
        ...
        if not (user_id and client):
            logger.debug(
                f"Memory fetch skipped: user_id={user_id}, client={'yes' if client else 'no'}"
            )
            return None
        ...
        try:
            ...
            memories = await memory_retrieval_service.retrieve_memories(
                account_id=user_id, query_text=query_text, tier_name=tier_name
            )
            ...
            logger.info(
                f"Retrieved {len(memories)} memories for user {user_id} (will inject as context message)"
            )
    ...
    def _log_prompt_stats(system_content: str):
        ...
        logger.info(
            f"✅ [DYNAMIC TOOLS] Final system prompt: {final_prompt_size:,} chars (est. {reduction_pct:.0f}% reduction vs legacy)"
        )
    ...
    async def _get_free_tier_promo(user_id: Optional[str]) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(
                f"[PROMO] Failed to check tier for {user_id}: {e} - skipping promo (fail safe)"
            )
            return None
...
💡 Enjoying Carbon BIM? Upgrade for more credits and unlimited chats!
...
1. Subtle hint after task (occasional):
   ...
   💡 Enjoying Carbon BIM? Upgrade for more credits!
   <upgrade/>
   ```
