...
class MCPManager:
    ...
    async def initialize_jit_loader(
        self, agent_config: Dict[str, Any], cache_only: bool = False
    ) -> None:
        if not config.ACTIVATE_MCPS_TRIG:
        ...
        if agent_id and (needs_loading or not has_mcps):
            ...
            try:
                ...
                logger.info(
                    f"⏱️ [MCP JIT TIMING] get_current_mcp_config: {(time.time() - version_start) * 1000:.1f}ms"
                )
            ...
            logger.debug(
                f"⚡ [MCP JIT] Skipping version service - MCPs already loaded (has_mcps={has_mcps})"
            )
        ...
        logger.debug(
            f"⚡ [MCP JIT] Loading MCPs: {len(custom_mcps)} custom, {len(configured_mcps)} configured"
        )
        ...
        if custom_mcps or configured_mcps:
            try:
                ...
                if (
                    not hasattr(self.thread_manager, "mcp_loader")
                    or self.thread_manager.mcp_loader is None
                ):
                ...
                logger.info(
                    f"⏱️ [MCP JIT TIMING] build_tool_map: {(time.time() - build_map_start) * 1000:.1f}ms"
                )

                # Get stats (quick, no timeout needed)
                stats = (
                    self.thread_manager.mcp_loader.get_activation_stats()
                    if self.thread_manager.mcp_loader
                    else {"total_tools": 0}
                )
                toolkits = (
                    await self.thread_manager.mcp_loader.get_toolkits()
                    if self.thread_manager.mcp_loader
                    else []
                )
                ...
                logger.info(
                    f"⚡ [MCP JIT] Initialized: {stats.get('total_tools', 0)} tools from {len(toolkits)} toolkits ({mode_str})"
                )
                logger.info(
                    f"⏱️ [MCP JIT TIMING] Total initialize_jit_loader: {(time.time() - jit_start) * 1000:.1f}ms"
                )
    ...
    def clean_legacy_mcp_tools(self) -> None:
        ...
        for tool_name in list(self.thread_manager.tool_registry.tools.keys()):
            ...
            should_remove = (
                hasattr(instance, "__class__") and "MCPToolWrapper" in instance.__class__.__name__
            ) or len(tool_name) > 64
        ...
        if removed_count > 0:
            logger.info(
                f"⚡ [MCP JIT] Registry cleaned: {tools_before} → {tools_after} tools ({removed_count} legacy tools removed)"
            )
