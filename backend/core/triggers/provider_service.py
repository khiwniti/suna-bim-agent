...
class ScheduleProvider(TriggerProvider):
    ...
    async def setup_trigger(self, trigger: Trigger) -> bool:
        try:
            ...
            logger.error(
                f"Failed to setup Supabase Cron schedule for trigger {trigger.trigger_id}: {e}"
            )
            return False
    ...
    async def teardown_trigger(self, trigger: Trigger) -> bool:
        try:
            ...
            try:
                ...
                logger.debug(
                    f"Unschedule requested for Supabase Cron job '{job_name}' (trigger {trigger.trigger_id})"
                )
                return True
            except Exception as rpc_err:
            ...
            logger.error(
                f"Failed to teardown Supabase Cron schedule for trigger {trigger.trigger_id}: {e}"
            )
            return False
    ...
    async def process_event(self, trigger: Trigger, event: TriggerEvent) -> TriggerResult:
        try:
            ...
            if not agent_prompt or not str(agent_prompt).strip():
                raise ValueError(
                    "agent_prompt is required for agent execution. Please update this trigger with valid worker instructions."
                )
            ...
            return TriggerResult(
                success=False, error_message=f"Error processing schedule event: {str(e)}"
            )
    ...
    def _convert_cron_to_utc(self, cron_expression: str, user_timezone: str) -> str:
        ...
        try:
            ...
            if "*" in minute or "*" in hour or "/" in minute or "/" in hour:
                logger.debug(
                    f"Cron expression {cron_expression} contains wildcards/intervals - will run in UTC"
                )
                return cron_expression
                ...
                for h in hours:
                    for m in minutes:
                        ...
                        user_time_naive = datetime.combine(
                            reference_date, datetime.min.time().replace(hour=h, minute=m)
                        )
                ...
                if day_offset != 0:
                    if has_specific_day:
                        ...
                        if day.isdigit():
                            ...
                            if 1 <= new_day <= 31:
                                ...
                                logger.warning(
                                    f"Day boundary crossing results in invalid day {new_day}, keeping original"
                                )
                        else:
                            logger.warning(
                                f"Complex day specification '{day}' with day boundary crossing - timing may be off by one day"
                            )
                    ...
                    if has_specific_weekday:
                        ...
                        if weekday.isdigit():
                            ...
                            logger.warning(
                                f"Complex weekday specification '{weekday}' with day boundary crossing - timing may be off by one day"
                            )

                converted = (
                    f"{utc_minute_str} {utc_hour_str} {converted_day} {month} {converted_weekday}"
                )
                logger.debug(
                    f"Converted cron '{cron_expression}' from {user_timezone} to UTC: '{converted}'"
                )
                return converted

            except Exception as e:
                logger.warning(
                    f"Failed to convert timezone for cron expression {cron_expression}: {e}"
                )
                return cron_expression
        ...
        if provider_id == "schedule":
            return {
                ...
                "properties": {
                    ...
                    "agent_prompt": {"type": "string", "description": "Prompt for agent execution"},
                    "timezone": {"type": "string", "description": "Timezone for cron expression"},
                },
...
class ComposioEventProvider(TriggerProvider):
    ...
    async def _count_triggers_with_composio_id(
        self, composio_trigger_id: str, exclude_trigger_id: Optional[str] = None
    ) -> int:
        ...
        query = (
            ...
            .select("trigger_id", count="exact")
            .eq("trigger_type", "webhook")
            .eq("config->>composio_trigger_id", composio_trigger_id)
        )
    ...
    async def _count_active_triggers_with_composio_id(
        self, composio_trigger_id: str, exclude_trigger_id: Optional[str] = None
    ) -> int:
    ...
    async def setup_trigger(self, trigger: Trigger) -> bool:
        ...
        try:
            ...
            other_active_count = await self._count_active_triggers_with_composio_id(
                composio_trigger_id, trigger.trigger_id
            )
            logger.debug(
                f"Setup trigger {trigger.trigger_id}: other_active_count={other_active_count} for composio_id={composio_trigger_id}"
            )

            if other_active_count > 0:
                # Other active triggers exist, don't touch Composio - just mark our trigger as active locally
                logger.debug(
                    f"Skipping Composio enable - {other_active_count} other active triggers exist"
                )
                return True

            # We're the first/only active trigger, enable in Composio
            logger.debug(
                f"Enabling trigger in Composio - first active trigger for {composio_trigger_id}"
            )
                    ...
                    for body in payload_candidates:
                        try:
                            ...
                            if resp.status_code in (200, 204):
                                logger.debug(
                                    f"Successfully enabled trigger in Composio: {composio_trigger_id}"
                                )
                                return True
                        except Exception:
                            continue
    ...
    async def teardown_trigger(self, trigger: Trigger) -> bool:
        ...
        try:
            ...
            if other_active_count > 0:
                ...
                logger.info(
                    f"TEARDOWN: Skipping Composio disable - {other_active_count} other active triggers exist"
                )
                return True
                ...
                for api_base in self._api_bases():
                    ...
                    for body in payload_candidates:
                        try:
                            ...
                            continue
            logger.warning(
                f"TEARDOWN: Failed to disable trigger in Composio: {composio_trigger_id}"
            )
            return True
        except Exception as e:
    ...
    async def delete_remote_trigger(self, trigger: Trigger) -> bool:
        ...
        try:
            ...
            other_count = await self._count_triggers_with_composio_id(
                composio_trigger_id, trigger.trigger_id
            )
            if other_count > 0:
    ...
    async def process_event(self, trigger: Trigger, event: TriggerEvent) -> TriggerResult:
        try:
            ...
            provider_event_id = (
                raw.get("eventId") or raw.get("payload", {}).get("id") or raw.get("id")
            )
            ...
            return TriggerResult(
                success=False, error_message=f"Error processing Composio event: {str(e)}"
            )
