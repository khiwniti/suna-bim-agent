...
async def get_agent_triggers(
    agent_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)
):
...
async def get_all_user_triggers(user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    try:
...
async def get_agent_upcoming_runs(
    ...
    try:
        ...
        return UpcomingRunsResponse(upcoming_runs=upcoming_runs, total_count=len(upcoming_runs))
...
async def create_agent_trigger(
    ...
    try:
        if config.ENV_MODE != EnvMode.LOCAL:
            ...
            provider_service = get_provider_service(db)
            provider_trigger_type = await provider_service.get_provider_trigger_type(
                request.provider_id
            )
            ...
            if not limit_check["can_create"]:
                ...
                logger.warning(
                    f"Trigger limit exceeded for account {user_id}: {limit_check['current_count']}/{limit_check['limit']} {trigger_type_str} triggers"
                )
...
async def get_trigger(trigger_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    """Get a trigger by ID"""
...
async def delete_trigger(trigger_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    """Delete a trigger"""
...
async def get_trigger_executions(
    ...
    try:
        ...
        runs_result = (
            ...
            .select("id, thread_id, agent_id, status, created_at, completed_at, error, metadata")
            .eq("metadata->>trigger_id", trigger_id)
...
async def trigger_webhook(trigger_id: str, request: Request):
    ...
    try:
        ...
        if not result.success:
            return JSONResponse(
                status_code=400, content={"success": False, "error": result.error_message}
            )
        ...
        if result.should_execute_agent:
            ...
            if trigger:
                ...
                execution_result = await execution_service.execute_trigger_result(
                    agent_id=trigger.agent_id, trigger_result=result, trigger_event=event
                )
        ...
        return JSONResponse(
            content={
                ...
                "trigger_result": {"should_execute_agent": result.should_execute_agent},
            }
        )
