...
async def initialize_user_account(
    account_id: str, email: Optional[str] = None, user_record: Optional[dict] = None
) -> Dict:
    try:
        ...
        if not result.get("success"):
            ...
            if "Already subscribed" in error_msg or "already" in error_msg.lower():
                logger.info(
                    f"[SETUP] User {account_id} already has subscription, proceeding with agent install"
                )
            else:
        ...
        if user_record:
            ...
            if referral_code:
                ...
                try:
                    ...
                    logger.info(
                        f"[SETUP] Validated referral code {referral_code} -> referrer_id: {referrer_id}"
                    )
...
def _send_welcome_email_async(email: str, user_name: str):
    ...
    try:
        return email_service.send_welcome_email(user_email=email, user_name=user_name)
    except Exception as e:
...
async def initialize_account(account_id: str = Depends(verify_and_get_user_id_from_jwt)):
    # Use singleton - already initialized at startup
    db = DBConnection()
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=500, detail=result.get("message", "Failed to initialize account")
        )
...
async def handle_user_created_webhook(
    ...
    try:
        ...
        if not user_id:
            ...
            return WebhookResponse(success=False, message="No user ID in user record")

        if not email:
            logger.warning("User created webhook received without email")
            return WebhookResponse(success=False, message="No email in user record")
...
async def receive_vercel_drain(
    request: Request, x_vercel_signature: Optional[str] = Header(None, alias="x-vercel-signature")
):
    """
    ...
    try:
        ...
        for event in events:
            ...
            await client.rpc(
                "upsert_vercel_pageview", {"p_date": event_date, "p_device_id": device_id}
            ).execute()
            processed += 1

        logger.info(
            f"Vercel drain: {processed} pageviews processed, {skipped} non-pageview skipped, {len(events)} total events"
        )
