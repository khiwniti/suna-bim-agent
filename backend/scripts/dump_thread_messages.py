...
async def dump_thread_messages(thread_id: str, output_file: str = None):
    ...
    messages = await threads_repo.get_thread_messages(
        thread_id=thread_id, order="asc", optimized=False
    )
    ...
    output = {"thread_id": thread_id, "message_count": len(messages), "messages": []}
    ...
    for msg in messages:
        if msg.get("type") == "assistant":
            ...
            print(
                f"  Content keys: {list(content.keys()) if isinstance(content, dict) else type(content)}"
            )
            print(
...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python scripts/dump_thread_messages.py b9ee92d2-c636-43da-b04c-b1a2d3cc98b9"
        )
        sys.exit(1)
