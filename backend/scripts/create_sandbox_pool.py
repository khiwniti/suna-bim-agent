...
async def create_sandboxes(count: int, batch_size: int, delay_seconds: int):
    ...
    logger.info(
        f"Starting sandbox pool creation: {count} sandboxes, batch_size={batch_size}, delay={delay_seconds}s"
    )
    ...
    try:
        ...
        for batch_num in range(batches):
            ...
            logger.info(
                f"[Batch {batch_num + 1}/{batches}] Creating {current_batch_size} sandboxes..."
            )
            ...
            logger.info(
                f"[Batch {batch_num + 1}/{batches}] Created {batch_created}/{current_batch_size} sandboxes (total: {created_total}/{count})"
            )
        ...
        logger.info(
            f"✅ Sandbox pool creation complete: {created_total} created, {failed_total} failed"
        )
...
def main():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=5, help="Sandboxes per batch (default: 5)"
    )
    parser.add_argument(
        "--delay", type=int, default=10, help="Seconds between batches (default: 10)"
    )
