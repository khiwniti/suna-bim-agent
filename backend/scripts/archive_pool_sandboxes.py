    ...
    try:
        ...
        if sandbox.state == SandboxState.STARTED:
            ...
            for i in range(60):
                ...
                logger.error(
                    f"Sandbox {sandbox_id} did not stop within 120 seconds, current state: {sandbox.state}"
                )
                return False
...
async def archive_pool_sandboxes(count: int, batch_size: int, dry_run: bool = False):
    ...
    logger.info(
        f"Starting pool sandbox archival and deletion: {count} sandboxes, batch_size={batch_size}, dry_run={dry_run}"
    )
    ...
    try:
        ...
        for batch_num in range(batches):
            ...
            logger.info(
                f"[Batch {batch_num + 1}/{batches}] Processing {len(batch_items)} sandboxes..."
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Stop, archive, and delete sandboxes from the pool"
    )
