...
async def check_wal_health():
    ...
    if metrics_dict["dlq_entries"] > 0:
        issues.append(
            f"🚨 CRITICAL: {metrics_dict['dlq_entries']} entries in DLQ - data loss risk!"
        )

    if metrics_dict["writes_dropped"] > 0:
        issues.append(
            f"🚨 CRITICAL: {metrics_dict['writes_dropped']} writes dropped - data loss confirmed!"
        )

    if metrics_dict["heartbeat_critical_runs"] > 0:
        issues.append(
            f"🚨 CRITICAL: {metrics_dict['heartbeat_critical_runs']} runs at risk of orphan takeover!"
        )

    # WARNING CHECKS
    if wal_stats["total_pending"] > 1000:
        warnings.append(
            f"⚠️  WARNING: {wal_stats['total_pending']} pending entries (threshold: 1000)"
        )

    if wal_stats["runs_with_pending"] > 50:
        warnings.append(
            f"⚠️  WARNING: {wal_stats['runs_with_pending']} runs with pending writes (threshold: 50)"
        )

    if metrics_dict.get("flush_latency_p99", 0) > 10.0:
        warnings.append(
            f"⚠️  WARNING: Flush latency P99 is {metrics_dict['flush_latency_p99']:.2f}s (threshold: 10s)"
        )
    ...
    if wal_stats["local_buffer_runs"] > 0:
        warnings.append(
            f"⚠️  WARNING: {wal_stats['local_buffer_runs']} runs using local buffer (Redis issues)"
        )

    if metrics_dict["pending_writes"] > config.PENDING_WRITES_WARNING_THRESHOLD:
        warnings.append(
            f"⚠️  WARNING: {metrics_dict['pending_writes']} pending writes (threshold: {config.PENDING_WRITES_WARNING_THRESHOLD})"
        )

    if metrics_dict["active_runs"] > config.ACTIVE_RUNS_WARNING_THRESHOLD:
        warnings.append(
            f"⚠️  WARNING: {metrics_dict['active_runs']} active runs (threshold: {config.ACTIVE_RUNS_WARNING_THRESHOLD})"
        )
