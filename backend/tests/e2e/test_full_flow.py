...
async def test_complete_api_flow(
    client: httpx.AsyncClient, test_config: E2ETestConfig, test_user: dict
):
    """
    ...
    print(
        f"✅ Account verified: tier={tier_key} ({tier_display}), credits={credits.get('total', 0)}"
    )
    ...
    response = await client.post(
        "/agent/start",
        data={
            "prompt": "Create a hello.py file that prints 'Hello World'. Use the attached test.py as reference."
        },
        files=files,
    )
    ...
    try:
        async with client.stream(
            "GET", f"/agent-run/{agent_run_id}/stream", timeout=test_config.agent_timeout
        ) as stream_response:
            print(f"✅ Stream connected, status: {stream_response.status_code}")
            assert stream_response.status_code == 200, (
                f"Stream failed: {stream_response.status_code}"
            )
