...
def create_tool_result_message(
    content: str, tool_call_id: str, message_id: Optional[str] = None
) -> Dict[str, Any]:
...
def generate_conversation_messages(
            ...
            if include_tool_calls and message_count % 6 == 1:
                ...
                messages.append(
                    create_tool_result_message(
                        result_content, tool_call["id"], f"msg_{message_count}"
                    )
                )
            else:
    ...
    print(
        f"Generated {len(messages)} messages with {final_tokens:,} tokens (target: {target_total_tokens:,})"
    )
...
class TestCompressionBasics:
    ...
    def test_remove_old_tool_outputs_preserves_recent(self, context_manager):
        ...
        for i in range(10):
            ...
            messages.append(
                create_tool_result_message(
                    generate_text_content(1000), tool_call["id"], f"msg_{i * 2 + 1}"
                )
            )
...
class TestLargeContextCompression200k:
    ...
    async def test_compression_happens_when_needed(self, context_manager):
        ...
        with patch.object(context_manager, "save_compressed_messages", new_callable=AsyncMock):
            model = "carbon-bim/basic"  # 200k context window
            ...
            assert len(result) <= len(large_conversation), (
                "Message count should be reduced or equal"
            )
    ...
    async def test_compression_respects_threshold(self, context_manager):
        ...
        # Mock save_compressed_messages to avoid DB calls
        with patch.object(context_manager, "save_compressed_messages", new_callable=AsyncMock):
            model = "carbon-bim/basic"  # 200k context window -> 168k max tokens
            system_prompt = create_system_prompt(6000)
            ...
            assert final_tokens <= model_threshold, (
                f"Final tokens ({final_tokens}) should be under threshold ({model_threshold})"
            )

            # Should have reduced from initial
            assert final_tokens < initial_tokens, (
                f"Should compress: {initial_tokens} -> {final_tokens}"
            )
...
class TestLargeContextCompression500k:
    ...
    async def test_compression_handles_500k_context(self, context_manager):
        ...
        conversation = generate_conversation_messages(
            target_total_tokens=30000, tokens_per_message=1000, tool_result_size=2000
        )
...
class TestLargeContextCompression1M:
    ...
    async def test_compression_handles_1m_context(self, context_manager):
        ...
        conversation = generate_conversation_messages(
            target_total_tokens=40000, tokens_per_message=1200, tool_result_size=3000
        )
    ...
    async def test_extreme_compression_preserves_structure(self, context_manager):
        ...
        conversation = generate_conversation_messages(
            target_total_tokens=60000, tokens_per_message=1500, tool_result_size=5000
        )
        ...
        with patch.object(context_manager, "save_compressed_messages", new_callable=AsyncMock):
            ...
            assert is_valid, (
                f"Tool call pairing must be valid. Orphaned: {orphaned}, Unanswered: {unanswered}"
            )
...
class TestTieredCompression:
    ...
    async def test_tier1_tool_output_compression(self, context_manager):
        ...
        for i in range(15):
            tool_call = create_tool_call(f"tool_{i}", {})
            messages.append(
                create_assistant_message(f"Calling tool {i}", f"msg_{i * 2}", [tool_call])
            )
            messages.append(
                create_tool_result_message(
    ...
    async def test_tier2_user_message_compression(self, context_manager):
        ...
        messages = []
        for i in range(20):
            messages.append(create_user_message(generate_text_content(1000), f"msg_{i}"))
    ...
    async def test_tier3_assistant_message_compression(self, context_manager):
        ...
        messages = []
        for i in range(20):
            messages.append(create_assistant_message(generate_text_content(1000), f"msg_{i}"))
...
class TestMiddleOutCompression:
    ...
    async def test_middle_out_preserves_tool_pairing(self, context_manager, mock_threads_repo):
        ...
        for i in range(30):
            if i % 3 == 1:
                tool_call = create_tool_call(f"tool_{i}", {})
                messages.append(create_assistant_message(f"Calling tool", f"msg_{i}", [tool_call]))
                messages.append(
                    create_tool_result_message(f"Result {i}", tool_call["id"], f"msg_{i}_result")
                )
            else:
                messages.append(create_user_message(f"Message {i}", f"msg_{i}"))
...
class TestCompressionTokenLimits:
    ...
    async def test_compression_200k_model_limit(self, context_manager):
        ...
        with patch.object(context_manager, "save_compressed_messages", new_callable=AsyncMock):
            result = await context_manager.compress_messages(
                messages, model, system_prompt=system_prompt, thread_id="test_thread"
            )
    ...
    async def test_no_compression_when_under_limit(self, context_manager, small_conversation):
        ...
        with patch.object(context_manager, "save_compressed_messages", new_callable=AsyncMock):
            result = await context_manager.compress_messages(
                small_conversation, model, system_prompt=system_prompt, thread_id="test_thread"
            )
...
class TestPromptCaching:
    ...
    async def test_supports_prompt_caching(self):
        ...
        assert supports_prompt_caching("carbon-bim/basic") is True
        assert supports_prompt_caching("carbon-bim/power") is True
    ...
    async def test_calculate_optimal_cache_threshold(self):
        ...
        early_threshold = calculate_optimal_cache_threshold(
            context_window=200000, message_count=10, current_tokens=5000
        )

        # Mature conversation (≤500 messages) should have higher threshold
        mature_threshold = calculate_optimal_cache_threshold(
            context_window=200000, message_count=300, current_tokens=100000
        )
        ...
        large_context_threshold = calculate_optimal_cache_threshold(
            context_window=1000000, message_count=10, current_tokens=5000
        )
    ...
    async def test_add_cache_control_preserves_structure(self):
        ...
        if isinstance(cached_msg["content"], list):
            has_cache_control = any(
                isinstance(c, dict) and "cache_control" in c for c in cached_msg["content"]
            )
    ...
    async def test_apply_caching_strategy(self):
        ...
        result = await apply_anthropic_caching_strategy(
            system_prompt, messages, "carbon-bim/basic", thread_id=None, force_recalc=True
        )
        ...
        has_cache_blocks = any(
            ...
            and any(isinstance(c, dict) and "cache_control" in c for c in msg.get("content", []))
            for msg in result
        )
...
class TestRealLargeContext200k:
    ...
    async def test_real_200k_token_compression(self, context_manager):
        ...
        conversation = generate_conversation_messages(
            target_total_tokens=TARGET_TOKENS, tokens_per_message=3000, tool_result_size=10000
        )
        ...
        print(
            f"✅ Generated {len(conversation)} messages with {actual_generated:,} tokens in {gen_time:.1f}s"
        )
        ...
        with patch.object(context_manager, "save_compressed_messages", new_callable=AsyncMock):
            ...
            print(
                f"✅ 200k test PASSED - {initial_tokens:,} -> {final_tokens:,} tokens ({final_tokens / initial_tokens:.1%})"
            )
...
class TestRealLargeContext500k:
    ...
    async def test_real_500k_token_compression(self, context_manager):
        ...
        conversation = generate_conversation_messages(
            target_total_tokens=TARGET_TOKENS, tokens_per_message=4000, tool_result_size=15000
        )
        ...
        with patch.object(context_manager, "save_compressed_messages", new_callable=AsyncMock):
            ...
            assert final_tokens < initial_tokens * 0.5, (
                f"Should compress significantly: {initial_tokens:,} -> {final_tokens:,}"
            )
            ...
            print(
                f"✅ 500k test PASSED - {initial_tokens:,} -> {final_tokens:,} tokens ({final_tokens / initial_tokens:.1%})"
            )
...
class TestRealLargeContext1M:
    ...
    async def test_real_1m_token_compression(self, context_manager):
        ...
        conversation = generate_conversation_messages(
            target_total_tokens=TARGET_TOKENS, tokens_per_message=5000, tool_result_size=20000
        )
        ...
        with patch.object(context_manager, "save_compressed_messages", new_callable=AsyncMock):
            ...
            assert final_tokens < initial_tokens * 0.25, (
                f"Should compress >75%: {initial_tokens:,} -> {final_tokens:,}"
            )
            ...
            print(
                f"✅ 1M test PASSED - {initial_tokens:,} -> {final_tokens:,} tokens ({final_tokens / initial_tokens:.1%})"
            )
