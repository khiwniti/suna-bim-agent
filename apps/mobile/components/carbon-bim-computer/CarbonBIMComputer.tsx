...
interface CarbonBIMComputerProps {
...
export function CarbonBIMComputer({
  toolMessages,
  currentIndex,
  onNavigate,
  messages,
  ...
  streamingText,
  sandboxId,
}: CarbonBIMComputerProps) {
  log.log('[CarbonBIMComputer] Render - toolMessages:', toolMessages.length, 'currentIndex:', currentIndex);
  log.log('[CarbonBIMComputer] project:', project ? { id: project.id, name: project.name, hasSandbox: !!project.sandbox } : 'undefined');
  log.log('[CarbonBIMComputer] sandboxId prop:', sandboxId);
  ...
  log.log('[CarbonBIMComputer] currentPair:', currentPair ? 'has pair' : 'undefined');
  log.log('[CarbonBIMComputer] currentPair.toolMessage:', currentPair?.toolMessage?.message_id || 'null');
  log.log('[CarbonBIMComputer] currentPair.assistantMessage:', currentPair?.assistantMessage?.message_id || 'null');
  
  const { toolCall, toolResult, isSuccess, assistantTimestamp, toolTimestamp } = useMemo(() => {
    if (!currentPair?.toolMessage) {
      log.log('[CarbonBIMComputer] No toolMessage in currentPair, returning null');
      return { toolCall: null, toolResult: null, isSuccess: false, assistantTimestamp: undefined, toolTimestamp: undefined };
    }
    log.log('[CarbonBIMComputer] Calling extractToolCallAndResult');
