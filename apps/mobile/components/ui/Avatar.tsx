...
interface AvatarProps extends ViewProps {
  ...
  /** Use Carbon BIM symbol instead of icon (for SUNA agent) */
  useCarbonBIMSymbol?: boolean;
 ...
 * - Carbon BIM symbol: 50% of container (larger for brand recognition)
 * - Border: 1.5px solid
...
export function Avatar({
  ...
  const symbolSize = Math.round(size * 0.55); // 55% for Carbon BIM symbol (more prominent)
  ...
  const getDefaultColors = () => {
    ...
    // Carbon BIM symbol always uses solid black bg with white icon
    if (useCarbonBIMSymbol) {
      return {
        bg: '#000000',
        icon: '#FFFFFF',
    ...
    <View 
      ...
      style={[
        { 
          ...
          borderWidth: useCarbonBIMSymbol ? 0 : (showBorder ? 1.5 : 0),
