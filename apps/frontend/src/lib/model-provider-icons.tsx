...
/**
 * Check if a model ID corresponds to a Carbon BIM mode (Basic, Advanced, or Test)
 */
export function isCarbonBIMMode(modelId: string): boolean {
  // New Carbon BIM registry IDs
  if (modelId === 'carbon-bim/basic' || modelId === 'carbon-bim/power' || modelId === 'carbon-bim/test' ||
      modelId === 'carbon-bim-basic' || modelId === 'carbon-bim-power' || modelId === 'carbon-bim-test') {
    return true;
  }
...
export function ModelProviderIcon({
  ...
  // Special handling for Carbon BIM symbol - needs different invert behavior
  const isCarbonBim = provider === 'carbon-bim';
  ...
  return (
    ...
    >
      <Image
        ...
        className={cn(
          "object-contain",
          // Carbon BIM symbol: invert in dark mode (black symbol → white)
          // Other icons: invert in dark mode (black icons → white)
          isCarbonBim ? "dark:invert" : "dark:brightness-0 dark:invert"
        )}
