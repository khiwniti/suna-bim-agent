...
import { CarbonBIMLogo } from '@/components/sidebar/carbon-bim-logo';
...
const LoggedInMenu: React.FC<UnifiedConfigMenuProps> = memo(function LoggedInMenu({
    ...
    const renderAgentIcon = useCallback((agent: any, size: number = 32) => {
        if (!agent && (isLoading || sunaAgent)) {
            return <AgentAvatar isSunaDefault={true} agentName="Carbon BIM" size={size} className="flex-shrink-0 !border-0" />;
        }
    ...
    // Check if current agent is Carbon BIM (used in multiple places)
    const isCarbonBIMAgent = !displayAgent?.name || displayAgent?.name === 'Carbon BIM';
                        ...
                        <div className="px-4 pb-2">
                            ...
                            >
                                ...
                                <span className="flex-1 truncate text-base font-medium text-left min-w-0">
                                    {isCarbonBIMAgent ? 'Carbon BIM' : displayAgent?.name}
                                </span>
    ...
    }, [mobileSection, searchQuery, onAgentSelect, displayAgent, isLoading, placeholderSunaAgent, renderAgentIcon, selectedAgentId, AgentsList, CreateWorkerButton, WorkerSettingsButtons, isCarbonBIMAgent]);
                            ...
                            {onAgentSelect && (
                                <>
                                    ...
                                    <div className="px-2 pb-2">
                                        <SpotlightCard className={cn("transition-colors cursor-pointer bg-transparent", isCarbonBIMAgent ? "p-2" : "")}>
                                            <DropdownMenuSub>
                ...
                <span className="inline-block">
                    ...
                    >
                        <div className="flex items-center gap-2 min-w-0 max-w-[180px]">
                            ...
                            <span className="truncate text-sm font-medium">Carbon BIM</span>
                            <ChevronDown size={12} className="opacity-60 flex-shrink-0" />
                        </div>
                    </Button>
                </span>
