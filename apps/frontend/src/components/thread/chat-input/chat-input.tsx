...
// Carbon BIM agent modes switcher - isolated from typing state
        ...
        <TooltipTrigger asChild>
          ...
          >
            {((loading || isUploading) && !isAgentRunning) ? (
              <CarbonBIMLoader size="small" customSize={20} variant={buttonLoaderVariant} />
            ) : showAddToQueue ? (
...
export const ChatInput = memo(forwardRef<ChatInputHandles, ChatInputProps>(
  (
    ...
    const [isModeDismissing, setIsModeDismissing] = useState(false);    // Carbon BIM Agent Modes feature flag
    ...
    // Check if selected agent is Carbon BIM based on agent data
    // While loading, default to Carbon BIM (assume Carbon BIM is the default agent)
    const selectedAgent = agents.find(agent => agent.agent_id === selectedAgentId);
    const sunaAgent = agents.find(agent => agent.metadata?.is_suna_default === true);
    const isSunaAgent = isLoadingAgents 
        ? true // Show Carbon BIM modes while loading
