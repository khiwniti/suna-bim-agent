...
import { 
  ...
  useCarbonBIMComputerPendingToolNavIndex,
  useCarbonBIMComputerClearPendingToolNav,
} from '@/stores/carbon-bim-computer-store';
...
const FLOATING_LAYOUT_ID = 'carbon-bim-computer-float';

export const CarbonBIMComputer = memo(function CarbonBIMComputer({
  isOpen,
  onClose,
  toolCalls,
  currentIndex,
  ...
  const pendingToolNavIndex = useCarbonBIMComputerPendingToolNavIndex();
  const clearPendingToolNav = useCarbonBIMComputerClearPendingToolNav();
  ...
  useEffect(() => {
    ...
    if (projectChanged || sandboxChanged) {
      console.log('[CarbonBIMComputer] Project or sandbox changed, resetting local state', { projectId, sandboxId });
  ...
  if (isMobile) {
    const handleDrawerKeyDown = (e: React.KeyboardEvent) => {
      ...
      // Prevent Escape / Esc from closing the Carbon BIM Computer.
    ...
    return (
      <Drawer
        ...
        // Never allow Esc/Escape to dismiss the Carbon BIM Computer.
  ...
  return (
    <motion.div
      ...
      layoutId="carbon-bim-computer-window"
      initial={{ opacity: 1 }}
      animate={{ opacity: 1 }}
      transition={{
        layout: {
