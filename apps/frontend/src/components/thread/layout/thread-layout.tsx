...
import { CarbonBIMComputer, ToolCallInput } from '@/components/thread/carbon-bim-computer';
...
export const ThreadLayout = memo(function ThreadLayout({
  ...
  // Carbon BIM Computer Store - for handling file open requests
  const { shouldOpenPanel, clearShouldOpenPanel, openFileInComputer, openFileBrowser } = useCarbonBIMComputerStore();
  ...
  // Handle file click - now opens in Carbon BIM Computer instead of modal
  ...
  const selectedFilePath = useCarbonBIMComputerStore((state) => state.selectedFilePath);
