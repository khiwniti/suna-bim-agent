...
import { ToolCallInput } from '@/components/thread/carbon-bim-computer';
...
import { useCarbonBIMComputerStore, useIsSidePanelOpen, useSetIsSidePanelOpen } from '@/stores/carbon-bim-computer-store';
...
export function useThreadToolCalls(
  ...
  const navigateToToolCall = useCarbonBIMComputerStore((state) => state.navigateToToolCall);
  ...
  const handleToolClick = useCallback((clickedAssistantMessageId: string | null, clickedToolName: string, toolCallId?: string) => {
    ...
    const navigateToIndex = (index: number) => {
      ...
      setIsSidePanelOpen(true);
      // Use store action to ensure CarbonBIMComputer switches to tools view
