...
export function useMarketplaceTemplates(params?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      ...
      if (params?.is_carbon_bim_team !== undefined) searchParams.set('is_carbon_bim_team', params.is_carbon_bim_team.toString());
...
export function useCarbonBIMTeamTemplates(options?: { enabled?: boolean }) {
  return useQuery({
    queryKey: ['secure-mcp', 'carbon-bim-templates-all'],
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      const supabase = createClient();
      const { data: { session } } = await supabase.auth.getSession();

      if (!session) {
        throw new Error('You must be logged in to view Carbon BIM templates');
      }

      const response = await fetch(`${API_URL}/templates/carbon-bim-all`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      });
